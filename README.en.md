...
Product version: `v0.3.3`. Runbook version: **Agent Installation 1.2.0**. These are separate versions: this is not GitHub `v1.0.0` or a complete copy of the maintainers' private platform.

## Installation Entry

Give your own agent [AGENT_INSTALL.md](AGENT_INSTALL.md) and the matching code archive from the [v0.3.3 Release](https://github.com/wzser/supermatrix/releases/tag/v0.3.3). The runbook verifies the exact Release, tag, asset digests and `SHA256SUMS` first. Do not substitute `main`, GitHub's automatic source archive or an existing installation.
...
v0.3.3 fixes onboarding-v1 handling of exit-zero Lark CLI JSON, bot identity readback and the provider-routed Claude probe budget; it adds no platform, installer or authorization capability.

## Updated Platform Capabilities

- **Publication contract:** the 57-check Agent Installation 1.2.0 contract remains unchanged, but is bound to the matching `v0.3.3` tag, archive, checksum manifest and extraction prefix.
...
An already completed v0.3.1 isolated installation needs no runtime migration or new authorization. New installations must use the v0.3.3 `AGENT_INSTALL.md`, `supermatrix-v0.3.3.tar` and `SHA256SUMS`; do not mix Release assets. Keep existing credentials, databases, production table records and event subscriptions. Any cleanup needs separate authorization.
