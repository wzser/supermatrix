...
产品版本：`v0.3.1`。安装文档版本：**Agent Installation 1.2.0**。两者是不同版本号；这不是 GitHub `v1.0.0`，也不代表维护者整套私有平台均已开源。

## 安装入口

把 [AGENT_INSTALL.md](AGENT_INSTALL.md) 和 [v0.3.1 Release](https://github.com/wzser/supermatrix/releases/tag/v0.3.1) 中的同版本代码归档交给你自己的 agent。文档要求先核对 Release、tag、资产 digest 与 `SHA256SUMS`，再安装；不要以 `main`、GitHub 自动源码包或旧安装目录代替。
...
v0.3.1 不新增运行时平台、命令、adapter 或安装器。本补丁只补齐公开包的许可证文件和可追溯的许可证边界。

## 更新的平台能力

- **发布合同**：安装文档继续保持 57 项检查和 Agent Installation 1.2.0 合同，但明确绑定同版本的 `v0.3.1` tag、归档、校验清单和解压前缀。
- **公开包边界**：根目录与 `platform/gitmaster/public-release/` 都携带 MIT、Apache-2.0 和第三方 notices 输入，构建器用精确 allowlist 与回归测试校验这两处字节闭合。
- **脱敏发布**：本补丁从已发布的 v0.3.0 快照出发，只修改许可证、安装合同、版本元数据和发布文档；其他平台文件保持原字节。

## 从 v0.3.0 升级

已完成的 v0.3.0 隔离安装不需要 runtime 迁移或重新授权。新安装必须使用 v0.3.1 的 `AGENT_INSTALL.md`、`supermatrix-v0.3.1.tar` 和 `SHA256SUMS`；不要混用旧 Release 资产。继续保留既有凭据、数据库、生产表记录和事件订阅，任何清理都需要单独授权。
