...
产品版本：`v0.3.5`。安装文档版本：**Agent Installation 1.2.0**。两者是不同版本号；这不是 GitHub `v1.0.0`，也不代表维护者整套私有平台均已开源。

## 安装入口

把 [AGENT_INSTALL.md](AGENT_INSTALL.md) 和 [v0.3.5 Release](https://github.com/wzser/supermatrix/releases/tag/v0.3.5) 中的同版本代码归档交给你自己的 agent。文档要求先核对 Release、tag、资产 digest 与 `SHA256SUMS`，再安装；不要以 `main`、GitHub 自动源码包或旧安装目录代替。
...
v0.3.5 修复 onboarding-v1 的 Lark CLI exit-zero JSON 判定、bot 身份读回和 provider-routed Claude 探针预算；不新增平台、安装器或授权能力。

## 更新的平台能力

- **发布合同**：安装文档继续保持 57 项检查和 Agent Installation 1.2.0 合同，但明确绑定同版本的 `v0.3.5` tag、归档、校验清单和解压前缀。
...
已完成的 v0.3.1 隔离安装不需要 runtime 迁移或重新授权。新安装必须使用 v0.3.5 的 `AGENT_INSTALL.md`、`supermatrix-v0.3.5.tar` 和 `SHA256SUMS`；不要混用旧 Release 资产。继续保留既有凭据、数据库、生产表记录和事件订阅，任何清理都需要单独授权。
