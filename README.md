...
> 下班路上想到一个 bug，在飞书里发一句话，让家里 Mac 上的 Claude 改完；到家直接 review diff、跑测试、merge。
>
> Super Matrix 把你电脑里的 Claude Code / Codex CLI / Kimi CLI，变成飞书群里随叫随到的同事——保留本地执行、本地数据、本地凭证，但入口、协作和派活都搬到群聊里。

<!--
═══════════════════════════════════════════════════════════════════════
图 1 / 6 — HERO（README 最顶部首屏）
═══════════════════════════════════════════════════════════════════════
内容：飞书群里的一段对话截图。
  - 用户消息：一句自然语言任务，例如「帮我把 dispatcher.ts 里的错误日志加上 chat_id」
  - 机器人/session 回复：状态卡片（执行中 → 已完成），附 diff 摘要或文件列表
建议尺寸：横图 1600×900 左右，PNG 或 SVG。
上传后：把仓库里的 docs/images/01-hero-chat.svg 替换成实图，路径保持不变；也可以换成 PNG 并同步改下面路径。
═══════════════════════════════════════════════════════════════════════
-->
![Hero：飞书群里直接给 session 派任务](docs/images/01-hero-chat.svg)

## 30 秒了解

- **入口在飞书，执行在本地**：手机或群里发任务，本机 Claude Code / Codex CLI / Kimi CLI 接手干活，结果回到群里。
- **每个 session 是一个长期同事**：有名字、群聊、工作区、后端 CLI、状态和任务记录；不是临时聊天窗口。
- **多 session 协作**：可以让 Codex 写 plan、Claude 执行、Kimi review，全程在飞书里用自然语言派活，不用复制粘贴 prompt。

<!--
═══════════════════════════════════════════════════════════════════════
图 2 / 6 — 多 session 协作场景
═══════════════════════════════════════════════════════════════════════
内容：飞书桌面端截图，左侧群列表展示多个 session 群（例如 `plan-codex`、
`alpha-claude`、`review-kimi`），右侧打开其中一个对话，能看到「上一个 session
把结果传给下一个」的协作痕迹。
建议尺寸：横图 1600×1000，可以打码处理头像/群名敏感字段。
═══════════════════════════════════════════════════════════════════════
-->
![多 session 协作：Codex 写 plan，Claude 执行，Kimi review](docs/images/02-multi-session.svg)
...
Super Matrix 的做法是：**把"入口"放到飞书，把"执行"留在本地**，把"协作规则"沉淀成 session、scheduler、watchdog 和共享 skills。不是再造一个聊天机器人，而是把你已经在用的 CLI Agent 接进一个可管理、可协作、可审计的工作台。
...
<!--
═══════════════════════════════════════════════════════════════════════
图 3 / 6 — 安装流程（建议 GIF，可选）
═══════════════════════════════════════════════════════════════════════
内容：录屏 GIF，展示 `npm run init` 从扫码到 self-check 通过的全过程，
30~60 秒即可。如果做 GIF 麻烦，也可以放 3 张静态截图：
  (1) 二维码出现
  (2) 飞书里扫码并授权
  (3) self-check 全绿
建议尺寸：横图 1200×750；GIF ≤ 5 MB，或用 SVG/PNG 静态占位。
═══════════════════════════════════════════════════════════════════════
-->
![npm run init 安装流程](docs/images/03-init-flow.svg)
...
图 4 / 6 — /status 卡片
═══════════════════════════════════════════════════════════════════════
内容：root console 群里发 `/status` 之后，机器人回复的飞书卡片截图。
能看到 session 列表（name / backend / status / 上次活跃时间）。
建议尺寸：竖图 800×1200 或方图 1000×1000，PNG，群名/敏感 ID 可以打码。
═══════════════════════════════════════════════════════════════════════
-->
![/status 输出的 session 列表卡片](docs/images/04-status-card.svg)
...
图 5 / 6 — 架构示意图
═══════════════════════════════════════════════════════════════════════
内容：一张干净的方块图，展示「飞书群 → SM API → SQLite → 工作区 → 后端 CLI」
的数据流。可以用 Excalidraw、draw.io、Figma 画，风格手绘/极简都行。
建议尺寸：横图 1600×900，PNG 或 SVG。
═══════════════════════════════════════════════════════════════════════
-->
![Super Matrix 架构示意：飞书 → API → SQLite → 工作区 → CLI](docs/images/05-architecture.svg)
...
<!--
═══════════════════════════════════════════════════════════════════════
图 6 / 6 — autobitable 多维表格触发示例
═══════════════════════════════════════════════════════════════════════
内容：飞书多维表格的截图。展示一张表（比如"待处理任务"），其中一行的状态
字段从「待处理」变成「已派单」，并且对应行后面出现一条机器人留言/回填，
说明已经触发了哪个 session、谁去执行。
建议尺寸：横图 1600×900，PNG，业务字段可以打码。
═══════════════════════════════════════════════════════════════════════
-->
![autobitable：多维表格触发 session 并回填结果](docs/images/06-autobitable.svg)
...
我是 Nacle，微信 `AtThePinnacle`，公众号「硅基山」，跨境 AI 人。大厂商务出身，实体和虚拟产品都做过，**不是程序员**。
