...
**Delegate tasks via `/api/spawn2.0` (the only cross-session delegation contract; legacy `/api/spawn` is retired — returns HTTP 410 on every call — and is not a valid call mechanism):**

```bash
curl -s -X POST http://localhost:3501/api/spawn2.0 \
  -H "Content-Type: application/json" \
  -d '{
    "target": "<session-name-or-alias>",
...
- **`from` is required** — set it to `$SM_SESSION_NAME` (framework-injected env var; never hard-code the session name). It is the attribution key and the return address for async fallback.
- **`client_request_id` is required** — must match `^\d{4}-\d{2}-\d{2}:` and stay ≤240 chars. Use a stable business key like `<date>:<from>:<target>:<topic>`; retries with the same id dedupe at admission.
- **`closure` is required** — `{kind:"message", target:{type: "inline"|"session"|"topic"|"todo_pool"}}`. `inline` is the recommended default (sync first, framework auto-falls-back to async via Heartbeat). `todo_pool` is for explicit fire-and-forget kickoff.
- **Caller does not pick sync vs async.** The framework decides from `closure.target` plus the run state. No `mode` field. `/api/spawn2.0` rejects `mode` with HTTP 400. (Legacy `/api/spawn` is retired and returns HTTP 410 — not a valid call mechanism; its old silently-ignore-`mode` behavior is gone.)
- **`verification_predicate` is optional but recommended.** Five types: `inbox-message` / `git-log` / `db-row` / `file-mtime` / `http-get`. Without it the framework degrades to the implicit "output non-empty" check.
- **`execution.backend` / `execution.model`** are optional overrides — put them under `execution`, never as top-level fields.
- **Long tasks need no special handling** — if the child outruns the caller's run timeout, the framework auto-switches the spawn to an async fallback, returns a `switched_async` receipt, and Heartbeat injects the late result back. The caller never selects async; if it intentionally needs the result first, only the original caller may `POST resultUrl` to take it. Transport watchers use read-only GET and never take.
- **"A waits for B to finish before continuing"** — this is just the default behavior with `closure.target.type=inline`: call `/api/spawn2.0` and block. Closure is framework-verified. Details in `console-principles.md` → Cross-Session Coordination Patterns and Spawn Closure.
...
curl -s -X POST http://localhost:3501/api/spawn2.0 \
  ...
  -d '{
    ...
    "client_request_id": "'"$(date +%F)"':'"$SM_SESSION_NAME"':business-knowledge:<topic>",
    "closure": { "kind": "message", "target": { "type": "inline" } }
  }'
```
...
Use `lark-cli` to send a direct Feishu notification within a business workflow. Resolve the recipient to an `open_id`, then address the user explicitly; files and images use paths relative to the current directory.

**Usage**:
```bash
lark-cli im +messages-send --as bot --user-id <ou_xxx> --text <plain text>
lark-cli im +messages-send --as bot --user-id <ou_xxx> --markdown <Markdown>
lark-cli im +messages-send --as bot --user-id <ou_xxx> --file ./<file>
lark-cli im +messages-send --as bot --user-id <ou_xxx> --image ./<image>
```

**Name matching**: Supports full name or the latter part of a name (e.g., "PERSON_REDACTED" matches "PERSON_REDACTED"). Reports an error when multiple people match; use a more precise name in that case.

**Registered users**: PERSON_REDACTED、PERSON_REDACTED、PERSON_REDACTED、PERSON_REDACTED、王环
...
- **Path**: `<LOCAL_HOME>/workspaces/business-knowledge/lingxing_help/`
...
ls "<LOCAL_HOME>/workspaces/business-knowledge/lingxing_help/" | grep -i <keyword>
# Or search content
grep -rl "<keyword>" "<LOCAL_HOME>/workspaces/business-knowledge/lingxing_help/"
```
...
4. [ ] Are the collaboration interfaces between Agents clearly defined? Which tasks are delegated via `/api/spawn2.0`?
5. [ ] What is the failure handling strategy? How to degrade when spawn returns an error or `switched_async` receipt?
