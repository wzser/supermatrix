...
GENERIC_PATTERNS = (
    ...
    ("lark_app_id", re.compile(r"\bcli_(?![xX]{12,}\b)[A-Za-z0-9]{12,}\b")),
...
PUBLIC_LARK_HOSTS = {
    ...
    for subdomain in ("open", "www", "accounts", "passport", "login", "docs", "support")
}

REMOTE_RESOURCE_PATTERNS = (
    (
        "bitable_field_or_view_id",
        re.compile(r"\b(?P<value>(?:fld|vew)[A-Za-z0-9]{6,})\b"),
    ),
    (
        "lark_tenant_host",
        re.compile(r"(?i)\b(?P<value>(?:[a-z0-9-]+\.)+(?:feishu\.cn|larksuite\.com))\b"),
    ),
    (
        "lark_resource_url",
        re.compile(
            r"(?i)https?://(?:[a-z0-9_-]+\.)*(?:feishu\.cn|larksuite\.com)/"
            r"(?:wiki|base|docx|docs|sheets)/(?P<value>[a-z0-9]{12,})\b"
        ),
    ),
    (
        "remote_resource_assignment",
        re.compile(
            r"(?i)\b\w*(?:base_?token|table_?id|node_?token|space_?id)\b['\"]?\s*[:=]\s*"
            r"(?:process\.env\.\w+\s*(?:\|\||\?\?)\s*|z\.string\(\)\.default\(\s*)?"
            r"(?P<quote>['\"])(?P<value>[a-z0-9]{16,})(?P=quote)"
        ),
    ),
)
...
AUTO_REDACTIONS = (
    ...
    ("lark_app_id", re.compile(r"\bcli_(?![xX]{12,}\b)[A-Za-z0-9]{12,}\b"), "cli_REDACTEDAPPID"),
...
def path_policy_hit(
    relative: str,
    allowed_artifact_patterns: Iterable[str],
    allowed_static_paths: Iterable[str] = (),
) -> str | None:
    pure = PurePosixPath(relative)
    if relative == ".env.example":
        return None
    denied_parts = {part.casefold() for part in pure.parts[:-1]} & DENIED_COMPONENTS
    if denied_parts:
        if not (denied_parts == {"data"} and pure.suffix == ".json" and relative in allowed_static_paths):
...
def apply_replacements(
    ...
    for detector, value, start, end in sorted(remote_resource_hits(text), key=lambda hit: hit[2], reverse=True):
        replacement = "YOUR_RESOURCE_ID"
        if detector == "lark_tenant_host":
            domain = "larksuite.com" if value.casefold().endswith("larksuite.com") else "feishu.cn"
            replacement = f"example.{domain}"
        elif detector == "bitable_field_or_view_id":
            replacement = f"{value[:3]}REDACTED{sha256_bytes(value.encode('utf-8'))[:16]}"
        text = text[:start] + replacement + text[end:]
        counts[f"pattern:{detector}"] += 1
...
def remote_resource_hits(text: str) -> list[tuple[str, str, int, int]]:
    hits: list[tuple[str, str, int, int]] = []
    for detector, pattern in REMOTE_RESOURCE_PATTERNS:
        for match in pattern.finditer(text):
            value = match.group("value")
            if is_placeholder(value):
                continue
            if detector == "lark_tenant_host" and value.casefold() in PUBLIC_LARK_HOSTS:
                continue
            hits.append((detector, value, *match.span("value")))
    return hits
...
def scan_text(relative: str, text: str, keywords: list[Keyword]) -> list[dict[str, Any]]:
    ...
    for detector, value, start, _ in remote_resource_hits(text):
        add(detector, value, start)
    for keyword in keywords:
        pattern = re.compile(re.escape(keyword.term), re.IGNORECASE if keyword.term.isascii() else 0)
        for match in pattern.finditer(text):
...
def build_snapshot(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if config.get("schema_version") != 1 or not isinstance(config.get("mappings"), list) or not config["mappings"]:
        raise ReleaseError("export config must use schema_version=1 and a nonempty mappings array")
    ...
    allowed_static_paths = [str(item) for item in config.get("allowed_static_paths", [])]
    ...
    # Validate every module before copying any files; a partial package is not a successful build.
    selections: dict[str, list[tuple[Path, str, str]]] = {}
    ...
    for mapping in config["mappings"]:
        ...
        if name in selections:
            raise ReleaseError(f"duplicate mapping name: {name}")
        required = mapping.get("required_files", [])
        if not isinstance(required, list) or any(
            not isinstance(path, str) or not path or path == "."
            or PurePosixPath(path).is_absolute() or ".." in PurePosixPath(path).parts
            or PurePosixPath(path).as_posix() != path or any(char in path for char in "*?[]\\")
            for path in required
        ):
            raise ReleaseError(f"mapping {name} required_files must contain exact relative paths")
        selected = selected_files(mapping, args.supermatrix_root, args.workspaces_root, public_base_root)
        if not selected:
            raise ReleaseError(f"mapping {name} selected no files")
        missing = set(required) - {relative for _, relative, _ in selected}
        if missing:
            raise ReleaseError(f"mapping {name} missing required files: {', '.join(sorted(missing))}")
        selections[name] = selected
        required_files[name] = sorted(set(required))
    ...
    for mapping_name, selected in selections.items():
        for source, source_relative, destination_relative in selected:
            ...
            policy_hit = path_policy_hit(destination_relative, allowed_artifacts, allowed_static_paths)
            if policy_hit:
            ...
            if not source.is_file():
                raise ReleaseError(f"selected source is not a regular file: {source}")
    ...
    scan = scan_tree(output, keywords, max_bytes, allowed_artifacts, allowed_static_paths)
    payload = {
        "schema_version": 1,
        "ok": scan["ok"],
...
def scan_tree(
    ...
    for path in sorted(root.rglob("*")):
        ...
        byte_count += size
        policy_hit = path_policy_hit(relative, allowed_artifacts, allowed_static_paths)
        if policy_hit:
...
def scan_existing(args: argparse.Namespace) -> dict[str, Any]:
    ...
    scan = scan_tree(args.root.resolve(), keywords, max_bytes, allowed_artifacts, allowed_static_paths)
    payload = {
        "schema_version": 1,
        "ok": scan["ok"],
