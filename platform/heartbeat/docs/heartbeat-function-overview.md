...
```bash
/Users/LOCAL_USER/SuperMatrixRuntime/workspaces/heartbeat/scripts/heartbeat-patrol
```
...
scripts/enqueue-heartbeat-todo \
  ...
  --batch-key <shared-batch-key> \
  --todo-type <type> \
  --expected-count <n>
```

`--batch-key` 和 `--expected-count` 适合 fan-out/fan-in 场景：例如一次 run spawn 出 15 个子 session，可以用同一个发起 run id 作为 `batch_key`，这样每条结果保留自己的 `source_ref`，heartbeat 仍会在同一批里汇总注入。
...
```text
base_token: EAQubOoVhaQfFjs7Sgpc9YL9nOf
...
| `SM_DB_PATH` | `/Users/LOCAL_USER/SuperMatrixRuntime/data/supermatrix.db` | SuperMatrix 主库 |
...
MINIMAX_API_KEY
/Users/LOCAL_USER/CodexSkills/smallmodel-manager/catalog/secrets.local.yaml
```
