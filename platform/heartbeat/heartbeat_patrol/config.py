...
def load_config() -> Config:
    runtime_root = Path(os.environ.get("SM_RUNTIME_ROOT", str(WORKSPACE / ".runtime"))).expanduser()
    ...
    return Config(
        ...
        state_db_path=Path(
            os.environ.get("HEARTBEAT_STATE_DB", str(runtime_root / "heartbeat" / "heartbeat.sqlite"))
        ),
        lark_cli=os.environ.get("SM_LARK_CLI_PATH", "lark-cli"),
        heartbeat_session=os.environ.get("HEARTBEAT_SESSION", "heartbeat"),
        todomaster_session=os.environ.get("HEARTBEAT_TODOMASTER_SESSION", "").strip(),
        controller_provider=os.environ.get("HEARTBEAT_CONTROLLER_PROVIDER", "spawn"),
        controller_model=os.environ.get("HEARTBEAT_CONTROLLER_MODEL", "gpt-5.4-mini"),
...
def _load_minimax_registry() -> dict[str, str]:
    configured_root = os.environ.get("SMALLMODEL_MANAGER_ROOT", "").strip()
