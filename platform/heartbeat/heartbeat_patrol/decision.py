...
def build_controller_prompt(packet: dict[str, Any], *, controller_model: str, escalation_model: str) -> str:
    ...
    return "\n".join(
        [
            ...
            "Use gpt-5.5 for every emitted child decision.",
            ...
            "Schema: {\"session\":\"<session>\",\"items\":[{\"logical_key\":\"<session>:<stable-key>\",\"severity\":\"info|warn|error\",\"decision\":\"skip|alert|spawn_collect|spawn_execute|escalate|user_resume\",\"reason\":\"<bounded reason>\",\"target_session\":\"<session>\",\"child_model\":\"gpt-5.5\",\"prompt\":\"<bounded child prompt or user-reply composer guidance>\"}]}",
