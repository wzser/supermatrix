...
class ProviderFailureEpisode:
    ...
    model: str
    effort: str
    human_attention_reported: bool


PROVIDER_EMPTY_COMPLETION_RE = re.compile(
    r"(?:provider\.filtered|filtered).{0,120}(?:empty\s+(?:completion|content|response)|no\s+content)"
    r"|empty\s+(?:completion|content|response)"
    r"|(?:completion|content|response).{0,80}(?:empty|blank)"
    r"|returned\s+empty\s+content",
    re.IGNORECASE | re.DOTALL,
)
HUMAN_ATTENTION_NEEDED_RE = re.compile(
    r"(?:human\s+attention\s+needed|needs\s+human\s+attention|人工(?:介入|处理|关注)|需要人工)",
    re.IGNORECASE,
)
HUMAN_ATTENTION_NOT_NEEDED_RE = re.compile(
    r"human\s+attention\s+needed\s*[:：-]\s*(?:no|false|none|not\s+needed|不需要|否)",
    re.IGNORECASE,
)
    ...
    *,
    ...
    if (
        ...
        if provider_episode is not None:
            reasons.append(
                f"{provider_episode.failure_class} occurrences={provider_episode.occurrences} "
                f"logical_key={provider_episode.logical_key}"
            )
...
def provider_empty_completion_episode(
    packet: dict[str, Any],
    *,
    now_ms: int,
    candidate_max_age_minutes: int = DEFAULT_CANDIDATE_MAX_AGE_MINUTES,
) -> ProviderFailureEpisode | None:
    runs = [run for run in packet.get("recent_runs") or [] if isinstance(run, dict)]
    latest_run = runs[0] if runs else None
    ...
    if not _within_candidate_window(_run_reference_time(latest_run), now_ms, candidate_max_age_minutes):
        return None
    if not _run_has_provider_empty_completion(latest_run):
        return None

    session = packet.get("session") if isinstance(packet.get("session"), dict) else {}
    session_name = str(session.get("name") or "unknown").strip() or "unknown"
    backend = str(session.get("backend") or "unknown").strip() or "unknown"
    model = str(session.get("model") or "unknown").strip() or "unknown"
    effort = str(session.get("effort") or "unknown").strip() or "unknown"
    occurrences = sum(
        1
        for run in runs
        if _within_candidate_window(_run_reference_time(run), now_ms, candidate_max_age_minutes)
        and _run_has_provider_empty_completion(run)
    )
    logical_key = ":".join(
        [
            _logical_key_part(session_name),
            "provider-empty-completion",
            _logical_key_part(backend),
            _logical_key_part(model),
            _logical_key_part(effort),
        ]
    )
    return ProviderFailureEpisode(
        session_name=session_name,
        failure_class="provider_filtered_empty_completion",
        logical_key=logical_key[:160],
        occurrences=occurrences,
        backend=backend,
        model=model,
        effort=effort,
        human_attention_reported=_packet_has_human_attention_child_result(packet),
    )
...
def _run_has_provider_empty_completion(run: dict[str, Any]) -> bool:
    status = str(run.get("status") or "").lower()
    if status not in RUN_CANDIDATE_STATUSES:
        return False
    text = "\n".join(
        str(value)
        for value in (run.get("error_message"), run.get("final_message"))
        if isinstance(value, str) and value.strip()
    )
    return provider_empty_completion_text(text)


def provider_empty_completion_text(text: str) -> bool:
    return isinstance(text, str) and bool(text.strip() and PROVIDER_EMPTY_COMPLETION_RE.search(text))


def _packet_has_human_attention_child_result(packet: dict[str, Any]) -> bool:
    for item in packet.get("recent_cross_session") or []:
        if not isinstance(item, dict):
            continue
        ...
        result_text = "\n".join(
            ...
            for value in (
                ...
                item.get("final_message"),
                item.get("error_message"),
            )
            if isinstance(value, str) and value.strip()
        )
        if "Heartbeat follow-up for `" not in prompt_text:
            continue
        if HUMAN_ATTENTION_NOT_NEEDED_RE.search(result_text):
            continue
        if HUMAN_ATTENTION_NEEDED_RE.search(result_text):
            return True
    return False


def _logical_key_part(value: str) -> str:
