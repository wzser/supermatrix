...
from .decision import (
    ...
    contains_runtime_config_mutation_intent,
    parse_decision,
)
from .prefilter import (
    NO_LOCAL_CANDIDATE_SIGNAL,
    DEFAULT_PROVIDER_LIMIT_PAUSE_MINUTES,
    ProviderFailureEpisode,
    has_user_confirmation_gate,
    provider_empty_completion_episode,
    provider_empty_completion_text,
...
from .todo_dedupe import problem_type_from_logical_key, problem_type_marker
...
SCHEDULER_SESSION_NAME = "scheduler"
RECOVERY_TODO_TYPES = {
...
PATROL_TODO_ACTIONS = {"alert", "escalate"}
PATROL_TODO_ACTION_TYPE = "patrol_todo"
USER_RESUME_MESSAGE_MAX_CHARS = 3000
USER_RESUME_FIXED_MESSAGE = (
    "请基于当前上下文继续处理仍未完成的事项，避免重复已完成的工作，并在完成后直接回报结果。"
)
USER_RESUME_FIXED_MESSAGE_EN = (
...
PROVIDER_FAILURE_ALERT_THRESHOLD = 2
READ_ONLY_RUNTIME_CONFIG_NO_MUTATION_CLAUSE = (
    "READ-ONLY/RUNTIME-CONFIG-NO-MUTATION (heartbeat code guard, not controller-provided): "
    "You may inspect and report evidence only. Do not run SQL writes or modify any SuperMatrix "
    "runtime DB. Do not change backend, model, effort, backend_session_id, "
    "session_runtime_settings, child_session_defaults, session defaults, scheduler tasks, "
    "catalog entries, or /backend state. If recovery would require any of these changes, stop "
    "and report human attention needed with the exact approval/path requirement."
)
SPAWN_EXECUTE_RUNTIME_CONFIG_NO_MUTATION_CLAUSE = (
    "RUNTIME-CONFIG-NO-MUTATION (heartbeat code guard, not controller-provided): "
    "This spawn_execute may only perform the explicit target-owned reversible action in the item "
    "prompt. It must not write the SuperMatrix runtime DB or change backend, model, effort, "
    "backend_session_id, session_runtime_settings, child_session_defaults, session defaults, "
    "scheduler tasks, catalog entries, or /backend state. Stop and report human attention needed "
    "if that is required."
)
...
def child_prompt(item: DecisionItem) -> str:
    if item.decision in {"spawn_collect", "escalate"}:
        mode_guard = READ_ONLY_RUNTIME_CONFIG_NO_MUTATION_CLAUSE
    elif item.decision == "spawn_execute":
        mode_guard = SPAWN_EXECUTE_RUNTIME_CONFIG_NO_MUTATION_CLAUSE
    else:
        mode_guard = ""
    guard_lines = [
        "No-cascade constraint (heartbeat code guard): Do not spawn other sessions, call scheduler, or delegate this task.",
    ]
    if mode_guard:
        guard_lines.append(mode_guard)
    return (
...
@dataclass
class PatrolTodoGate:
    existing_problem_types: set[str] = field(default_factory=set)
    seen_problem_types: set[str] = field(default_factory=set)
    lookup_error: str = ""

    def claim(self, problem_type: str) -> bool:
        if problem_type in self.seen_problem_types:
            return False
        self.seen_problem_types.add(problem_type)
        return True
...
class PatrolRunner:
    ...
    def run_once(self, *, session_names: list[str] | None = None) -> dict[str, Any]:
        ...
        stats = {
            ...
            "patrol_todos_submitted": 0,
            "patrol_todos_skipped_duplicate": 0,
        }
        errors: list[str] = []
    ...
    def _verify_injected_landings(self, *, patrol_id: str, stats: dict[str, int]) -> None:
        checker = getattr(self.reader, "session_todo_run_landed_since", None)
        ...
        for todo in todos:
            ...
            expected_prompt = str(todo.get("detail") or "")
            injected_ms = _iso_to_ms(todo.get("injected_at"))
            if injected_ms is None:
                continue
            landed = False
            try:
                landed = bool(checker(target_session, injected_ms, expected_prompt))
            except Exception:
            ...
            if landed:
                if self.state.mark_todos_landing_verified(
                    todo_ids=[todo_id],
                    detail="landing confirmed: matching synthetic message_run observed after injection",
                ):
                    ...
                    self.state.log_event(
                        ...
                        logical_key=logical_key,
                        status="completed",
                        summary="injected todo landed as a matching synthetic run in the target session",
                ...
                if self.state.mark_todos_landing_unconfirmed(
                    todo_ids=[todo_id],
                    detail=(
                        f"no matching synthetic message_run observed in target session within "
                    ...
                    self.state.log_event(
                        ...
                        logical_key=logical_key,
                        status="failed",
                        summary="injected todo showed no matching synthetic landing run before deadline",
                        trigger_source="todo_pool",
    ...
    def _reconcile_unrecovered_targets(self, *, patrol_id: str, stats: dict[str, int]) -> None:
        ...
        superseded = self.state.supersede_unrecovered_user_resume_handoffs_from_later_progress()
        for row in superseded:
            target_session = str(row.get("target_session") or "")
            logical_key = str(row.get("last_logical_key") or "")
            handoff_ids = [str(value) for value in row.get("handoff_ids") or []]
            self.state.log_event(
                patrol_id=patrol_id,
                event_type="unrecovered_handoff_superseded",
                target_session=target_session,
                logical_key=logical_key,
                decision="user_resume",
                status="superseded",
                summary=(
                    f"superseded handoffs={','.join(handoff_ids)} after different-key user_resume "
                    "and later target landing; exact business recovery is not inferred"
                ),
                trigger_source="unrecovered_target",
                trigger_cause="later_target_progress",
                trigger_location=target_session,
            )
            stats["unrecovered_handoffs_superseded"] += len(handoff_ids)
    ...
    def _process_session(
        ...
        try:
            ...
            provider_episode = provider_empty_completion_episode(
                packet,
                now_ms=packet["heartbeat_policy"]["now_ms"],
                candidate_max_age_minutes=candidate_max_age_minutes,
            )
            if provider_episode is not None:
                decision = self._guard_provider_failure_episode(
                    decision,
                    provider_episode,
                    patrol_id=patrol_id,
                )
        ...
        todo_gate = self._prepare_patrol_todo_gate(decision.items)
        ...
        for item in decision.items:
            try:
                ...
                if item.decision in PATROL_TODO_ACTIONS or contains_runtime_config_mutation_intent(
                    "\n".join((item.reason, item.prompt))
                ):
                    self._register_patrol_todo(item, patrol_id=patrol_id, stats=stats, gate=todo_gate)
                ...
                if item.decision in PATROL_TODO_ACTIONS | {"spawn_collect", "spawn_execute"}:
                    try:
                        self._register_patrol_todo(
                            item,
                            patrol_id=patrol_id,
                            stats=stats,
                            gate=todo_gate,
                        ...
                        errors.append(f"{item.target_session}:{item.logical_key}: patrol todo failed: {todo_exc}")
    ...
    def _register_patrol_todo(
        ...
        *,
        ...
        issue_key = item.logical_key
        problem_type = problem_type_from_logical_key(issue_key)
        if not gate.claim(problem_type):
            stats["patrol_todos_skipped_duplicate"] += 1
            self.state.log_event(
                patrol_id=patrol_id,
                event_type="patrol_todo_skipped_duplicate",
                target_session=item.target_session,
                logical_key=issue_key,
                decision=item.decision,
                child_model=item.child_model,
                status="skipped",
                summary=f"same problem type already considered in this patrol run: {problem_type}",
                trigger_source="patrol_unresolved_issue",
                trigger_cause="problem_type_same_patrol_duplicate",
                trigger_location=item.target_session,
            )
            return
        if problem_type in gate.existing_problem_types:
            stats["patrol_todos_skipped_duplicate"] += 1
            self.state.log_event(
                patrol_id=patrol_id,
                event_type="patrol_todo_skipped_existing_type",
                target_session=item.target_session,
                logical_key=issue_key,
                decision=item.decision,
                child_model=item.child_model,
                status="skipped",
                summary=f"same problem type already exists in the complete todolist: {problem_type}",
                trigger_source="patrol_unresolved_issue",
                trigger_cause="problem_type_full_table_duplicate_any_status",
                trigger_location=item.target_session,
            )
            return
        if gate.lookup_error:
            self.state.log_event(
                patrol_id=patrol_id,
                event_type="patrol_todo_skipped_dedupe_unavailable",
                target_session=item.target_session,
                logical_key=issue_key,
                decision=item.decision,
                child_model=item.child_model,
                status="failed",
                summary="full todolist lookup failed; fail closed without creating a new Todo",
                error=gate.lookup_error,
                trigger_source="patrol_unresolved_issue",
                trigger_cause="problem_type_full_table_lookup_failed",
                trigger_location=item.target_session,
            )
            return
        claimed = self.state.try_claim_action(
            action_type=PATROL_TODO_ACTION_TYPE,
            target_session=item.target_session,
            logical_key=issue_key,
        )
        if not claimed:
            ...
            self.state.log_event(
                ...
                child_model=item.child_model,
                status="skipped",
                summary="same unresolved patrol issue already has an in-flight todo claim",
                trigger_source="patrol_unresolved_issue",
                trigger_cause="stable_issue_key_duplicate",
                trigger_location=item.target_session,
            )
            return

        body = _patrol_todo_body(item)
        register = getattr(self.api, "register_agent_todo", None)
        if not callable(register):
            ...
            raise RuntimeError("api.register_agent_todo is unavailable")
        try:
            response = register(issue_key=issue_key, body=body)
        except Exception as exc:
            if _is_ambiguous_spawn_timeout(exc):
                self.state.finish_action(
                    action_type=PATROL_TODO_ACTION_TYPE,
                    target_session=item.target_session,
                    logical_key=issue_key,
                    status="submitted_async",
                    detail=f"spawn2.0 synchronous window timed out; framework async delivery continues: {exc}",
                )
                stats["patrol_todos_submitted"] += 1
                self.state.log_event(
                    patrol_id=patrol_id,
                    event_type="patrol_todo_submitted_async",
                    target_session=item.target_session,
                    logical_key=issue_key,
                    decision=item.decision,
                    child_model=item.child_model,
                    status="submitted_async",
                    summary="shared agent todo spawn timed out synchronously; framework async delivery continues",
                    error=str(exc),
                    trigger_source="patrol_unresolved_issue",
                    trigger_cause="spawn2_sync_timeout",
                    trigger_location=item.target_session,
                )
                return
            self.state.release_action_claim(
            ...
            self.state.log_event(
                patrol_id=patrol_id,
                event_type="patrol_todo_failed",
                target_session=item.target_session,
                logical_key=issue_key,
                decision=item.decision,
                child_model=item.child_model,
                status="failed",
                summary="shared agent todo registration failed",
                error=str(exc),
                trigger_source="patrol_unresolved_issue",
                trigger_cause="spawn2_registration_failed",
                trigger_location=item.target_session,
            )
            raise

        response_status = str(response.get("status") or "") if isinstance(response, dict) else ""
        status = "submitted_async" if response_status == "switched_async" else "submitted"
        self.state.finish_action(
            action_type=PATROL_TODO_ACTION_TYPE,
            target_session=item.target_session,
            logical_key=issue_key,
            status=status,
            detail=f"todomaster spawn accepted: {json.dumps(response, ensure_ascii=False, default=str)[:3500]}",
        )
        stats["patrol_todos_submitted"] += 1
        self.state.log_event(
            patrol_id=patrol_id,
            event_type="patrol_todo_submitted",
            target_session=item.target_session,
            logical_key=issue_key,
            decision=item.decision,
            child_model=item.child_model,
            status=status,
            summary="unresolved patrol issue submitted to todomaster as agent todo",
            trigger_source="patrol_unresolved_issue",
            trigger_cause="outside_current_patrol_recovery_scope",
            trigger_location=item.target_session,
        )

    def _prepare_patrol_todo_gate(self, items: list[DecisionItem]) -> "PatrolTodoGate":
        candidates = {
            problem_type_from_logical_key(item.logical_key)
            for item in items
            if _requires_patrol_todo(item)
        }
        candidates.discard("")
        lookup = getattr(self.api, "find_registered_todo_problem_types", None)
        if not candidates or not callable(lookup):
            return PatrolTodoGate()
        try:
            existing = lookup(problem_types=candidates)
            if not isinstance(existing, set):
                existing = set(existing or [])
            return PatrolTodoGate(existing_problem_types={str(value) for value in existing})
        except Exception as exc:
            return PatrolTodoGate(lookup_error=str(exc))
    ...
    def _handle_item(
        ...
        *,
        ...
        if item.decision == "alert":
            ...
            user_message = f"/next {user_message}"
        ...
        if item.decision in {"spawn_collect", "spawn_execute", "escalate"}:
            if contains_runtime_config_mutation_intent("\n".join((item.reason, item.prompt))):
                alert_item = _runtime_config_mutation_alert_item(item)
                self.state.log_event(
                    patrol_id=patrol_id,
                    event_type=f"{item.decision}_runtime_config_mutation_blocked",
                    target_session=item.target_session,
                    logical_key=item.logical_key,
                    decision=item.decision,
                    ...
                    status="blocked",
                    summary=(
                        f"{item.decision} requested runtime config or SuperMatrix runtime DB mutation; "
                        "downgraded to alert"
                    ),
                    trigger_source="decision_safety",
                    trigger_cause="runtime_config_mutation",
                    trigger_location=item.target_session,
                )
                self._handle_item(
                    alert_item,
    ...
    def _guard_provider_failure_episode(
        self,
        decision: PatrolDecision,
        episode: ProviderFailureEpisode,
        *,
        patrol_id: str,
    ) -> PatrolDecision:
        local_human_attention = self._provider_failure_has_local_child_attention(episode)
        requires_alert = (
            episode.occurrences >= PROVIDER_FAILURE_ALERT_THRESHOLD
            or episode.human_attention_reported
            or local_human_attention
        )
        if requires_alert:
            cause = (
                "prior_human_attention_needed"
                if episode.human_attention_reported or local_human_attention
                else "repeated_provider_empty_completion"
            )
            self.state.log_event(
                patrol_id=patrol_id,
                event_type="provider_failure_episode_downgraded_to_alert",
                target_session=episode.session_name,
                logical_key=episode.logical_key,
                decision="alert",
                child_model=SUPPORTED_BOUNDED_CHILD_MODEL,
                status="blocked",
                summary=(
                    f"class={episode.failure_class}; occurrences={episode.occurrences}; "
                    f"backend={episode.backend}; model={episode.model}; effort={episode.effort}; "
                    "mutation-capable heartbeat child spawn is forbidden"
                ),
                trigger_source="decision_safety",
                trigger_cause=cause,
                trigger_location=episode.session_name,
            )
            return PatrolDecision(
                session=decision.session,
                items=[_provider_failure_alert_item(episode)],
            )
        ...
        for item in decision.items:
            if item.decision == "skip":
                continue
            if item.decision in {"spawn_collect", "spawn_execute", "escalate", "user_resume"}:
                items.append(
                    replace(
                        item,
                        logical_key=episode.logical_key,
                        reason=(
                            f"{item.reason}; provider_failure_class={episode.failure_class}; "
                            f"occurrences={episode.occurrences}; stable_key={episode.logical_key}"
                        ),
                    )
                )
            else:
                items.append(item)
        return PatrolDecision(session=decision.session, items=items)

    def _provider_failure_has_local_child_attention(self, episode: ProviderFailureEpisode) -> bool:
        summaries = getattr(self.state, "completed_child_spawn_summaries_for_target", None)
        if not callable(summaries):
            return True
        try:
            rows = summaries(target_session=episode.session_name)
        except Exception:
            return True
        for row in rows:
            final_summary = str(row.get("final_summary") or "")
            if (
                provider_empty_completion_text(final_summary)
                and _spawn_final_message_has_terminal_blocker(final_summary)
            ):
                return True
        return False
    ...
    def _emit_unrecovered_escalation(
        ...
        *,
        ...
        handoff_claim = self.state.claim_unrecovered_escalation_handoff(
        ...
        handoff_id = str(handoff_claim["handoff_id"])
        if not handoff_claim["created"]:
            self.state.log_event(
                patrol_id=patrol_id,
                event_type="unrecovered_handoff_duplicate_episode_skipped",
                target_session=item.target_session,
                logical_key=item.logical_key,
                decision=item.decision,
                child_model=child_model,
                status="skipped",
                summary=(
                    f"episode already owns handoff_id={handoff_id}; "
                    "duplicate Console notification was not sent"
                ),
                trigger_source="unrecovered_target",
                trigger_cause="episode_handoff_already_claimed",
                trigger_location=item.target_session,
            )
            return None
    ...
    def _send_user_resume_for_item(
        ...
        *,
        ...
        try:
            message = _normalize_user_resume_message(_fixed_user_resume_message(item))
    ...
    def _drain_todo_if_idle(
        ...
        *,
        ...
        try:
            ...
            message = f"{message}\n\n参考编号：{_todo_delivery_reference(claim)}"
            self.api.send_user_message(chat_id, f"/next {message}")
        except Exception as exc:
...
def _fixed_user_resume_message(item: DecisionItem) -> str:
    context = "\n".join((item.reason, item.prompt))
    return (
        USER_RESUME_FIXED_MESSAGE
        if re.search(r"[\u4e00-\u9fff]", context)
        else USER_RESUME_FIXED_MESSAGE_EN
    )
...
def _spawn_final_message_has_terminal_blocker(message: str) -> bool:
    ...
    if _human_attention_is_affirmative(normalized):
        return True
    terminal_markers = (
        "unrecoverable",
        "not recoverable",
...
def _human_attention_is_affirmative(text: str) -> bool:
    for match in re.finditer(r"human attention needed\s*[:：-]\s*([^\n.;。；]+)", text, re.IGNORECASE):
        value = re.sub(r"^(?:[-*+•]\s*)+", "", match.group(1).strip().lower())
        if value.startswith(("no", "false", "none", "not needed")) or value in {"否", "不需要"}:
            continue
        return True
    return bool(re.search(r"\bneeds human attention\b|\brequires human attention\b|需要人工(?:介入|处理|关注)", text, re.IGNORECASE))
...
def _normalize_outgoing_user_message(
    ...
    *,
    ...
    if not isinstance(value, str):
        raise RuntimeError("outgoing user message must be a string")
    text = value.strip()
    ...
    if not text:
        raise RuntimeError("outgoing user message is empty")
...
def _trigger_cause_from_item(item: DecisionItem) -> str:
    ...
    if "provider_filtered_empty_completion" in text or "empty completion" in text:
        return "provider_empty_completion"
...
def _provider_failure_alert_item(episode: ProviderFailureEpisode) -> DecisionItem:
    prompt = (
        f"{episode.session_name} 连续 {episode.occurrences} 次遇到 provider.filtered empty completion"
        f"（backend={episode.backend}, model={episode.model}, effort={episode.effort}）。"
        "请只向用户说明：调整 backend/model/effort、backend_session_id、"
        "session_runtime_settings 或 child_session_defaults 必须先取得用户明确批准，"
        "并通过正式 /backend 或框架 owner 路径执行；不要自行修改运行配置或直写运行库。"
    )
    reason = (
        f"{episode.failure_class} repeated for {episode.session_name}; "
        f"occurrences={episode.occurrences}; human_attention_reported={episode.human_attention_reported}; "
        "runtime config mutation is outside heartbeat automatic authority"
    )
    return DecisionItem(
        logical_key=episode.logical_key,
        severity="error",
...
def _runtime_config_mutation_alert_item(item: DecisionItem) -> DecisionItem:
    prompt = (
        "这一步涉及运行配置或 SuperMatrix 运行库修改。请只向用户说明：调整 "
        "backend/model/effort、backend_session_id、session_runtime_settings 或 "
        "child_session_defaults 必须先取得用户明确批准，并通过正式 /backend 或框架 owner 路径执行；"
        "不要自行修改运行配置或直写运行库。"
    )
    return DecisionItem(
        logical_key=f"{item.target_session}:runtime-config-mutation-required",
        severity="error",
        decision="alert",
        reason=(
            f"{item.reason}; heartbeat blocked {item.decision} because runtime config or "
            "SuperMatrix runtime DB mutation was requested"
        ),
        target_session=item.target_session,
        child_model=SUPPORTED_BOUNDED_CHILD_MODEL,
        prompt=prompt,
...
def _todo_declares_no_action(todo: Any) -> bool:
    ...
    if not _closure_result_declares_no_action(result_text):
        return False
    if str(getattr(todo, "source_session", "") or "").strip() == SCHEDULER_SESSION_NAME:
        return _scheduler_write_closure_is_verified(result_text)
    return True


def _closure_result_declares_no_action(result_text: str) -> bool:
    if CLOSURE_NO_ACTION_RE.search(result_text):
        return True
    return any(
        receipt.get("SM_CLOSURE_ACTION") == "no_action"
        for receipt in _json_objects_in_text(result_text)
    )


def _json_objects_in_text(text: str) -> list[dict[str, Any]]:
    """Extract complete JSON objects from a closure result without guessing prose."""
...
def _scheduler_write_closure_is_verified(result_text: str) -> bool:
    """Require an unambiguous v2 task and matching mutation readback before clearing."""
    receipts = [receipt for receipt in _json_objects_in_text(result_text) if "status" in receipt]
    if len(receipts) != 1:
        return False
    receipt = receipts[0]
    ...
    task_id = receipt.get("task_id") or receipt.get("taskId")
    if not isinstance(task_id, str) or not task_id.strip():
        return False
    if receipt.get("unique_match_count") != 1:
        return False
    ...
    if (type(mutation_id) not in {int, str}) or (isinstance(mutation_id, str) and not mutation_id.strip()):
        return False
    if not isinstance(mutation.get("action"), str) or not mutation["action"].strip():
        return False
    mutation_task_id = mutation.get("taskId")
    if mutation_task_id is None:
        mutation_task_id = mutation.get("task_id")
    if not isinstance(mutation_task_id, str) or mutation_task_id.strip() != task_id.strip():
        return False
    return True
...
def _todo_delivery_reference(claim: Any) -> str:
    batch_key = str(getattr(claim, "batch_key", "") or "")
    if batch_key:
        identity = f"batch:{batch_key}"
    else:
        logical_keys = sorted(
            str(getattr(todo, "logical_key", "") or "")
            for todo in (getattr(claim, "todos", []) or [])
        )
        identity = "todos:" + "\0".join(logical_keys)
...
def _patrol_todo_body(item: DecisionItem) -> str:
    reason = str(item.reason).strip()
    issue_summary = re.split(r"[;；\n]", reason, maxsplit=1)[0].strip()
    problem_type = problem_type_from_logical_key(item.logical_key)
    if not issue_summary:
        issue_summary = "巡检发现一项仍未完成的问题"
    if not issue_summary.endswith(("。", ".", "！", "!", "？", "?")):
        issue_summary += "。"
    action = " ".join(str(item.prompt).strip().split())
    if action:
        action = f"2. 按当前处置要求推进：{action}"
    else:
        action = "2. 由对应 owner 给出明确处置并回传可核验结果。"
    return "\n".join(
        [
            "① 存在什么问题",
            problem_type_marker(problem_type),
            f"heartbeat 巡检发现：{issue_summary}该问题超出本轮可自动收口范围，若继续放着，原任务会保持停滞或风险继续存在。",
            "",
            "② 要做什么",
            "1. 由 heartbeat 跟进对应 owner，确认问题仍存在并按责任边界推进。",
            action,
            "3. 如果需要他人配合、跨 session 或排期处理，转给对应 owner；heartbeat 只跟进回执，不代改别人的域。",
            "",
            "③ 完成标准",
            "对应 owner 已完成处置并留下可核验回执，且后续 heartbeat 巡检不再把同一问题判为未解决。",
        ]
    )


def _requires_patrol_todo(item: DecisionItem) -> bool:
    return item.decision in PATROL_TODO_ACTIONS | {"spawn_collect", "spawn_execute"} or contains_runtime_config_mutation_intent(
        "\n".join((item.reason, item.prompt))
    )


def _is_ambiguous_spawn_timeout(exc: Exception) -> bool:
    text = str(exc).lower()
...
def build_default_runner(cfg: Config, *, state: HeartbeatState | None = None) -> PatrolRunner:
    ...
    return PatrolRunner(
        ...
        api=HeartbeatApi(
            ...
            todomaster_session=cfg.todomaster_session,
