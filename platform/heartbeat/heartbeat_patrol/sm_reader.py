...
class SuperMatrixReader:
    ...
    def session_todo_run_landed_since(
        self, session_name: str, since_ms: int, expected_prompt: str
    ) -> bool:
        """True only when the injected todo became a matching synthetic run."""
        if not session_name or not expected_prompt:
            return False
        legacy_prompt = f"Δ{expected_prompt}"
        with self._connect() as conn:
            row = conn.execute(
                """
                SELECT 1
                ...
                WHERE s.name = ?
                  AND mr.started_at >= ?
                  AND (
                    (mr.origin = 'framework_synthetic' AND mr.prompt = ?)
                    OR (mr.origin = 'lark_user_synthetic' AND mr.prompt = ?)
                  )
                LIMIT 1
                """,
                (session_name, since_ms, expected_prompt, legacy_prompt),
            ).fetchone()
