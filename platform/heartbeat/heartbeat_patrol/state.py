...
class HeartbeatState:
    ...
    def _init_schema(self) -> None:
        with self._connect() as conn:
            try:
                ...
                conn.execute(
                    """
                    CREATE INDEX IF NOT EXISTS idx_heartbeat_events_user_resume_sent_progress
                      ON heartbeat_events(target_session, created_at, event_id, logical_key)
                      WHERE event_type = 'user_resume_sent' AND status = 'sent'
                    """
                )
                conn.execute(
                    """
                    CREATE INDEX IF NOT EXISTS idx_heartbeat_events_todo_landing_verified_progress
                      ON heartbeat_events(target_session, created_at, event_id, logical_key)
                      WHERE event_type = 'todo_landing_verified' AND status = 'completed'
                    """
                )
                ...
                self._supersede_duplicate_unrecovered_handoff_episodes(conn)
                conn.execute(
                    """
                    CREATE UNIQUE INDEX IF NOT EXISTS idx_unrecovered_handoffs_episode_owner
                      ON unrecovered_escalation_handoffs(action_type, target_session, window_started_at)
                      WHERE is_episode_owner = 1
                    """
                )
                conn.execute("COMMIT")
            except Exception:
    ...
    def _supersede_duplicate_unrecovered_handoff_episodes(self, conn: sqlite3.Connection) -> None:
        """Keep historical rows while quieting repeated notifications for one episode."""
        episodes = conn.execute(
            """
            SELECT action_type, target_session, window_started_at
            FROM unrecovered_escalation_handoffs
            GROUP BY action_type, target_session, window_started_at
            HAVING COUNT(*) > 1
            ORDER BY window_started_at, target_session, action_type
            """
        ).fetchall()
        open_statuses = (
            "awaiting_delivery",
        ...
        for action_type, target_session, window_started_at in episodes:
            rows = conn.execute(
                """
                SELECT handoff_id
                FROM unrecovered_escalation_handoffs
                WHERE action_type = ? AND target_session = ? AND window_started_at = ?
                ORDER BY created_at ASC, handoff_id ASC
                """,
                (action_type, target_session, window_started_at),
            ).fetchall()
            if len(rows) < 2:
                continue
            canonical_handoff_id = str(rows[0][0])
            duplicate_handoff_ids = [str(row[0]) for row in rows[1:]]
            conn.execute(
                """
                UPDATE unrecovered_escalation_handoffs
                SET is_episode_owner = 0
                WHERE action_type = ? AND target_session = ? AND window_started_at = ?
                """,
                (action_type, target_session, window_started_at),
            )
            conn.execute(
                """
                UPDATE unrecovered_escalation_handoffs
                SET is_episode_owner = 1
                WHERE handoff_id = ?
                """,
                (canonical_handoff_id,),
            )
            placeholders = ",".join("?" for _ in duplicate_handoff_ids)
            ...
            conn.execute(
                ...
                SET status = 'superseded',
                    recovery_event_type = 'duplicate_episode_handoff',
                    recovery_summary = ?, recovery_at = ?, updated_at = ?
                ...
                (
                    (
                        "duplicate notification for the same unrecovered episode; "
                        f"canonical_handoff_id={canonical_handoff_id}; retained for audit; "
                        "exact business recovery is not inferred"
                    ),
                    now,
                    now,
                    *duplicate_handoff_ids,
                    *open_statuses,
                ),
            )
    ...
    def purge_old_events(self, *, retention_days: int, history_directory: Path | None = None) -> int:
        """Delete heartbeat_events older than the retention window. 0 disables purging."""
        if retention_days <= 0:
            return 0
        if history_directory is None:
            raise ValueError("history_directory is required before event TTL deletion")
        from .history import export_state_history

        export_state_history(state_db_path=self.path, canonical_dir=history_directory)
    ...
    def record_unrecovered_target(
        ...
        *,
        ...
        with self._connect() as conn:
            ...
            episode_already_handed_off = conn.execute(
                """
                SELECT 1
                FROM unrecovered_escalation_handoffs
                WHERE action_type = ?
            ...
            escalate = False
            if threshold > 0 and failure_count >= threshold and not episode_already_handed_off:
                if (
        ...
        with self._connect() as conn:
            ...
            rows = conn.execute(
                ...
                WHERE window_started_at <= ?
                  AND NOT EXISTS (
                    SELECT 1
                    FROM unrecovered_escalation_handoffs handoff
                    WHERE handoff.action_type = unrecovered_targets.action_type
                      AND handoff.target_session = unrecovered_targets.target_session
                      AND handoff.window_started_at = unrecovered_targets.window_started_at
                  )
                  AND (
    ...
    def claim_unrecovered_escalation_handoff(
        self,
        *,
        ...
        """Atomically claim the single durable handoff row for an unrecovered episode."""
        required = {
        ...
        with self._connect() as conn:
            cursor = conn.execute(
                ...
                INSERT INTO unrecovered_escalation_handoffs (
                  handoff_id, escalation_event_id, patrol_id, owner_session, ledger_ref,
                  action_type, target_session, logical_key,
                  failure_count, window_started_at, error, created_at, updated_at, is_episode_owner
                )
                VALUES (?, ?, ?, 'heartbeat', ?, ?, ?, ?, ?, ?, ?, ?, ?, 1)
            ...
            row = conn.execute(
                """
                SELECT handoff_id, escalation_event_id
                FROM unrecovered_escalation_handoffs
                WHERE action_type = ?
                  AND target_session = ?
                  AND window_started_at = ?
                  AND is_episode_owner = 1
                """,
                (action_type, target_session, window_started_at),
            ).fetchone()
        if row is None:
            raise RuntimeError(
                "escalation handoff episode owner was not recorded: "
                f"{action_type}:{target_session}:{window_started_at}"
            )
        return {
            "handoff_id": str(row[0]),
            "created": cursor.rowcount == 1 and str(row[1]) == escalation_event_id,
        }
    ...
    def record_unrecovered_escalation_handoff(
        ...
        *,
        ...
        """Compatibility wrapper returning the episode owner's durable handoff id."""
        claim = self.claim_unrecovered_escalation_handoff(
            escalation_event_id=escalation_event_id,
            patrol_id=patrol_id,
            action_type=action_type,
            target_session=target_session,
    ...
    def resolve_unrecovered_target(
        ...
        *,
        ...
        with self._connect() as conn:
            ...
            episode_window_started_at = (
                str(aggregate[0]) if aggregate is not None and aggregate_cursor.rowcount == 1 else None
            )
            handoff_ids = self._recoverable_handoff_ids(
                conn,
                action_type=action_type,
                target_session=target_session,
                logical_key=logical_key,
                window_started_at=episode_window_started_at,
            )
            if handoff_ids:
                ...
                conn.execute(
                    ...
                    WHERE handoff_id IN ({placeholders})
                      AND status IN (
                        'awaiting_delivery', 'awaiting_acceptance', 'notify_failed',
                        'legacy_unverifiable', 'superseded'
                      )
                    """,
                    (
                        recovery_status,
    ...
    def _recoverable_handoff_ids(
        ...
        *,
        ...
        """Resolve a current aggregate by episode, while keeping legacy exact-key behavior."""
        status_sql = (
            "'awaiting_delivery', 'awaiting_acceptance', 'notify_failed', "
            "'legacy_unverifiable', 'superseded'"
        )
            ...
            for target in targets:
                ...
                if cursor.rowcount:
                    handoff_ids = self._recoverable_handoff_ids(
                        ...
                        target_session=str(target["target_session"]),
                        logical_key=logical_key,
                        window_started_at=str(target["window_started_at"]),
                    )
                    if handoff_ids:
                        placeholders = ",".join("?" for _ in handoff_ids)
                        now = now_iso()
    ...
    def supersede_unrecovered_user_resume_handoffs_from_later_progress(
        self,
        *,
        limit: int = 500,
    ) -> list[dict[str, Any]]:
        """Quiet an old user-resume handoff after a different-key nudge and later target activity.

        This transitions only the human-notification ledger.  The exact-key unrecovered aggregate
        remains authoritative, so later target activity cannot be mistaken for business recovery.
        """
        open_statuses = ("awaiting_delivery", "awaiting_acceptance", "notify_failed", "legacy_unverifiable")
        with self._connect() as conn:
            conn.row_factory = sqlite3.Row
            targets = conn.execute(
                """
                SELECT action_type, target_session, last_logical_key, window_started_at, last_failed_at
                FROM unrecovered_targets
                WHERE action_type = 'user_resume'
                ORDER BY last_failed_at ASC
                LIMIT ?
                """,
                (max(1, limit),),
            ...
            for target in targets:
                ...
                landing = conn.execute(
                    ...
                    LIMIT 1
                    """,
                    (str(target["target_session"]), str(sent["created_at"])),
                ).fetchone()
                if landing is None:
                    continue
                handoff_rows = conn.execute(
                    ...
                    (
                        str(target["target_session"]),
                        str(target["window_started_at"]),
                        *open_statuses,
                    ),
                ).fetchall()
                handoff_ids = [str(row["handoff_id"]) for row in handoff_rows]
                if not handoff_ids:
                    continue
                summary = (
                    f"later different-key user_resume_sent event={sent['event_id']} "
                    f"logical_key={sent['logical_key']}; later target landing event={landing['event_id']} "
                    f"logical_key={landing['logical_key']}; exact business recovery is not inferred"
                )
                placeholders = ",".join("?" for _ in handoff_ids)
                now = now_iso()
                conn.execute(
                    f"""
                    UPDATE unrecovered_escalation_handoffs
                    SET status = 'superseded', recovery_event_id = ?,
                        recovery_event_type = 'later_target_progress', recovery_summary = ?,
                        recovery_at = ?, updated_at = ?
                    WHERE handoff_id IN ({placeholders})
                      AND status IN ({','.join('?' for _ in open_statuses)})
                    """,
                    (
                        str(landing["event_id"]),
                        summary[:4000],
                        str(landing["created_at"]),
                        now,
                        *handoff_ids,
                        *open_statuses,
                    ),
                ...
                row = dict(target)
                row["sent_event_id"] = str(sent["event_id"])
                row["landing_event_id"] = str(landing["event_id"])
                row["landing_created_at"] = str(landing["created_at"])
                row["handoff_ids"] = handoff_ids
                row["summary"] = summary
    ...
    def try_claim_todo_watch(
        self, *, target_session: str, stale_after_seconds: int, lease_id: str
    ) -> bool:
            ...
            try:
                conn.execute(
                    ...
                    INSERT INTO action_claims (
                      action_type, target_session, logical_key, status, created_at, detail
                    )
                    VALUES ('todo_watch', ?, 'watch', 'claimed', ?, ?)
                    """,
                    (target_session, now, lease_id),
                )
                return True
            except sqlite3.IntegrityError:
                cursor = conn.execute(
                    ...
                    WHERE action_type = 'todo_watch' AND target_session = ? AND logical_key = 'watch'
                      AND created_at < ?
                    """,
                    (now, lease_id, target_session, cutoff),
                )
    ...
    def release_todo_watch(self, target_session: str, *, lease_id: str | None = None) -> None:
        """Ensure that a target has no todo-watch claim.

        Watcher shutdown supplies its lease so a stale worker cannot delete a
        replacement claim.  External orphan recovery (for example localwatch)
        has no lease and intentionally clears the target singleton; the
        caller has already established that no watcher process is present.
        Missing claims are a successful no-op by design.
        """
        with self._connect() as conn:
            if lease_id is None:
                conn.execute(
                    """
                    DELETE FROM action_claims
                    WHERE action_type = 'todo_watch' AND target_session = ?
                ...
                conn.execute(
                    ...
                    DELETE FROM action_claims
                    WHERE action_type = 'todo_watch' AND target_session = ?
                      AND logical_key = 'watch' AND detail = ?
                    """,
                    (target_session, lease_id),
                )
    ...
    def record_self_spawn_closure(
        ...
        *,
        ...
        heartbeat 自己 spawn 的 child（controller/bounded follow-up）超出同步窗口后，框架会把
        结果按 spawn_closure 投回 target=heartbeat；2026-08-25 前的 composer closure 也可能晚到。
        heartbeat 不在心跳覆盖范围内，以前一律拒绝导致结果丢失 + 框架无限复读。现在按 comm id
        匹配 child_spawns 收口；匹配不到（controller 或历史 composer child 不进 child_spawns）也
        接受落事件，返回终态止住复读。
        """
        logical_key = logical_key.strip()
        if not logical_key:
        ...
        if matched:
            ...
            summary_parts.append(
                "no matching child_spawns row (controller or historical composer child, or already closed)"
            )
    ...
    def list_injected_todos_for_landing_check(
        ...
        *triggered* delivery, not proof a run *landed* in the owner session. Returns
        ...
        with self._connect() as conn:
            ...
            rows = conn.execute(
                """
                SELECT todo_id, target_session, logical_key, injected_at, detail
                FROM session_todos
                WHERE status = 'injected'
                  AND injected_at IS NOT NULL
                  AND injected_at <= ?
        ...
        with self._connect() as conn:
            ...
            if injected:
                conn.execute(
                    ...
                    WHERE todo_id IN ({placeholders}) AND status = 'claimed'
                    """,
                    (status, finished_at, detail, *todo_ids),
                )
            else:
                conn.execute(
                    f"""
    ...
    def completed_child_spawn_summaries_for_target(
        self, *, target_session: str, limit: int | None = None
    ) -> list[dict[str, Any]]:
        if not isinstance(target_session, str) or not target_session:
            return []
        query = """
            SELECT logical_key, final_summary, last_polled_at, created_at
            FROM child_spawns
            WHERE target_session = ?
              AND status IN ('completed', 'failed')
              AND final_summary != ''
            ORDER BY COALESCE(last_polled_at, created_at) DESC
            """
        params: tuple[Any, ...] = (target_session,)
        if limit is not None:
            bounded_limit = max(1, min(int(limit), 500))
            query = f"{query}\nLIMIT ?"
            params = (target_session, bounded_limit)
        with self._connect() as conn:
            conn.row_factory = sqlite3.Row
