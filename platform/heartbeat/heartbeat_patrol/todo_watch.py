...
from .history import open_enqueue_log_for_child
...
def run_todo_watch(
    *,
    ...
    try:
        ...
        state.release_todo_watch(target_session, lease_id=lease_id)
        state.log_event(
...
def ensure_todo_watch(
    *,
    ...
    env = os.environ.copy()
    env["HEARTBEAT_TRIGGER_POINTER_MODE"] = "1"
    env["HEARTBEAT_TRIGGER_ORIGINAL_LOG_PATH"] = str(log_path)
    try:
        output, writer_lease, _ = open_enqueue_log_for_child(log_path)
        try:
