...
class CompletionSummaryTest(unittest.TestCase):
    ...
    def test_write_completion_summary_also_writes_history_manifest_when_configured(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            ...
            summary = build_completion_summary(
                state=state,
                completion_id="attempt-history",
                patrol_id=patrol_id,
                status="completed",
                started_at="2026-08-20T01:00:00+00:00",
                finished_at="2026-08-20T01:00:02+00:00",
                stats={"coverage_scope": "targeted", "eligible_sessions": 1, "sessions_scanned": 1},
                errors=[],
                feishu_sync={},
                landing_deadline_seconds=3600,
            )

            manifest = write_completion_summary(
                summary=summary,
                directory=root / "completion",
                state=state,
                canonical_directory=root / "canonical",
                manifest_directory=root / "manifests",
                history_created_at="2026-08-20T02:00:00+00:00",
            )

            self.assertIsNotNone(manifest)
            assert manifest is not None
            self.assertIn(event_id, manifest["event_ids"])
            manifest_path = Path(manifest["manifest_path"])
            self.assertTrue(manifest_path.is_file())
            self.assertEqual(json.loads(manifest_path.read_text()), {
                key: value for key, value in manifest.items() if not key.startswith("manifest_")
            })
            pointer = json.loads((root / "completion" / "attempt-history.json").read_text())
            self.assertEqual(pointer["schema"], "heartbeat.completion-pointer/v1")
            self.assertNotIn("todos", pointer)
            self.assertEqual(json.loads((root / "completion" / "latest.json").read_text()), summary)
            from heartbeat_patrol.history import reconstruct_completion_pointer_bytes

            rebuilt = json.loads(reconstruct_completion_pointer_bytes(pointer))
