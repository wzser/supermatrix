...
class DecisionTest(unittest.TestCase):
    ...
    def test_bare_empty_array_normalizes_to_skip_decision(self) -> None:
        decision = parse_decision("[]", expected_session="scheduler")

        self.assertEqual(decision.session, "scheduler")
        self.assertEqual(decision.items, [])

    def test_noop_skip_item_with_null_child_model_is_dropped(self) -> None:
        payload = json.loads(raw_decision())
        payload["items"][0]["decision"] = "skip"
        payload["items"][0]["child_model"] = None
        payload["items"][0]["prompt"] = "No action."

        decision = parse_decision(json.dumps(payload), expected_session="scheduler")

        self.assertEqual(decision.items, [])

    def test_non_skip_item_with_null_child_model_still_fails(self) -> None:
        payload = json.loads(raw_decision())
        payload["items"][0]["child_model"] = None

        with self.assertRaisesRegex(DecisionError, "child_model"):
            parse_decision(json.dumps(payload), expected_session="scheduler")
    ...
    def test_unsafe_prompt_raises(self) -> None:
        unsafe_prompts = [
            ...
            "Switch backend to codex and set model to gpt-5.6-terra.",
            "Reroute mythos from kimi to codex.",
            "Change mythos from kimi/kimi-code/k3 to codex/gpt-5.6-terra/max.",
            "Change\nmythos from kimi/kimi-code/k3 to codex/gpt-5.6-terra/max.",
            "Set `backend` to codex.",
            "Directly modify the runtime database.",
            "Directly change runtime configuration.",
            "Directly modify <runtime-root>/data/supermatrix.db.",
            "Do not only report; directly change runtime configuration.",
            "Update session_runtime_settings directly.",
            "Write SuperMatrix runtime DB using direct SQL.",
            "sqlite3 supermatrix.db \"UPDATE sessions SET backend='codex'\"",
            "Set backend_session_id to an empty value.",
            "Modify child_session_defaults for mythos.",
            "把 mythos 后端切换成 codex，并更新 session_runtime_settings。",
        ]
    ...
    def test_safe_negative_prompt_parses(self) -> None:
        safe_prompts = [
            ...
            "Do not change mythos from kimi/kimi-code/k3 to codex/gpt-5.6-terra/max.",
            "Do not change backend to codex. Report evidence only.",
            "Do not switch backend or model; report evidence only.",
            "不得修改 session_runtime_settings；只读检查证据。",
        ]
    ...
    def test_prompt_contract(self) -> None:
        ...
        controller_model="MiniMax-M2.7",
            escalation_model="gpt-5.5",
        )

        self.assertIn("Return JSON only", prompt)
        self.assertIn("response root must always be a JSON object", prompt)
        self.assertIn('{"session":"scheduler","items":[]}', prompt)
        self.assertIn("Do not return skip items", prompt)
        self.assertIn("Never use null for child_model", prompt)
        ...
        self.assertIn("external file sharing", prompt)
        self.assertIn("already asked the human", prompt)
        self.assertIn("do not rely on alert to interrupt active work", prompt)
        self.assertIn("user_resume", prompt)
        self.assertIn("fixed pre-audited language template", prompt)
        self.assertNotIn("separate child composer", prompt)
        self.assertNotIn("composer guidance", prompt)
        self.assertIn("latest run is not running", prompt)
        self.assertIn("non-stale running run", prompt)
        self.assertIn("temporary provider or API rate limit", prompt)
        self.assertIn("Runtime config is outside heartbeat's automatic authority", prompt)
        self.assertIn("provider.filtered empty completion", prompt)
        self.assertIn("use alert only; do not spawn another investigation", prompt)
        ...
        self.assertIn("use alert instead of escalate", prompt)
        ...
        self.assertNotIn("Use gpt-5.4-mini for bounded collection or execution.", prompt)
        self.assertIn("Set child_model to gpt-5.5 on every emitted item", prompt)
        self.assertIn("user_resume does not spawn a child", prompt)
