...
class PrefilterTest(unittest.TestCase):
    ...
    def test_provider_empty_completion_episode_uses_stable_failure_class_key(self):
        packet = {
            "session": {
                "name": "mythos",
                "status": "idle",
            ...
            "recent_runs": [
                ...
                },
                {
                    ...
                    "status": "failed",
                    "started_at": 1_000,
                    "error_message": "empty completion from provider",
                },
            ],
            "recent_cross_session": [
                {
                    "prompt": "Heartbeat follow-up for `mythos:old-key`.\nReason: collect.",
                    "status": "completed",
                    "result_preview": "evidence found. human attention needed: yes.",
                }
            ],
        }

        episode = provider_empty_completion_episode(
            packet,
            now_ms=4_000,
        ...
        self.assertEqual(episode.logical_key, "mythos:provider-empty-completion:kimi:kimi-code:k3")
        self.assertEqual(episode.occurrences, 3)
        self.assertTrue(episode.human_attention_reported)
        self.assertTrue(should_check)
        self.assertIn("provider_filtered_empty_completion occurrences=3", "\n".join(reasons))

    def test_completed_run_final_message_about_empty_completion_does_not_start_episode(self):
        packet = {
            "session": {
                "name": "mythos",
                "status": "idle",
            ...
            "recent_runs": [
                {
                    ...
                    "status": "completed",
                    "started_at": 3_000,
                    "final_message": "I reviewed an earlier empty completion and no action is needed.",
                }
            ],
            "recent_cross_session": [],
        }
