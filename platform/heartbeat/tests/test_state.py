...
class HeartbeatStateTest(unittest.TestCase):
    ...
    def test_open_existing_releases_watch_claim_without_rerunning_schema_init(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertTrue(
                state.try_claim_todo_watch(
                    target_session="alpha", stale_after_seconds=3000, lease_id="open-existing"
                )
            )
            ...
            with patch.object(HeartbeatState, "_init_schema", side_effect=AssertionError("schema init called")):
                ...
                existing.release_todo_watch("alpha", lease_id="open-existing")

            self.assertFalse(state.todo_watch_claim_active("alpha", stale_after_seconds=3000))

    def test_orphan_watch_release_without_lease_is_idempotent(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            state = self.new_state(d)
            self.assertTrue(
                state.try_claim_todo_watch(
                    target_session="alpha", stale_after_seconds=3000, lease_id="orphan-watch"
                )
            )
    ...
    def test_schema_adds_bounded_later_progress_event_indexes(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            state = self.new_state(d)
            with sqlite3.connect(state.path) as conn:
                indexes = {
                    row[1]
                    for row in conn.execute("PRAGMA index_list(heartbeat_events)").fetchall()
                }
                sent_plan = " ".join(
                    str(row[3])
                    for row in conn.execute(
                        """
                        EXPLAIN QUERY PLAN
                        SELECT event_id, created_at, logical_key, summary
                        FROM heartbeat_events
                        WHERE target_session = 'alpha'
                          AND event_type = 'user_resume_sent'
                          AND status = 'sent'
                          AND created_at >= '2026-08-25T00:00:00+00:00'
                          AND logical_key != 'alpha:old-key'
            ...
            self.assertTrue(
                {
                    "idx_heartbeat_events_user_resume_sent_progress",
                    "idx_heartbeat_events_todo_landing_verified_progress",
                }.issubset(indexes)
            )
            self.assertIn("idx_heartbeat_events_user_resume_sent_progress", sent_plan)
            self.assertIn("idx_heartbeat_events_todo_landing_verified_progress", landing_plan)
    ...
    def test_purge_old_events_respects_retention_window(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            ...
            history_dir = Path(d) / "canonical"
            purged = state.purge_old_events(retention_days=30, history_directory=history_dir)
            ...
            segment_receipts = list((history_dir / "state" / "heartbeat_events").glob("**/*.segment.json"))
            self.assertEqual(len(segment_receipts), 1)
            segment = json.loads(segment_receipts[0].read_text())
            self.assertEqual(segment["schema_version"], "heartbeat.state-history-segment/v1")
            self.assertEqual(segment["table"], "heartbeat_events")
            self.assertEqual(segment["row_count"], 2)
            self.assertEqual(len(segment["sha256"]), 64)

    def test_event_ttl_refuses_to_delete_without_history_export_destination(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            state = self.new_state(d)
            state.log_event(event_type="session_prefilter_skip", status="skipped", target_session="alpha")

            with self.assertRaisesRegex(ValueError, "history_directory"):
                state.purge_old_events(retention_days=30)

    def test_event_ttl_keeps_events_when_the_history_export_itself_fails(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            state = self.new_state(d)
            state.log_event(event_type="session_prefilter_skip", status="skipped", target_session="alpha")
            old = (datetime.now(timezone.utc) - timedelta(days=40)).isoformat(timespec="seconds")
            with sqlite3.connect(state.path) as conn:
                conn.execute("UPDATE heartbeat_events SET created_at = ?", (old,))

            with patch(
                "heartbeat_patrol.history.export_state_history",
                side_effect=RuntimeError("state history export already running"),
            ):
                with self.assertRaisesRegex(RuntimeError, "state history export already running"):
                    state.purge_old_events(
                        retention_days=30,
                        history_directory=Path(d) / "canonical",
                    )

            with sqlite3.connect(state.path) as conn:
                remaining = int(conn.execute("SELECT COUNT(*) FROM heartbeat_events").fetchone()[0])
    ...
    def test_unrecovered_target_does_not_realert_an_existing_episode_after_interval(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            state = self.new_state(d)
            knobs = {"threshold": 3, "window_minutes": 1440, "reescalate_minutes": 720}
            for n in range(3):
                outcome = state.record_unrecovered_target(
                    action_type="user_resume",
                    target_session="alpha",
                    logical_key=f"alpha:resume-{n}",
                    error="send failed",
                    **knobs,
                )
            ...
            escalation_event_id = state.log_event(
                ...
                status="escalated",
                summary="three user resume failures",
                error="send failed",
            )
            state.record_unrecovered_escalation_handoff(
                escalation_event_id=escalation_event_id,
                patrol_id="patrol-first-alert",
                action_type="user_resume",
                target_session="alpha",
                logical_key="alpha:resume-2",
                failure_count=3,
            ...
            later = state.record_unrecovered_target(
                action_type="user_resume",
                target_session="alpha",
                logical_key="alpha:resume-3",
                error="send failed again",
                **knobs,
            )

            self.assertEqual(later["failure_count"], 4)
            self.assertFalse(later["escalate"])
    ...
    def test_episode_handoff_claim_is_atomic_across_concurrent_callers(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            state = self.new_state(d)
            ...
            event_ids = [
                state.log_event(
                    event_type="unrecovered_item_escalated",
                    target_session="alpha",
                    logical_key=f"alpha:concurrent-{index}",
                    decision="user_resume",
                    status="escalated",
                    summary="concurrent escalation",
            ...
            def claim(index: int) -> dict[str, object]:
                barrier.wait()
                return state.claim_unrecovered_escalation_handoff(
                    escalation_event_id=event_ids[index],
                    patrol_id=f"patrol-{index}",
                    action_type="user_resume",
                    target_session="alpha",
                    logical_key=f"alpha:concurrent-{index}",
            ...
            self.assertEqual(len({str(item["handoff_id"]) for item in claims}), 1)
            with sqlite3.connect(state.path) as conn:
                rows = conn.execute(
                    "SELECT handoff_id, is_episode_owner FROM unrecovered_escalation_handoffs"
                ).fetchall()
            self.assertEqual(len(rows), 1)
            self.assertEqual(rows[0][1], 1)

    def test_schema_init_supersedes_duplicate_handoffs_without_deleting_history(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            state = self.new_state(d)
            window_started_at = "2026-08-10T23:10:48+00:00"
            first_event_id = state.log_event(
                event_type="unrecovered_item_escalated",
                target_session="alpha",
                logical_key="alpha:resume-0",
                decision="user_resume",
                status="escalated",
                summary="same unresolved episode",
                error="unsafe composer text",
            )
            first_handoff_id = state.record_unrecovered_escalation_handoff(
                escalation_event_id=first_event_id,
                patrol_id="patrol-0",
                action_type="user_resume",
                target_session="alpha",
                logical_key="alpha:resume-0",
            ...
            duplicate_event_id = state.log_event(
                event_type="unrecovered_item_escalated",
                target_session="alpha",
                logical_key="alpha:resume-1",
                decision="user_resume",
                status="escalated",
                summary="historical duplicate episode",
                error="unsafe composer text",
            )
            duplicate_handoff_id = "ueh_historical_duplicate"
            handoff_ids = [first_handoff_id, duplicate_handoff_id]
            with sqlite3.connect(state.path) as conn:
                conn.execute("DROP INDEX idx_unrecovered_handoffs_episode_owner")
                conn.execute(
                    "UPDATE unrecovered_escalation_handoffs SET created_at = ?, updated_at = ? "
                    "WHERE handoff_id = ?",
                    ("2026-08-11T00:00:00+00:00", "2026-08-11T00:00:00+00:00", handoff_ids[0]),
                )
                conn.execute(
                    """
                    INSERT INTO unrecovered_escalation_handoffs (
                      handoff_id, escalation_event_id, patrol_id, owner_session, ledger_ref,
                      action_type, target_session, logical_key, failure_count, window_started_at,
                      error, created_at, notify_status, notify_message_id, notify_attempted_at,
                      status, updated_at, is_episode_owner
                    )
                    VALUES (?, ?, 'patrol-1', 'heartbeat', ?, 'user_resume', 'alpha',
                            'alpha:resume-1', 1, ?, 'unsafe composer text', ?, 'delivered',
                            'om_1', ?, 'awaiting_acceptance', ?, 1)
                    """,
                    (
                        duplicate_handoff_id,
                        duplicate_event_id,
                        f"unrecovered_escalation_handoffs/{duplicate_handoff_id}",
                        window_started_at,
            ...
            with sqlite3.connect(state.path) as conn:
                rows = conn.execute(
                    "SELECT handoff_id, status, recovery_event_type, recovery_summary, updated_at, "
                    "is_episode_owner "
                    "FROM unrecovered_escalation_handoffs ORDER BY created_at, handoff_id"
                ).fetchall()
            self.assertEqual(len(rows), 2)
            self.assertEqual(rows[0][0:3], (handoff_ids[0], "awaiting_acceptance", ""))
            self.assertEqual(rows[1][0:3], (handoff_ids[1], "superseded", "duplicate_episode_handoff"))
            self.assertIn(handoff_ids[0], rows[1][3])
            self.assertEqual((rows[0][5], rows[1][5]), (1, 0))
            migrated_updated_at = rows[1][4]
            ...
            with sqlite3.connect(state.path) as conn:
                ...
                duplicate_updated_at = conn.execute(
                    "SELECT updated_at FROM unrecovered_escalation_handoffs WHERE handoff_id = ?",
                    (handoff_ids[1],),
                ).fetchone()[0]
            self.assertEqual(row_count, 2)
            self.assertEqual(duplicate_updated_at, migrated_updated_at)

    def test_later_different_key_user_resume_and_landing_supersede_handoff_without_fake_recovery(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            state = self.new_state(d)
            outcome = state.record_unrecovered_target(
                action_type="user_resume",
            ...
            landing_event_id = state.log_event(
            ...
            superseded = state.supersede_unrecovered_user_resume_handoffs_from_later_progress()

            self.assertEqual(len(superseded), 1)
            self.assertEqual(superseded[0]["handoff_ids"], [handoff_id])
            self.assertEqual(superseded[0]["sent_event_id"], sent_event_id)
            self.assertEqual(superseded[0]["landing_event_id"], landing_event_id)
            with sqlite3.connect(state.path) as conn:
                aggregate_count = conn.execute("SELECT COUNT(*) FROM unrecovered_targets").fetchone()[0]
                handoff = conn.execute(
                    "SELECT status, recovery_event_id, recovery_event_type, recovery_summary "
                    "FROM unrecovered_escalation_handoffs WHERE handoff_id = ?",
                    (handoff_id,),
                ).fetchone()
            self.assertEqual(aggregate_count, 1)
            self.assertEqual(handoff[0:3], ("superseded", landing_event_id, "later_target_progress"))
            self.assertIn("exact business recovery is not inferred", handoff[3])

            exact_event_id = state.log_event(
                event_type="todo_landing_verified",
                target_session="alpha",
                logical_key="alpha:stale-resume",
                status="completed",
                summary="exact original resume landed",
            )
            resolved = state.resolve_unrecovered_target(
                action_type="user_resume",
                target_session="alpha",
                logical_key="alpha:stale-resume",
                recovery_status="recovered",
                recovery_event_id=exact_event_id,
                recovery_event_type="todo_landing_verified",
                recovery_summary="exact original resume landed",
            )

            self.assertTrue(resolved["aggregate_resolved"])
            with sqlite3.connect(state.path) as conn:
                final_status = conn.execute(
                    "SELECT status FROM unrecovered_escalation_handoffs WHERE handoff_id = ?",
                    (handoff_id,),
                ).fetchone()[0]
            self.assertEqual(final_status, "recovered")

    def test_exact_latest_key_recovery_closes_the_episode_handoff_after_key_churn(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            state = self.new_state(d)
            knobs = {"threshold": 1, "window_minutes": 360, "reescalate_minutes": 720}
            first = state.record_unrecovered_target(
                action_type="user_resume",
                target_session="alpha",
                logical_key="alpha:first-failed-key",
                error="user message transport failed",
                **knobs,
            )
            escalation_event_id = state.log_event(
                ...
                decision="user_resume",
                status="escalated",
                summary="first failure opened the episode handoff",
                error="user message transport failed",
            )
            handoff_id = state.record_unrecovered_escalation_handoff(
                escalation_event_id=escalation_event_id,
                patrol_id="patrol-first",
                action_type="user_resume",
                target_session="alpha",
                logical_key="alpha:first-failed-key",
                failure_count=1,
            ...
            second = state.record_unrecovered_target(
                action_type="user_resume",
                target_session="alpha",
                logical_key="alpha:latest-failed-key",
                error="user message transport failed again",
                **knobs,
            )
            self.assertEqual(second["window_started_at"], first["window_started_at"])
            ...
            recovery_event_id = state.log_event(
                ...
                logical_key="alpha:latest-failed-key",
                status="sent",
                summary="latest exact key was accepted",
            )

            outcome = state.resolve_unrecovered_target(
                action_type="user_resume",
                target_session="alpha",
                logical_key="alpha:latest-failed-key",
                ...
                recovery_summary="latest exact key was accepted",
            )

            self.assertTrue(outcome["aggregate_resolved"])
            self.assertEqual(outcome["handoff_ids"], [handoff_id])
            with sqlite3.connect(state.path) as conn:
                handoff = conn.execute(
                    "SELECT status, recovery_event_id, recovery_event_type "
                    "FROM unrecovered_escalation_handoffs WHERE handoff_id = ?",
                    (handoff_id,),
                ).fetchone()
            self.assertEqual(handoff, ("accepted", recovery_event_id, "user_resume_sent"))
    ...
    def test_existing_handoff_ledger_migrates_owner_and_ledger_reference(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            ...
            with sqlite3.connect(state.path) as conn:
                row = conn.execute(
                    "SELECT owner_session, ledger_ref, notify_message_id, status, is_episode_owner "
                    "FROM unrecovered_escalation_handoffs WHERE handoff_id = 'ueh_old_1'"
                ).fetchone()
                episode_owner_index = conn.execute(
                    "SELECT 1 FROM sqlite_master WHERE type = 'index' "
                    "AND name = 'idx_unrecovered_handoffs_episode_owner'"
                ).fetchone()
            self.assertEqual(
                row,
                (
            ...
            self.assertEqual(episode_owner_index, (1,))
    ...
    def _inject_todo(self, state, *, target_session: str, logical_key: str, injected_minutes_ago: int) -> str:
        ...
        with sqlite3.connect(state.path) as conn:
            conn.execute(
                ...
                SET status='injected', injected_at=?, finished_at=?, detail='do X'
                WHERE todo_id=?
                """,
    ...
    def test_injected_landing_check_window_and_transitions(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertEqual(next(row["detail"] for row in due if row["todo_id"] == ripe), "do X")
    ...
    def test_mark_todos_injected_keeps_full_landing_body(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            state = self.new_state(d)
            state.enqueue_todo(
                target_session="alpha",
                logical_key="alpha:long-task",
                message="long",
                batch_mode="single",
            )
            claim = state.claim_next_todo_batch(target_session="alpha")
            sent_body = f"{'x' * 5000}A"

            state.mark_todos_injected(
                todo_ids=[claim.todos[0].todo_id],
                detail=sent_body,
            )

            with sqlite3.connect(state.path) as conn:
                detail = conn.execute(
                    "SELECT detail FROM session_todos WHERE todo_id = ?",
                    (claim.todos[0].todo_id,),
                ).fetchone()[0]
            self.assertEqual(detail, sent_body)
