...
class TodoWatchLoopTest(unittest.TestCase):
    ...
    def test_fires_after_busy_to_idle_debounce_then_exits_drained(self):
        ...
        self.assertTrue(
            state.try_claim_todo_watch(
                target_session="alpha", stale_after_seconds=3000, lease_id="post-run-watch"
            )
        )
    ...
    def test_second_watcher_exits_already_active(self):
        ...
        self.assertTrue(
            state.try_claim_todo_watch(
                target_session="alpha",
                stale_after_seconds=3000,
                lease_id="foreign-watch",
            )
        )
        ...
        self.assertFalse(
            state.try_claim_todo_watch(
                target_session="alpha", stale_after_seconds=3000, lease_id="second-watch"
            )
        )

    def test_stale_watch_owner_cannot_release_replacement_lease(self):
        state = self.make_state()
        self.assertTrue(
            state.try_claim_todo_watch(
                target_session="alpha",
        ...
        stale_created_at = (datetime.now(timezone.utc) - timedelta(hours=2)).isoformat(timespec="seconds")
        with sqlite3.connect(state.path) as conn:
            conn.execute(
        ...
        state.release_todo_watch("alpha", lease_id="watch-a")
        self.assertFalse(
            state.try_claim_todo_watch(
                target_session="alpha",
                stale_after_seconds=3000,
        ...
        state.release_todo_watch("alpha", lease_id="watch-b")
        self.assertTrue(
            state.try_claim_todo_watch(
                target_session="alpha",
                stale_after_seconds=3000,
...
class TodoWatchStateTest(unittest.TestCase):
    ...
    def test_claim_is_exclusive_until_released(self):
        ...
        self.assertTrue(
            state.try_claim_todo_watch(
                target_session="alpha", stale_after_seconds=3000, lease_id="watch-a"
            )
        )
        self.assertFalse(
            state.try_claim_todo_watch(
                target_session="alpha", stale_after_seconds=3000, lease_id="watch-b"
            )
        )
        ...
        self.assertTrue(
            state.try_claim_todo_watch(
                target_session="beta", stale_after_seconds=3000, lease_id="watch-beta"
            )
        )
        state.release_todo_watch("alpha", lease_id="watch-a")
        ...
        self.assertTrue(
            state.try_claim_todo_watch(
                target_session="alpha", stale_after_seconds=3000, lease_id="watch-c"
            )
        )
