...
## Prerequisites
- git CLI、codex bin（由安装环境提供）可用；`data/git-ledger.jsonl` 可追加；`registry/repo-policies/` 目录存在；文末「参数表」全部有值。
...
| R6 | NOISE | 命中 `references/daily-commit-ignore-policy.md` §2 allowlist 类目 | 若该 repo 的全部可见脏项均为未跟踪 Tier-0 直接清理项，可连同匹配的 ignored Tier-0 项处理（其它 ignored 路径不触碰）；仅对 `.DS_Store`、AppleDouble `._*`、`__pycache__/` 的直接子文件且 ≤10 MiB 的 `*.pyc` 过 secret/DB/artifact deny gate 后永久直删；每文件先 fsync intent、后 completion，缺任一即 localgit failure。其余 NOISE 才按既有 auto-remediate 边界追加窄 `.gitignore` 并重筛；不计入待提交集 |
...
- **处理**：按 `rules[]` 顺序做 glob 首条命中。`action` 三值语义：`commit`=并入待提交集；`ignore`=按 R6 边界追加 `.gitignore`；`keep_dirty`=仅表示 owner 明确要求暂留，必须带 `keep_until` 期限，期限前不重复通知但仍是待收敛状态。未命中 → disposition=`pending_owner`；首次 repo-local pending 必须按 `references/owner-decision-rubric.md` §D 组装 `spawn2.0` owner delegation：附 §A 段摘要、枚举动作 `commit|ignore|keep_dirty` 及后果、localgit 预判默认项、准确 repo path/fingerprint 与 decision receipt 命令。owner session 在准则内检查并执行 durable 动作；不得把现成技术裁决再交给用户。未获得 owner decision 的同一 `(repo, 首段目录)` 连续 pending ≥ `param.artifact_digest_days` 天 → 列入每周裁决 digest（digest 机制与升级线见 `SOP-daily-commit-skip-handling-*`）。答案不在枚举内不回写，追问一次仍无效按 rubric §C 升级。owner/用户裁决后回写 manifest 一条 rule（幂等键 = `(repo, pattern)`，重复写覆盖旧值并保留 `decided_at` 历史于 `superseded[]`）。
- **manifest 样本行**：`{"repo":"budiansha","trunk_branch":"main","rules":[{"pattern":"data/**","action":"ignore","decided_by":"budiansha","decided_at":"2026-08-19","source":"owner-delegation-2026-08-19","rubric":"v2","note":"ABA 周数据为可重建导出，不入库"}]}`
...
- **处理**：进入 L2 前先做 artifact 目录级分组（2026-09-02 起）：预判为 artifact 类的文件在同一顶层目录 ≥20 件时合成一条组 disposition（`file: "<dir>/ (grouped)"`、`grouped_count`、≤5 个样本文件，verdict=PENDING_OWNER 永不入提交集），不逐件耗 reviewer 预算；secrets / 数据库 / 机器噪音 / manifest 已裁决文件一律保持逐文件。L2 候选按 behavior-first 排序（行为目录 → 灰区 → artifact 尾），候选数超 `param.l2_max_files` 时 cap 截断只砍便宜尾巴，溢出件 disposition=`pending_owner`（理由注明 cap；巨量灰区堆走 Step 3 digest/manifest 收编，不产生 ETIMEDOUT 级巨型 prompt）。跑 `references/SOP-daily-commit-judgment-matrix-l2-review-prompt.md`（唯一 prompt 权威，禁止内联改写），喂 `{{repo_name}}` + `{{files_evidence_json}}`；输出 schema：`[{"file":"<path>","verdict":"SAFE|RISKY|OWNER","reason":"<引用 prompt 内枚举理由编号>"}]`。输出不合 schema → 同 prompt 重试 1 次；仍失败 → 该仓本轮记 localgit-owned issue（异常表 Case-1），不占 owner。单次调用超时 `param.reviewer_timeout_s` 秒，单仓两次调用上限。
...
- **下一步消费方**：Step 5。
- **失败回滚**：LLM 不可用时 R9 文件全部 disposition=`UNREVIEWED`（reason `reviewer_unavailable: <失败原文>`）留工作区，次日重排队首（Step 6 排序自然保证）；默认仍提交不依赖 L2、已独立判定的 L0/manifest SAFE 子集（含行为快车道），该仓记 localgit-owned。repo-policy 声明 `reviewer_outage.partial_commit=false` 的仓（如 yolo，todo-553）整仓 skip、不提交任何文件，skippedReason 保留 `blocked: must-review` 前缀并追加策略原因，等 reviewer 恢复后整批判定。
...
- **要解决的问题**：字母序 + 固定预算把尾部仓永久饿死（wendangwang 244 文件连续 8 天未被审）；现行活跃闸门把「不活跃+stale」仓塞进无人消费的 backlog，方向反了。
- **处理**：①处理顺序 = 连续容量挨饿天数降序优先（2026-09-02 起；streak ≥2 的仓排最前，定义与升级线见 `SOP-daily-commit-skip-handling-*` Step 2），其次 `last_reviewed_at` 升序（从 ledger 派生；从未审过的排最前，其间按最早脏龄升序，再同分按 repo 名）——被预算跳过的仓次日自动排到队首，无需独立 backlog 队列。②活跃闸门反转：session 在 `param.inflight_window_h` 小时内有非 daily-commit message run → 本轮 defer（reason=`in-flight`，避免与在途工作竞态）；**除此之外一律处理**——「不活跃 + stale」不再是跳过理由，恰是最安全的提交窗口。③主循环预算 `param.loop_budget_min` 分钟，且在进入可能消耗 L2 长调用前同时保留当前主审最坏的 `2 × param.reviewer_timeout_s` 与一整次 `2 × param.capacity_retry_reviewer_timeout_s + param.capacity_retry_start_reserve_s` 的 retry-tail 启动余量；当前 `15min - (2×120s + 2×45s+10s) = 9min20s` 到达 reserve boundary 后，剩余仓记 `time-budget` 且标 `localgit_owned=true`，先写原 ledger skip。④主循环后仅对③的仍脏仓按原顺序做一轮 `param.capacity_retry_budget_min` 分钟尾部重审；每仓最多一次，只有同时仍装得下 `2 × param.capacity_retry_reviewer_timeout_s + param.capacity_retry_start_reserve_s`，且整个主循环加尾部不超过 18 分钟时才启动。重审走同一 L0/L1/L2 管道；L2 每次调用改用短超时。重审成功/内容性 owner 判定才替换当日最终 result；容量失败保持 localgit-owned，绝不路由 owner。18 分钟之后保留 9 分钟给日志、Bitable、通知与 reload，以匹配 scheduler 的 27 分钟脚本 timeout。
- **产物**：本轮处理队列、预算截断点，以及 `data/daily-commits.log.capacity_retry`（candidates/attempted/reviewed/exhausted/budget_ms/elapsed_ms）与逐仓 `retry_attempted` / `retry_reviewed`。
...
| 3 R1 secrets 命中 | Step 2 R1 判定为真 | 模式/正则清单命中记录 | 不提交不 ignore；按 rubric §D 向 repo owner session 发可审计 delegation，附 §A 敏感内容处置段与默认项（同 fingerprint 只派一次） | repo owner session | owner 3 天无处理 → 周 digest 标红升级用户 |
...
| 5 session 治理读取失败 | `sqlite3 -readonly` 非 0，或 sessions 行不满足预期列数 | execFileSync / 行解析 throw | 中止本轮、写单行 log + notify，不发 owner delegation | Console（level=error） | 当日用 refs 的 SQLite 命令复测；连续 2 run → 找 supermatrix-root 恢复 runtime DB / SQLite 可读性 |
| 6 reviewer 宕机 vs 时间预算耗尽两条出口（todo-553） | reviewer 调用抛错/超时/输出非法（`review.ok=false`）；或主循环在 `canStartPrimaryLoopWork` 处触达 retry-tail reserve 边界 | `judgeRepo` reviewer 分支 / `daily-commit.ts` 时间预算闸 | 时间预算耗尽：该仓不进判定、不提交任何文件，全部 disposition 标 deferred，localgit-owned，走一轮 bounded capacity retry。reviewer 宕机：默认提交 L0/manifest SAFE 子集、灰区 UNREVIEWED 留工作区；该仓 manifest 声明 `reviewer_outage.partial_commit=false` 时整仓 skip，与时间预算出口同形。两者 skippedReason 均保留 `blocked: must-review dirty set could not be reviewed by localgit` 前缀；`PENDING_OWNER`（cap 溢出、artifact 分组）不参与该闸 | Console（source=localgit, level=warn），不新增通知路径 | 同 Case-1；某仓因 partial commit 切开「测试+契约」与「实现」致 HEAD 红 → 该仓 owner 声明 `reviewer_outage` 策略 |
...
- **不准发不带裁决准则与默认建议的 owner delegation**。**Why**：裸问题让几十个仓的 owner 各自自由决策，同类内容跨仓五花八门、不可审计不可比。**How to apply**：Step 3 owner delegation、digest 项与异常表 Case-3 告警一律按 `references/owner-decision-rubric.md` §D 模板组装（问题 + 枚举动作 + 准则摘要 + 预判默认项），缺任一要素不发。

## Inputs & Outputs 契约

- **Inputs**：repo 清单 — 来源 `sqlite3 -readonly "$SM_RUNTIME_ROOT/data/supermatrix.db"` 的 `sessions`，过滤 `status!='deleted' AND scope!='child' AND affiliated_to='first-principle' AND category NOT IN ('外部','员工')`，再取唯一的现存 git `workdir`；不读取 Bitable `Daily Commit` 列。样本行 `{"name":"example-repo","path":"/path/to/example-repo"}`。
- **Outputs**：①commit（可选）；②ledger 行（通常每仓每 run 一行；time-budget 尾部重审时保留原 skip 并追加第二条 retry 结果；Tier-0 永久删除每文件额外写 `purge_phase=intent` 与 `purge_phase=completed` 两条 fsync 行，任一缺失即 verifier fail）扩展字段样本：`{"run_id":"daily-2026-07-04T19:15:00.000Z","repo":"ad-adjust","action":"commit","head_after":"ab12cd3","per_file_dispositions":[{"file":"sop/INDEX.md","class":"R9","verdict":"SAFE","source":"l2-fresh"},{"file":"full-docs","class":"R9","verdict":"RISKY","reason":"E5","source":"l2-fresh"}],"decision_source":"l2-fresh"}`；③manifest 增量 rule（样本见 Step 3）。
...
- `references/owner-decision-rubric.md`：owner 裁决准则与 delegation payload 模板唯一权威（与分支巡检 SOP 共用；owner 按它判，不自由发挥）。
...
| param | 值 | 说明 |
|---|---|---|
| loop_budget_min | 15 | 主循环 wall-clock 预算（27 分钟 scheduler timeout 内留出 3 分钟尾部重审和 9 分钟收尾） |
| reviewer_timeout_s | 120 | 单次 codex 调用超时 |
| capacity_retry_budget_min | 3 | time-budget 仓的同进程单轮尾部重审预算 |
| capacity_retry_reviewer_timeout_s | 45 | 尾部重审中每次 codex 调用超时（单仓最多两次） |
| capacity_retry_start_reserve_s | 10 | 只有 `2×45+10` 秒完整装得下时才启动该仓重审 |
| direct_purge_discovery_budget_s | 60 | Tier-0 ignored-only 候选发现总预算；耗尽后只延后该类清理，不拖垮主循环 |
