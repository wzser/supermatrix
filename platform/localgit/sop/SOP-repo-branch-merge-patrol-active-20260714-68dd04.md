...
| C2 可 ff 合回 | 普通分支（非 `localgit/hold/*`），is-ancestor(trunk→branch) 为真（trunk 零独有 commit）且 trunk 的 worktree 中脏文件与 branch 变更文件集无交集 | trunk 未被任何 worktree checkout → `git fetch . <branchSha>:refs/heads/<trunk>`（2026-09-02 起；git 原生保证非 ff 拒绝、目标分支在任一 worktree 被 checkout 即拒绝，无 TOCTOU 窗口）；fetch 被拒且复验 trunk 已被 checkout → 在该 worktree 内 `git merge --ff-only`（含 show-current 复验）；其余失败 fail-closed。成功后按 C1 清理（只用 `git branch -d`，它原生拒绝删除任何 worktree checkout 的分支——`update-ref -d` 无此保护，实测禁用）；分支保留记 `branchRetained:true`。action=`fast_forwarded`（分支保留）或 `fast_forwarded_and_deleted`，前后 sha 记 evidence |
| C3 干净可合 | 双向都有独有 commit，且临时 detached worktree（`git worktree add --detach`）中 `git merge --no-commit --no-ff <branch>` 无冲突（试完即 `git worktree remove`） | **docs 快车道（2026-09-02 起）**：ahead ≤5 且变更文件全部命中安全 allowlist（路径在 `docs/` 或 `architecture/` 下，或 basename 为 `.md`；任意深度 `AGENTS.md`/`CLAUDE.md`/`LOCALGIT-GOVERNANCE.md` 及 `sop/`、`principles/`、`registry/`、`.github/` 一律排除，身份治理文档永不自动合）→ `param.auto_docs_merge=true` 时自动在临时 detached worktree 产 merge commit（`--no-ff`，parents 断言 = `[trunk, branch]`），trunk 按 C2 同款 `git fetch` 边界移动，action=`auto_merged_docs`，evidence 记 `mergeSha`+`parents`（回滚路径 `git revert -m 1 <mergeSha>`），随后 C1 清理。**其余 C3**：`param.auto_clean_merge=false` 维持只报告（用户书面批准翻 true 才自动合）；ageDays ≥2 → 走 C4 同款 notify gate 发 owner hint（fingerprint `patrol:<repo>:<branch>:C3`，rubric §D 模板，默认合回），不再干等周 digest |
...
| C5 常驻非 trunk | current ≠ trunk 的**连续巡检日**（按 branch-patrol.jsonl `recorded_at` UTC 日历日自当日起连续回溯；中断或回到 trunk 即断；换新分支不打断；首次见到计第 1 天，2026-09-02 起替代旧 ageDays 判定）≥ `param.parked_days` | 问 owner 一次（C4 同款 hint 通道，fingerprint `patrol:<repo>:<branch>:C5`），按 rubric §B（默认「合回」，仅 main 已事实废弃才「登记 trunk」）+ §D 模板附准则摘要与默认项；答案回写 manifest `trunk_branch`，此后本仓按新 trunk 巡检不再报 |
...
- **处理**：每仓每分支写一行 `data/branch-patrol.jsonl`，**幂等键 = `run_id` + `repo` + `branch`**。action 取值含 `report` / `deleted` / `fast_forwarded` / `fast_forwarded_and_deleted` / `auto_merged_docs` / `failed`；执行类 action 带 `shaBefore`/`shaAfter`，`auto_merged_docs` 另带 `mergeSha`+`parents`，分支保留带 `branchRetained:true`。巡检主程序完成后必须单独运行 `npm run branch-patrol-verify`，按 live git 重新核对每条分支恰有一条 evidence；apply 模式的 C1/C2 还要验证 ref 已删除及 trunk SHA 与 evidence 一致，`fast_forwarded`/`auto_merged_docs` 核验 live trunk SHA；`auto_merged_docs` 还要验证 `mergeSha === shaAfter` 且真实 Git commit 父链与 evidence `parents`、`shaBefore`、branch head 一致；`fast_forwarded` 缺 `branchRetained` 或非 C1/C2 级的 `failed` 行由 verifier 报错。样本行：`{"run_id":"patrol-2026-07-14T04:00:00.000Z","repo":"amz-sql","branch":"feat/aba-loader-v2","class":"C3","action":"report","age_days":21,"ahead_behind":"2 5","dry_run_conflicts":[],"sha_before":"ab12cd3","sha_after":"ab12cd3","routed_to":"digest"}`
...
- **Inputs**：repo 清单（同判定矩阵 SOP 样本）；触发事件样本：scheduler v2 任务 `localgit-branch-patrol`（`cron: 0 4 * * *`，type=script，command=`env LOCALGIT_BRANCH_PATROL_MODE=apply bash "$LOCALGIT_REPO_ROOT/scripts/run-branch-patrol.sh"`，cwd=`$LOCALGIT_REPO_ROOT`，timeout=1200000）。scheduler success 只证脚本被触发；业务完成只认独立 verifier 退出 0。
...
| auto_clean_merge | false | 非 docs 的 C3 只报告；用户书面批准后翻 true |
| auto_docs_merge | true | C3 docs 快车道（ahead≤5 且变更文件全部命中 docs 安全 allowlist，见 C3 行）自动合；`LOCALGIT_BRANCH_PATROL_AUTO_DOCS_MERGE=false` 关闭 |
| trunk_advance | 按仓政策 | `registry/repo-policies/<repo>.json` 的 `branch_policy.trunk_advance=owner_only`（2026-09-03 起，首例 ad-adjust）：仓级 hook（D-58）保留 trunk 推进给 owner，C2/C3 只分级+报告（C3 ageDays≥2 / C5 照常 owner hint），C1 清理非 trunk 动作照常执行 |
| worktree_stale_days | 30 | C0 报「疑似遗忘」阈值 |
| parked_days | 7 | C5 常驻判定：连续 off-trunk 巡检日 |
