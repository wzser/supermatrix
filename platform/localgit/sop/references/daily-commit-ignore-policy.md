...
#### 2.1 Tier-0 direct coding-garbage cleanup

Daily commit may permanently delete, without moving to Trash, only this exact
subset: `.DS_Store`, AppleDouble `._*`, and `*.pyc` files no larger than 10 MiB
only as direct children of `__pycache__/`. It also scans matching ignored files, so
an existing `.gitignore` does not prevent cleanup. This is deliberately not a
general cleanup of cache directories: `.coverage`, `.pytest_cache/`,
`.mypy_cache/`, `.ruff_cache/`, and `.turbo/` stay outside direct deletion.

All of the following gates are required: the repo has no visible dirty path
outside that subset; every visible candidate is untracked; matching ignored
candidates may be added but all other ignored paths remain untouched; every target is a
regular file inside the repo (no symlink); secret-name/content, database,
10 MiB size, artifact-first, and resolved-path containment checks all pass; and each deletion writes an
fsync'd intent then a completion with reclaimed bytes in `git-ledger.jsonl`.
An intent without completion, a failed target, or any ledger-write error fails
the daily run rather than reporting cleanup success. The scheduled path performs
at most one ignored-candidate Git scan per repo, capped at 60 seconds total
inside the daily loop budget, and reuses the normal status read for visible
candidates. This tier never deletes
`node_modules/`, `dist/`, `build/`, `.next/`, `.cache/`, `coverage/`, `tmp/`,
`temp/`, logs, databases, archives, media, `.coverage`, or any artifact-first /
owner-routed directory.
