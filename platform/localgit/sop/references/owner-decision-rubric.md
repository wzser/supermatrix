...
> 消费方：`SOP-daily-commit-judgment-matrix`（owner delegation、周 digest、R1/R4 owner 告警）与 `SOP-repo-branch-merge-patrol`（C3-C6 ask、周 digest）。
> 用途：**所有发给 repo owner session 的裁决任务必须附带本准则的相关段摘要 + localgit 按准则预判的默认项**。owner 在准则框架内做 repo-local 判断、只从枚举动作中选择；不同意默认项时附一句理由。目的 = 同类内容跨仓同判，裁决可审计、可比对。
> 版本：v2（2026-08-19）。修订本文件必须 bump 版本号；裁决消息与 manifest 回写引用的版本以本文件为准。
...
4. **拿不准** → `keep_dirty`（合法终态；必须记录具体原因与后续触发条件，之后想改随时改 manifest）。
...
每条 owner delegation / digest 行必须包含：

1. 一句话问题（哪个仓、哪个路径或分支、卡在哪）；
2. 枚举动作清单及各自后果（§A 或 §B 的答案空间，不给自由文本空间）；
3. 本准则**相关段摘要 ≤8 行** + 本文件绝对路径 + 版本号；
4. **localgit 按本准则预判的默认项 + 一句理由**。初次 repo-local 事项由 localgit 用 `spawn2.0` 定向交给 owner session：owner 必须检查并执行 `commit|ignore|keep_dirty` 的耐久动作，再写 decision receipt；不得把已有技术判断再抛回真人。只有 owner 无法化解、或周 digest 升级时，才由用户在同一枚举内代裁。

裁决回写 manifest 时在 rule 里记录 `"rubric":"v2"`；答案不在枚举内的回复不回写，追问一次后仍无效 → 按未响应走 §C 升级线。
