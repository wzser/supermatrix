...
const FP_DISCIPLINE_DOC = process.env.LOCALGIT_FP_DISCIPLINE_DOC?.trim();
...
export function buildFpIdentityDocEscalation(input: {
  ...
  const prompt = [
    ...
    FP_DISCIPLINE_DOC
      ...
      : `Read the installed first-principle module's "Session Identity Document Change Discipline", then inspect ${input.repo} identity-doc diff.`,
