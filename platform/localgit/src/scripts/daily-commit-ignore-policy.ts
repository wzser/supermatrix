...
- Tier-0 direct cleanup is narrower than auto-ignore: only when every visible dirty entry is Tier-0 may it permanently delete regular in-repo .DS_Store, AppleDouble ._*, and .pyc files no larger than 10 MiB only as direct children of __pycache__ after secret-name/content, database, size, and artifact-first gates. Matching ignored candidates may join, but other ignored paths remain untouched. Fsync an intent before each unlink and a completion with reclaimed bytes after; an audit failure fails the run. Never direct-delete symlinks, source, DBs, logs, build products, node_modules, .next, .cache, tmp/temp, .coverage, or artifact-first paths.
