...
import type { NotifyRequest } from "../notify/console.js";
...
export const OWNER_DECISION_RUBRIC_VERSION = "v2";
...
export const JUDGMENT_PARAMS = {
  loopBudgetMs: 15 * 60 * 1000,
  reviewerTimeoutMs: 120_000,
  capacityRetryBudgetMs: 3 * 60 * 1000,
  capacityRetryReviewerTimeoutMs: 45_000,
  capacityRetryStartReserveMs: 10_000,
  directPurgeDiscoveryBudgetMs: 60_000,
...
export type FileDisposition = {
  ...
  // Set only on directory-group entries (groupArtifactNoiseByDirectory): one row
  // summarizes N artifact files, carrying count + samples instead of N file rows.
...
export function isDatabasePath(filePath: string): boolean {
  const normalized = normalizePath(filePath);
  return DB_EXTS.has(extension(normalized)) || DB_EXT_RE.test(basename(normalized));
}
...
export function classifyFileL0(
  ...
  if (isDatabasePath(file)) return "deny_db";
...
// Reviewer-budget ordering for L2 candidates: must-review behavior dirs
// (src/scripts/tests/config/docs/…) first, gray-zone residue next, artifact-ish
// tails last, so a truncated review still covers the risky files (ad-adjust
// time-budget starvation incident). Stable: in-tier order is preserved.
export function prioritizeL2Candidates<T extends { file: string }>(candidates: T[]): T[] {
  const tier = (file: string): number => {
    const first = normalizePath(file).split("/")[0] ?? "";
    if (BEHAVIOR_FAST_PATH_DIRS.has(first)) return 0;
    if (isArtifactFirstPath(file) || noiseIgnoreEntryFor(file) !== null) return 2;
    return 1;
  };
  return [...candidates].sort((a, b) => tier(a.file) - tier(b.file));
}

// ---------------------------------------------------------------------------
// Artifact directory grouping — a huge artifact-only directory gets ONE group
// disposition instead of hundreds of per-file reviews that eat the loop budget
// (2026-08-23 ad-adjust: 324/331 dirty files were reports/ dry-run noise).
// ---------------------------------------------------------------------------

export const ARTIFACT_GROUP_MIN_FILES = 20;

export function groupArtifactNoiseByDirectory(
  files: string[],
  opts: { minGroupSize?: number; keepPerFile?: (file: string) => boolean } = {},
): { dispositions: FileDisposition[]; groupedFiles: Set<string>; groupMembers: Map<string, string[]> } {
  const minGroupSize = opts.minGroupSize ?? ARTIFACT_GROUP_MIN_FILES;
  const buckets = new Map<string, string[]>();
  for (const raw of files) {
    const file = normalizePath(raw);
    if (!isArtifactFirstPath(file)) continue;
    // Path-level deny classes (secrets, databases), allowlisted machine noise and
    // manifest-decided files keep their per-file judgment — grouping must never
    // widen or narrow their verdict (noise already has a deterministic IGNORE +
    // .gitignore remediation path).
    if (matchesSecretName(file) || isDatabasePath(file)) continue;
    if (noiseIgnoreEntryFor(file) !== null) continue;
    if (opts.keepPerFile?.(file)) continue;
  ...
  for (const dir of [...buckets.keys()].sort()) {
    ...
    for (const file of bucket) groupedFiles.add(file);
    // 组内文件逐文件审也只会得到同一 PENDING_OWNER verdict（无 manifest 规则的
    // artifact 类唯一去向就是等 owner 裁决）——分组不改变判定结果，只压缩
    // ledger 证据粒度；完整成员清单由调用方写进 run log（见 groupMembers）。
    groupMembers.set(dir, bucket);
    dispositions.push({
      file: `${dir}/ (grouped)`,
      class: "artifact",
      verdict: "PENDING_OWNER",
      reason: `grouped ${bucket.length} artifact files under ${dir}/ (samples: ${bucket.slice(0, 5).join(", ")}); awaiting owner decision per rubric §A`,
      source: "l0",
      grouped_count: bucket.length,
      sample_files: bucket.slice(0, 5),
    });
  }
  return { dispositions, groupedFiles, groupMembers };
}
...
export type RepoPolicy = {
  ...
  // Branch-patrol governance for this repo. trunk_advance=owner_only means a
  // repo-local hook (ad-adjust D-58) reserves trunk moves to the owner: patrol
  // classifies + reports (+ owner hint) but never executes C2/docs-C3 merges.
  branch_policy?: {
    trunk_advance?: "localgit" | "owner_only";
    decided_by?: string;
    decided_at?: string;
    source?: string;
    note?: string;
  };
  // Reviewer-outage governance (todo-553). partial_commit=false means that when the
  // L2 reviewer is down the whole repo skips instead of committing the L0/manifest
  // SAFE subset, so tests/contracts never land without the implementation they cover.
  reviewer_outage?: {
...
export function buildVerdictCache(entries: Iterable<GitLedgerEntry>): Map<string, CachedVerdict> {
...
export function deriveLastReviewedAt(entries: Iterable<GitLedgerEntry>): Map<string, number> {
...
export function orderReposForProcessing<T extends { name: string }>(
  ...
  capacitySkipDays: Map<string, number> = new Map(),
): T[] {
  return [...repos].sort((a, b) => {
    // Capacity-starved repos jump to the front of the queue; the remaining keys
    // keep the least-recently-reviewed order (SOP Step 6 ①).
    const ca = capacitySkipDays.get(a.name) ?? 0;
    const cb = capacitySkipDays.get(b.name) ?? 0;
...
// skip-handling SOP class not_reviewed_time_budget: both the "exceeded" and the
// "reached retry-tail reserve boundary" phrasings count as capacity starvation.
export const CAPACITY_SKIP_REASON_RE = /daily-commit time budget|reached retry-tail reserve boundary/i;

// Escalation thresholds (consecutive starved days): >=2 aggregates into one warn
// notification per run; >=3 additionally aggregates into one error notification
// plus an idempotent capacity-repair decision record per repo per day.
export const CAPACITY_WARN_DAYS = 2;
export const CAPACITY_REPAIR_REQUIRED_DAYS = 3;

// Aggregated capacity escalation shape: at most TWO Console notifications per
// run. A per-repo serial notify would burn one client timeout per repo during a
// notify outage and inflate the very time budget this escalation is about.
export function buildCapacityEscalationNotifyRequests(
  date: string,
  escalations: Array<{ repo: string; days: number }>,
): NotifyRequest[] {
  const starved = escalations
    .filter((entry) => entry.days >= CAPACITY_WARN_DAYS)
    .sort((a, b) => b.days - a.days || a.repo.localeCompare(b.repo));
  if (starved.length === 0) return [];

  const requests: NotifyRequest[] = [{
    source: "localgit",
    title: "daily-commit 容量告警",
    body: [
      "以下 repo 因 daily-commit 时间预算连续未能审查：",
      ...starved.map((entry) => `- ${entry.repo}：已连续 ${entry.days} 天`),
      "请修复 reviewer 容量或拆分审查范围。",
    ].join("\n"),
    level: "warn",
    metadata: { date, repos: starved.length, maxConsecutiveCapacitySkipDays: starved[0]?.days ?? 0 },
  }];

  const repairRequired = starved.filter((entry) => entry.days >= CAPACITY_REPAIR_REQUIRED_DAYS);
  if (repairRequired.length > 0) {
    requests.push({
      source: "localgit",
      title: "daily-commit 容量修复必需",
      body: [
        "以下 repo 已连续 ≥3 天因时间预算未能审查：",
        ...repairRequired.map((entry) => `- ${entry.repo}：已连续 ${entry.days} 天`),
        "它们已排在今晚队列最前；若明日仍因预算 skip，说明其脏集在单仓预算内审不完——本通知即当日人工修复信号。",
      ].join("\n"),
      level: "error",
      metadata: { date, repos: repairRequired.length, maxConsecutiveCapacitySkipDays: repairRequired[0]?.days ?? 0 },
    });
  }
  return requests;
}

// Count, per repo, the most recent run of consecutive calendar days whose LAST
// ledger record of the day is a time-budget skip. A same-day retry success means
// the budget held, so that day does not count. The live streak must reach today
// or yesterday (tonight's skip is not recorded yet at ordering time); older
// streaks are ancient history, not active starvation.
export function deriveConsecutiveCapacitySkipDays(
  entries: Iterable<GitLedgerEntry>,
  now: Date = new Date(),
): Map<string, number> {
  const DAY_MS = 24 * 60 * 60 * 1000;
  const dayKey = (d: Date): string => d.toISOString().slice(0, 10);

  // Slim per-day rows only — retaining whole entries here would pin the full
  // history in memory for the duration of the scan.
  const lastEntryByRepoDay = new Map<string, { operation?: string; skipped_reason?: string }>();
  for (const entry of entries) {
    if (!entry.repo || !entry.recorded_at) continue;
    const ts = Date.parse(entry.recorded_at);
    if (!Number.isFinite(ts)) continue;
    // Entries arrive chronologically, so the last write per repo+day wins.
    lastEntryByRepoDay.set(`${entry.repo} ${entry.recorded_at.slice(0, 10)}`, {
      operation: entry.operation,
      skipped_reason: entry.skipped_reason,
    });
  }

  const starvedDaysByRepo = new Map<string, Set<string>>();
  for (const [key, entry] of lastEntryByRepoDay) {
    if (entry.operation !== "skip") continue;
    if (!CAPACITY_SKIP_REASON_RE.test(entry.skipped_reason ?? "")) continue;
    const repo = key.slice(0, -11);
    const day = key.slice(-10);
    const days = starvedDaysByRepo.get(repo) ?? new Set<string>();
    days.add(day);
    starvedDaysByRepo.set(repo, days);
  }

  const streaks = new Map<string, number>();
  const today = dayKey(now);
  const yesterday = dayKey(new Date(now.getTime() - DAY_MS));
  for (const [repo, days] of starvedDaysByRepo) {
    let cursor: string | null = days.has(today) ? today : days.has(yesterday) ? yesterday : null;
    if (cursor === null) continue;
    let streak = 0;
    while (days.has(cursor)) {
      streak++;
      cursor = dayKey(new Date(Date.parse(`${cursor}T00:00:00.000Z`) - DAY_MS));
    }
    streaks.set(repo, streak);
  }
  return streaks;
}
...
export function summarizeLeftovers(dispositions: FileDisposition[]): string {
  ...
  for (const d of dispositions) {
    if (d.verdict === "SAFE" || d.verdict === "PURGED") continue;
