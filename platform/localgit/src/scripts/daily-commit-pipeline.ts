...
// execFileSync error messages lead with the full argv; chunked `git add` invocations carry
// hundreds of paths, so slicing err.message at 100 chars recorded only the command line and
// dropped the stderr that explained the failure (ad-adjust commit-failure incident, 2026-09).
export function formatExecFailure(err: unknown, maxLen = 2000): string {
  ...
  const detail = (stderr.trim() || message).replace(/\s+/g, " ").trim();
  ...
  try {
    ...
    return {
      ...
      result: {
        ...
        // Ledger contract: commit rows list the files actually committed, not the
        // whole dirty set — partial commits leave pending_owner files dirty by
        // design, and the verifier checks committed files are clean.
        changedFiles: [...files],
      },
    };
  } catch (err) {
    return {
      ...
      result: {
        ...
        message: "",
        filesChanged: files.length,
        skippedReason: `git commit failed: ${formatExecFailure(err)}`,
        branch,
        headBefore,
...
function runL2Review(
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    try {
      ...
      lastError = formatExecFailure(err);
      continue;
    }
...
export function judgeRepo(
  ...
  // Pre-pass (SOP Step 2 budget guard): a huge artifact-only directory collapses
  // into ONE group disposition. Per-file evidence collection (a git diff exec per
  // tracked file) on hundreds of artifact files is what starved ad-adjust's
  // 15-minute budget while source files waited unreviewed.
  const preGroup = groupArtifactNoiseByDirectory(changedFiles, {
    keepPerFile: (file) => matchPolicyRule(rules, file) !== null,
  });
  const ungroupedEntries = changedEntries.filter((entry) => !preGroup.groupedFiles.has(entry.file.replace(/\/+$/, "")));
  // 证据保全：组内文件逐文件审也只会得到同一 PENDING_OWNER verdict，分组不改变
  // 判定结果、只压缩 ledger 证据粒度（ledger 行保持 grouped_count+≤5 样本）；
  // 完整成员清单写进 run log（console 输出随 wrapper 进 data/run-logs）。
  for (const [dir, members] of preGroup.groupMembers) {
    console.log(`  artifact_group: ${dir}/ → ${members.length} files as one PENDING_OWNER ledger entry; full member list:`);
    for (const member of members) {
      console.log(`    ${member}`);
    }
  }

  const ungroupedFiles = ungroupedEntries.map((e) => e.file);
  const { sample, truncated } = prioritizeForContentScan(ungroupedFiles, JUDGMENT_PARAMS.contentScanMaxFiles);
  if (truncated > 0) {
  ...
  const evidences = ungroupedEntries.map((entry) => collectFileEvidence(repo.path, entry, sample.has(entry.file)));
  const dispositions: FileDisposition[] = [...preGroup.dispositions];
  ...
  if (l2Candidates.length > 0) {
    // Reviewer budget goes to must-review classes first (behavior dirs before
    // gray-zone tails), so the l2MaxFiles cap truncates the cheap tail, not the
    // risky head.
    const orderedCandidates = prioritizeL2Candidates(l2Candidates);
    ...
    const overflow = orderedCandidates.splice(JUDGMENT_PARAMS.l2MaxFiles);
    ...
    if (overflow.length > 0) {
      console.log(`  l2_candidate_cap: reviewing ${orderedCandidates.length}/${orderedCandidates.length + overflow.length}`);
    }
    const review = runL2Review(repo, orderedCandidates, deps);
    if (!review.ok) {
      for (const c of orderedCandidates) {
        dispositions.push({ file: c.file, class: "source", verdict: "UNREVIEWED", reason: `reviewer_unavailable: ${formatExecFailure(review.error, 500)}`, source: "l2-fresh" });
      }
      const reviewerFailure = buildMustReviewFailureReason(
        `codex reviewer likely stalled or returned invalid schema: ${formatExecFailure(review.error, 500)}`,
      );
      ...
      // Unless the repo policy refuses partial commits under outage (todo-553): then
      // the whole repo skips, same as the time-budget exit. PENDING_OWNER plays no part.
      const outagePolicy = policy?.reviewer_outage;
      const refusePartial = outagePolicy?.partial_commit === false;
      if (!refusePartial && buildCommitSet(dispositions).length > 0) {
      ...
      return {
        ...
        message: "",
        filesChanged: changedFiles.length,
        skippedReason: refusePartial
          ? `${reviewerFailure}. Repo policy refuses partial commit while the reviewer is down (reviewer_outage.partial_commit=false, source: ${outagePolicy?.source ?? "unknown"}): whole repo skipped so tests/contracts are not committed apart from their implementation; the full batch is judged once the reviewer recovers`
          : reviewerFailure,
        reviewerFailure,
        changedFiles,
        dirtyFingerprint: fingerprint,
