...
根节点（Wiki）：https://jxs9pwkdvwn.feishu.cn/wiki/DsfxwFqgCicGZ0koeDYcDb8Onwd

根节点下固定子节点：

| 本地文件 | 飞书节点 | URL |
|---------|---------|-----|
| `kb/CHARTER.md` | docx | https://jxs9pwkdvwn.feishu.cn/wiki/IUVcwVzZoiMseNkDTDdcz52UnFh |
| `kb/MAP.md` | docx | https://jxs9pwkdvwn.feishu.cn/wiki/C4FCwZMqoi0FRWkFBdncpLctnEe |
| `kb/sources.jsonl` | bitable "Sources" 表 | https://jxs9pwkdvwn.feishu.cn/wiki/DXwGwLJsiiJCEukHQoQctA4ontc |
...
**Base token**：`C6njbTVJeaiQVVsmOiycuXe8nxb`（Sources 与 Queries 共享同一 base，分别为不同 table）

**Sources 表**（`tblREDACTEDTABLEID`）——一行一 source，主键 `source_id`（= jsonl 的 `id`，形如 `S0001`）：
...
**Queries 表**（`tblREDACTEDTABLEID`）——一行一次咨询，行级唯一键 `timestamp`（微秒 ISO-8601，由 `scripts/log-query.py` 保证非空且唯一）：
...
- **bitable**（Sources / Queries）：走 wendangwang `feishu-sync-enqueue` 队列做 `bitable_rows_upsert`（accept-only，只入队不 inline drain，终态查 `feishu-sync-status`）——Sources 按 `source_id` 幂等、Queries 按 `timestamp` 幂等；**不再 delete-all + `record-batch-create`**（delete-all 会制造审计采样空窗 / 幻影漂移）。
- **concepts**：扫 `kb/concepts/*.md`，manifest 里没有的 slug 先 `wiki +node-create --obj-type docx`，然后 docs +update。

### 8.4 触发时机

1. 本地 `SOP-kb-capture-active-*.md` SOP 完成 Step 6（更新 MAP）后，紧接跑一次 `./scripts/sync-kb.sh`（推全量，包括 Queries 兜底）。
2. 手动修改 CHARTER / MAP 后立刻跑。
3. `SOP-kb-query-review-active-*.md` 周度 review SOP 完成后跑一次 `./scripts/sync-kb.sh queries`（每周日志兜底入飞书）。
...
- Queries 表**不做实时单条 push**——只在 review 周结时全量 `timestamp` 幂等 queue upsert（accept-only）。real-time 推送会让每次咨询多一次入队调用，成本不值，且与"飞书是只读副本"的语义不符。

---

## 9. 本地索引与审计（agent 检索层）

KB 本体是给人看的 markdown + jsonl。但 agent 在被 `/api/spawn2.0` 咨询时还需要两类机器可读产物：
...
- 2026-04-29 v9：飞书镜像扩出 Queries 表（§8）。`logs/queries/queries.jsonl` 走 `sync-kb.sh queries`（同 base `C6njbTVJeaiQVVsmOiycuXe8nxb`、新 table `tblREDACTEDTABLEID`），字段对应 `intent / kb_state / caller / prompt / concepts / sources / routing_target / answer_summary / notes`。同步语义和 Sources 一致（删完重灌、chunk 100），但只在周度 review 后兜底跑一次，不做 real-time 单条 push。
- 2026-07-11 v10：Queries 表飞书同步从 delete-all + batch-insert 改为 wendangwang 队列 `bitable_rows_upsert` 按 `timestamp` 幂等（`sync-kb.sh queries`，accept-only，只入队不 inline drain，终态查 `feishu-sync-status`）——消除 delete-all 造成的审计采样空窗（幻影漂移根因）。配套 `scripts/log-query.py` 默认时间戳升到微秒精度、并在 append 前对空 / 重复 timestamp fail closed（调用方自带 timestamp 同样校验），使 `timestamp` 成为稳定行级唯一键。live 生效待 owner ACK 资产合同 `unique_key=["timestamp"]`；query-log schema 变更已按约通报 codingmaster。
