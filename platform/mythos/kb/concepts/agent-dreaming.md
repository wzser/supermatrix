...
**2026-06-04 OpenAI 正式把『dreaming』命名搬到亿级用户产品上** [S0193]——这是 dreaming 这条路线首次在大规模商业部署里直接沿用 Letta 的术语：ChatGPT 的 memory 演进被官方分为三代（2024 Saved Memories → 2025 Dreaming V0 → 2026 Dreaming V3），定义『dreaming』为『background process that allows ChatGPT to learn from many conversations and synthesize ChatGPT's memory state』——与 Letta/OpenClaw 共识相同：异步、跨多对话、合成记忆状态。OpenAI 同时披露了一项工程数字：**Dreaming V3 服务 Free 用户的算力比之前下降约 5×**，这是这条路线第一次给出"通过架构优化、不靠模型小型化、就把 dreaming 成本压到 free tier 可负担"的公开证据，回应了本文 §8 列出的 compute 成本失控风险。架构形态对应 Letta 的 sleep-time compute（背景跨对话 synthesize），但走得更远：用户层暴露了一个『memory summary page』，让用户审阅、编辑、增删 dreaming 合成出的 memory state——这是首个直接给终端用户开放的 dreaming 控制面，OpenClaw 的 DREAMS.md 是开发者审查、OpenAI memory summary 是消费者审查，两者都贯彻了"反思必须可审查"原则（见 §7.2）。
...
- **Dreaming 产出的评估**：如何判断一次 dreaming "有价值"？当前两个公开指标：①test-time compute 降低 [S0044]；②OpenAI 的三轴评估框架——**carry-forward context / follow preferences / stay-current-over-time**——分别衡量"该记的有没有记住"/"该用上下文有没有用上"/"过时的有没有自动更新（时间推移类）" [S0193]。第三轴尤其关键：『去新加坡 → 已去过新加坡（July 2026）』的自动改写是 dreaming 区别于 saved-memories 的硬指标。但跨各家系统的可比基准仍缺。
