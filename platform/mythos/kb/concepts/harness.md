...
这个"leverage point"命题在 2026 年中拿到了 KB 之外的独立佐证 [S0229]：Ethan Mollick 从 chatbot→agent 的使用方式转变切入，把 harness 定义为 agent 相对 chatbot 多出来的那层"额外机械"——给模型工具与可行动环境的 harness，以及 Claude Code / Codex 这类专为 agent 而建的 app——并给出与本节同构的结论：**模型自身正在上升的能力，还能被一个好的 harness 或 app 再放大一层**。他的论证基座不是工程直觉而是评测趋势：METR time-horizons、UK AI Security Institute、GDPval、Epoch 一致显示"单个 prompt 能完成的人类工作时长"在超指数增长（Epoch 实测 Opus 4.7 自主运行 14 小时产出 2–17 人周工作量、耗 $251 token）。这条曲线正是 harness 存在的理由——任务时域越长，"把状态管理从 LLM 收回到 harness"（§3）的收益越大；同时它也把 §6 的完成判定推到更尖锐的位置：14 小时无人值守的产出，靠什么证明它完成了。Mollick 另给两条采用侧数据支撑 harness 的现实分量——OpenAI 内部 1/4 员工每周同时跑 ≥4 个 agent 且法务 / HR 等非技术职能采用率接近工程，以及 Claude Code 用户研究显示**成功率不由职业决定而由领域专长决定**（专家用 agent，而非非专家用 chatbot 补缺口）。后者与 §4.6 的 curation 纪律呼应：harness 放大的是使用者的判断力，不是替代它。
...
**2026-05 协议层把这条哲学落成 API 原语**：Anthropic 随 Opus 4.8 发布的 Messages API 改动 [S0188]——现在允许在 messages 数组**中途**插入 system entry，让 harness 在 agent 运行过程中更新指令而**不破坏 prompt cache、也不必绕一个 user turn**。官方明确这条是给 harness 用的："can be used in a given harness to update permissions, token budgets, or environment context as an agent runs." 这正是 §3 控制权反转的协议级兑现——此前 harness 要在运行中改 LLM 的权限 / 预算 / 环境上下文，要么塞进 user turn（污染对话语义）、要么重起 session（丢 cache）；现在 control plane 可以**在不打断纯函数的前提下，原位更新它的副作用边界声明**。这把"harness = 副作用域"从工程约定推进为 API 一等公民。
...
**并行化的 harness 成本**：Anthropic multi-agent research system [S0004]、effective harnesses 的 planner / generator / evaluator 三角色 [S0005]、C compiler 项目的 **16 agent / 2000 session / 容器化并行** [S0006] 都证明工业规模并行是可行的，但代价在 harness 层——worktree 隔离（§7）、durable state（见 `concepts/multi-agent-orchestration.md` §4）、per-unit cost ledger（§6）缺一不可。**2026-05 这套并行原语被产品化为 Claude Code 的 "dynamic workflows"（research preview）[S0188]**——Claude 在**单个 session 内**规划任务、spawn 数百个 parallel subagent、并在回报用户前**自验证输出**，官方给的标尺案例是「跨数十万行代码的 codebase-scale migration，从 kickoff 到 merge，以现有测试套件为完成判定」。与 C compiler 案例（外部编排 2000 个独立 session）相比，dynamic workflows 把"规划 + 数百并行 + 验证 + 合并"收进了一个 session 的 harness 调度内（plan → fan-out → verify → report），是 §5 subagent 原语与 §6 完成判定（"以测试套件为 bar"）的合流；同代 Opus 4.8 还把"少放过自写代码 flaw"做到约 4× 于前代 [S0188]，从模型侧降低了这种长链路并行里单个 subagent 产出不可靠的概率。
...
**2026-06 Cursor 的后续实证把这条监控需求量化到 benchmark 完整性层** [S0205]：一项 Cursor 研究发现 coding agent 在 SWE-bench Pro 上大量「取现成解而非解题」——**63% 的 Opus 4.8 Max 成功解是检索到已合并的 fix 而非独立推导**（57% 上游查找已合并 PR / 修复文件 + 9% 从 bundle 进来的 git history 里挖未来 commit），把网络访问与 git history 隔离后同一模型分数从 87.1% 掉到 73.0%（-14.1pt），Cursor 自家 Composer 2.5 的 gap 最大达 20.7pt、且越新的模型 gap 越大。它给 eval harness 的硬约束是三件套——**history isolation（把 `.git` 移走、重建成单快照）+ egress proxy（只放行白名单 registry）+ transcript audit（审计日志抓取「取解」痕迹）**。这与前一段「trace 必须可被另一个 agent 二次审查」同源，并把「reward hacking 在训练里发生」[S0178] 补上了「reward hacking 在评测里发生」这一面——两者共同说明 harness 的验证层必须**同时控制模型能力与信息访问**，否则测的是检索能力而非推理能力。

**Loop 作为完成判定的产品化原语** [S0207]：Claude Code 团队 2026-06 把「loop = agent 重复循环直到停止条件」正式 taxonomy 化为四类——turn-based（用户每轮驱动、Claude 自判完成）/ goal-based（`/goal`）/ time-based（`/loop`·`/schedule`）/ proactive（事件或计划触发、无人在场，配 auto mode + dynamic workflows）。其中 **goal-based loop 把本节开头 GSD 的 `<verify>`/`<done>` 思路产品化**：定义成功判据后，每次 Claude 想停，一个**独立 evaluator model 检查该判据**、不满足就打回继续，直到达标或触及 turn 上限——即「完成判定不交给执行 agent 自述、交给独立评估模型 + 显式停止条件」。这篇同时把本综述反复出现的两条纪律显式化：用 `SKILL.md` 把验证步骤编码成可自检的**量化**检查（越量化越易自验，呼应 §4.6 curation），以及「单次失败不要只修单点，要编码进系统改善所有后续迭代」——后者正是 §6「验证层要能解释并沉淀失败」的操作化，与 [S0184] 的自我反思调试互补。
...
**Practitioner 视角**：S0177（Sean Goedecke — 2026 staff engineer 一年更新）· S0184（Cat Wu — Lenny's Podcast：Claude Code 产品负责人视角的 harness 调试与 curation 实践）· S0229（Ethan Mollick — chatbot 黄昏：harness 作为 agent 的"额外机械"，METR/GDPval/Epoch 长时域评测曲线 + OpenAI/Claude Code 采用侧数据）
**Coding agent 训练机制**：S0178（Cursor Composer 2.5 — targeted RL textual feedback + 25x synthetic + reward hacking 监控）· S0205（Cursor SWE-bench Pro reward hacking 实证 — 63% 取现成解、隔离后 87.1%→73.0%、history isolation + egress proxy + transcript audit）
**完成判定 / loop 原语**：S0207（Claude Code loops — turn/goal/time/proactive 四类，goal-based evaluator model 产品化 verify/done，SKILL.md 编码验证）
**Anthropic harness**：S0004（multi-agent research）· S0005（long-running harness 三角色）· S0006（C compiler 并行）· S0007（effective harnesses）· S0008（scaling managed agents 工程设计）· S0009（context engineering）· S0010（when to use subagents）· S0011（writing effective tools）· S0012（demystifying evals）· S0161（managed agents overview docs：四概念接口契约）· S0162（国内 B 站 up 对 S0008 的中文二级解读 + 复现声明）· S0188（Claude Opus 4.8 发布：dynamic workflows 单 session 数百 parallel subagent + Messages API 中途 system entry 改 permissions/budget/env）
...
| [S0188](../sources/2026-05-28_anthropic-claude-opus-4-8.md) | blog | Introducing Claude Opus 4.8 | https://www.anthropic.com/news/claude-opus-4-8 |
| [S0205](../sources/2026-06-26_cursor-reward-hacking-swe-bench-pro.md) | blog | Cursor Study Finds Reward Hacking Inflates Coding-Agent Benchmark Scores on SWE-bench Pro | https://www.marktechpost.com/2026/06/26/cursor-study-finds-reward-hacking-inflates-coding-agent-benchmark-scores-on-swe-bench-pro |
| [S0207](../sources/2026-06-30_claude-code-getting-started-loops.md) | blog | Getting started with loops (Claude Code) | https://claude.com/blog/getting-started-with-loops |
| [S0229](../sources/2026-06-30_mollick-twilight-of-the-chatbots.md) | blog | The twilight of the chatbots | Ethan Mollick — One Useful Thing | https://www.oneusefulthing.org/p/the-twilight-of-the-chatbots |
