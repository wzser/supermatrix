...
**EverOS 给「markdown 检索弱」这个老问题一个工程化回应** [S0206]：EverMind 2026-06 开源的这个 Apache-2.0 memory runtime 明确以 **markdown 文件为唯一真相源**，但在其上叠了一层混合检索——SQLite 管状态与队列、LanceDB 管向量 + BM25 + scalar 过滤，等于把上面三条优点（可审查 / 可携带 / LLM 友好）保留、同时用 BM25+向量补上 markdown 天然缺的"大规模精确检索"。它还同时回应本综述另外两条线：(1) **多 agent 隔离**——检索可按 `user_id / agent_id / app_id / project_id / session_id` 过滤，正是 §9 讲的 `(tenant, user, scope)` 三元组治理的一种落地；(2) **procedural memory 自进化**——完成的任务沉淀为 Cases、离线蒸馏成可复用 Skills，把 §2 的 distillation / consolidation 从"事实压缩"扩到"技能提炼"，与 [S0167] Khaos Brain 的 experience card、[agent-dreaming §4](agent-dreaming.md) 的 KB Sleep 同脉。它把 markdown-as-memory 从"人审友好但检索弱"推进到"人审友好 + 混合检索 + scope 治理 + 技能自进化"的组合形态（vendor 报 LoCoMo 93.05% / LongMemEval 83.00% 等数字未经独立复核）。
...
**2026-06-04 OpenAI 把同一架构推到 ChatGPT 全量** [S0193]：memory 演进官方分为三代（2024 Saved Memories → 2025 Dreaming V0 → 2026 Dreaming V3），Dreaming V3 服务 Free 用户的算力比之前下降约 5×——这是『sleep-time compute / 异步反思』路线第一次在亿级用户产品上给出"算力可负担"的工程证据。配套用户层新增 **memory summary page**，让终端用户审阅 / 编辑 / 增删 dreaming 合成出的 memory state——OpenClaw 的 DREAMS.md 是开发者侧审查界面、OpenAI memory summary 是消费者侧审查界面，两套实现共同夯实"memory 必须可审查"原则（§7）。完整的 dreaming 流程对照见 [concepts/agent-dreaming.md](agent-dreaming.md) §2.5。
...
| [S0193](../sources/2026-06-04_openai-chatgpt-memory-dreaming.md) | blog | Dreaming: Better memory for a more helpful ChatGPT (OpenAI release) | https://openai.com/index/chatgpt-memory-dreaming/ |
| [S0206](../sources/2026-06-29_everos-markdown-agent-memory-runtime.md) | blog | Meet EverOS: Open Source Markdown-First Agent Memory Runtime (hybrid BM25+vector, self-evolving skills) | https://www.marktechpost.com/2026/06/29/meet-everos-an-open-source-markdown-first-agent-memory-runtime-with-hybrid-bm25-vector-retrieval-and-self-evolving-skills |
