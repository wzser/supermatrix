...
Google ADK 把多 agent 的常见 topology 系统化成**八条模式 + 原语集**[S0072] [S0073]，这是目前最完整的公开目录。**ADK 2.0.0 GA（2026-05-19）** [S0169] 把这套目录从 SDK 升级为 production-grade 引擎：model-agnostic 的 Multi-Agent Workflow Engine 支持 non-linear / conditional / cyclical 执行图（覆盖到 §6 generator-critic / loop），模块化抽象同时支持 parallel sub-agent worker / nested hierarchical team / resilient dynamic scheduling；Dynamic Agent Collaboration 主线提供 native inter-agent routing + 控制状态 handoff + context variable 跨 agent 传播。**ADK Go 2.0（2026-06-30）** [S0209] 把同一套 graph-workflow 引擎带到 Go SDK——节点+边组成执行图（typed function / agent / tool / join / dynamic / workflow 节点，边带 routing condition），同样覆盖 sequential / conditional router / fan-out-fan-in / first-class loop，是 Python 版 [S0169] 同一设计的双语言落地。两个值得单记的工程点：**HITL 作内建原语**（任意节点 `NewRequestInputEvent()` 暂停等人，handoff / re-entry 两种 resume 模式）、**durable state 可在进程重启后甚至跨不同 runtime 续跑**——把 §4 durable execution 的「crash 后接着跑」延伸到「换执行环境也能接着跑」，且统一 runtime 让单 agent 与整图共用同一执行模型。下面的分类吸收 ADK 的命名，跨对其他源补证据：
...
**产品化的混合形态（2026-08）**：xAI 的 Grok Bot [S0276] 把 hierarchical 与 peer messaging 揉进一个消费级产品——用户跑多个 Bot 并行、一个 chief-of-staff Bot 管各 lane 的 specialist（inbox / expenses / recruiting / bug fixes / ops），同时 Bot 之间可以**独立互发消息、在共享 thread 里传递 context、甚至被放进同一个 group chat 自行协调分工**，只在 judgment call 时才拉回人。两个工程点值得注意：(a) 每个 Bot 有自己的云端 computer（§8 工作空间隔离的消费级版本——人关机任务不停）；(b) workflow 学习走"跟看人做一遍存为 routine"，把 §7"人工编排压缩到只剩高层目标"落到了个体 workflow 层。这是迄今把"多 agent 编排 + 异步持久执行"包装成普通用户产品的最完整样本。
...
| [S0209](../sources/2026-06-30_adk-go-2-multi-agent.md) | blog | Build reliable multi-agent applications with ADK Go 2.0 (graph workflow engine + HITL + durable cross-runtime resume) | https://developers.googleblog.com/announcing-adk-go-20 |
| [S0276](../sources/2026-08-13_xai-grok-bot.md) | blog | Introducing Grok Bot (xAI：云端 computer 的 AI teammate，chief-of-staff 多 Bot 编排 + Bot 间互发消息/group chat 协调) | https://x.ai/news/introducing-grok-bot |
