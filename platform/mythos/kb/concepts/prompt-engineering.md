...
**GPT-5.6 专项更新（2026-07）**：GPT-5.6 Sol 的专项指南 [S0227] 沿 5.5 的"越强越抽象"轴继续，但把重心从"怎么写目标"移到"怎么删冗余"和"怎么划授权边界"。三处相对 5.5 的实质演进：(1) *Simplify prompts first*——把"精简 prompt"从建议升级为**首要方法论**并给出可操作循环：从一个能用的 prompt/工具集出发，一次移除一组指令/示例/工具再重跑同一份 eval；给出明确 trim/keep 清单（trim 重复规则、不改行为的 style/process 指令、模型已可靠执行的过程说明、与任务无关的工具；keep 用户可见 outcome、成功/停止条件、safety/business/evidence/permission 约束、上下文相关的 tool-routing、输出 schema）。这是本代少数带公开实验数字的改动——内部 coding-agent eval 里精简 system prompt 使评分升约 10–15%、总 token 降 41–66%、成本降 33–67%（OpenAI 自标 directional，需在自有任务上复验）。(2) *Autonomy & approval boundaries*——新增独立一节，主张用一段紧凑策略声明"每个请求授权到哪一层"：answer/explain/review/plan 类只读并报告、不落地改动；change/build/fix 类做 in-scope 本地改动 + 跑非破坏性校验、不先问；外部写 / 破坏性 / 购买 / 实质扩 scope 才要确认。配套要点：显式命名"安全本地动作"（读文件、看日志、改 in-scope 代码、跑测试）、每条规则只写一次（重复"ask first"会诱发对安全动作的多余审批）、长任务显式声明当前 work layer（research/design/implementation/review/coordination）避免模型静默跨层——这与 [harness](harness.md) 的 autonomy gate、[multi-agent-orchestration](multi-agent-orchestration.md) 的层级控制同源，但落在 prompt-level policy。(3) *Programmatic Tool Calling（PTC）*——把 [S0224] GPT-5.6 GA 页宣布的 PTC 能力落成 prompt 规范：PTC 只适用于 bounded record-reduction（过滤/join/排序/去重/聚合/批处理/可缩成紧凑 schema 的大结构结果），并明确"多调用/并行/依赖调用本身不构成用 PTC 的理由"；何时反而用 direct call（单次够、中间结果已小、每步结果改变下一步、需审批、要保留 citation/原生 artifact、步间需语义判断）逐条列出；红线是 `program_output` 与最终 assistant `message` 是两个独立输出、须分别测（程序可能返回对的记录但 message 漏字段/引用/caveat）。此外 5.6 默认比 5.5 更简洁，迁移时要重估"be concise"类旧指令（可能多余甚至过短）。整体看，5.6 指南是"按版本切片"策略从"教你写什么"走向"教你删什么 + 授权到哪"的一代。
...
- [S0227] OpenAI *Prompting guidance for GPT-5.6 Sol*——GPT-5.6 专项指南（2026-07）：simplify-prompts-first（带 +10-15% eval / -41-66% token / -33-67% cost 内部实测）/ autonomy & approval boundaries / Programmatic Tool Calling 规范 / 5.6 默认更简洁。是 [S0097][S0111] 的下一代延续，配套 [S0224] GA 页。
