...
type JobEntry = {
  ...
  cron: string;
  job: Cron;
  handler: (scheduledAt: number) => void;
};

export type CronEngine = {
  register(name: string, cronExpr: string, handler: (scheduledAt: number) => void): void;
...
export function createCronEngine(): CronEngine {
  ...
  return {
    register(name, cronExpr, handler) {
      let job: Cron;
      job = new Cron(cronExpr, { paused: false }, () => {
        handler(job.currentRun()?.getTime() ?? Date.now());
      });
