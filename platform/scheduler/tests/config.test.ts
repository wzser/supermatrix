...
describe("loadConfig", () => {
  it("defaults port to 3502 and uses scheduler-v2.db", () => {
    ...
    expect(c.smBaseUrl).toBe("http://127.0.0.1:3501");
    expect(c.mirrorEnqueueBin).toBe("feishu-sync-enqueue");
  });
  ...
  it("uses portable defaults when no runtime root is supplied", () => {
    const c = loadConfig({});
    expect(c.dbPath).toBe(`${process.cwd()}/data/scheduler-v2.db`);
    expect(c.smDbPath).toBe(`${process.cwd()}/data/supermatrix.db`);
  });

  it("allows the mirror queue executable to be supplied by the installation", () => {
    const c = loadConfig({ SCHEDULER_MIRROR_ENQUEUE_BIN: "/opt/bin/feishu-sync-enqueue" });
    expect(c.mirrorEnqueueBin).toBe("/opt/bin/feishu-sync-enqueue");
  });
