...
> Gatekeeper of the skill registry and canonical skill pool spanning both backends (Claude Code + Codex CLI). **Backend: claude** — `CLAUDE.md` is canonical; this `AGENTS.md` is kept in sync. Roles flip if the backend switches to codex in the future.
