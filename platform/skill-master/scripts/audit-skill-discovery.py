...
def _default_repo_file(relative_path: str, public_path: str) -> Path:
    configured = os.environ.get(
        "SKILL_MASTER_CONFIG" if relative_path.startswith("config/") else "SKILL_MASTER_INDEX"
    )
    if configured:
        return Path(os.path.expandvars(os.path.expanduser(configured)))
    private_path = ROOT / relative_path
    return private_path if private_path.exists() else ROOT / public_path


DEFAULT_CONFIG = _default_repo_file(
    "config/skill-retirement-audit.json", "docs/onboarding-v1-skill-discovery.json"
)
DEFAULT_INDEX = _default_repo_file(
    "skills/INDEX.md", "docs/onboarding-v1-skill-index.md"
)
...
def run_audit(
    *,
    ...
    index_is_allowlist = bool(config.get("index_is_allowlist", False))
    if index_is_allowlist:
        for name, targets in sorted(occurrences.items()):
            discovered_targets = {
                path: source
                for path, source in targets.items()
                if source["discovery_roots"]
            }
            if name not in active_managed and discovered_targets:
                findings.append({
                    "code": "skill_not_in_allowlist",
                    "name": name,
                    "paths": sorted(discovered_targets),
                })
    for skill_file in canonical_files:
        name = _frontmatter_name(skill_file)
        if name and name not in rows and not index_is_allowlist:
            findings.append({
