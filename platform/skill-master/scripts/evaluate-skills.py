...
PATROL_TODO_LEDGER = ROOT / "metrics" / "patrol-todos.jsonl"
REVIEWS_DIR = ROOT / "metrics" / "reviews"
LAST_RUN = ROOT / "metrics" / "last-run.txt"
FANOUT_STATE = ROOT / "metrics" / "eval-fanout-state.json"
INDEX = Path(os.environ.get("SKILL_MASTER_INDEX", "")) if os.environ.get("SKILL_MASTER_INDEX") else (
    ROOT / "skills" / "INDEX.md" if (ROOT / "skills" / "INDEX.md").exists()
    else ROOT / "docs" / "onboarding-v1-skill-index.md"
)
...
SPAWN_URL = f"{API_BASE}/api/spawn2.0"
SELF = "skill-master"
WINDOW_HOURS = 72
SPAWN_TIMEOUT_S = 45
FANOUT_PARALLEL = 12
...
PATROL_TODO_TIMEOUT_S = 45
PATROL_TODO_BASE_TOKEN = os.environ.get("SKILL_MASTER_TODO_BASE_TOKEN", "")
PATROL_TODO_TABLE_ID = os.environ.get("SKILL_MASTER_TODO_TABLE_ID", "")
PATROL_TODO_CONTENT_FIELD_ID = os.environ.get("SKILL_MASTER_TODO_CONTENT_FIELD_ID", "")
PATROL_TODO_SOURCE_FIELD_ID = os.environ.get("SKILL_MASTER_TODO_SOURCE_FIELD_ID", "")
PATROL_TODO_PAGE_SIZE = 200
...
def require_session_catalog():
    """Fail closed before fanout when the native session catalog is unavailable."""
    if not SESSION_CATALOG.is_file():
        raise RuntimeError(
            f"fanout unavailable: required session-catalog.json is missing at {SESSION_CATALOG}"
        )
    try:
        data = json.loads(SESSION_CATALOG.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        raise RuntimeError(
            f"fanout unavailable: required session-catalog.json is unreadable at {SESSION_CATALOG}"
        ) from exc
    sessions = data.get("sessions") if isinstance(data, dict) else None
    if not isinstance(sessions, list) or not sessions:
        raise RuntimeError(
            f"fanout unavailable: session-catalog.json has no non-empty sessions list at {SESSION_CATALOG}"
        )
    malformed = [
        index for index, record in enumerate(sessions)
        if not isinstance(record, dict)
        or not isinstance(record.get("name"), str)
        or not record["name"].strip()
        or not isinstance(record.get("backend"), str)
        or not record["backend"].strip()
    ]
    if malformed:
        indexes = ",".join(str(index) for index in malformed[:10])
        raise RuntimeError(
            f"fanout unavailable: session-catalog.json has malformed session entries at indexes {indexes}"
        )
    return data
...
def upsert(table_id, fields, record_id=None):
    if not APP_TOKEN or not table_id:
        raise RuntimeError(
            "private Feishu mirror configuration is not supplied; set the SKILL_MASTER_* mirror variables"
        )
    args = [
        "base", "+record-upsert",
...
INDEX_SEPARATOR = re.compile(r"^\|\s*:?-+:?\s*(?:\|\s*:?-+:?\s*)+\|$")
...
def patrol_todo_type_key(issue):
    """Return the stable problem-type key used by the patrol Todo gate.

    The lifecycle issue key includes the reporting session.  A patrol Todo
    must not: the same skill/title problem reported by several sessions is
    one type for this run and for the shared Todo table.
    """
    explicit = issue.get("ProblemType") or issue.get("IssueType") or issue.get("TodoTypeKey")
    if explicit:
        return normalize_key_part(explicit)
    return f"skill-master-patrol-type:{group_key(issue)}"
...
def load_index_owners():
    ...
    for line in INDEX.read_text(encoding="utf-8").splitlines():
        ...
        if not in_skills or not line.startswith("|"):
            continue
        if line.startswith("|------") or " Name " in line or INDEX_SEPARATOR.fullmatch(line):
            continue
...
def build_patrol_todo_content(issue, responsible_owner):
    """Create the fixed three-part human-readable body for an unresolved finding."""
    skill = issue.get("Skill") or "未标明 skill"
    session = issue.get("Session") or "未标明 session"
    title = issue.get("Title") or "未命名问题"
    description = issue.get("Description") or "巡检已记录问题，但反馈没有提供进一步描述。"
    return "\n".join([
        "① 存在什么问题：",
        f"skill「{skill}」在 session「{session}」的日常使用中出现「{title}」。具体表现是：{description}。",
        "如果不处理，后续使用仍会重复遇到同一问题，导致能力触发、执行或验收继续不稳定。",
        "",
        "② 要做什么：",
        "1. 先用本次巡检证据确认问题仍然存在，并确认责任 owner；",
        f"2. 若责任属于 skill-master（当前判定 owner={responsible_owner}），在本仓修复并补充可复验的测试或命令证据；",
        "3. 若责任属于其他 session、需要跨 session 协作或需要排期，使用 spawn2.0 转交准确 owner，跟踪其修复与回执；若复核证明不再复现，记录不再复现的证据；",
        "4. 复跑对应检查，将 fix/recheck/resolved 证据写回 issue lifecycle 与本巡检台账。",
        "",
        "③ 完成标准：",
        "问题已由责任 owner 修复并通过独立复验，或已证明不再复现并留下证据；issue lifecycle 与本 ToDo 均进入终态。",
    ])


def load_patrol_todo_ledger():
    latest = {}
    if not PATROL_TODO_LEDGER.exists():
        return latest
    with PATROL_TODO_LEDGER.open(encoding="utf-8") as f:
        for line in f:
            try:
                record = json.loads(line)
            except json.JSONDecodeError:
...
def load_remote_patrol_todos():
    """Read every row of the shared Todo table for source-side deduplication.

    No status filter is intentional: a completed, cancelled, idea, or
    in-progress row still blocks a new row of the same patrol problem type.
    """
    records = []
    offset = 0
    while True:
        response = lark(
            "base", "+record-list",
            "--as", "user",
            "--base-token", PATROL_TODO_BASE_TOKEN,
            "--table-id", PATROL_TODO_TABLE_ID,
            "--field-id", PATROL_TODO_CONTENT_FIELD_ID,
            "--field-id", PATROL_TODO_SOURCE_FIELD_ID,
            "--limit", str(PATROL_TODO_PAGE_SIZE),
            "--offset", str(offset),
            "--format", "json",
        )
        if not response or response.get("ok") is False:
            raise RuntimeError(f"shared Todo full-table lookup failed: {response}")
        data = response.get("data") or {}
        rows = data.get("data")
        record_ids = data.get("record_id_list")
        if not isinstance(rows, list) or not isinstance(record_ids, list) or len(rows) != len(record_ids):
            raise RuntimeError("shared Todo full-table lookup returned invalid rows")
        field_ids = data.get("field_id_list") or []
        try:
            content_index = field_ids.index(PATROL_TODO_CONTENT_FIELD_ID)
            source_index = field_ids.index(PATROL_TODO_SOURCE_FIELD_ID)
        except ValueError as exc:
            raise RuntimeError("shared Todo full-table lookup omitted the content/source field") from exc
        for record_id, row in zip(record_ids, rows):
            content = row[content_index] if isinstance(row, list) and content_index < len(row) else None
            source = row[source_index] if isinstance(row, list) and source_index < len(row) else None
            records.append({"record_id": record_id, "content": content or "", "todo_source": source or ""})
        if not data.get("has_more"):
            return records
        if not rows:
            raise RuntimeError("shared Todo full-table lookup did not advance pagination")
        offset += len(rows)


def remote_patrol_todo_matches(records, issue):
    """Find all-state remote rows matching this patrol problem type."""
    type_key = patrol_todo_type_key(issue)
    current_issue_key = issue_key(issue)
    title_pattern = re.compile(
        r"skill「(?P<skill>[^」]+)」.*?出现「(?P<title>[^」]+)」",
        re.DOTALL,
    )
    matches = []
    for record in records:
        content = "\n".join(str(record.get(field) or "") for field in ("content", "todo_source"))
        if (
            re.search(rf"问题类型键\s*[:：]\s*{re.escape(type_key)}(?:\s|$)", content)
            or re.search(rf"问题键\s*[:：]\s*{re.escape(current_issue_key)}(?:\s|$)", content)
        ):
            matches.append(record)
            continue
        legacy = title_pattern.search(content)
        if legacy and patrol_todo_type_key({"Skill": legacy.group("skill"), "Title": legacy.group("title")}) == type_key:
            matches.append(record)
    return matches


def append_patrol_todo_ledger(record):
    PATROL_TODO_LEDGER.parent.mkdir(parents=True, exist_ok=True)
    with PATROL_TODO_LEDGER.open("a", encoding="utf-8") as f:
        f.write(json.dumps(record, ensure_ascii=False, sort_keys=True) + "\n")


def patrol_todo_ledger_path():
    try:
        return str(PATROL_TODO_LEDGER.relative_to(ROOT))
    except ValueError:
        return str(PATROL_TODO_LEDGER)


def spawn_patrol_todo(issue, responsible_owner, timeout_s=PATROL_TODO_TIMEOUT_S):
    today = datetime.now(timezone.utc).date().isoformat()
    key = issue_key(issue)
    type_key = patrol_todo_type_key(issue)
    client_request_id = f"{today}:skill-master:patrol-todo:{type_key}"
    prompt = "\n".join([
        "[skill-master unresolved patrol finding]",
        "请按 agent Todo 路径登记一条共享 To-do，canonical owner 固定为 skill-master；不要代替 skill-master 执行修复。",
        "来源侧已先读取共享 Feishu Bitable「PRODUCT_REDACTED / todolist」全表且确认没有同问题类型记录；你仍须在登记前复核全表，不得按状态筛选。任意状态（在途、已完成、已取消、idea、其他终态）命中同一问题类型，都禁止新建，只返回既有 record_id；只有全表无命中才允许新建。",
        "同一巡检运行同一问题类型最多一条；本请求已由来源侧按类型聚合，不得拆成多条。",
        "以下正文必须按三段原样写入 Todo 的内容字段：",
        "",
        build_patrol_todo_content(issue, responsible_owner),
        "",
        f"问题键：{key}",
        f"问题类型键：{type_key}",
        f"稳定 client_request_id：{client_request_id}",
    ])
    payload = {
        "from": SELF,
        "target": "todomaster",
    ...
    try:
        ...
        data["_client_request_id"] = client_request_id
        if exc.code == 409 and data.get("duplicate"):
            return data, "duplicate_in_flight"
        return data, "failed"
    except Exception as exc:
        return {"error": str(exc), "_client_request_id": client_request_id}, "delivery_unknown"


def register_patrol_todos(issues):
    """Register at most one Todo per type, after an all-state full-table check."""
    open_keys = load_current_open_issue_keys() or set()
    by_type = {}
    for issue in issues:
        key = issue_key(issue)
        if key in open_keys:
            by_type.setdefault(patrol_todo_type_key(issue), []).append(issue)
    if not by_type:
        return {"candidates": 0, "created": 0, "followed": 0, "failed": 0, "coalesced": 0, "ledger": patrol_todo_ledger_path()}

    latest = load_patrol_todo_ledger()
    owners = load_index_owners()
    latest_by_type = {}
    for record in latest.values():
        type_key = record.get("todo_type_key")
        if not type_key and record.get("skill") and record.get("title"):
            type_key = patrol_todo_type_key(record)
        if type_key:
            latest_by_type[type_key] = record

    representatives = {}
    coalesced = 0
    failed = 0
    for type_key, candidates in sorted(by_type.items()):
        valid = [candidate for candidate in candidates if all(candidate.get(field) for field in ("Skill", "Session", "Title"))]
        invalid = [candidate for candidate in candidates if candidate not in valid]
        for issue in invalid:
            append_patrol_todo_ledger({
                "at": int(time.time() * 1000),
                "issue_key": issue_key(issue),
                "todo_type_key": type_key,
                "status": "failed",
                "error": "missing required issue field: Skill/Session/Title",
            })
            failed += 1
        if not valid:
            continue
        valid.sort(key=lambda item: (item.get("Skill") or "", item.get("Title") or "", item.get("Session") or "", issue_key(item)))
        representatives[type_key] = valid[0]
        for issue in valid[1:]:
            append_patrol_todo_ledger({
                "at": int(time.time() * 1000),
                "issue_key": issue_key(issue),
                "todo_type_key": type_key,
                "status": "same_type_suppressed",
                "note": "same patrol run and same problem type; representative issue handled once",
            })
            coalesced += 1

    remote_records = None
    created = followed = 0
    for type_key, issue in sorted(representatives.items()):
        key = issue_key(issue)
        previous = latest.get(key)
        previous_by_type = latest_by_type.get(type_key)
        previous = previous or previous_by_type
        if not all(issue.get(field) for field in ("Skill", "Session", "Title")):
            append_patrol_todo_ledger({
                "at": int(time.time() * 1000),
                "issue_key": key,
                "todo_type_key": type_key,
        ...
        if previous and previous.get("status") in {
            "requested", "queued", "duplicate_in_flight", "delivery_unknown", "in_flight_followed", "completed",
        }:
            append_patrol_todo_ledger({
                "at": int(time.time() * 1000),
                "issue_key": key,
                "todo_type_key": type_key,
                "status": "in_flight_followed" if previous.get("status") != "completed" else "completed",
                "client_request_id": previous.get("client_request_id"),
                "note": "same problem type already tracked locally; no new Todo row",
            })
            followed += 1
            continue

        responsible_owner = owners.get(issue.get("Skill")) or issue.get("Session") or SELF
        if remote_records is None:
            try:
                remote_records = load_remote_patrol_todos()
            except Exception as exc:
                append_patrol_todo_ledger({
                    "at": int(time.time() * 1000),
                    "issue_key": key,
                    "todo_type_key": type_key,
                    "status": "dedupe_check_failed",
                    "error": str(exc),
                })
                failed += 1
                continue
        remote_matches = remote_patrol_todo_matches(remote_records, issue)
        if remote_matches:
            append_patrol_todo_ledger({
                "at": int(time.time() * 1000),
                "issue_key": key,
                "todo_type_key": type_key,
                "status": "remote_duplicate",
                "record_ids": [record.get("record_id") for record in remote_matches],
                "note": "full shared todolist matched this problem type in any state; no new row; follow-up target remains the existing row",
            })
            followed += 1
            continue

        client_request_id = f"{datetime.now(timezone.utc).date().isoformat()}:{SELF}:patrol-todo:{type_key}"
        append_patrol_todo_ledger({
            "at": int(time.time() * 1000),
            "issue_key": key,
            "todo_type_key": type_key,
        ...
        response, status = spawn_patrol_todo(issue, responsible_owner)
        append_patrol_todo_ledger({
            "at": int(time.time() * 1000),
            "issue_key": key,
            "todo_type_key": type_key,
            "status": status,
            "client_request_id": response.get("_client_request_id") if isinstance(response, dict) else client_request_id,
            "comm_id": extract_comm_id(response),
            "receipt": response,
        })
        if status in {"queued", "duplicate_in_flight"}:
            created += 1
        else:
            failed += 1
    return {
        ...
        "failed": failed,
        "coalesced": coalesced,
        "ledger": patrol_todo_ledger_path(),
    }
...
def write_review(
    ...
    lines.append("")
    lines.append("## Patrol ToDo handoff")
    lines.append("")
    todo_summary = patrol_todo_summary or {}
    lines.append(
        "- Unresolved findings from this run: "
        f"{todo_summary.get('candidates', 0)}; new/in-flight admissions: "
        f"{todo_summary.get('created', 0)}; followed without a new row: "
        f"{todo_summary.get('followed', 0)}; same-type suppressed: "
        f"{todo_summary.get('coalesced', 0)}; failed/unknown: {todo_summary.get('failed', 0)}"
    )
    lines.append(f"- Patrol ToDo ledger: `{todo_summary.get('ledger', patrol_todo_ledger_path())}`")
    lines.append("- Rule: one Todo maximum per problem type per run; before any new admission, the full shared `PRODUCT_REDACTED / todolist` is read without a status filter, and any matching row in any state blocks creation.")
...
def main():
    ...
    patrol_todo_summary = register_patrol_todos(all_issues)
    print(
        "patrol todo: "
        f"candidates={patrol_todo_summary['candidates']} "
        f"created={patrol_todo_summary['created']} "
        f"followed={patrol_todo_summary['followed']} "
        f"failed={patrol_todo_summary['failed']}",
        flush=True,
    )
    ...
    path = write_review(
        ...
        patrol_todo_summary=patrol_todo_summary,
    )
