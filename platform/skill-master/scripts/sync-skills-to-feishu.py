...
The target asset and private transport configuration are supplied by the
owner-side queue environment; this public package contains no remote IDs.
"""
import json
import os
import sys
...
REGISTRY_ASSET = os.environ.get("SKILL_MASTER_REGISTRY_ASSET", "skill-master.registry.技能清单")
...
SOURCE_REGISTRY = Path(os.environ.get("SKILL_MASTER_SOURCE_REGISTRY", "")) if os.environ.get("SKILL_MASTER_SOURCE_REGISTRY") else (
    ROOT / "config" / "skill-upgrade-sources.json" if (ROOT / "config" / "skill-upgrade-sources.json").exists()
    else ROOT / "docs" / "onboarding-v1-skill-upgrade-sources.json"
)
HOSTED_BY = os.environ.get("SKILL_MASTER_HOSTED_BY", "skill-master")

VALID_ORIGINS = {"skill-master", "claude-builtin", "codex-builtin"}
VALID_SCOPES = {"shared", "claude-only", "codex-only", "inventory-only"}


def source_registry_label() -> str:
    """Return a stable non-absolute label for registry provenance."""
    try:
        return SOURCE_REGISTRY.relative_to(ROOT).as_posix()
    except ValueError:
    ...
    for package in registry.get("packages", []):
        ...
        for mapping in package.get("mappings", []):
            ...
            metadata[local] = {
                ...
                "RegistrySource": f"skills/INDEX.md + {source_registry_label()}#{package_id}",
                "HostedBy": HOSTED_BY,
            }
