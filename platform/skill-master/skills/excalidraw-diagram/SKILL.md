...
```
bash <SM_WORKSPACE_ROOT>/skill-master/scripts/record-tick.sh excalidraw-diagram
```
...
```bash
cd <SM_WORKSPACE_ROOT>/skill-master/skills/excalidraw-diagram/references && uv run python render_excalidraw.py <path-to-file.excalidraw>
```
...
```bash
cd <SM_WORKSPACE_ROOT>/skill-master/skills/excalidraw-diagram/references
uv sync
