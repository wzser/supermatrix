...
function formatCursorKey(cursor) {
  if (!cursor) return "initial";
  return `${cursor.eventTimeMs}-${cursor.id}`;
}

function buildReviewSpawnRequestKey({ phase, mode, previousCursor, nextCursor }) {
  return [
    "cross-session-review",
    phase,
    mode,
    formatCursorKey(previousCursor),
    formatCursorKey(nextCursor),
  ].join(":");
}
...
async function main() {
  ...
  const fromSession = process.env.SM_SESSION_NAME || workspace.sessionName;
  ...
  const draftSpawn = await spawnAndCollectResult({
    ...
    prompt: draftPrompt,
    clientRequestId: buildSpawnClientRequestId({
      nowMs,
      from: fromSession,
      target: targetSession,
      businessKey: buildReviewSpawnRequestKey({
        phase: "draft",
        mode: args.mode,
        previousCursor,
        nextCursor,
