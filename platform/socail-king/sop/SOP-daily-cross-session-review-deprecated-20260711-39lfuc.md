---
id: 39lfuc
name: daily-cross-session-review
description: 【已废弃 2026-04-25】旧 PMO 七问八答报告流程；不要按它工作，日常复盘走 SOP-judgment-via-interview。仅留作反思材料。
status: deprecated
owner: socail-king
created: 2026-04-19
updated: 2026-07-11
...
> **不要按这个 SOP 工作。** 新流程见 `sop/SOP-judgment-via-interview-active-20260711-gq1fni.md`。
>
...
>
> **2026-06-02 补充**：06:00 每日任务是 scheduler fire-and-forget 触发，不再把完整 REPORT、摘要或判断正文回流给 scheduler。复盘成果只在 SK 自己侧闭环：`state/judgments.jsonl`、飞书 Bitable 记录、读回校验和规则沉淀。
>
...
2. 主 session 通过 `/api/spawn2.0` 拉起一个子 session 做首轮 draft：
   - body 必须带 `from`、`target`、`prompt`、`client_request_id`、`closure`
   - `closure.kind=message`，同步等待结果时用 `closure.target.type=inline`
   - backend（执行后端）放在 `execution.backend`，不要发送顶层 `mode`
...
4. 主 session 再次 `/api/spawn2.0` 子 session，要求它根据 feedback 修订：
  ...
  ```json
  {
    "overrides": {
      "receiptProof": {
        ...
        "engine": "file",
        "target": { "path": "/Users/LOCAL_USER/SuperMatrixRuntime/workspaces/socail-king/state/cross-session-review-state.json" },
