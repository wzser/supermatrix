...
test("buildSpawnClientRequestId uses a date prefix and stable business key", () => {
  const clientRequestId = buildSpawnClientRequestId({
    nowMs: new Date(2026, 4, 28, 1, 2, 3).getTime(),
    from: "socail-king",
    target: "socail-king",
    businessKey: "cross-session-review:draft:incremental:initial:comm_123",
  });

  assert.equal(
    clientRequestId,
    "2026-05-28:socail-king:socail-king:cross-session-review:draft:incremental:initial:comm_123",
  );
});

test("buildSessionResultUrl derives the result endpoint from the spawn endpoint", () => {
  assert.equal(
    buildSessionResultUrl("http://127.0.0.1:3501/api/spawn2.0", "sess_child_123"),
    "http://127.0.0.1:3501/api/sessions/sess_child_123/result",
  );
  assert.equal(
    buildSessionResultUrl("http://localhost:3501/api/spawn2.0", "sess_child_123"),
    "http://127.0.0.1:3501/api/sessions/sess_child_123/result",
  );
});

test("spawnAndCollectResult sends the spawn2.0 contract and polls the child result endpoint", async () => {
  const requests = [];

  const result = await spawnAndCollectResult({
    spawnUrl: "http://localhost:3501/api/spawn2.0",
    target: "mythos",
    from: "socail-king",
    backend: "claude",
    prompt: "draft prompt",
    clientRequestId: "2026-05-28:socail-king:mythos:cross-session-review:draft:initial-to-empty",
  ...
  assert.equal(requests[0].url, "http://127.0.0.1:3501/api/spawn2.0");
  assert.equal(Object.hasOwn(requests[0].body, "mode"), false);
  assert.equal(Object.hasOwn(requests[0].body, "backend"), false);
  assert.equal(Object.hasOwn(requests[0].body, "delivery_address"), false);
  assert.equal(Object.hasOwn(requests[0].body, "supermatrix_internal"), false);
  assert.equal(requests[0].body.target, "mythos");
  assert.equal(requests[0].body.from, "socail-king");
  assert.equal(
    requests[0].body.client_request_id,
    "2026-05-28:socail-king:mythos:cross-session-review:draft:initial-to-empty",
  );
  assert.deepEqual(requests[0].body.closure, {
    kind: "message",
    target: { type: "inline" },
  });
  assert.deepEqual(requests[0].body.execution, { backend: "claude" });
...
test("spawnAndCollectResult recovers the child session after spawn fetch fails and polls until completion", async () => {
  ...
  assert.deepEqual(fetchCalls, [
    { url: "http://127.0.0.1:3501/api/spawn2.0", method: "POST" },
...
test("spawnAndCollectResult surfaces a clear error when the child session cannot be recovered", async () => {
  await assert.rejects(
    () =>
      spawnAndCollectResult({
        spawnUrl: "http://127.0.0.1:3501/api/spawn2.0",
        target: "mythos",
        from: "socail-king",
        backend: "claude",
        prompt: "draft prompt",
...
主洞察：与上一次相比，本次发生了 sync_inline 长任务被父流程局部等待预算切断的问题；事实链是父流程 POST /api/spawn2.0、child 继续运行并写文件、父流程先报超时；偏差是实际机制偏离 async_kickoff + pollable result；下一步修正是改为 async_kickoff 并轮询结果端点。
