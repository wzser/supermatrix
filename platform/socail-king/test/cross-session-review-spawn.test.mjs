...
test("spawnAndCollectResult sends the spawn2.0 contract", async () => {
  const requests = [];
  ...
  const result = await spawnAndCollectResult({
    ...
    backend: "claude",
    prompt: "draft prompt",
    clientRequestId: "2026-05-28:socail-king:mythos:cross-session-review:draft",
  ...
  assert.equal(requests[0].url, "http://localhost:3501/api/spawn2.0");
  ...
  assert.equal(requests[0].body.client_request_id, "2026-05-28:socail-king:mythos:cross-session-review:draft");
