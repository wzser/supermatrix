...
Safe help: this command does not open, create, or migrate a database.
For an isolated instance set WATCHDOG_DB_PATH and SM_API_BASE explicitly.
Remote Bitable sync is disabled unless both WATCHDOG_BITABLE_BASE_TOKEN and
WATCHDOG_BITABLE_TABLE_ID are configured.`);
}
...
function weeklyReviewDate(issue: Issue): string {
...
function syncWeeklyReviewClosureIfNeeded(db: Database.Database, issue: Issue): void {
...
async function runCommand(): Promise<void> {
  ...
  case "start": {
    ...
    syncWeeklyReviewClosureIfNeeded(db, startedIssue);
    output(startedIssue);
  ...
  case "pending": {
    ...
    syncWeeklyReviewClosureIfNeeded(db, pendingIssue);
  ...
  case "done": {
    ...
    syncWeeklyReviewClosureIfNeeded(db, issue);
  ...
  case "failed": {
    ...
    syncWeeklyReviewClosureIfNeeded(db, failedIssue);
  ...
  case "set-verification": {
    ...
    syncWeeklyReviewClosureIfNeeded(db, verifiedIssue);
  ...
  case "set-required-evidence": {
    ...
    syncWeeklyReviewClosureIfNeeded(db, updatedIssue);
    output(updatedIssue);
