...
const MIGRATION_V4 = `
CREATE TABLE IF NOT EXISTS weekly_review_patrol_todos (
...
export function applyMigrations(db: Database.Database): void {
  ...
  // V4: patrol findings handed to todomaster need a local idempotency ledger;
  // the shared Bitable remains the source of truth for the live Todo row.
  db.exec(MIGRATION_V4);

  // V5: weekly-review closure must use the reviewer-selected canonical owner
