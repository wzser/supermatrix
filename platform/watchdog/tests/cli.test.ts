...
describe("CLI", () => {
  ...
  it("prints help without opening or migrating the configured database", () => {
    const helpDbPath = join(tmpDir, "help.db");
    const help = run(["--help"], { WATCHDOG_DB_PATH: helpDbPath });

    expect(help).toContain("Safe help: this command does not open, create, or migrate a database.");
