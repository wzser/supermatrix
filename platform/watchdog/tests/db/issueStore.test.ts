...
describe("IssueStore", () => {
  ...
  it("persists canonical weekly-review owner and completion evidence", () => {
    const issue = store.createIssue({
      title: "Opaque weekly-review finding",
      source: "weekly-review-watchdog",
      description: "No owner names are embedded in this text",
      verification: "echo ok",
  ...
  it("persists an explicit unclassified weekly-review evidence state", () => {
    const issue = store.createIssue({
      title: "Derived follow-up without reviewer owner",
      source: "weekly-review-watchdog",
      description: "Ownership requires a governance decision",
      verification: null,
      requiredOwner: null,
