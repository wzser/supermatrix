...
describe("applyMigrations", () => {
  ...
  it("creates the patrol Todo idempotency ledger", () => {
    db = new Database(":memory:");
    applyMigrations(db);

    const table = db.prepare(
      "SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'weekly_review_patrol_todos'",
    ).get();
    expect(table).toEqual({ name: "weekly_review_patrol_todos" });
  });
