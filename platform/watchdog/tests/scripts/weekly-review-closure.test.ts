...
`,
  ...
  it("standing sweep exposes unclassifiable issues explicitly and never redispatches them", async () => {
    ...
    try {
      ...
      const outcome = await runWeeklyReviewStandingSweepWithEscalation(db, {
        ...
        notifier: { notify },
        redispatch,
        todoDispatch: vi.fn().mockResolvedValue({ status: "accepted", ref: "todo_unclassified" }),
        sharedTodoLookup: vi.fn().mockResolvedValue([]),
      });
  ...
  it("registers one self-owned agent Todo per unresolved problem and suppresses duplicates", async () => {
    const tmpDir = mkdtempSync(join(tmpdir(), "watchdog-weekly-review-standing-todo-"));
    const reportsRoot = join(tmpDir, "reports");
    try {
      const db = new Database(join(tmpDir, "watchdog.db"));
      applyMigrations(db);
      db.prepare(
        ...
        "todo-issue-1",
        "2026-09-09 weekly-review owner follow-up",
        "weekly-review-watchdog",
        "owner needs to close the finding",
        "rg -q 'codexroot completion: verified' receipts.md",
        "in_progress",
      ...
      db.prepare(
        ...
        "2026-09-09 weekly-review owner follow-up",
        "weekly-review-watchdog",
        "same problem type from another issue instance",
        "rg -q 'codexroot completion: verified' receipts.md",
        "in_progress",
        Date.parse("2026-09-09T08:30:00.000Z"),
      );

      const todoDispatch = vi.fn().mockResolvedValue({
      ...
      expect(todoDispatch).toHaveBeenCalledTimes(1);
      expect(todoDispatch).toHaveBeenCalledWith(expect.objectContaining({
        target: "todomaster",
        from: "watchdog",
        client_request_id: "2026-09-09:watchdog:patrol-todo:weekly-review-issue:weekly-review-owner-follow-up",
        prompt: expect.stringContaining("owner=watchdog"),
      }));
      const prompt = (todoDispatch.mock.calls[0]?.[0] as { prompt: string }).prompt;
      expect(prompt).toContain("① 存在什么问题");
      expect(prompt).toContain("② 要做什么");
      expect(prompt).toContain("③ 完成标准");
      expect(prompt).not.toContain("rg -q");
      expect(first.patrolTodos).toEqual([
        expect.objectContaining({ problemKey: "weekly-review-issue:todo-issue-1", status: "accepted" }),
      ]);
      expect(second.patrolTodos).toEqual([
        expect.objectContaining({ problemKey: "weekly-review-issue:todo-issue-1", status: "skipped_existing" }),
      ]);
      expect(db.prepare("SELECT status FROM weekly_review_patrol_todos WHERE problem_key = ?").get(
        "weekly-review-issue:weekly-review-owner-follow-up",
  ...
  it("does not create a Todo when the same problem type exists in any shared-table status", async () => {
    const tmpDir = mkdtempSync(join(tmpdir(), "watchdog-weekly-review-standing-todo-existing-status-"));
    const reportsRoot = join(tmpDir, "reports");
    try {
      const db = new Database(join(tmpDir, "watchdog.db"));
      applyMigrations(db);
      db.prepare(
        ...
        "todo-issue-existing-status",
        "2026-09-10 weekly-review owner follow-up",
        "weekly-review-watchdog",
        "owner needs to close the finding",
        "rg -q 'codexroot completion: verified' receipts.md",
        "in_progress",
      ...
      const outcome = await runWeeklyReviewStandingSweepWithEscalation(db, {
        ...
        sharedTodoLookup: vi.fn().mockResolvedValue([
          {
            recordId: "rec_completed_same_type",
            content: "① 存在什么问题：问题类型键：weekly-review-issue:weekly-review-owner-follow-up",
            humanStatus: "已完成",
            agentStatus: "已完成",
          },
        ]),
      ...
      expect(todoDispatch).not.toHaveBeenCalled();
      expect(outcome.patrolTodos).toEqual([
        expect.objectContaining({
          problemTypeKey: "weekly-review-issue:weekly-review-owner-follow-up",
  ...
  it("escalates stranded owners through notify and one idempotent re-dispatch", async () => {
    ...
    try {
      ...
      const outcome = await runWeeklyReviewStandingSweepWithEscalation(db, {
        ...
        notifier: { notify },
        redispatch,
        todoDispatch: vi.fn().mockResolvedValue({ status: "accepted", ref: "todo_escalation" }),
        sharedTodoLookup: vi.fn().mockResolvedValue([]),
      });
  ...
  it("keeps retry_count unchanged when an owner re-dispatch was not admitted", async () => {
    ...
    try {
      ...
      const outcome = await runWeeklyReviewStandingSweepWithEscalation(db, {
        ...
        todoDispatch: vi.fn().mockResolvedValue({ status: "accepted", ref: "todo_retry_failure" }),
        sharedTodoLookup: vi.fn().mockResolvedValue([]),
      });
  ...
  it("does not let a verification retry suppress the first durable E7 owner re-dispatch", async () => {
    ...
    try {
      ...
      const options = {
        ...
        redispatch,
        todoDispatch: vi.fn().mockResolvedValue({ status: "accepted", ref: "todo_existing_retry" }),
        sharedTodoLookup: vi.fn().mockResolvedValue([]),
      };
      const first = await runWeeklyReviewStandingSweepWithEscalation(db, options);
      const second = await runWeeklyReviewStandingSweepWithEscalation(db, options);
