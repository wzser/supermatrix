...
### Spawn2 immutable-ingress pre-admission (synthetic only)

适用：target session declares `capability_payload.immutableIngress` per [the exact contract](spawn2-immutable-ingress.md). The tracked activation receipt and live capability readback must exist before any production ingress can be considered enabled.

- [ ] Use a temporary runtime root and test receiver. Confirm the receiver gets the exact UTF-8 prompt artifact bytes and emits one terminal JSON receipt whose declared identity pointers match the carrier.
- [ ] For the HDQ declaration with `completionMode=receiver_terminal`, confirm an accepted carrier returns the receiver's bare terminal JSON as Spawn2 `finalMessage` with non-empty `spawnCommId` in both body/header, starts zero child/backend processes, and retains the exact receipt in the immutable-ingress audit artifact. A slow receiver must be aborted, its full process group reaped, and return a durable `target_accepted:false` terminal response with no `resultUrl`/`switched_async` fallback. Include a synthetic receiver whose leader exits on `SIGTERM` while a same-group descendant ignores `TERM`/`HUP`/`INT`; completion must wait until escalation reaps that descendant. Inject one negative-PGID `SIGKILL` failure and confirm admission remains unsettled while the group is alive, then restore the real kill primitive, reap the group, and confirm the structured cleanup failure is returned before receipt parsing.
- [ ] In a temporary/in-memory spawn-closure store, confirm only the complete fixed sanitized legacy metadata shape is fenced. Underscore-key substitutions, trailing fields, wrong digest widths, or artifact/prompt digest mismatch must recover and classify as ordinary spawn; an injected receiver-terminal identity lookup failure must remain `noop` with no delivery, model, redrive, or adjudication action.
...
### 3b. /new provisioning root on the sm-c volume (only when `SM_NEW_WORKSPACE_ROOT` is set)

Skip this whole section if `.env.local` has no `SM_NEW_WORKSPACE_ROOT`; without it `/new` provisions
into `$SM_WORKSPACE_ROOT` exactly as in step 3 and nothing below applies.

- [ ] Preconditions — the provisioning root is a *separate* mounted volume, checked by device number
      (never by device node name: `/dev/diskNsN` changes on every remount, so do not assert on it):
      ```sh
      grep -n '^SM_NEW_WORKSPACE_ROOT=' /Users/LOCAL_USER/SuperMatrix/.env.local
      #  expect exactly one line → SM_NEW_WORKSPACE_ROOT=/Volumes/EXAMPLE_VOLUME/workspaces
      stat -f '%d %N' /Volumes/EXAMPLE_VOLUME/workspaces "$SM_WORKSPACE_ROOT"
      #  expect two DIFFERENT device numbers in column 1; equal numbers mean sm-c is NOT mounted
      #  (a leftover writable /Volumes/EXAMPLE_VOLUME stub on the boot disk) and `/new` must refuse — see 3c.
      ```
- [ ] In root, send `/new claude smoketest-505 <purpose>`; root replies `✓ 已创建 session ...` as in step 3.
- [ ] The workspace **entity** is on sm-c and the legacy root holds only a compatibility symlink:
      ```sh
      readlink "$SM_WORKSPACE_ROOT/smoketest-505"
      #  expect → /Volumes/EXAMPLE_VOLUME/workspaces/smoketest-505
      test -d /Volumes/EXAMPLE_VOLUME/workspaces/smoketest-505 && echo entity-ok
      stat -f '%d' "$SM_WORKSPACE_ROOT/smoketest-505" ; stat -f '%d' "$SM_WORKSPACE_ROOT"
      #  expect the two device numbers to DIFFER (stat follows the symlink onto sm-c)
      git -C "$SM_WORKSPACE_ROOT/smoketest-505" rev-parse HEAD   # expect one initial commit
      ls /Volumes/EXAMPLE_VOLUME/workspaces/smoketest-505/CLAUDE.md /Volumes/EXAMPLE_VOLUME/workspaces/smoketest-505/AGENTS.md \
         /Volumes/EXAMPLE_VOLUME/workspaces/smoketest-505/session-catalog.json
      ```
- [ ] The database still records the **legacy** path — every existing prefix guard depends on this:
      ```sh
      sqlite3 "$SM_RUNTIME_ROOT/data/supermatrix.db" \
        "SELECT workdir FROM sessions WHERE name='smoketest-505';"
      #  expect → /Users/LOCAL_USER/SuperMatrixRuntime/workspaces/smoketest-505 (NOT /Volumes/EXAMPLE_VOLUME/...)
      ```
- [ ] Cleanup: `/delete smoketest-505` in root, then remove both ends —
      `rm "$SM_WORKSPACE_ROOT/smoketest-505"` (the symlink) **and**
      `rm -rf /Volumes/EXAMPLE_VOLUME/workspaces/smoketest-505` (the entity). Deleting only the symlink leaves a
      dirty entity that a later same-name `/new` would git-init on top of.

### 3c. /new fails closed when the provisioning root is not a separate mounted volume

Proves the guard refuses rather than silently provisioning onto the boot disk. No need to physically
unmount sm-c — pointing the env at any boot-disk directory reproduces the leftover-mount-stub case,
because the guard compares `stat().dev` against the workspace root instead of matching `/Volumes`.

- [ ] Point `SM_NEW_WORKSPACE_ROOT` at a boot-disk directory and restart SuperMatrix through the
      controlled path (`scripts/platform-maintenance-gate.sh`; never `kill` / `pkill` / `launchctl`):
      ```sh
      mkdir -p /tmp/sm-fake-root
      stat -f '%d' /tmp/sm-fake-root ; stat -f '%d' "$SM_WORKSPACE_ROOT"
      #  expect the SAME device number — that is exactly what the guard must reject
      #  then set SM_NEW_WORKSPACE_ROOT=/tmp/sm-fake-root in .env.local and reload
      ```
- [ ] In root, send `/new claude smoketest-505-fail <purpose>`.
- [ ] Root replies a Chinese `UserError` (message prefixed `❌`) containing 「未挂载」 and both real paths.
      A raw `EACCES` / `ENOENT` errno, or 「内部错误」, is a FAILURE of this step.
- [ ] Nothing was provisioned and **no Feishu group was created** — the guard runs before `mkdir` and
      before `lark.createGroup`:
      ```sh
      ls /tmp/sm-fake-root                      # expect empty
      ls "$SM_WORKSPACE_ROOT" | grep smoketest-505-fail   # expect no output
      sqlite3 "$SM_RUNTIME_ROOT/data/supermatrix.db" \
        "SELECT count(*) FROM sessions WHERE name='smoketest-505-fail';"   # expect 0
      ```
      Also confirm in Feishu that no group named `smoketest-505-fail-claude` appeared.
- [ ] Restore `.env.local` to `SM_NEW_WORKSPACE_ROOT=/Volumes/EXAMPLE_VOLUME/workspaces`, reload through the same
      gate, and re-run step 3b to confirm the normal path still works. `rmdir /tmp/sm-fake-root`.
...
适用：改动触及 `scripts/localwatch.sh`、`scripts/localwatch-managed-services.ts` 或 live `localwatch-services.json`。helper 的 `check` 可能发出真实 `open -g` / Clash 优雅退出，**不能在 busy session、未批准的维护窗口或只做源码验证时手工运行**。首次源码接入需在安全窗口经 codexroot maintenance gate 重启 localwatch 一次；busy 非零时直接拒绝。
...
3. 仅在确认所有目标 session idle、获得维护窗口批准后，由 codexroot 执行 `./scripts/platform-maintenance-gate.sh restart-localwatch --reason '<变更原因>'`。以 gate 的旧/新 PID、permit 消费和 `/api/health status=ok` 终局回执为准；不得另起第二个 supervisor。
...
7. **重启 reconcile** — 通过 spawn2.0 交由 codexroot maintenance gate 执行安全 reload；重启后 `ps aux | grep "kimi acp"` 应只有一个 PID（旧 ACP 已清理 / 新 ACP 重启）。
8. **共享单进程验证** — 同时打开 2-3 个 kimi session 各发一轮，`ps aux | grep "kimi acp"` 全程只有 1 个 PID。
9. **kimi term 不被误杀** — 在另一个 terminal 跑 `kimi`（交互式），由 codexroot gate 执行安全 reload，期望那个交互式 kimi **不被 reconcile 杀**（cmd 不含 acp）。
