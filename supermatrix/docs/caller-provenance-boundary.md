...
- `POST /api/caller-identity` on the existing loopback API server — body is
  ...
  `messageRunId` / `sessionName` / `ownerSessionName`. `messageRunId` binds the
  provenance record to the exact in-flight run rather than merely its session.
  A resolved-token response and an
