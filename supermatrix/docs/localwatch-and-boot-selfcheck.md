...
| `takeover()` | `pgrep -f 'localwatch\.sh' / 'dev-loop\.sh' / 'tsx.*src/cli/main\.ts'`、4322/4323 两个 business-screen node。SIGTERM 2s → SIGKILL。 | 新 localwatch 启动时把上一任全杀干净，避免双胞胎。 |
...
- EVA 看板使用独立的 `BUSINESS_SCREEN_EVA_PORT=4323`、`server-eva.js`、stdout/stderr 日志和 PID；启动前只清理 4323 占用。
...
| 3min | %18 | `/api/health`（SM）、`:3500/health`（scheduler）、`:4322/`（business-screen）、`:4323/`（business-screen EVA） | 连续 `HEALTH_FAIL_THRESHOLD=3` 次失败 → 飞书告警 + 强杀该进程让 handle_exit 重拉 |
...
收到 SIGINT/SIGTERM → `cleanup`：对四个被管 PID 各发 SIGTERM，再 `pkill -TERM` 兜底子孙 SM/business-screen，`wait`，退出。锁通过 `trap EXIT` 自动释放。
...
# 完全停掉（启动也不会自动恢复）
launchctl unload ~/Library/LaunchAgents/com.LOCAL_USER.localwatch.plist

# 重新装载
launchctl load ~/Library/LaunchAgents/com.LOCAL_USER.localwatch.plist
