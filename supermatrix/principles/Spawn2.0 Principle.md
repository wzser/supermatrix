...
> Last verified: 2026-09-02（点名的路径/命令/环境变量请定期校验仍存在）
> 实现位置：`src/cli/apiServer.ts`（`/api/spawn2.0` handler + `resolveSpawn2ClosureTarget` / `withSpawn2DeliveryInstruction` / linked-worktree admission）、`src/app/childSession.ts`（child runtime config 继承与 opaque handoff 执行）、`src/app/immutableIngress.ts`（target 声明的 immutable ingress bridge）。本文件给契约与坑，机制细节读代码。
...
需要让 child 在同一目标仓的 linked Git worktree 中执行时，可传 `execution.workdir`；它必须是绝对路径、Git worktree 根目录，并与 target 的规范 workdir 共享同一 Git common-dir。未传时仍使用 target 规范 workdir；本字段只改变本次 child cwd，不改 session 持久配置。
...
- **`execution.workdir` 越过目标仓边界** → 现象：`400 invalid execution.workdir`。原因：只接受绝对 Git worktree 根目录，且必须与 target 规范 workdir 共享 Git common-dir；相对路径、子目录、缺失路径和无关仓库均拒绝。正确做法：先创建同仓 linked worktree，再传其根目录；不需要隔离目录就省略该字段。
- **`inbox-message` 断言过弱** → 现象：`400 invalid verification_predicate: weak inbox-message predicate`。原因：`contains_all`/`contains_any` 至少一个 token 必须含 concrete anchor；只有泛化词的断言会被拒绝。正确做法：带 hex、UUID、`comm_`、epoch 或 ISO datetime 等具体锚点。
...
- **`POST /api/spawn_async_items/<ref>/take`**：取结果 + 原子记账（`verdict='caller_consumed'`）。调用方跟进异步 spawn 的**唯一规范入口**——switched_async / todo_pool / 409 响应里的 `resultUrl` 就是它。`commStatus="completed"` 或终态失败 `commStatus="failed"` 且 `finalMessage` 非空时才算结果就绪并记账；未跑完返回当前状态不记账；重复 take 幂等（`alreadyConsumed:true`）。
- **`GET /api/spawn_async_items/<ref>`、`GET /api/spawn_async_items/by-comm/<comm_id>` 与 `GET /api/sessions/:id/result` 保持纯读**：诊断、监控可用，前两者会返回可直接使用的 `resultUrl`，但永不记账，也不会抑制后续推送。
...
- **caller 关闭 Heartbeat 时不可依赖推送**：closure fast-path / watcher 会把结果终态停放为 `delivery_unsupported_caller_heartbeat_disabled`，并提示 caller 通过 `POST /api/spawn_async_items/<ref>/take` 自取；不要为同一 `client_request_id` 重发。
- **`resultUrl` 是路径不是完整 URL**（形如 `/api/spawn_async_items/<ref>/take`），自己拼 host：
  `curl -s -X POST "http://localhost:3501$RESULT_URL"`。返回 `200 { ok:true, ...item }`；**判「结果到手」看 `commStatus` 为 `"completed"` 或 `"failed"` 且 `finalMessage` 非空**，其它 status 只是当前进度（不记账，可稍后再 take）。

### 5.2 Target 声明的 immutable ingress

target session 可在 machine-owned `capability_payload.immutableIngress` 声明 `supermatrix-immutable-ingress/v1` 接收器。声明存在时，框架要求 prompt 中恰好一对 marker，将原始 prompt 以 UTF-8 artifact 写入 runtime data 目录并读回，再以 `shell=false`、target workdir 和固定 argv 调 receiver；receiver 必须返回 terminal JSON，且 `accepted` 与声明的 identity pointers 全部通过。缺 carrier、receipt、身份绑定、artifact 读回或内容变更任一项都返回 `400 target_accepted:false`，不创建 child/model；没有声明的 target 不会触发这套解释。

声明可选 `completionMode:"receiver_terminal"`，目前只允许 `closure.target.type="inline"`：成功后直接把 receiver terminal JSON 作为 `finalMessage`，不启动 child/model，也不走 `switched_async`；该模式用按 `client_request_id` 确定的 `cross_session_log.kind="receiver_terminal"` 做持久完成收据，重试只复读同一完成/失败结果。此模式的 `verification_predicate` 明确绕过，因为 verified terminal receipt identity 本身就是完成证明；普通 Spawn2 路径仍正常校验并转发 predicate。完整 receiver contract 与 cleanup 细节见 `docs/spawn2-immutable-ingress.md`。

### 5.3 Opaque handoff

若 prompt 含框架登记的 opaque envelope，Spawn2 会在 admission 阶段按固定 marker、canonical base64 和固定命令契约解析；非法 framing 直接 `400`。child 执行时由框架用固定 argv、`shell=false` 将精确解码字节经 stdin 交给登记 consumer，并绑定 framework `comm_id`；不把 opaque bytes 交给模型，也不允许 prompt 自选可执行文件或参数。该路径的 consumer 失败是终态失败，不能靠换 key 重发绕过原有幂等；相关已完成 transport-failure recovery 只能按精确 comm/client key、envelope 和 no-side-effect 证据走 owner 的 dry-run repair。
