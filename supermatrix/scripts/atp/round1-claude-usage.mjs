...
// Round 1 ATP: invoke Claude backend with a trivial prompt and confirm
// that the streamParser emits a `usage` AgentEvent carrying real token
// counts from a fresh claude-cli result event.
...
// NOTE: do NOT unref this timer — child process is unref'd inside spawnAndStream,
// so without an active timer the Node event loop would exit prematurely with the
// "unsettled top-level await" warning before stdin from claude can be consumed.
const hardTimer = setTimeout(() => {
  console.error("FAIL: ATP hit 60s hard timeout");
  process.exit(2);
}, 60_000);

const workdir = await mkdtemp(join(tmpdir(), "atp-token-usage-"));

const backend = new ClaudeBackend({});
const session = {
  id: "atp-sess",
  name: "atp-token-usage",
  alias: "",
  avatar: "",
  scope: "user",
  backend: "claude",
...
console.log("PASS: Claude side emits usage event with non-zero tokens.");

// End-to-end: feed the same stream shape through collectStream + a real
// SqliteBindingStore, then read the row back.
...
// Replay captured events through collectStream
