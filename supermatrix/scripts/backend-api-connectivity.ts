...
type CheckSummary = {
  ...
  /** A repair changed local configuration; codexroot may evaluate maintenance. */
  manualReloadRequired: boolean;
  /** @deprecated Automatic reload is disabled by platform policy. */
...
async function runConnectivityCheck(args: string[]): Promise<CheckSummary> {
  ...
  const restartRecommended = false;
  ...
  if (repair && codexRoute !== "deepseek" && codexProbe && !codexProbe.ok && codexProbe.failureKind === "model_access") {
    ...
    if (fallback.probe?.ok) {
      ...
      // The old signal caused localwatch to terminate SuperMatrix immediately.
      // A changed config is observable, but only the codexroot maintenance gate
      // (manually reasoned or reached from the daily scheduler task) may apply it.
