...
# kimi-code 0.38.0's esbuild bundle renames the native ACP bootstrap symbol to
# `bootstrap$1`; both exact, unique V2 bootstrap forms are accepted below.
#
...
ANCHOR_V2_BOOTSTRAP_BUNDLER_SUFFIX = ANCHOR_V2_BOOTSTRAP.replace(
    "bootstrap({", "bootstrap$1({"
)
REPLACEMENT_V2_BOOTSTRAP_BUNDLER_SUFFIX = REPLACEMENT_V2_BOOTSTRAP.replace(
    "bootstrap({", "bootstrap$1({"
)
...
def apply_js_patch(js: str) -> str:
    ...
    # A compact native-V2 loader intentionally replaces the ctor anchor. Check
    # its complete marker pair before counting pristine anchors so a repeat run
    # remains idempotent after upstream bundle formatting/layout changes.
    if PATCH_MARKER_V2_ENGINE in js or PATCH_MARKER_SKILLS_DIR_V2 in js:
        verify_sm_patch_markers(js)
        return js
...
def patch_v2_bootstrap(js: str) -> str:
    """Inject bootstrap args across esbuild's stable and suffixed symbols."""
    candidates = (
        (ANCHOR_V2_BOOTSTRAP, REPLACEMENT_V2_BOOTSTRAP),
        (
            ANCHOR_V2_BOOTSTRAP_BUNDLER_SUFFIX,
            REPLACEMENT_V2_BOOTSTRAP_BUNDLER_SUFFIX,
        ),
    )
    total = sum(js.count(anchor) for anchor, _ in candidates)
    if total != 1:
        raise fail(
            "anchor 'v2 acp bootstrap' found "
            f"{total}x (expected 1) — kimi-code version drift?"
        )
    anchor, replacement = next(
        candidate for candidate in candidates if js.count(candidate[0]) == 1
    )
...
def main() -> None:
    ...
    if patched is js:
        marker_count = verify_sm_patch_markers(js)
        verify_runtime_sidecar(binary, js)
        print(f"already patched — markerCount={marker_count}")
        return
