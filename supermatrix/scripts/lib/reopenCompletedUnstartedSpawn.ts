...
function readSnapshot(
  ...
  const snapshot = db.prepare(
    `SELECT c.status AS comm_status,
            ...
            run.final_message AS run_final_message,
  ...
  if (!snapshot || !snapshot.message_run_id) return snapshot;
  const finalMessage = visibleMessageRunContent(
    db,
    snapshot.message_run_id,
    "final_message",
    snapshot.run_final_message,
  );
  snapshot.run_final_has_refusal_marker = finalMessage?.includes(input.refusalMarker) ? 1 : 0;
  return snapshot;
}
