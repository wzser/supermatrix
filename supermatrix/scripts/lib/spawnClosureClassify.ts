...
import { visibleMessageRunContent } from "../../src/adapters/store-sqlite/messageRunCanonical.ts";
import {
  describeHeartbeatEnqueueFailure,
  HeartbeatEnqueueRejected,
} from "../../src/app/spawnClosure/heartbeatEnqueue.ts";
import { receiverTerminalCommSqlPredicate } from "../../src/app/spawnClosure/receiverTerminalFence.ts";
...
export function classifyAsyncItem(item: SpawnAsyncItem, db: Database.Database): RouteDecision {
  ...
  const receiverTerminalIdentity = classifyReceiverTerminalComm(item.comm_id, db);
  if (receiverTerminalIdentity !== "ordinary") {
    return {
      route: "noop",
      reason: receiverTerminalIdentity === "receiver_terminal"
        ? "receiver-terminal comm is terminal-only; async recovery and redrive suppressed"
        : "receiver-terminal identity lookup failed; generic classification suppressed",
    };
  }
...
function classifyReceiverTerminalComm(
  commId: string,
  db: Database.Database,
): "receiver_terminal" | "ordinary" | "lookup_failed" {
  try {
    const row = db
      .prepare(
        `SELECT 1 AS matched
          ...
          WHERE c.id = ?
            AND ${receiverTerminalCommSqlPredicate("c")}
          LIMIT 1`,
      )
      .get(commId) as { matched: number } | undefined;
    return row?.matched === 1 ? "receiver_terminal" : "ordinary";
  } catch {
...
function readClosureSnapshot(item: SpawnAsyncItem, db: Database.Database): ClosureSnapshot {
  ...
  const runFinalMessage = row.message_run_id
    ? visibleMessageRunContent(db, row.message_run_id, "final_message", row.run_final_message)
    : row.run_final_message;
  const finalMessage = runFinalMessage ?? row.comm_final_message ?? "";
