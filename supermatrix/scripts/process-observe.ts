...
const DEFAULT_EVENTS_MAX_BYTES = 256 * 1024 * 1024;
const DEFAULT_ARCHIVE_MAX_FILES = 8;
...
function eventsMaxBytes(): number {
  return boundedIntegerEnv("PROCESS_OBSERVE_EVENTS_MAX_BYTES", DEFAULT_EVENTS_MAX_BYTES, 1, DEFAULT_EVENTS_MAX_BYTES);
}

function archiveMaxFiles(): number {
  return boundedIntegerEnv("PROCESS_OBSERVE_ARCHIVE_MAX_FILES", DEFAULT_ARCHIVE_MAX_FILES, 1, DEFAULT_ARCHIVE_MAX_FILES);
}

function archiveRetentionDays(): number {
  return boundedIntegerEnv("PROCESS_OBSERVE_ARCHIVE_RETENTION_DAYS", DEFAULT_ARCHIVE_RETENTION_DAYS, 1, DEFAULT_ARCHIVE_RETENTION_DAYS);
}
...
async function durableWrite(path: string, value: Buffer): Promise<void> {
...
async function durableAppend(path: string, value: string): Promise<void> {
...
async function compressRotatingLog(path: string): Promise<string> {
  const archivePath = path.replace(/\.jsonl\.rotating$/u, ".jsonl.gz");
  if (archivePath === path) throw new Error(`invalid rotating event log name: ${basename(path)}`);
  const source = await readFile(path);
  if (await pathExists(archivePath)) {
    const archived = gunzipSync(await readFile(archivePath));
    if (sha256Buffer(archived) !== sha256Buffer(source)) throw new Error(`event archive collision: ${basename(archivePath)}`);
    await unlink(path);
    return archivePath;
  }
  const temporary = `${archivePath}.${process.pid}.tmp`;
  await durableWrite(temporary, gzipSync(source, { level: 9 }));
  await rename(temporary, archivePath);
  await unlink(path);
  return archivePath;
}

async function enforceEventArchiveRetention(dir: string, atMs: number): Promise<void> {
  const entries = await readdir(dir, { withFileTypes: true });
  const archives = await Promise.all(entries
    .filter((entry) => entry.isFile() && /^events-.*\.jsonl\.gz$/u.test(entry.name))
    .map(async (entry) => ({ path: join(dir, entry.name), name: entry.name, stats: await stat(join(dir, entry.name)) })));
  archives.sort((left, right) => right.stats.mtimeMs - left.stats.mtimeMs || right.name.localeCompare(left.name));
  const cutoffMs = atMs - archiveRetentionDays() * DAY_MS;
  for (const [index, archive] of archives.entries()) {
    if (index >= archiveMaxFiles() || archive.stats.mtimeMs < cutoffMs) await unlink(archive.path);
  }
}

async function rotateEventLogIfNeeded(dir: string, eventsPath: string, at: string): Promise<void> {
  const entries = await readdir(dir, { withFileTypes: true });
  for (const entry of entries.filter((item) => item.isFile() && /^events-.*\.jsonl\.rotating$/u.test(item.name))) {
    await compressRotatingLog(join(dir, entry.name));
  }
  if (await pathExists(eventsPath)) {
    const current = await stat(eventsPath);
    if (current.size > eventsMaxBytes()) {
      const stamp = at.replace(/[:.]/gu, "-");
      const identity = createHash("sha256").update(`${at}\u0000${current.size}\u0000${current.mtimeMs}`).digest("hex").slice(0, 12);
      const rotatingPath = join(dir, `events-${stamp}-${identity}.jsonl.rotating`);
      await rename(eventsPath, rotatingPath);
      await compressRotatingLog(rotatingPath);
    }
  }
  await enforceEventArchiveRetention(dir, new Date(at).valueOf());
}
...
async function observe(): Promise<{
  ...
  coverageGaps: number;
  eventLog: { activeMaxBytes: number; archiveMaxFiles: number; archiveRetentionDays: number };
}> {
  ...
  if (events.length > 0) await durableAppend(eventsPath, `${events.map((event) => JSON.stringify(event)).join("\n")}\n`);
  await rotateEventLogIfNeeded(dir, eventsPath, at);
...
async function readEvents(path: string): Promise<ObserveEvent[]> {
  try {
    ...
    const raw = path.endsWith(".gz") ? gunzipSync(bytes).toString("utf8") : bytes.toString("utf8");
...
async function readRetainedEvents(dir: string): Promise<ObserveEvent[]> {
  const entries = await readdir(dir, { withFileTypes: true });
  const paths = entries
    .filter((entry) => entry.isFile() && (entry.name === "events.jsonl" || /^events-.*\.jsonl\.(?:gz|rotating)$/u.test(entry.name)))
    .map((entry) => join(dir, entry.name))
    .sort();
  const groups = await Promise.all(paths.map(readEvents));
  return groups.flat().sort((left, right) => left.at.localeCompare(right.at));
}
...
async function report(): Promise<Record<string, unknown>> {
  ...
  const events = await readRetainedEvents(dir);
