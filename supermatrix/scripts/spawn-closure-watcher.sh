...
try {
  ...
  const hasCanonicalPointerTable = db.prepare(
    "SELECT 1 FROM sqlite_master WHERE type = 'table' AND name = 'message_run_canonical_pointers'",
  ).get() !== undefined;
  const canonicalFinalReadySql = hasCanonicalPointerTable
    ? ` OR EXISTS (
    ...
    .prepare(
      `SELECT sai.ref, sai.comm_id, sai.caller_session, sai.target_session,
       ...
       WHERE sai.status IN ('pending', 'waiting_child')
         AND sai.created_at >= ?
         AND (TRIM(COALESCE(mr.final_message, c.final_message, '')) <> ''${canonicalFinalReadySql})
