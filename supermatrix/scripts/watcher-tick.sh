...
import {
  ...
  visibleMessageRunContentAtPath,
...
const skTarget = process.env.SPAWN_CLOSURE_ADJUDICATION_SESSION?.trim() || "sk-watcher";
const sourceSession = "supermatrix-root";
const sopPath = "/Users/LOCAL_USER/SuperMatrixRuntime/workspaces/socail-king/sop/spawn-exception-transaction.md";

function cstDate(): string {
  return new Intl.DateTimeFormat("en-CA", { timeZone: "Asia/Shanghai" }).format(new Date());
}

function spawn2ClientRequestId(prefix: string, key: string): string {
  const safeKey = key.replace(/[^A-Za-z0-9:._-]/g, "_");
  return `${cstDate()}:${prefix}:${safeKey}`.slice(0, 240);
}
...
function getLatestMessageRun(db: Database.Database, row: OpenPredicateRow): MessageRunSnapshot | null {
  ...
  if (row.message_run_id) {
    ...
    if (byId) {
      byId.final_message = visibleMessageRunContent(db, byId.id, "final_message", byId.final_message);
      return byId;
    }
  }
  ...
  if (latest) {
    latest.final_message = visibleMessageRunContent(db, latest.id, "final_message", latest.final_message);
  }
  return latest;
}
...
async function routeToSkOrFallback(db: Database.Database, payload: SkPayload): Promise<boolean> {
  ...
  const requestBody = {
    target: skTarget,
    from: skTarget,
    client_request_id: spawn2ClientRequestId("watcher-tick", payload.tx_id),
  ...
  const spawnResult = curlPostJson(`${apiBase}/api/spawn2.0`, requestBody, 30);
...
async function runTick(db: Database.Database): Promise<TickSummary> {
  ...
  for (const row of rows) {
    ...
    const result = await evaluateSpawnPredicate(predicate, {
      ...
      resolveMessageRunContent: visibleMessageRunContentAtPath,
    });
