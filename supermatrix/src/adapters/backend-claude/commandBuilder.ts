...
function buildBaseArgs(input: RunInput): string[] {
  ...
  const args: string[] = [
    ...
    "--output-format",
    "stream-json",
    // Every run reads its prompt from stream-json stdin and asks Claude Code
    // to echo stdin user envelopes back (--replay-user-messages). The replay
    // is the only acceptance proof for /now steering: a successful stdin
    // write alone never counts as "injected".
    "--input-format",
    "stream-json",
  ...
  const model = execution.model ?? FALLBACK_DEFAULT_MODEL;
  ...
  if (cardAskConfig) {
    args.push("--mcp-config", buildClaudeCardAskMcpConfig(cardAskConfig));
  }
  // answer-only mode: no resume — external non-owner gets a fresh ephemeral context.
  const forkSource = !answerOnly ? input.conversationFork?.sourceBackendSessionId : undefined;
  if (forkSource) {
    args.push("--resume", forkSource, "--fork-session");
  } else if (!answerOnly && input.session.backendSessionId) {
...
export function buildClaudeCommand(input: RunInput): ClaudeCommand {
  ...
  const prompt = prependSystemHint(
    buildPromptWithAttachments(input.prompt, hintAttachments, input.session.workdir),
    input.systemHint,
  );
