...
import {
  filterCodexModelsForCurrentLogin,
...
export async function resolveCodexDefaultModel(
  ...
  parsed.models.forEach((entry, index) => {
    ...
    candidates.push({ slug, supportedEfforts: extractSupportedEfforts(entry), priority, index });
  });
  ...
  const availableModels = new Set(
    filterCodexModelsForCurrentLogin(candidates.map((candidate) => candidate.slug)),
  );
  const availableCandidates = candidates.filter((candidate) => availableModels.has(candidate.slug));
  if (availableCandidates.length === 0) {
    return {
      kind: "fail",
      error: "no candidate models remain after current-login availability filter",
    };
  }
  const models = availableCandidates.map((candidate) => candidate.slug);
  const hasEffortDetails = availableCandidates.some(
    (candidate) => candidate.supportedEfforts.length > 0,
  );
  return {
    kind: "ok",
    slug: models[0]!,
    ...
    ...(hasEffortDetails
      ? {
          modelDetails: availableCandidates.map((candidate) => ({
            slug: candidate.slug,
...
function extractSupportedEfforts(entry: Record<string, unknown>): string[] {
