...
function startCodexAppServerRun(opts: StartCodexAppServerRunOptions): CodexRunHandle {
  ...
  let canonicalRefAnnounced = false;
  ...
  const announceCanonicalRef = () => {
    if (canonicalRefAnnounced || !threadId || !activeTurnId) return;
    canonicalRefAnnounced = true;
    push({
      kind: "canonical_ref",
      backendSessionId: threadId,
      canonicalObjectId: activeTurnId,
    });
  };
