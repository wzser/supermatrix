...
// codex-cli's background model-catalog refresher logs this exact line to stderr
// when its child process times out (verified in live mr_f9c9e220, alongside a
// real "at capacity" API error already on stdout). It is secondary noise: when
// a real error surfaced on stdout it must not mask it, and when it appears
// alone the caller still surfaces the non-zero exit so no false success. The
// leading RUST_LOG timestamp/level varies, so match the stable message tail.
const CODEX_MODEL_MANAGER_CHILD_EXIT_TIMEOUT =
  /ERROR codex_models_manager::manager: failed to refresh available models: timeout waiting for child process to exit$/u;

function isKnownCodexStderrNoise(line: string): boolean {
  const trimmed = line.trim();
  return (
    trimmed === CODEX_STDIN_PROMPT_NOISE ||
    CODEX_MODEL_MANAGER_CHILD_EXIT_TIMEOUT.test(trimmed)
  );
}
...
function filterKnownCodexStderrNoise(stderr: string): string {
  return stderr
    .split(/\r?\n/u)
    .filter((line) => !isKnownCodexStderrNoise(line))
    .join("\n")
    .trim();
}
...
export function normalizeCodexChildEnv(env: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
