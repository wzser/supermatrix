...
//
// The per-run `codex app-server` transport (appServerProcess.ts) delivers
// camelCase JSON-RPC notifications instead of the exec-json snake_case line
// records above. This mapper translates them into the same AgentEvent
// contract with the same staging semantics: assistant text is held pending
// and flushed as `final` only at turn completion (or demoted to `thinking`
// when more items follow), commentary stays out of the final body, and one
// merged usage event is committed per turn.

export type CodexAppServerEventState = {
  sessionAnnounced: boolean;
  model?: string;
  pendingAgentMessage?: string | undefined;
  /** Latest thread/tokenUsage/updated snapshot; committed at turn end. */
  lastTokenUsage?: JsonRecord | undefined;
  lastContextWindowTokens?: number | undefined;
  sawError: boolean;
  /** Set once a terminal turn/completed notification has been mapped. */
  turnFinished: boolean;
};

export function createCodexAppServerEventState(
  fallbackModel?: string | null,
): CodexAppServerEventState {
  return {
    sessionAnnounced: false,
    sawError: false,
    turnFinished: false,
...
/**
 * Announce the backend session id from a thread/start | thread/resume RPC
 * response (`{thread:{id}, model}`). The response is authoritative: the real
 * app-server does not always emit a thread/started notification (verified on
 * codex-cli 0.146.0 with ephemeral threads). Idempotent with the
 * notification path via sessionAnnounced.
 */
export function announceCodexAppServerThread(
  result: unknown,
  state: CodexAppServerEventState,
): AgentEvent[] {
  if (!isRecord(result)) return [];
  const model = getNonEmptyString(result.model);
  if (model) state.model = model;
  const thread = getNestedRecord(result, "thread");
  const threadId = getNonEmptyString(thread?.id);
  if (!threadId || state.sessionAnnounced) return [];
  state.sessionAnnounced = true;
  return [{ kind: "started", backendSessionId: threadId }];
}
...
export function mapCodexAppServerNotification(
  ...
  switch (method) {
    case "thread/started": {
      const thread = getNestedRecord(record, "thread");
      const threadId = getNonEmptyString(thread?.id);
      if (threadId && !state.sessionAnnounced) {
        state.sessionAnnounced = true;
        events.push({ kind: "started", backendSessionId: threadId });
      }
      break;
    }
    case "item/started": {
      ...
      if (item?.type === "commandExecution") {
        flushPendingAppServerMessage(events, state);
        const command = getNonEmptyString(item.command) ?? "command_execution";
        events.push({ kind: "tool_call", name: command, args: {} });
      } else if (item?.type === "mcpToolCall") {
        flushPendingAppServerMessage(events, state);
        const name = getNonEmptyString(item.tool) ?? "mcp_tool_call";
        const event: Extract<AgentEvent, { kind: "tool_call" }> = {
          kind: "tool_call",
          name,
          args: parseFunctionArguments(item.arguments),
        };
        const callId = getNonEmptyString(item.id);
        if (callId) event.callId = callId;
        events.push(event);
      }
      break;
    ...
    case "item/completed": {
      ...
      if (item?.type === "agentMessage") {
        ...
        events.push({
          ...
          name: command,
          result: {
            output: getString(item.aggregatedOutput) ?? "",
            exitCode: getNumber(item.exitCode) ?? null,
          },
        });
      } else if (item?.type === "mcpToolCall") {
        flushPendingAppServerMessage(events, state);
        ...
        const event: Extract<AgentEvent, { kind: "tool_result" }> = {
          kind: "tool_result",
          name,
          result: { output: item.error ?? item.result ?? "" },
        };
        const callId = getNonEmptyString(item.id);
        if (callId) event.callId = callId;
        events.push(event);
    ...
    case "thread/tokenUsage/updated": {
      const tokenUsage = getNestedRecord(record, "tokenUsage");
      const last = tokenUsage ? getNestedRecord(tokenUsage, "last") : undefined;
      if (last) {
        state.lastTokenUsage = last;
        const window = toPositiveIntOrUndefined(tokenUsage?.modelContextWindow);
        if (window) state.lastContextWindowTokens = window;
      }
      break;
    }
    case "error": {
      // willRetry errors are transient stream retries codex handles itself
      // (exec-era equivalents only reached stderr and were filtered as
      // noise); surfacing them would fail runs that go on to succeed.
      if (record.willRetry === true) break;
      const error = getNestedRecord(record, "error");
      const message = getNonEmptyString(error?.message) ?? "unknown codex error";
      state.sawError = true;
      events.push({ kind: "error", message, recoverable: false });
      break;
    }
    case "turn/completed": {
      const turn = getNestedRecord(record, "turn");
      const status = getString(turn?.status);
      state.turnFinished = true;
      // Usage first, matching the exec parser's token_count-before-completed
      // ordering that downstream consumers already handle.
      const usage = commitAppServerUsage(state);
      if (usage) events.push(usage);
      if (status === "failed") {
        // Verbatim message pass-through: capacity/model-unavailable matchers
        // in the runtime recovery layer key on the raw API error text.
        if (!state.sawError) {
          const error = turn ? getNestedRecord(turn, "error") : undefined;
          const message = getNonEmptyString(error?.message) ?? "codex turn failed";
          state.sawError = true;
          events.push({ kind: "error", message, recoverable: false });
        }
      } else if (status === "interrupted") {
        events.push({ kind: "error", message: "cancelled by user", recoverable: false });
      } else if (state.pendingAgentMessage) {
        const text = state.pendingAgentMessage;
        delete state.pendingAgentMessage;
        events.push({ kind: "assistant_message", text, final: true });
        events.push({ kind: "completed", finalMessage: text });
      } else if (!state.sawError) {
        // Same guard as the exec parser's task_complete handling: a turn
        // that completes with no assistant message must surface a stable
        // error, or downstream sinks silently drop the empty reply.
        events.push({
          kind: "error",
...
function flushPendingAppServerMessage(
  events: AgentEvent[],
  state: CodexAppServerEventState,
): void {
  const text = state.pendingAgentMessage;
...
/**
 * Commit (and clear) the last observed tokenUsage snapshot. Called at
 * turn/completed for the normal path, and by the adapter's process-exit
 * handler so a killed run (timeout kill, crash) still accounts its tokens —
 * exec-era token_count lines were committed incrementally and survived kills.
 * Clearing on commit makes a second call return undefined (no double count).
 */
export function commitAppServerUsage(state: CodexAppServerEventState): UsageEvent | undefined {
  const last = state.lastTokenUsage;
  delete state.lastTokenUsage;
  if (!last) return undefined;
  const inputTokens = toInt(last.inputTokens);
  const cacheReadTokens = toInt(last.cachedInputTokens);
  const rawOutputTokens = toInt(last.outputTokens);
  const reasoningTokens = toInt(last.reasoningOutputTokens);
  if (inputTokens + cacheReadTokens + rawOutputTokens + reasoningTokens === 0) return undefined;
  ...
  return {
    ...
    cacheWriteTokens: 0,
    reasoningTokens,
    ...(state.lastContextWindowTokens
      ? { contextWindowTokens: state.lastContextWindowTokens }
      : {}),
    // Same snake_case raw evidence shape as the exec parser so stream_log
    // consumers see one format across both transports.
    rawUsage: stripUndefined({
      input_tokens: inputTokens,
      cached_input_tokens: cacheReadTokens,
      output_tokens: rawOutputTokens,
