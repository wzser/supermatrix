...
/**
 * session/update kinds kimi-code emits that the ACP SDK 0.4.5 union cannot
 * decode; they are dropped before the lib instead of logging -32602 each turn.
 * Ruling (Todo #549): 0.4.5 is npm latest, so there is no SDK to upgrade to;
 * SM consumes none of their semantics (token usage comes from wire.jsonl via
 * usageWire.ts, ACP session titles are unused, eventTranslator ignores plans).
 * config_option_update still records the session's model/thinking first.
 * Any kind not listed here still reaches the SDK and fails loudly.
 */
export const KNOWN_IGNORABLE_SESSION_UPDATES = [
...
// 30s (was 10s): the shared ACP child multiplexes every kimi session; under
// load (e.g. K3 MAX turns streaming) an idle-period roundtrip already takes
// ~5s, so 10s turned ordinary RPC starvation into spurious timeouts.
const DEFAULT_SESSION_RPC_TIMEOUT_MS = 30_000;
// The health probe stays inside the 15s HTTP budget its callers allow
// (DEFAULT_KIMI_ACP_HEALTH_TIMEOUT_MS in backend-api-connectivity.ts,
// localwatch curl --max-time 15) — it must NOT follow the session-RPC bump.
const DEFAULT_HEALTH_RPC_TIMEOUT_MS = 10_000;
...
/** The shared client was recycled mid-flight (dispose/invalidate). */
export class AcpDisposedError extends Error {
  constructor(reason?: string) {
    super(reason ? `AcpClient has been disposed: ${reason}` : "AcpClient has been disposed");
    this.name = "AcpDisposedError";
  }
}

/** An RPC was attempted before ensureReady established the connection. */
export class AcpNotReadyError extends Error {
  constructor() {
    super("AcpClient not ready — call ensureReady() first");
    this.name = "AcpNotReadyError";
  }
}
...
export class AcpClient {
  ...
  private async invalidate(reason: string): Promise<never> {
    await this.dispose();
    throw new AcpDisposedError(reason);
  }

  /**
   * Recycle the shared child only when it is provably not runnable. A bare
   * RPC timeout does NOT prove wedging — a busy shared process simply answers
   * slowly, and disposing it fails every other session multiplexed on it
   * (2026-09-10 mr_e263beae collateral failure). "unknown" (ps hiccup) is not
   * evidence either: only an explicit stopped/dead verdict recycles. When the
   * child is alive the caller's original error propagates and the client
   * stays usable.
   */
  private async invalidateIfChildUnrunnable(reason: string): Promise<void> {
    const liveness = await this.childLiveness().catch(() => "unknown" as const);
    if (liveness === "stopped" || liveness === "dead") {
      await this.invalidate(`${reason}; managed ACP child is ${liveness}`);
    }
  }
  ...
  async ensureReady(): Promise<void> {
    ...
    if (this.state === "dead") throw new AcpDisposedError();
  ...
  private async start(): Promise<void> {
    ...
    nodeStdout.on("data", (chunk: Buffer) => {
      ...
      while ((idx = lineBuf.indexOf("\n")) >= 0) {
        ...
        if (!this.routeRawResponse(line) && !this.consumeKnownIgnorableUpdate(line)) {
  ...
  private async runBoundedSessionRpc<T>(
    ...
    try {
      ...
      if (err instanceof AcpSessionRpcTimeoutError) {
        await this.invalidateIfChildUnrunnable(err.message);
      }
      throw err;
    } finally {
  ...
  async newSession(params: { cwd: string; mcpServers?: any[] }): Promise<string> {
    ...
    if (!conn) throw new AcpNotReadyError();
  ...
  async prompt(args: PromptArgs): Promise<PromptResponse> {
    if (!this.conn) throw new AcpNotReadyError();
  ...
  private sendRawRequest(method: string, params: unknown, timeoutMs = DEFAULT_SESSION_RPC_TIMEOUT_MS): Promise<unknown> {
    const stdin = this.rawStdin;
    if (!stdin) throw new AcpNotReadyError();
   ...
   */
  async probeHealth(timeoutMs = DEFAULT_HEALTH_RPC_TIMEOUT_MS): Promise<{ rttMs: number }> {
    ...
    try {
      ...
      await this.invalidateIfChildUnrunnable(`shared ACP roundtrip failed: ${message}`);
      throw err;
    }
  ...
  /**
   * Drop session notifications whose kind is in KNOWN_IGNORABLE_SESSION_UPDATES
   * before the 0.4.5 lib sees them (its schema predates them and logs a noisy
   * "Error handling notification" for every one). `config_option_update`
   * (kimi-code 0.26.0+, e.g. after session/set_model) first records the
   * current model. Returns true when the line was such a notification.
   */
  private consumeKnownIgnorableUpdate(line: string): boolean {
    // cheap pre-filter
    if (!KNOWN_IGNORABLE_SESSION_UPDATES.some((kind) => line.includes(`"${kind}"`))) return false;
    let msg: {
      method?: string;
      params?: {
        sessionId?: string;
    ...
    if (
      ...
      typeof kind !== "string" ||
      !(KNOWN_IGNORABLE_SESSION_UPDATES as readonly string[]).includes(kind)
    ) {
      return false;
    }
  ...
  async dispose(): Promise<void> {
    ...
    for (const [, pending] of this.rawPending) {
      clearTimeout(pending.timer);
      pending.reject(new AcpDisposedError("raw request pending during dispose"));
    }
