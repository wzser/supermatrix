...
const isDisposedAcpClientError = (err: unknown): boolean =>
  err instanceof AcpDisposedError ||
  /AcpClient (has been )?disposed/.test(errorMessage(err));

// Thrown when a concurrent run recycled the shared ACP client between this
// run's ensureReady and a later RPC (2026-09-10 mr_e263beae). Recoverable:
// re-ensure and retry once instead of failing the run.
const isNotReadyAcpClientError = (err: unknown): boolean =>
  err instanceof AcpNotReadyError ||
  /AcpClient not ready — call ensureReady\(\) first/.test(errorMessage(err));
...
export class KimiBackend implements AgentBackend {
  ...
  /**
   * Run one shared-client RPC, retrying ONCE after re-ensure when the client
   * was yanked mid-setup by a concurrent run (disposed / not-ready errors).
   * The operation closure must read `this.acp` at call time so the retry can
   * land on a recreated client. Genuine RPC errors (bad params, timeouts)
   * propagate immediately.
   *
   * `reattach` re-loads the ACP session on the (possibly recreated) client
   * before the retry: a fresh shared process has never loadSession'd this
   * session, so retrying set_model/set_config without it would fail or apply
   * to nothing (rereview 2026-09-10). Not needed for the acquisition RPCs
   * (newSession / loadSession) themselves.
   */
  private async withSharedAcpRetry<T>(
    operation: (acp: AcpClient) => Promise<T>,
    reattach?: (acp: AcpClient) => Promise<void>,
  ): Promise<T> {
    try {
      return await operation(this.acp);
    } catch (err) {
      if (!isDisposedAcpClientError(err) && !isNotReadyAcpClientError(err)) throw err;
      await this.ensureAcpReady();
      if (reattach) await reattach(this.acp);
      return await operation(this.acp);
    }
  }
    ...
    return {
      async *[Symbol.asyncIterator]() {
        ...
        try {
          ...
          if (!acpSessionId) {
            // Retry policy for session/new (review rounds 3-4, 2026-09-10):
            // AcpNotReadyError is thrown by the pre-I/O `if (!conn)` guard —
            // the request provably never reached the server, so one retry
            // after re-ensure cannot create an orphan session. Any other
            // failure (including AcpDisposedError from a post-I/O
            // timeout-invalidate, where the server may already have
            // committed) fails the run instead of risking a duplicate.
            try {
              ...
              if (!isNotReadyAcpClientError(err)) throw err;
              await self.ensureAcpReady();
              acpSessionId = await self.acp.newSession({
                cwd: safeInput.session.workdir,
                mcpServers,
            ...
            await self.withSharedAcpRetry((acp) =>
              acp.loadSession({
        ...
        // Re-load this ACP session on a recreated shared client before
        // retrying per-session RPCs (see withSharedAcpRetry). Idempotent, and
        // valid for resumed and this-run-created sessions alike: kimi-code
        // persists sessions on session/new, so the id is loadable from disk.
        const reattachSession = async (acp: AcpClient) => {
