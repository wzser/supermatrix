...
export const DEFAULT_MCP_ASK_SERVER_PATH =
...
type BrokerEnv = {
  ...
  SM_CARD_ASK_MCP_SERVER_PATH?: string | undefined;
};
...
function isUsableMcpAskServerPath(candidate: string): boolean {
  if (!path.isAbsolute(candidate) || candidate.includes("\0") || path.normalize(candidate) !== candidate) return false;
  try {
...
/**
 * Isolated onboarding instances provide their materialized public larkc path.
 * An omitted override keeps the existing live-workspace fallback for backward
 * compatibility; an explicit but unusable override disables card-ask instead
 * of silently routing an agent to a different workspace.
 */
export function resolveMcpAskServerPath(env: BrokerEnv = process.env): string | null {
  const configured = env.SM_CARD_ASK_MCP_SERVER_PATH;
  if (configured !== undefined) {
    const candidate = configured.trim();
    return candidate && isUsableMcpAskServerPath(candidate) ? candidate : null;
  }
  return DEFAULT_MCP_ASK_SERVER_PATH;
}
...
export function buildCardAskRuntimeConfig(
  ...
  const mcpAskServerPath = resolveMcpAskServerPath(env);
  if (!mcpAskServerPath) return null;
