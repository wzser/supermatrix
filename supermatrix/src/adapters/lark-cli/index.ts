...
export class LarkCliGateway implements LarkGateway {
  ...
  private async handleRaw(raw: LarkRawInbound, handler: InboundHandler): Promise<void> {
    ...
    const msg: InboundMessage = {
      ...
      origin: classifyInboundOrigin(messageRaw.text),
...
/**
 * The user-scoped lark-cli path cannot add a trustworthy event-side marker:
 * Feishu reports the authenticated human sender for `--as user`. The Δ
 * prefix is the existing framework contract for that path, so classify it at
 * the single inbound boundary and persist the classification downstream.
 */
export function classifyInboundOrigin(text: string): MessageOrigin {
  return text.startsWith("Δ") ? "lark_user_synthetic" : "lark_user";
}
