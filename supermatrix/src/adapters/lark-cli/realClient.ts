...
import type {
  ...
  DriveCommentPollWatch as DriveCommentPollWatchConfig,
  ReferencedMessage,
...
export type RealLarkClientConfig = {
  larkCliPath: string;
  /** Optional profile. When set every CLI call is pinned to this profile. */
  profile?: string;
  /** Optional complete process environment for isolated callers. */
  ...
  mentionRegistryPollLoader?: () => Promise<DriveCommentPollWatchConfig[]>;
  wsClientFactory?: (params: LarkWsClientParams) => LarkWsClient;
  eventDispatcherFactory?: () => LarkWsEventDispatcher;
};

type LarkCliRunConfig = Pick<RealLarkClientConfig, "larkCliPath" | "noProxy" | "profile" | "env">;
...
type DriveCommentPollWatch = DriveCommentPollWatchConfig & {
  // Which configuration source produced this watch. Provenance is what breaks a
  // requireMention tie when the watches file and the mention registry describe
  // the same watch key.
  origin?: "watch_file" | "mention_registry";
  stateTargets?: DriveCommentPollStateTarget[];
};

type DriveCommentPollStateTarget =
  | { kind: "watch_file"; path: string; watchKey: string }
  | { kind: "mention_registry"; persistSince: (since: number) => Promise<void> };

function resolveEnv(cfg: LarkCliRunConfig): NodeJS.ProcessEnv {
  const base = cfg.env ?? process.env;
  if (cfg.noProxy === false) return base;
  return { ...base, LARK_CLI_NO_PROXY: "1" };
}

function cliArgs(cfg: LarkCliRunConfig, args: string[]): string[] {
  return cfg.profile ? ["--profile", cfg.profile, ...args] : args;
}
...
async function runLarkCli<T>(
  ...
  let stdout = "";
  try {
    const result = await execFileP(cfg.larkCliPath, cliArgs(cfg, args), {
      env: resolveEnv(cfg),
...
function pickDriveCommentAnchor(
  ...
  const reply = replyId
    ...
    : undefined;
  const rootReply = item.reply_list?.replies?.find(
    (candidate) => candidate.reply_id === item.comment_id,
  ) ?? item.reply_list?.replies?.[0];
  const extras = [
    pickRecordField(reply, "extra"),
    pickRecordField(rootReply, "extra"),
...
function driveCommentPollWatchKey(watch: DriveCommentPollWatch): string {
  return `${watch.fileType}:${watch.fileToken}:${watch.tableId ?? ""}:${watch.recordId ?? ""}`;
}

function minDriveCommentPollIntervalSec(
...
// Floor for per-watch polling. Registry owners choose the cadence, but the
// adapter refuses to hammer the Drive comment API faster than this.
const MIN_DRIVE_COMMENT_POLL_INTERVAL_SEC = 60;

function isRegistryDerivedDriveCommentWatch(watch: DriveCommentPollWatch): boolean {
  return watch.origin === "mention_registry";
}

// The mention registry is the only source allowed to open the unmentioned gate;
// the watches file stays a mention-only fallback. When both describe the same
// watch key with different gates, the registry-derived value wins, and an
// unresolvable tie fails closed (mention required) with a log line. Never OR /
// AND the two flags together: an implicit merge lets a stale watches-file entry
// silently widen — or a leftover registry entry silently narrow — the gate.
function mergedDriveCommentWatchOrigin(
  existing: DriveCommentPollWatch,
  incoming: DriveCommentPollWatch,
): DriveCommentPollWatch["origin"] {
  return isRegistryDerivedDriveCommentWatch(existing) || isRegistryDerivedDriveCommentWatch(incoming)
    ? "mention_registry"
    : existing.origin ?? incoming.origin;
}

function resolveMergedRequireMention(
  existing: DriveCommentPollWatch,
  incoming: DriveCommentPollWatch,
): boolean {
  if (existing.requireMention === incoming.requireMention) return existing.requireMention;
  const existingFromRegistry = isRegistryDerivedDriveCommentWatch(existing);
  const incomingFromRegistry = isRegistryDerivedDriveCommentWatch(incoming);
  const watchKey = driveCommentPollWatchKey(existing);
  if (existingFromRegistry !== incomingFromRegistry) {
    const requireMention = existingFromRegistry
      ? existing.requireMention
      : incoming.requireMention;
    console.warn("[lark-cli] drive comment watch requireMention conflict resolved by registry", {
      watchKey,
      requireMention,
    });
    return requireMention;
  }
  console.warn("[lark-cli] drive comment watch requireMention conflict; failing closed", {
    watchKey,
  });
  return true;
}
  ...
  for (const raw of rawWatches) {
    ...
    watches.push({
      ...
      stateTargets: [{
        kind: "watch_file",
        path: pathname,
        watchKey: `${fileType}:${fileToken}:${tableId ?? ""}:${recordId ?? ""}`,
      }],
    });
  }
  return watches;
...
async function readMentionRegistryPollWatches(
  loader: () => Promise<DriveCommentPollWatchConfig[]>,
): Promise<DriveCommentPollWatch[]> {
  try {
    const coldStartSince = Math.floor(Date.now() / 1_000);
    const watches: DriveCommentPollWatch[] = [];
    for (const watch of await loader()) {
      // A newly enabled registry watch starts at activation time. Treat zero
      // like an absent cursor so it cannot be used to replay all history.
      const since = watch.requireMention === false
        && (watch.since === undefined || watch.since === 0)
        ? coldStartSince
        : watch.since;
      if (since !== undefined && since !== watch.since) await watch.persistSince?.(since);
      watches.push({
        ...
        ...(since !== undefined ? { since } : {}),
        requireMention: watch.requireMention,
        ...(watch.intervalSec !== undefined ? { intervalSec: watch.intervalSec } : {}),
        ...(watch.url ? { url: watch.url } : {}),
        origin: "mention_registry" as const,
        ...(watch.persistSince
          ? { stateTargets: [{ kind: "mention_registry" as const, persistSince: watch.persistSince }] }
          : {}),
      });
    }
    return watches;
  } catch (err) {
    console.warn(`[lark-cli] mention registry poll watch read failed: ${(err as Error).message}`);
    return [];
  }
}
...
function mergeDriveCommentWatches(watches: DriveCommentPollWatch[]): DriveCommentPollWatch[] {
  ...
  for (const watch of watches) {
    const key = driveCommentPollWatchKey(watch);
    ...
    if (watch.since === undefined || existing.since === undefined) {
      const intervalSec = minDriveCommentPollIntervalSec(existing.intervalSec, watch.intervalSec);
      const origin = mergedDriveCommentWatchOrigin(existing, watch);
      const next = {
        ...existing,
        ...watch,
        requireMention: resolveMergedRequireMention(existing, watch),
        ...(origin !== undefined ? { origin } : {}),
        ...(intervalSec !== undefined ? { intervalSec } : {}),
        stateTargets: [...(existing.stateTargets ?? []), ...(watch.stateTargets ?? [])],
      };
...
async function persistDriveCommentPollSince(
  watch: DriveCommentPollWatch,
  since: number,
): Promise<void> {
  for (const target of watch.stateTargets ?? []) {
    if (target.kind === "mention_registry") {
      await target.persistSince(since);
      continue;
    }
    ...
    const rawWatches = Array.isArray(parsed)
      ...
      ? parsed.watches
      : [];
    const rawWatch = rawWatches.find((candidate) => {
      if (!isRecord(candidate)) return false;
      const fileToken = pickStringField(candidate, "fileToken");
      const fileType = normalizeDriveCommentFileType(pickStringField(candidate, "fileType"));
      const tableId = pickStringField(candidate, "tableId") ?? pickStringField(candidate, "table_id");
      const url = pickStringField(candidate, "url");
      const recordId = pickStringField(candidate, "recordId")
        ?? pickStringField(candidate, "record_id")
        ?? recordIdFromDriveCommentUrl(url);
      return fileToken && fileType
        ? `${fileType}:${fileToken}:${tableId ?? ""}:${recordId ?? ""}` === target.watchKey
        : false;
    });
    if (!isRecord(rawWatch)) continue;
    rawWatch.since = since;
...
async function fetchBotOpenId(cfg: RealLarkClientConfig): Promise<string | undefined> {
  ...
  try {
    const result = await execFileP(cfg.larkCliPath, cliArgs(cfg, [
      "api", "GET",
...
export function createRealLarkClient(cfg: RealLarkClientConfig): RealLarkClient {
  ...
  return {
    ...
    async downloadAttachment(opts: {
      ...
      try {
        await execFileP(cfg.larkCliPath, cliArgs(cfg, [
    ...
    subscribeInbound(cb: (raw: LarkRawInbound) => void): () => Promise<void> {
      ...
      let pollRefreshTimer: ReturnType<typeof setTimeout> | undefined;
      ...
      const pollWatches = new Map<string, DriveCommentPollWatch>();
      const pollInFlight = new Set<string>();
      const activePollPromises = new Set<Promise<void>>();
      let pollStateWriteChain: Promise<void> = Promise.resolve();
      ...
      const trackPollPromise = (promise: Promise<void>): void => {
        activePollPromises.add(promise);
        void promise.then(
          () => activePollPromises.delete(promise),
          () => activePollPromises.delete(promise),
        );
      };

      const persistPollSince = async (
        watch: DriveCommentPollWatch,
        since: number,
      ): Promise<void> => {
        const write = pollStateWriteChain.catch(() => {}).then(
          () => persistDriveCommentPollSince(watch, since),
        );
        pollStateWriteChain = write;
        await write;
      };

      const isCurrentPollWatch = (
        watchKey: string,
        _watch: DriveCommentPollWatch,
        generation: number,
      ): boolean => !stopped
        && generation === cancellationGeneration
        && pollWatches.has(watchKey);

      const emitPolledDriveComments = async (
        watchKey: string,
        generation: number,
      ): Promise<void> => {
        const watch = pollWatches.get(watchKey);
        if (!watch || pollInFlight.has(watchKey) || stopped) return;
        pollInFlight.add(watchKey);
        const startedAt = Date.now();
        let scannedCount = 0;
        let emittedCount = 0;
        let nextSince = watch.since;
        try {
          const botOpenId = await getBotOpenId();
          if (!isCurrentPollWatch(watchKey, watch, generation)) return;
          if (!botOpenId) {
            console.warn(
              "[lark-cli] drive comment poll skipped: bot open_id unavailable; self-reply suppression is fail-closed",
            );
            return;
          }
          ...
          do {
            ...
            for (const item of data.items ?? []) {
              ...
              if (watch.since !== undefined && updateTime !== undefined && updateTime <= watch.since) {
                continue;
              }
              if (updateTime !== undefined) nextSince = Math.max(nextSince ?? updateTime, updateTime);
              ...
              for (const { commentId: collectedCommentId, reply } of collected) {
                ...
                scannedCount += 1;
                nextSince = Math.max(nextSince ?? createTime, createTime);
                if (watch.since !== undefined && createTime <= watch.since) continue;
                if (reply.user_id === botOpenId) continue;
                const mentioned = driveReplyMentionsBot(reply, botOpenId);
                if (watch.requireMention !== false && !mentioned) continue;
                const anchor = pickDriveCommentAnchor(item, replyId);
                if (watch.tableId && anchor.tableId !== watch.tableId) continue;
                if (watch.recordId && anchor.recordId !== watch.recordId) continue;
          ...
          if (nextSince !== undefined && nextSince !== watch.since) {
            if (!isCurrentPollWatch(watchKey, watch, generation)) return;
            await persistPollSince(watch, nextSince);
            if (!isCurrentPollWatch(watchKey, watch, generation)) return;
            watch.since = nextSince;
          }
        } finally {
          pollInFlight.delete(watchKey);
          console.log("[lark-cli] drive comment poll swept", {
            watchKey,
            scannedCount,
            emittedCount,
      ...
      const watchIntervalMs = (watch: DriveCommentPollWatch): number =>
        watch.intervalSec !== undefined
          ? Math.max(watch.intervalSec, MIN_DRIVE_COMMENT_POLL_INTERVAL_SEC) * 1_000
          : pollIntervalMs;

      const scheduleDriveCommentPoll = (watchKey: string, delayMs?: number): void => {
        if (stopped || pollTimers.has(watchKey)) return;
        const watch = pollWatches.get(watchKey);
        if (!watch) return;
        const timer = setTimeout(() => {
          pollTimers.delete(watchKey);
          const generation = cancellationGeneration;
          const pollPromise = emitPolledDriveComments(watchKey, generation);
          trackPollPromise(pollPromise);
          void pollPromise
            .catch((err) => {
              console.warn(`[lark-cli] drive comment poll failed: ${(err as Error).message}`);
            })
            .finally(() => {
              if (!stopped && generation === cancellationGeneration && pollWatches.has(watchKey)) {
                scheduleDriveCommentPoll(watchKey);
              }
            });
        }, delayMs ?? watchIntervalMs(watch));
        pollTimers.set(watchKey, timer);
      };

      const readConfiguredPollWatches = async (): Promise<DriveCommentPollWatch[]> => {
        const sources = await Promise.all([
          cfg.driveCommentPollPath ? readDriveCommentPollWatches(cfg.driveCommentPollPath) : [],
          cfg.mentionRegistryPollLoader
            ? readMentionRegistryPollWatches(cfg.mentionRegistryPollLoader)
            : [],
        ]);
        return mergeDriveCommentWatches(sources.flat());
      };

      const refreshDriveCommentPollWatches = async (runNewImmediately: boolean): Promise<void> => {
        if (pollRefreshInFlight || stopped) return;
        pollRefreshInFlight = true;
        try {
          const watches = await readConfiguredPollWatches();
          if (stopped) return;
          const nextKeys = new Set(watches.map(driveCommentPollWatchKey));
          for (const watchKey of pollWatches.keys()) {
            if (nextKeys.has(watchKey)) continue;
            const timer = pollTimers.get(watchKey);
            if (timer) {
              clearTimeout(timer);
              pollTimers.delete(watchKey);
            }
            pollWatches.delete(watchKey);
          }
          for (const watch of watches) {
            const watchKey = driveCommentPollWatchKey(watch);
            const previous = pollWatches.get(watchKey);
            pollWatches.set(watchKey, watch);
            if (!previous) {
              scheduleDriveCommentPoll(watchKey, runNewImmediately ? 0 : undefined);
              continue;
            }
            if (watchIntervalMs(previous) !== watchIntervalMs(watch)) {
              const timer = pollTimers.get(watchKey);
              if (timer) {
                clearTimeout(timer);
                pollTimers.delete(watchKey);
      ...
      const schedulePollWatchRefresh = (): void => {
        if (
          stopped
          || (!cfg.driveCommentPollPath
            && !cfg.mentionRegistryPollLoader)
        ) return;
        pollRefreshTimer = setTimeout(() => {
          pollRefreshTimer = undefined;
          const generation = cancellationGeneration;
          const refreshPromise = refreshDriveCommentPollWatches(true);
          trackPollPromise(refreshPromise);
          void refreshPromise
            .catch((err) => {
              console.warn(`[lark-cli] drive comment poll watch refresh failed: ${(err as Error).message}`);
            })
            .finally(() => {
              if (!stopped && generation === cancellationGeneration) schedulePollWatchRefresh();
            });
        }, pollIntervalMs);
      };
      ...
      const initialPollPromise = refreshDriveCommentPollWatches(true);
      trackPollPromise(initialPollPromise);
      void initialPollPromise
        .catch((err) => {
          console.warn(`[lark-cli] drive comment poll watch refresh failed: ${(err as Error).message}`);
        })
        .finally(() => {
          if (!stopped) schedulePollWatchRefresh();
        });
      ...
      return async () => {
        ...
        await Promise.allSettled([...activePollPromises]);
        await pollStateWriteChain.catch(() => {});
      };
    },
  };
}
