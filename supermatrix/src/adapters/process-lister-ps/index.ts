...
export function createPsProcessLister(): ProcessLister {
  return {
    ...
    async killAll(pids: number[]): Promise<number[]> {
      // Scope is deliberately narrow: boot reconciliation has already matched
      // these PIDs to orphan backend runs. Never pass SuperMatrix/localwatch PIDs
      // here or copy this SIGKILL escalation into a platform lifecycle path;
      // platform maintenance belongs to codexroot's audited gate.
      const killed: number[] = [];
