...
import type { MessageOrigin } from "../../domain/messageOrigin.ts";
...
import type {
  ...
  BeginSchedulerCleanupRequestInput,
...
import { LEGACY_MAIN_SESSION_DEFAULTS } from "../../ports/SessionRuntimeSettings.ts";
...
import {
  defaultCodexCanonicalStateDbPath,
  indexMessageRunCanonicalPointers,
  visibleMessageRunContent,
  type MessageRunCanonicalRef,
} from "./messageRunCanonical.ts";
...
type SchedulerCleanupRequestSqlRow = {
  client_request_id: string;
  session_id: string;
  session_name: string;
  status: SchedulerCleanupRequest["status"];
  attempt_count: number;
...
function mapSchedulerCleanupRequestRow(row: SchedulerCleanupRequestSqlRow): SchedulerCleanupRequest {
  return {
    ...
    submittedAt: row.submitted_at as Timestamp | null,
  };
}
...
const RUNTIME_CONFIG_BACKEND_WRITE_GUARD_FUNCTION = "sm_runtime_config_backend_write_allowed";

function exposeDbWithReservedRuntimeConfigGuard(db: Db): Db {
  const protectedFunction = ((name: string, ...args: unknown[]) => {
    if (name === RUNTIME_CONFIG_BACKEND_WRITE_GUARD_FUNCTION) {
      throw new Error("runtime config backend write guard is reserved");
    }
    return Reflect.apply(db.function, db, [name, ...args]) as Db;
  }) as Db["function"];
  const facade = Object.create(null) as Db;
  return new Proxy(facade, {
    get(_target, property) {
      if (typeof property === "symbol") return undefined;
      if (property === "function") return protectedFunction;
      const value = Reflect.get(db, property, db) as unknown;
      return typeof value === "function" ? value.bind(db) : value;
    },
    set(_target, property, value) {
      if (typeof property === "symbol" || property === "function") {
        throw new Error("runtime config backend write guard is reserved");
      }
      return Reflect.set(db, property, value, db);
    },
    defineProperty(_target, property, attributes) {
      if (typeof property === "symbol" || property === "function") {
        throw new Error("runtime config backend write guard is reserved");
      }
      return Reflect.defineProperty(db, property, attributes);
    },
    deleteProperty(_target, property) {
      if (typeof property === "symbol" || property === "function") {
        throw new Error("runtime config backend write guard is reserved");
      }
      return Reflect.deleteProperty(db, property);
    },
    getPrototypeOf: () => null,
    has: (_target, property) => typeof property !== "symbol" && Reflect.has(db, property),
  });
}
...
export class SqliteBindingStore implements BindingStore {
  ...
  #runtimeConfigBackendWriteDepth = 0;

  constructor(path: string, options: { codexStateDbPath?: string | null } = {}) {
    const db = openDb(path);
    db.function(
      RUNTIME_CONFIG_BACKEND_WRITE_GUARD_FUNCTION,
      () => this.#runtimeConfigBackendWriteDepth > 0 ? 1 : 0,
    );
    this.db = exposeDbWithReservedRuntimeConfigGuard(db);
    this.codexStateDbPath = options.codexStateDbPath === undefined
      ? defaultCodexCanonicalStateDbPath()
      : options.codexStateDbPath;
  }

  #withRuntimeConfigBackendWrite<T>(fn: () => T): T {
    this.#runtimeConfigBackendWriteDepth += 1;
    try {
      return fn();
    } finally {
      this.#runtimeConfigBackendWriteDepth -= 1;
  ...
  async applySessionRuntimeConfigMutations(
    ...
    this.#withRuntimeConfigBackendWrite(apply);
  ...
  private insertDefaultSessionRuntimeSettings(
    ...
    const defaults = this.readMainSessionDefaults(backend);
    this.db.prepare(
  ...
  private readMainSessionDefaults(
    backend: BackendKind,
  ): Pick<SessionRuntimeSettings, "mainModelDefault" | "mainEffortDefault"> {
    const fallback = LEGACY_MAIN_SESSION_DEFAULTS[backend];
    const table = this.db
      .prepare(
        "SELECT 1 AS present FROM sqlite_master WHERE type = 'table' AND name = 'backend_runtime_defaults'",
      )
      .get() as { present: 1 } | undefined;
    if (!table) return fallback;

    const row = this.db
      .prepare("SELECT model, effort FROM backend_runtime_defaults WHERE backend = ?")
      .get(backend) as { model: string | null; effort: EffortLevel | null } | undefined;
    return row
      ? { mainModelDefault: row.model, mainEffortDefault: row.effort }
      : fallback;
  }
  ...
  async beginSchedulerCleanupRequest(
    input: BeginSchedulerCleanupRequestInput,
  ): Promise<{ request: SchedulerCleanupRequest; shouldDispatch: boolean }> {
    const existing = this.db
      .prepare("SELECT * FROM scheduler_cleanup_requests WHERE client_request_id = ?")
      .get(input.clientRequestId) as SchedulerCleanupRequestSqlRow | undefined;
    if (existing) {
      if (existing.status === "failed") {
        const now = input.createdAt;
        this.db
          .prepare(
            `UPDATE scheduler_cleanup_requests
             SET status = 'pending', attempt_count = attempt_count + 1,
                 last_error = NULL, updated_at = ?, submitted_at = NULL
             WHERE client_request_id = ? AND status = 'failed'`,
          )
          .run(now, input.clientRequestId);
        const retried = this.db
          .prepare("SELECT * FROM scheduler_cleanup_requests WHERE client_request_id = ?")
          .get(input.clientRequestId) as SchedulerCleanupRequestSqlRow;
        return { request: mapSchedulerCleanupRequestRow(retried), shouldDispatch: true };
      }
      return { request: mapSchedulerCleanupRequestRow(existing), shouldDispatch: false };
    }

    this.db
      .prepare(
        `INSERT INTO scheduler_cleanup_requests
         (client_request_id, session_id, session_name, status, attempt_count,
          last_error, created_at, updated_at, submitted_at)
    ...
    const created = this.db
      .prepare("SELECT * FROM scheduler_cleanup_requests WHERE client_request_id = ?")
      .get(input.clientRequestId) as SchedulerCleanupRequestSqlRow;
    return { request: mapSchedulerCleanupRequestRow(created), shouldDispatch: true };
  }

  async markSchedulerCleanupSubmitted(clientRequestId: string, now: Timestamp): Promise<void> {
    this.db
      .prepare(
        `UPDATE scheduler_cleanup_requests
         SET status = 'submitted', last_error = NULL, updated_at = ?, submitted_at = ?
         WHERE client_request_id = ? AND status = 'pending'`,
      )
      .run(now, now, clientRequestId);
  }

  async markSchedulerCleanupFailed(clientRequestId: string, error: string, now: Timestamp): Promise<void> {
    this.db
      .prepare(
        `UPDATE scheduler_cleanup_requests
      ...
      .run(error, now, clientRequestId);
  }

  async listSchedulerCleanupRequests(limit = 100): Promise<SchedulerCleanupRequest[]> {
    const safeLimit = Number.isFinite(limit) ? Math.max(1, Math.min(1000, Math.trunc(limit))) : 100;
    const rows = this.db
      .prepare(
        `SELECT * FROM scheduler_cleanup_requests
         ORDER BY updated_at ASC, created_at ASC
         LIMIT ?`,
      )
      .all(safeLimit) as SchedulerCleanupRequestSqlRow[];
    return rows.map(mapSchedulerCleanupRequestRow);
  }
  ...
  async finishMessageRun(
    ...
    canonicalRef?: MessageRunCanonicalRef,
  ): Promise<void> {
    ...
    if (this.hasStreamLogColumn()) {
      ...
      if (canonicalRef && this.codexStateDbPath) {
        await this.indexCanonicalPointers(id, canonicalRef, this.codexStateDbPath);
      }
      return;
    }
    this.db
  ...
  /**
   * Canonical indexing is best-effort enrichment that runs after the run row is
   * already durable: a failure must leave the run finished with its stored text
   * intact and only be reported, never thrown back at the caller.
   */
  private async indexCanonicalPointers(
    id: MessageRunId,
    ...
    if (result.indexed.length === 0 && result.reason) {
      console.warn(
        `[store-sqlite] message run ${id} not canonicalised; keeping stored prompt/final_message`,
        { messageRunId: id, backendSessionId: ref.backendSessionId, reason: result.reason },
      );
    }
  }
  ...
  private originColumnCache: boolean | undefined;

  private hasOriginColumn(): boolean {
    if (this.originColumnCache !== undefined) return this.originColumnCache;
    const rows = this.db.prepare("PRAGMA table_info(message_runs)").all() as Array<{ name: string }>;
    this.originColumnCache = rows.some((r) => r.name === "origin");
    return this.originColumnCache;
  }
  ...
  private canonicalPointerTableCache: boolean | undefined;
  private hasCanonicalPointerTable(): boolean {
    if (this.canonicalPointerTableCache !== undefined) return this.canonicalPointerTableCache;
    const row = this.db.prepare(
      "SELECT 1 FROM sqlite_master WHERE type = 'table' AND name = 'message_run_canonical_pointers'",
    ).get();
    this.canonicalPointerTableCache = row !== undefined;
    return this.canonicalPointerTableCache;
  }
  ...
  private messageRunFromRow(row: MessageRunSqlRow): MessageRun {
    const prompt = visibleMessageRunContent(this.db, row.id, "prompt", row.prompt);
    if (prompt === null) throw new Error(`message run prompt unavailable: ${row.id}`);
    return {
      ...
      finalMessage: visibleMessageRunContent(this.db, row.id, "final_message", row.final_message),
      errorMessage: row.error_message,
      ...(row.origin !== null && row.origin !== undefined ? { origin: row.origin as MessageOrigin } : {}),
    };
  }
  ...
  async findRunningMessageRunBySession(sessionId: SessionId): Promise<MessageRun | null> {
    const row = this.db
      ...
      .get(sessionId) as MessageRunSqlRow | undefined;
    if (!row) return null;
    return this.messageRunFromRow(row);
  }
  ...
  async listRecentMessageRuns(sessionId: SessionId, limit: number, branchName?: string): Promise<MessageRun[]> {
    ...
    const rows = this.db
      ...
      .all(...params) as MessageRunSqlRow[];
    return rows.map((row) => this.messageRunFromRow(row));
  }

  async listRecentCompletedMessageRuns(sessionId: SessionId, limit: number): Promise<MessageRun[]> {
    const safeLimit = Number.isFinite(limit) ? Math.max(0, Math.trunc(limit)) : 0;
    if (safeLimit === 0) return [];
    const promptFilter = this.hasCanonicalPointerTable()
      ? `(prompt != '' OR EXISTS (
           SELECT 1 FROM message_run_canonical_pointers p
           WHERE p.message_run_id = message_runs.id AND p.content_kind = 'prompt'
         ))`
      : "prompt != ''";
    const finalFilter = this.hasCanonicalPointerTable()
      ? `(final_message != '' OR EXISTS (
           SELECT 1 FROM message_run_canonical_pointers p
           WHERE p.message_run_id = message_runs.id AND p.content_kind = 'final_message'
         ))`
      : "final_message IS NOT NULL AND final_message != ''";
    const rows = this.db
      .prepare(
        `SELECT * FROM message_runs
         WHERE session_id = ?
      ...
      .all(sessionId, safeLimit) as MessageRunSqlRow[];
    return rows.map((row) => this.messageRunFromRow(row));
  }
  ...
  async getRankStats(input: GetRankStatsInput): Promise<RankStats> {
    ...
    const runRows = this.db
      .prepare(
        `SELECT mr.id, mr.sender_id, mr.prompt, s.name as session_name
         FROM message_runs mr
         JOIN sessions s ON s.id = mr.session_id
         ${whereClause}
         ORDER BY mr.started_at ASC`
      )
      .all(...whereArgs) as Array<{ id: string; sender_id: string; prompt: string | null; session_name: string }>;
    ...
    for (const row of runRows) {
      const prompt = visibleMessageRunContent(this.db, row.id, "prompt", row.prompt);
      if (prompt === null) throw new Error(`message run prompt unavailable: ${row.id}`);
      if (prompt.startsWith("Δ")) continue;
      ...
      current.total += 1;
      current.inputChars += countRankInputChars(prompt);
  ...
  async findCrossSessionCommForDedup(clientRequestId: string): Promise<{
    ...
    const row = this.db
      .prepare(
        `SELECT id, status, child_session_id, created_at, final_message
      ...
      .get(clientRequestId) as
        | { id: string; status: string; child_session_id: string | null; created_at: number; final_message: string | null }
        | undefined;
    if (!row) return null;
    return {
      id: row.id,
  ...
  async finishCrossSessionComm(
    ...
    if (result.changes !== 1) throw new Error(`cross-session comm not found: ${id}`);
  }

  async finishReceiverTerminalComm(
    input: FinishReceiverTerminalCommInput,
  ): Promise<FinishReceiverTerminalCommResult> {
    return this.db.transaction((): FinishReceiverTerminalCommResult => {
      const read = () => this.db
        .prepare(
          `SELECT id, client_request_id, status, final_message
             FROM cross_session_log
            WHERE id = ?`
        )
        .get(input.id) as {
      ...
      const current = read();
      if (!current) throw new Error(`cross-session comm not found: ${input.id}`);
      if (current.client_request_id !== input.clientRequestId) {
        return { outcome: "not_committed", reason: "receiver-terminal completion identity mismatch" };
      }
      if (current.status === "completed") {
        return typeof current.final_message === "string"
          ? { outcome: "already_completed", finalMessage: current.final_message }
          : { outcome: "not_committed", reason: "receiver-terminal completion has no final message" };
      }
      if (current.status !== "pending") {
        return { outcome: "not_committed", reason: `receiver-terminal completion row is ${current.status}` };
      }

      const now = Date.now();
      if (input.accepted && now >= input.deadlineAt) {
        return { outcome: "not_committed", reason: "receiver-terminal completion deadline expired before persistence" };
      }
      const result = this.db
        .prepare(
          `UPDATE cross_session_log
              SET status = 'completed', child_session_id = NULL, result_preview = ?, error_message = ?,
                  final_message = ?, message_run_id = NULL, finished_at = ?
            WHERE id = ?
      ...
      if (result.changes !== 1) {
        const raced = read();
        return raced?.status === "completed" && typeof raced.final_message === "string"
          ? { outcome: "already_completed", finalMessage: raced.final_message }
          : { outcome: "not_committed", reason: "receiver-terminal completion CAS lost" };
      }

      const written = read();
      if (!written || written.status !== "completed" || written.final_message !== input.finalMessage) {
        throw new Error("receiver-terminal completion readback mismatch");
      }
      // Keep the success row uncommitted if the synchronous transaction itself
      // crossed the lease boundary. Throwing rolls back the UPDATE, after
      // which the API records the deterministic fail-closed result.
      if (input.accepted && Date.now() >= input.deadlineAt) {
        throw new Error("receiver-terminal completion deadline expired during persistence");
      }
      return { outcome: "committed", finalMessage: written.final_message };
    })();
  }
  ...
  private mapCommRow(r: {
    ...
    return {
      ...
      kind: r.kind as CrossSessionCommKind,
      prompt: r.prompt ?? "",
