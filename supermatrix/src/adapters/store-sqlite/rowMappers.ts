...
export type SessionRow = {
  ...
  backend_session_updated_at?: unknown;
...
export function rowToSession(row: SessionRow): Session {
  return {
    ...
    effortLocked: row.effort_locked === 1,
    workdir: asAbsolutePath(row.workdir),
    backendSessionId: row.backend_session_id,
    backendSessionUpdatedAt:
      typeof row.backend_session_updated_at === "number"
      && Number.isFinite(row.backend_session_updated_at)
      && Number.isInteger(row.backend_session_updated_at)
        ? asTimestamp(row.backend_session_updated_at)
        : null,
    chatName: row.chat_name ?? null,
    purpose: row.purpose,
    status: row.status as Session["status"],
    affiliatedTo: row.affiliated_to ?? null,
