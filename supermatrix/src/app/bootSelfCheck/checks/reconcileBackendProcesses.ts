...
import type { ProcessInfo } from "../../../ports/processLister.ts";
import type { BootCheck } from "../types.ts";

const BACKEND_PROCESS_RE = /(?:^|\s|\/)(claude|codex|kimi)(?:\s|$)/;

function compactRestartField(value: string): string {
  return value.replace(/\s+/g, " ").trim().slice(0, 240);
}

function bootOrphanReason(ctx: Parameters<BootCheck["run"]>[0]): string {
  const base = "boot reconcile: backend orphaned by console restart";
  const provenance = ctx.restartProvenance;
  if (!provenance) return base;
  return `${base} [restart_id=${compactRestartField(provenance.restartId)}`
    + ` source=${compactRestartField(provenance.source)}`
    + ` path=${compactRestartField(provenance.path)}`
    + ` reason=${compactRestartField(provenance.reason)}]`;
}
...
export const reconcileBackendProcessesCheck: BootCheck = {
  ...
  async run(ctx, mode) {
    ...
    // Backend children are spawned with `session.workdir` as cwd, but the
    // kernel (lsof) reports the resolved *physical* path. A workspace that was
    // provisioned under SM_NEW_WORKSPACE_ROOT and only symlinked back into the
    // legacy root therefore reports a cwd under provisioningRoot, which the
    // workspaceRoot prefix never matches. Scan both roots and merge by pid.
    const cwdPrefixes = [...new Set(
      [ctx.cfg.workspaceRoot, ctx.cfg.provisioningRoot].filter(
        (prefix): prefix is string => typeof prefix === "string" && prefix.length > 0,
      ),
    )];
    const byPid = new Map<number, ProcessInfo>();
    for (const cwdPrefix of cwdPrefixes) {
      const found = await ctx.processLister.list({
    ...
    const allCandidates = [...byPid.values()];
    ...
    for (const run of runningRuns) {
      ...
      const reason = bootOrphanReason(ctx);
      const failedComms = await store.timeoutMessageRunAndFailPendingSpawnComms(run.id, reason, now);
      for (const comm of failedComms) {
        failedSpawnComms.push(comm.commId);
        ctx.onBootOrphanedSpawnComm?.(comm);
      }
    ...
    return {
      ...
      detail: {
        ...
        actions,
        failedSpawnComms,
        ...(ctx.restartProvenance ? { restartProvenance: ctx.restartProvenance } : {}),
      },
    };
  },
};
