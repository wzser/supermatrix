...
export type BootCheckConfig = {
  ...
  /**
   * Optional second physical root where newly provisioned workspaces live
   * (SM_NEW_WORKSPACE_ROOT). Backend children keep the legacy symlink path as
   * their cwd, but the kernel reports the resolved physical path, so orphan
   * scans must accept this prefix too. Field name matches AppConfig so
   * bootstrap needs no extra wiring.
   */
