...
import {
  executeOpaqueHandoff,
  runOpaqueHandoff,
  type OpaqueHandoff,
  type OpaqueHandoffExecutor,
} from "./opaqueHandoff.ts";
...
export type ChildSessionDeps = {
  ...
  deliverSinks?: (
    session: Session,
    finalMessage: string,
    executionEvidence?: ChildExecutionEvidence,
  ...
  /** Direct executor for opaque handoffs; production uses a no-shell argv runner. */
  opaqueHandoffExecutor?: OpaqueHandoffExecutor;
};
...
export type SpawnChildInput = {
  ...
  senderId?: string;
  /** Framework-owned exact-byte consumer contract for an opaque handoff. */
...
export function createChildSessionService(deps: ChildSessionDeps) {
  ...
  async function startChildRun(
    ...
    try {
      const result = await runPrompt(session, input.prompt, {
        ...
        ...(input.opaqueHandoff ? { opaqueHandoff: input.opaqueHandoff } : {}),
  ...
  async function runPrompt(
    ...
    const opaqueAbortController = hooks.opaqueHandoff ? new AbortController() : undefined;
    let recovery: CodexRuntimeRecoveryRun | null = null;
    const stream = hooks.opaqueHandoff
      ? runOpaqueHandoff(
          {
            cwd: session.workdir,
            handoff: hooks.opaqueHandoff,
            commId: spawnCommId ?? "",
            ...(opaqueAbortController ? { signal: opaqueAbortController.signal } : {}),
          },
          deps.opaqueHandoffExecutor ?? executeOpaqueHandoff,
        )
      : session.backend === "codex" && deps.codexRuntimeRecovery
          ...
          if (outcome.res.error) {
            ...
            await deps.store.finishMessageRun(
              ...
              outcome.res.streamLog && outcome.res.streamLog.length > 0
                ...
                : undefined,
              session.backend === "codex" && outcome.res.backendSessionId && outcome.res.canonicalObjectId
                ? {
                    backendSessionId: outcome.res.backendSessionId,
                    canonicalObjectId: outcome.res.canonicalObjectId,
                  }
                : undefined,
            );
          } else {
    ...
    try {
      ...
      if (gating?.subscribeGatesCompletion && gating.subscribe) {
        ...
        if (gateOutcome.ok) {
          ...
          await deliverAndRecordSinks(
            sessionForSinks,
            collected.finalMessage,
            runId,
            spawnCommId,
            toExecutionEvidence(collected.streamLog),
          );
          return {
  ...
  async function deliverAndRecordSinks(
    ...
    try {
      const summary = await deps.deliverSinks(sessionForSinks, finalMessage, executionEvidence);
...
function toExecutionEvidence(streamLog: StreamResult["streamLog"]): ChildExecutionEvidence {
  return {
    toolResults: streamLog
      .filter((entry): entry is Extract<StreamResult["streamLog"][number], { kind: "tool_result" }> => entry.kind === "tool_result")
      .map((entry) => ({
