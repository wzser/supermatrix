...
export type ChildSessionPolicy = {
  ...
  /**
   * Idle timeout in seconds: a run is declared stuck and cancelled only
   * after this long with ZERO stream events. Any stream activity resets the
   * clock, so an actively-working child is never killed by this value
   * (changed from a wall-clock hard cap on 2026-08-03 after a 30-min cap
   * killed a still-running product-tracker child mid-task).
   */
