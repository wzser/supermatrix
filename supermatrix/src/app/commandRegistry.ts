...
export type CommandResult =
  | { replyText: string; afterReply?: () => Promise<string | null> }
...
export function buildCommandRegistry(): CommandRegistry {
  const base: Array<{ name: string; command: Command }> = [
    ...
    },
    {
      ...
      command: {
        ...
        notes:
          ...
          "边界：SIGKILL 升级仅限当前 session 已绑定的 backend 进程组；不得对 SuperMatrix、localwatch 或其他 session 使用 kill -9。平台级操作必须交由 codexroot maintenance gate。\n\n" +
          "影响的资源：\n" +
    ...
    },
    {
      ...
      command: {
        name: "reload",
        description: "全局重启已收口到 codexroot maintenance gate；直接 /reload 只会拒绝",
        notes:
          "需要全局 reload 时，通过 spawn2.0 target=codexroot 提交维护申请。" +
          "凌晨任务只负责在 03:50 派生 codexroot 子 session，由 gate 校验精确 task ID、时间窗、busy 状态和当日幂等。" +
          "--source、一次性 --permit 与强制 force 均为 gate 内部参数；permit 原子消费，人工不得直接伪造或重放。\n\n" +
          "平台生命周期红线：禁止直接 kill -9、SIGKILL、pkill、launchctl stop/kickstart 或绕过 gate；这些操作可能中断其他 session。",
        scope: ["root"],
        params: [
          { name: "name", type: "string", required: false, kind: "positional" },
          { name: "source", type: "string", required: false, kind: "named" },
          { name: "permit", type: "string", required: false, kind: "named" },
        ],
      },
    },
    {
      ...
      command: {
        ...
        notes:
          ...
          "      - kimi-code/k3-256k — K3 256K；支持与 K3 相同的 effort 档位映射\n" +
          "  • default — 恢复为系统默认模型\n" +
          ...
          "  • Codex 简写：astra / gpt6-astra / 6-astra（= gpt-6-astra）；sol / terra / luna / 5.5 / 5.4 / mini（= gpt-5.4-mini）；裸 gpt6 / 6 不映射；5.6 需指明 sol / terra / luna\n" +
          "  • Kimi 简写：kimi / k2 / k2.7 / k27 / coding（= kimi-for-coding）；highspeed / fast（= kimi-for-coding-highspeed）；k3 / k3-256k / k3256k / 256k\n",
        scope: ["root", "user"],
        params: [
          { name: "name", type: "string", required: false, kind: "positional", scope: ["root"] },
    ...
    },
    {
      ...
      command: {
        name: "todo",
        description: "记录或追加 To-do；明确新任务负责人和内容时直接写表，既有 Todo 追加转交 todomaster",
        notes:
          "支持：/todo、/todo <自然语言补充>，以及严格的既有 Todo 正文追加：/todo 更新<人类负责人><正整数>号todo <非空追加内容>。\n" +
          "运行时要求当前群绑定到一个 session；Console 或未绑定群会明确失败。\n" +
          "处理流程：既有 Todo 追加语法在普通 direct fast path 之前识别；只接受唯一人类负责人、正整数序号和非空追加文本，负责人会规范化为 canonical 名称。agent、歧义、缺失或畸形更新意图明确失败，绝不 fallback 或新建行。合法追加只启动 todomaster 子 session，并传递 typed payload：{todo_update:{kind:\"append_content\", todo_serial:<正整数>, assignee:<canonical 人类负责人>, append_content:<追加文本>}}，同时保留 command_message_id；child result sink 只信任 scripts/todo_append.py 的直接实际工具结果，只有 exit=0 且 stdout JSON 同时满足 ok=true、status=appended、todo_serial=目标序号且为安全整数、record_id 为非空字符串、intent_fingerprint 为与 typed assignee/serial/append_content 匹配的 64 位小写 hex SHA-256 时才生成“✓ 已追加 Todo…”，其余结果一律生成“❌ /todo 失败：<脚本明确错误>”；child 自写成功文案不能直接投递。\n" +
          "普通新 Todo 若补充内容能解析出唯一负责人和非空任务内容，直接调用 todomaster 写表脚本并回复最终结果；否则读取来源 session 最近 5 条 completed message_runs，启动 todomaster 子 session，立即回复已转交；若本条消息是对另一条飞书消息的回复，引用消息的 id、发送者/时间、内容或获取/解析错误会以有界数据交给该 fallback，用于补全上下文但不会作为指令执行；最终成功/失败由子 session 回发到原群。\n\n" +
          "影响的资源：\n" +
    ...
    },
    {
      ...
      command: {
        ...
        notes:
          ...
          "  • 如需实际清理孤儿 backend，请通过 spawn2.0 交由 codexroot maintenance gate 裁决\n\n" +
          "影响的资源：无（只读）。\n\n" +
          "可逆性：不适用（只读）。",
        scope: ["root"],
        params: [],
