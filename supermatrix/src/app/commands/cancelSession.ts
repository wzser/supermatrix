...
export function createCancelHandler(deps: CancelHandlerDeps): CommandHandler {
  return async ({ args, scope, msg }) => {
    const targetSpec = (args.target ?? args.name ?? args.next ?? "").trim();
    let targetName = "";
    let nextOnly = false;
    if (scope === "user") {
      ...
      const firstSpace = targetSpec.indexOf(" ");
      const firstToken = firstSpace === -1 ? targetSpec : targetSpec.slice(0, firstSpace);
      if (canonicalizeToken(firstToken) === "next") {
        targetName = firstSpace === -1 ? "" : targetSpec.slice(firstSpace + 1).trim();
        nextOnly = true;
        if (!targetName) throw new UserError("用法：/cancel next <name>");
      } else {
