...
export function createHeartbeatHandler(deps: HeartbeatHandlerDeps): CommandHandler {
  return async ({ args, scope, msg }) => {
    ...
    if (!sessionName || !state) {
      throw new UserError(
        scope === "root"
          ? "用法：/heartbeat <session-name> on|off|status|stop|resume [minutes|permanent]"
          : "用法：/heartbeat on|off|status|stop|resume [minutes|permanent]",
      );
    }

    if (!["on", "off", "status", "stop", "resume"].includes(state)) {
      throw new UserError(
        "heartbeat 状态必须是 on、off、status、stop 或 resume（stop 默认临时暂停 60 分钟，stop permanent / off 为永久停止）",
      );
    }
