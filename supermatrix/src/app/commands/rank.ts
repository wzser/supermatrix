...
const CONSOLE_GROUP_ID = "oc_REDACTEDCHATID" as LarkGroupId;
...
// Shared per-message average spec: inputChars / total, total=0 → 0, at most 1
// decimal. Single source so the personal-detail 平均 line and the per-row
// summary never drift in precision.
function formatAverage(row: RankRow): string {
  const average = row.total === 0 ? 0 : row.inputChars / row.total;
  return average.toLocaleString("en-US", { maximumFractionDigits: 1 });
}

function formatPersonalDetail(
  row: RankRow,
  displayName: string,
  rank: number,
  userCount: number,
  ...
  const sessions = row.allSessions.map(
    (session) =>
      `- ${session.sessionName}  ${formatCount(session.count)} 条（${formatPercent(session.count, row.total)}）`,
  );

  return [
    `📊 ${displayName} · 近 7 天详情（自 ${trackingSince ? formatDate(trackingSince) : "?"}）`,
    "",
    `全局排名：${rank} / ${userCount}`,
    `消息：${formatCount(row.total)} 条`,
    `输入：${formatCount(row.inputChars)} 字`,
    `平均：${formatAverage(row)} 字/条`,
    "",
    "Session 分布：",
    ...sessions,
  ].join("\n");
...
export function createRankHandler(deps: RankHandlerDeps): CommandHandler {
  ...
  return async (ctx: HandlerContext): Promise<{ replyText: string }> => {
    ...
    const requestedName = ctx.args.name?.trim();

    if (requestedName && !isRoot) {
      return { replyText: "个人详情仅 Console 群可查询。" };
    }
    ...
    if (requestedName) {
      const idMatch = stats.rows.find((row) => row.senderId === requestedName);
      const matches = idMatch
        ? [idMatch]
        : stats.rows.filter((row) => names.get(row.senderId) === requestedName);

      if (matches.length === 0) {
        return { replyText: `📊 近 7 天未找到「${requestedName}」的消息数据。` };
      }
      if (matches.length > 1) {
        const candidates = matches.map((row) => `- ${row.senderId}`);
        return {
          replyText: [
            `检测到重名「${requestedName}」，请使用 /rank <open_id> 查询：`,
            ...candidates,
          ].join("\n"),
        };
      }
      ...
      const row = matches[0];
      const displayName = names.get(row.senderId) ?? row.senderId.slice(-8);
      return {
        replyText: formatPersonalDetail(
          row,
          displayName,
          stats.rows.indexOf(row) + 1,
          stats.rows.length,
          stats.trackingSince,
        ),
      };
    ...
    const lines = stats.rows.map((row, i) => {
      ...
      const countPart = `${formatCount(row.total)} 条 / ${formatCount(row.inputChars)} 字 / 平均 ${formatAverage(row)} 字/条`;
