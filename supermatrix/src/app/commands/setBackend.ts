...
import type {
  SessionRuntimeSettingsSyncResult,
  SessionRuntimeSettingsSyncScope,
} from "../sessionRuntimeSettings.ts";
import type { CommandHandler } from "../commandRegistry.ts";
import { errorMessage } from "../errorMessage.ts";
import { isHumanLarkUserOrigin } from "../../domain/messageOrigin.ts";
import {
...
export type SetBackendHandlerDeps = {
  ...
  syncSessionTable?: (
    scope: SessionRuntimeSettingsSyncScope,
  ) => void | SessionRuntimeSettingsSyncResult | Promise<SessionRuntimeSettingsSyncResult>;
};

export function createSetBackendHandler(deps: SetBackendHandlerDeps): CommandHandler {
  return async ({ args, scope, msg }) => {
    if (!isHumanLarkUserOrigin(msg.origin)) {
      throw new UserError("backend 切换只能由用户显式 /backend 命令触发");
    }
    ...
    const syncSessionTable = deps.syncSessionTable;
    const afterReply = syncSessionTable
      ? async (): Promise<string | null> => {
          try {
            const syncResult = await syncSessionTable("backend-switch");
            if (!syncResult || syncResult.ok) return null;
            return `⚠ session「${sessionName}」已切换为 ${newBackend}，但飞书每日默认值同步失败：${errorMessage(syncResult.error)}`;
          } catch (err) {
            return `⚠ session「${sessionName}」已切换为 ${newBackend}，但飞书每日默认值同步失败：${errorMessage(err)}`;
          }
        }
      : undefined;
    ...
    //    reflects the new backend. Failure cannot roll back the DB mutation,
    //    so the returned receipt must say synchronization is incomplete.
    ...
    const parts = [
      warnings.length > 0
        ? `⚠ session「${sessionName}」已从 ${session.backend} 切换为 ${newBackend}，但后续同步或核查未完成`
        : `✓ session「${sessionName}」已从 ${session.backend} 切换为 ${newBackend}`,
      warnings.length > 0
        ? "（对话上下文与当前 model / effort 已清空；飞书每日默认值同步或相关核查未全部完成）"
