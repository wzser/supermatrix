...
const TIMEOUT_DEFAULT_ALIASES: ReadonlySet<string> = new Set(["default", "默认"]);
const MAXRUN_OFF_ALIASES: ReadonlySet<string> = new Set(["off", "关闭"]);
...
export function createSetTimeoutHandler(deps: SetTimeoutHandlerDeps): CommandHandler {
  return async ({ args, scope, msg }) => {
    ...
    if (timeoutArg) {
      if (TIMEOUT_DEFAULT_ALIASES.has(canonicalizeToken(timeoutArg))) {
    ...
    if (maxrunArg) {
      if (MAXRUN_OFF_ALIASES.has(canonicalizeToken(maxrunArg)) || maxrunArg === "0") {
