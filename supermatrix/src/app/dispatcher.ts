...
import {
  allowsLegacyCommandRouting,
  isHumanLarkUserOrigin,
  isLarkUserOrigin,
} from "../domain/messageOrigin.ts";
...
export function createDispatcher(deps: DispatcherDeps) {
  ...
  async function drainPendingNext(sessionId: SessionId): Promise<boolean> {
    ...
    try {
      while (pendingNext.has(sessionId)) {
        ...
        let admissionKind: MessageRunAdmissionResult["kind"] | undefined;
        try {
          await handleInbound({
          ...
          if (admissionKind !== "admitted") {
            log.warn("drainPendingNext admission not obtained, requeuing", {
              sessionId,
              admissionKind: admissionKind ?? "pre_admission_rejection",
            });
  ...
  async function handleInbound(
    msg: InboundMessage,
    onAdmission?: (kind: MessageRunAdmissionResult["kind"]) => void,
  ): Promise<void> {
    const rawInboundText = msg.text;
    ...
    const syntheticNextCommand =
      msg.origin === "lark_user_synthetic" && /^Δ\/next(?:\s|$)/u.test(rawInboundText);
    const commandTextCandidate = syntheticNextCommand
      ? rawInboundText.slice(1)
      : extractCommandTextCandidate(msg.text);
    const heartbeatControlCommand = normalizeBareHeartbeatControlCommand(msg.text);
    // Legacy callers may omit origin and still route commands. Explicit
    // synthetic origins are internal prompt delivery, except for the narrow
    // Δ/next user-synthetic entry point.
    const allowCommandRouting = allowsLegacyCommandRouting(msg.origin) || syntheticNextCommand;
    const isCommand =
      allowCommandRouting &&
    ...
    if (isCommand) {
      ...
      const afterReply = "afterReply" in result ? result.afterReply : undefined;
      ...
      if (afterReply) {
        const followUpText = await afterReply();
        if (followUpText) await lark.sendMessage(msg.groupId, followUpText);
      }
    ...
    try {
      ...
      deps.lifecycle?.runStarted(runId);
      try {
        const usageBaseline =
        ...
        if (
          isHumanLarkUserOrigin(msg.origin) &&
          !isExternalNonOwner
        ) {
        ...
        const result = await replier.consume({
          ...
          ...(msg.origin === "framework_synthetic" ? { syntheticOrigin: true } : {}),
          stream: iterable,
        });
        ...
        if (result.error) {
          await store.finishMessageRun(
            ...
            result.error,
            streamLogJson,
            session.backend === "codex" && result.backendSessionId && result.canonicalObjectId
              ? { backendSessionId: result.backendSessionId, canonicalObjectId: result.canonicalObjectId }
              : undefined,
          );
      ...
      deps.lifecycle?.runFinished(runId);
    }
  }
