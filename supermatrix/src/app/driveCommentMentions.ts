...
export type DriveCommentMentionRegistryTrigger = {
  id: string;
  priority?: number;
  // Normalized from the registry JSON field `ingest_modes`; defaults to
  // ["mention"] so legacy triggers stay immune to unmentioned poll intake.
  ingestModes?: Array<"mention" | "poll_unmentioned">;
  match: {
    all?: string[];
    any?: string[];
    none?: string[];
...
function registryTriggerMatches(
  ...
  const ingestMode = request.source.mentioned === false ? "poll_unmentioned" : "mention";
  if (!(trigger.ingestModes ?? ["mention"]).includes(ingestMode)) return false;
  if (!recordFieldConditionsMatch(trigger, request.bitableRecord?.fields)) return false;
  const matchTexts = ingestMode === "poll_unmentioned"
    ? [request.text, request.quote ?? ""]
    : [
        request.text,
        request.quote ?? "",
  ...
  const haystack = normalize(matchTexts.join("\n"));
...
export function buildDriveCommentTargetPrompt(
  ...
  return [
    ...
    request.source.mentioned === false
      ? "This request came from a Feishu Drive comment polled without an explicit SuperMatrix mention. Use the supplied context as evidence."
...
export function createDriveCommentMentionProcessor(deps: DriveCommentMentionProcessorDeps) {
  ...
  async function handle(source: DriveCommentSource): Promise<void> {
    ...
    if (source.mentioned === false && resolvedRoute.kind !== "registered") return;
