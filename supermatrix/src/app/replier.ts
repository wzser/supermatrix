...
export type ConsumeInput = {
  ...
  /**
   * True when this run was started by a framework-synthesized message
   * (continuation receipt, /next drain, spawn result reinjection) rather
   * than a live user message. Adds a Δ title prefix — matching the Δ the
   * framework prepends to injected prompts — and defaults the completed
   * header to violet, so a session acting with no user input is visually
   * distinct across the card's whole lifecycle.
   */
  ...
  async function consume(input: ConsumeInput): Promise<ConsumeResult> {
    ...
    const buildTitle = (suffix: string, currentUsage?: CollectedUsage) => {
      ...
      const displaySessionName = input.branchName
        ...
        : input.sessionName;
      const origin = input.syntheticOrigin ? "Δ " : "";
      const base = `${origin}${displaySessionName} | ${modelDisplay} · ${suffix}`;
    ...
    try {
      for await (const event of input.stream) {
        ...
        switch (event.kind) {
          ...
          case "canonical_ref":
            backendSessionId = event.backendSessionId;
            canonicalObjectId = event.canonicalObjectId;
            break;
          case "thinking":
    ...
    await deps.lark.finalizeCard(
      ...
      runStatus,
      input.completedTemplate ?? (input.syntheticOrigin ? "violet" : undefined),
    );
...
function renderProcessLog(
  ...
  // Tool names only — never args/results, which stay out of the card by
  // design (see the finalized-log tests). Names alone are what let a reader
  // tell an unproductive run from a swallowed answer.
  ...
  if (toolCallCount > 0 || toolResultCount > 0) {
    lines.push(
      `🧰 已隐藏 ${toolCallCount} 次工具调用和 ${toolResultCount} 次工具结果`
      + formatHiddenToolNames(toolNameCounts)
      + "；完整记录见 DB message_runs",
    );
  }
  return lines.join("\n");
}

// Insertion-ordered `名×次数` summary, bounded so a tool-heavy run cannot
// push the card past its size budget.
const HIDDEN_TOOL_NAME_LIMIT = 6;

function formatHiddenToolNames(counts: Map<string, number>): string {
  if (counts.size === 0) return "";
  const shown = [...counts.entries()]
    .slice(0, HIDDEN_TOOL_NAME_LIMIT)
    .map(([name, n]) => `${name}×${n}`);
  const overflow = counts.size > HIDDEN_TOOL_NAME_LIMIT ? " 等" : "";
