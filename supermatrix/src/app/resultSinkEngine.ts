...
import { formatTodoAppendResult } from "./todoAppendResultGate.ts";
...
export async function deliverResultSinks(
  ...
  return deliverDeclaredResultSinks(session, finalMessage, deps, {
    respectSyncInline: true,
...
export async function redeliverResultSinks(input: {
  ...
  const summary = await deliverDeclaredResultSinks(input.session, input.finalMessage, input.deps, {
    ...
    ...(input.executionEvidence ? { executionEvidence: input.executionEvidence } : {}),
  });
...
async function deliverDeclaredResultSinks(
  ...
  finalMessage: string,
  deps: ResultSinkDeps,
  options: { respectSyncInline: boolean; executionEvidence?: ChildExecutionEvidence },
): Promise<DeliverySummary> {
  ...
  if (options.respectSyncInline && session.callerInvocation === "sync_inline") {
    const continuationSinks = sinks.filter((sink) => sink.kind === "parent_continuation_inject");
    if (continuationSinks.length === 0) {
      summary.delivered.push({ sinkKind: "http_response", ok: true, note: "skipped: sync_inline handler owns delivery" });
      return summary;
    }
    for (const sink of continuationSinks) {
      const outcome = await deliverOneSafely(sink, session, finalMessage, deps, options.executionEvidence);
...
async function deliverOneSafely(
  ...
  try {
    return await deliverOne(sink, session, finalMessage, deps, executionEvidence);
  } catch (err) {
...
async function deliverOne(
  ...
  switch (sink.kind) {
    ...
    case "chat_post": {
      ...
      const deliveryText = sink.resultGate?.kind === "todo_append_v1"
        ? formatTodoAppendResult(finalMessage, sink.resultGate, executionEvidence)
        : finalMessage;
      await deps.postToChat(chatId, deliveryText, sink.identity);
      return { ok: true };
    }
