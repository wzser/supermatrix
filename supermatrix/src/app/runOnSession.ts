  ...
  try {
    ...
    if (collected.error) {
      await store.finishMessageRun(
        ...
        collected.error,
        streamLogJson,
        session.backend === "codex" && collected.backendSessionId && collected.canonicalObjectId
          ? { backendSessionId: collected.backendSessionId, canonicalObjectId: collected.canonicalObjectId }
          : undefined,
      );
