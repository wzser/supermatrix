...
): (mode?: SessionTableSyncMode, sessionNames?: readonly string[]) => void {
  const pending: Array<{
    mode: SessionTableSyncMode;
    sessionNames: string[];
  }> = [];
...
export function requestSessionTableSync(
  ...
  enqueueSessionTableSync(mode, sessionNames);
}
...
export function createSessionLifecycle(deps: SessionLifecycleDeps) {
  ...
  async function create(input: CreateInput): Promise<{ session: Session }> {
    ...
    try {
      ...
      requestSync("scoped-push-current", [input.name]);
  ...
  return {
    ...
    syncSessionTable: (scope: "current" | "backend-switch" = "current") => {
      requestSync("runtime-settings-push-current");
      if (scope === "backend-switch") {
        requestSync("runtime-settings-normalize-main-defaults");
        requestSync("runtime-settings-pull");
      }
    },
  };
}
