import { execFile } from "node:child_process";
import { join } from "node:path";
import { promisify } from "node:util";
import { createSessionCatalogService } from "./sessionCatalog.ts";
import { UserError, SystemError } from "../domain/errors.ts";
import {
  asAbsolutePath,
  asSessionId,
  type AbsolutePath,
  type SessionId,
  type Timestamp,
} from "../domain/ids.ts";
import type { SessionEvent } from "../domain/events/sessionEvent.ts";
import type {
  BackendKind,
  EffortLevel,
  Session,
  SessionCategory,
} from "../domain/session.ts";
import type { BindingStore } from "../ports/BindingStore.ts";
import type { Clock } from "../ports/Clock.ts";
import type { EventBus } from "../ports/EventBus.ts";
import type { LarkGateway } from "../ports/LarkGateway.ts";
import type { WorkspaceFs } from "../ports/WorkspaceFs.ts";
import { errorMessage } from "./errorMessage.ts";
import type { SessionRuntimeSettingsSyncResult } from "./sessionRuntimeSettings.ts";

const PRINCIPLES_FILES = [
  "console-principles.md",
  "coding-principles.md",
  "business-principles.md",
] as const;

export type SessionLifecycleDeps = {
  store: BindingStore;
  fs: WorkspaceFs;
  lark: LarkGateway;
  clock: Clock;
  workspaceRoot: AbsolutePath;
  // Where the *physical* directory of a brand-new workspace is created. When
  // unset, workspaceRoot is both the provisioning root and the legacy root and
  // behaviour is exactly the single-root one. When set, the entity lives under
  // provisioningRoot and workspaceRoot only gets a compatibility symlink — the
  // legacy path stays the one recorded in the DB and used by every consumer.
  // Note this deliberately does NOT touch the *other* meaning of
  // workspaceRoot (shared-asset base for principles/SOP templates).
  provisioningRoot?: AbsolutePath;
  // Throws a UserError when the provisioning volume is not ready (unmounted,
  // read-only, …). Called before any mkdir or Feishu group creation.
  assertProvisioningRootReady?: () => Promise<void>;
  // Absolute path of the single global session-catalog.json. Every workspace
  // gets a symlink pointing here; one regeneration updates the whole fleet.
  catalogPath: AbsolutePath;
  principlesTemplatesDir: AbsolutePath;
  claudeMdTemplatePath: AbsolutePath;
  agentsMdTemplatePath: AbsolutePath;
  gitignorePath: AbsolutePath;
  ownerUserId: string;
  idFactory?: () => string;
  cancelBackend?: (sessionId: string) => Promise<void>;
  eventBus?: EventBus;
  requestSessionTableSync?: (
    mode?: SessionTableSyncMode,
    sessionNames?: readonly string[],
  ) => void | Promise<SessionRuntimeSettingsSyncResult>;
  requestSchedulerCleanup?: (input: {
    sessionId: SessionId;
    sessionName: string;
    clientRequestId: string;
    requestedAt: Timestamp;
  }) => Promise<void>;
};

export type CreateInput = {
  backend: BackendKind;
  name: string;
  purpose: string;
  model?: string;
  effort?: EffortLevel | null;
  workdir?: AbsolutePath;
  chatName?: string;
  category?: SessionCategory;
  thinking?: boolean;
  inactivityTimeoutS?: number | null;
  maxRuntimeS?: number | null;
  heartbeatEnabled?: boolean;
  affiliatedTo?: string | null;
};

const NAME_RE = /^[a-z0-9][a-z0-9_-]{0,39}$/u;

const execFileP = promisify(execFile);
const SESSION_TABLE_SYNC_TIMEOUT_MS = 180_000;

function sessionTableSyncScript(): string {
  const runtimeRoot = process.env.SM_RUNTIME_ROOT || "/Users/LOCAL_USER/SuperMatrixRuntime";
  const workspaceRoot = process.env.SM_WORKSPACE_ROOT || join(runtimeRoot, "workspaces");
  return join(workspaceRoot, "first-principle/scripts/sync-session-table.sh");
}

export type SessionTableSyncMode =
  | "full"
  | "scoped-push-current"
  | "runtime-settings-pull"
  | "runtime-settings-push-current"
  | "runtime-settings-normalize-main-defaults";

export type SessionTableSyncRequestResult = SessionRuntimeSettingsSyncResult;

function normalizeSessionTableSyncNames(sessionNames: readonly string[]): string[] {
  return [...new Set(sessionNames.map((name) => name.trim()))].sort();
}

export async function syncSessionTableToLark(
  mode: SessionTableSyncMode = "full",
  timeoutMs = SESSION_TABLE_SYNC_TIMEOUT_MS,
  sessionNames: readonly string[] = [],
): Promise<void> {
  const args = [sessionTableSyncScript()];
  if (mode === "scoped-push-current") {
    args.push(
      "--sessions",
      normalizeSessionTableSyncNames(sessionNames).join(","),
      "--push-current",
    );
  }
  if (mode === "runtime-settings-pull") args.push("--runtime-settings", "pull");
  if (mode === "runtime-settings-push-current") args.push("--runtime-settings", "push-current");
  if (mode === "runtime-settings-normalize-main-defaults") {
    args.push("--runtime-settings", "normalize-main-defaults");
  }
  await execFileP("bash", args, { timeout: timeoutMs });
}

export function createSessionTableSyncRequester(
  runner: (
    mode: SessionTableSyncMode,
    sessionNames: readonly string[],
  ) => Promise<void>,
  reportError: (error: unknown) => void = (error) => {
    if (error) console.error("session-table sync failed:", errorMessage(error));
  },
): (
  mode?: SessionTableSyncMode,
  sessionNames?: readonly string[],
) => Promise<SessionTableSyncRequestResult> {
  const pending: Array<{
    mode: SessionTableSyncMode;
    sessionNames: string[];
    resolve: Array<(result: SessionTableSyncRequestResult) => void>;
  }> = [];
  let running = false;

  const drain = async (): Promise<void> => {
    if (running) return;
    running = true;
    try {
      while (pending.length > 0) {
        const request = pending.shift()!;
        let result: SessionTableSyncRequestResult = { ok: true };
        try {
          await runner(request.mode, request.sessionNames);
        } catch (error) {
          reportError(error);
          result = { ok: false, error };
        }
        for (const resolve of request.resolve) resolve(result);
      }
    } finally {
      running = false;
    }
  };

  return (
    mode: SessionTableSyncMode = "full",
    sessionNames: readonly string[] = [],
  ): Promise<SessionTableSyncRequestResult> => {
    return new Promise((resolve) => {
      const request = {
        mode,
        sessionNames: normalizeSessionTableSyncNames(sessionNames),
      };
      const tail = pending[pending.length - 1];
      if (
        tail?.mode === request.mode
        && tail.sessionNames.length === request.sessionNames.length
        && tail.sessionNames.every((name, index) => name === request.sessionNames[index])
      ) {
        tail.resolve.push(resolve);
        return;
      }
      pending.push({ ...request, resolve: [resolve] });
      void drain();
    });
  };
}

const enqueueSessionTableSync = createSessionTableSyncRequester((mode, sessionNames) =>
  syncSessionTableToLark(mode, SESSION_TABLE_SYNC_TIMEOUT_MS, sessionNames),
);

export function requestSessionTableSync(
  mode: SessionTableSyncMode = "full",
  sessionNames: readonly string[] = [],
): Promise<SessionTableSyncRequestResult> {
  return enqueueSessionTableSync(mode, sessionNames);
}

export function createSessionLifecycle(deps: SessionLifecycleDeps) {
  const emit = (event: SessionEvent) =>
    deps.eventBus ? deps.eventBus.publish(event) : Promise.resolve();
  const requestSync = deps.requestSessionTableSync ?? requestSessionTableSync;

  const catalog = createSessionCatalogService({
    store: deps.store,
    fs: deps.fs,
    catalogPath: deps.catalogPath,
    clock: deps.clock,
    ...(deps.eventBus !== undefined ? { eventBus: deps.eventBus } : {}),
  });

  // Ensure the workspace has a symlink to the global catalog. Idempotent: a
  // workspace that already has the link (re-adopted workdir, prior run) is
  // left untouched. Returns the link path, or null when it was already there.
  async function ensureCatalogSymlink(
    workdir: AbsolutePath,
  ): Promise<AbsolutePath | null> {
    const link = asAbsolutePath(`${workdir}/session-catalog.json`);
    if (await deps.fs.exists(link)) return null;
    await deps.fs.symlink(deps.catalogPath, link);
    return link;
  }
  const genId = deps.idFactory ?? (() => "sess_" + Math.random().toString(36).slice(2, 10));

  async function create(input: CreateInput): Promise<{ session: Session }> {
    if (!NAME_RE.test(input.name)) {
      throw new UserError("名称仅限小写字母 a-z、数字 0-9、下划线 _ 和连字符 -，首字符为字母或数字，最多 40 字符");
    }
    const existing = await deps.store.findSessionByName(input.name);
    if (existing) {
      throw new UserError(`名称已存在：${input.name}`);
    }

    const useExistingWorkdir = Boolean(input.workdir);
    const workdir = input.workdir ?? asAbsolutePath(`${deps.workspaceRoot}/${input.name}`);
    const rollback: Array<() => Promise<void>> = [];
    const runRollback = async () => {
      for (const step of rollback.reverse()) {
        try {
          await step();
        } catch {
          // keep going
        }
      }
    };

    try {
      if (useExistingWorkdir) {
        // Existing workdir: must already exist
        if (!(await deps.fs.exists(workdir))) {
          throw new UserError(`工作目录不存在：${workdir}`);
        }
      } else {
        // New workdir: create and scaffold.
        // Without a provisioningRoot this is the historical single-root path:
        // physicalDir === workdir, one exists() pre-check, one rmrf rollback.
        // With one, the entity goes to `${provisioningRoot}/${name}` and
        // `workdir` becomes a legacy-root symlink pointing at it — everything
        // downstream (gitInit, scaffold, commits, DB) keeps using `workdir`.
        const physicalDir = deps.provisioningRoot
          ? asAbsolutePath(`${deps.provisioningRoot}/${input.name}`)
          : workdir;
        if (deps.provisioningRoot) {
          // Readiness + both pre-checks run before mkdir and before
          // lark.createGroup, so an unmounted volume leaves no group behind.
          await deps.assertProvisioningRootReady?.();
          if (await deps.fs.entryExists(physicalDir)) {
            throw new UserError(`工作目录已存在：${physicalDir}`);
          }
        }
        if (
          deps.provisioningRoot
            ? await deps.fs.entryExists(workdir)
            : await deps.fs.exists(workdir)
        ) {
          throw new UserError(`工作目录已存在：${workdir}`);
        }
        await deps.fs.mkdir(physicalDir);
        rollback.push(() => deps.fs.rmrf(physicalDir));
        if (deps.provisioningRoot) {
          await deps.fs.symlink(physicalDir, workdir);
          // Pushed after the physical dir so the LIFO rollback drops the
          // legacy-root symlink first and the physical directory second.
          rollback.push(() => deps.fs.rmrf(workdir));
        }
        await deps.fs.gitInit(workdir);
        const gitignoreTarget = asAbsolutePath(`${workdir}/.gitignore`);
        await deps.fs.copyFile(deps.gitignorePath, gitignoreTarget);
        await deps.fs.gitCommit(workdir, `init: scaffold session ${input.name}`, [
          gitignoreTarget,
        ]);
      }

      // Step 8: createGroup — pattern: `{prefix}-{name}-{backend}` (prefix = trimmed --chat-name)
      const chatNamePrefix = input.chatName?.trim();
      const groupName = chatNamePrefix
        ? `${chatNamePrefix}-${input.name}-${input.backend}`
        : `${input.name}-${input.backend}`;
      const groupId = await deps.lark.createGroup({
        name: groupName,
        ownerUserId: deps.ownerUserId,
      });
      rollback.push(() => deps.lark.dissolveGroup(groupId));

      // Step 9: inviteUser
      await deps.lark.inviteUser(groupId, deps.ownerUserId);

      // Step 10: createSessionWithBinding
      // FP v1.0 contract §4 chat_name option (a): chatName is no longer
      // persisted. chatNamePrefix above is the only consumer (group naming).
      const { session } = await deps.store.createSessionWithBinding(
        {
          id: asSessionId(genId()),
          name: input.name,
          scope: "user",
          backend: input.backend,
          model: input.model ?? null,
          effort: input.effort ?? null,
          ...(input.thinking !== undefined ? { thinking: input.thinking } : {}),
          ...(input.inactivityTimeoutS !== undefined
            ? { inactivityTimeoutS: input.inactivityTimeoutS }
            : {}),
          ...(input.maxRuntimeS !== undefined ? { maxRuntimeS: input.maxRuntimeS } : {}),
          ...(input.heartbeatEnabled !== undefined
            ? { heartbeatEnabled: input.heartbeatEnabled }
            : {}),
          ...(input.category !== undefined ? { category: input.category } : {}),
          ...(input.affiliatedTo !== undefined ? { affiliatedTo: input.affiliatedTo } : {}),
          workdir,
          purpose: input.purpose,
          createdAt: deps.clock.now(),
        },
        groupId
      );

      // Past the rollback cliff — no more rollback from here.
      await emit({ kind: "session_created", session });

      // Steps 12-12c: only for new workdirs (existing workdirs already have these files)
      if (!useExistingWorkdir) {
        try {
          const link = await ensureCatalogSymlink(workdir);
          if (link) {
            await deps.fs.gitCommit(workdir, `catalog: link for ${input.name}`, [link]);
          }
        } catch (err) {
          await deps.lark.sendMessage(
            groupId,
            `⚠ session-catalog symlink 失败：${errorMessage(err)}`,
          );
        }

        try {
          const principleLinks: AbsolutePath[] = [];
          for (const file of PRINCIPLES_FILES) {
            const link = asAbsolutePath(`${workdir}/${file}`);
            await deps.fs.symlink(
              asAbsolutePath(`${deps.principlesTemplatesDir}/${file}`),
              link,
            );
            principleLinks.push(link);
          }
          await deps.fs.gitCommit(workdir, `principles: link for ${input.name}`, principleLinks);
        } catch (err) {
          await deps.lark.sendMessage(
            groupId,
            `⚠ principles symlink 失败：${errorMessage(err)}`,
          );
        }

        try {
          const sopDir = asAbsolutePath(`${workdir}/sop`);
          const sopIndex = asAbsolutePath(`${sopDir}/INDEX.md`);
          const sopTemplate = asAbsolutePath(`${sopDir}/TEMPLATE.md`);
          await deps.fs.mkdir(sopDir);
          await deps.fs.writeFile(
            sopIndex,
            `# SOP Index\n\n| SOP | Description |\n|-----|-------------|\n| (none yet) | |\n`,
          );
          // SOP template lives under workspaceRoot/first-principle/templates/,
          // matching bootstrap.ts's principlesTemplatesDir layout. The earlier
          // `${SM_RUNTIME_ROOT ?? workspaceRoot}/workspaces/...` formula
          // produced dangling `workspaces/workspaces/...` symlinks whenever
          // SM_RUNTIME_ROOT was unset (the common case in production).
          await deps.fs.symlink(
            asAbsolutePath(`${deps.workspaceRoot}/first-principle/templates/sop-template.md`),
            sopTemplate,
          );
          await deps.fs.gitCommit(workdir, `sop: init directory for ${input.name}`, [
            sopIndex,
            sopTemplate,
          ]);
        } catch (err) {
          await deps.lark.sendMessage(
            groupId,
            `⚠ SOP 目录初始化失败：${errorMessage(err)}`,
          );
        }
      }

      if (!useExistingWorkdir) {
        try {
          const agentMdPaths: AbsolutePath[] = [];
          for (const [tplPath, filename] of [
            [deps.claudeMdTemplatePath, "CLAUDE.md"],
            [deps.agentsMdTemplatePath, "AGENTS.md"],
          ] as const) {
            const tpl = await deps.fs.readFile(tplPath);
            const target = asAbsolutePath(`${workdir}/${filename}`);
            await deps.fs.writeFile(target, tpl.replace(/\{\{name\}\}/gu, input.name));
            agentMdPaths.push(target);
          }
          await deps.fs.gitCommit(
            workdir,
            `agent-md: add CLAUDE.md + AGENTS.md for ${input.name}`,
            agentMdPaths,
          );
        } catch (err) {
          await deps.lark.sendMessage(
            groupId,
            `⚠ CLAUDE.md/AGENTS.md 写入失败：${errorMessage(err)}`,
          );
        }
      }

      // Step 13: an adopted (pre-existing) workdir skips the scaffold blocks
      // above, but still needs the catalog symlink so the session can read
      // the fleet directory. No commit — we never inject into a repo we don't
      // own. A workdir that already has the link is left untouched.
      if (useExistingWorkdir) {
        try {
          await ensureCatalogSymlink(workdir);
        } catch (err) {
          await deps.lark.sendMessage(
            groupId,
            `⚠ session-catalog symlink 失败：${errorMessage(err)}`,
          );
        }
      }

      // Step 14: flip status from 'initializing' to 'idle' — session is ready
      // to accept prompts from its bound user group.
      await deps.store.updateSessionStatus(session.id, "idle", deps.clock.now());
      await emit({
        kind: "session_status_changed",
        sessionId: session.id,
        from: "initializing",
        to: "idle",
      });
      const ready: Session = { ...session, status: "idle" };

      // Step 15: regenerate the global catalog so the new session appears.
      // Done after the status flip so the catalog records 'idle', not the
      // transient 'initializing'. One JSON write replaces the old per-sibling
      // catalog regenerated.
      try {
        await catalog.regenerateCatalog(`new session: ${input.name}`);
      } catch (err) {
        await deps.lark.sendMessage(
          groupId,
          `⚠ 已创建 ${input.name}，但 session-catalog 重新生成失败：${errorMessage(err)}`,
        );
      }

      await requestSync("scoped-push-current", [input.name]);

      return { session: ready };
    } catch (err) {
      await runRollback();
      if (err instanceof UserError) throw err;
      throw new SystemError(errorMessage(err, "create failed"), err);
    }
  }

  async function remove(input: { name: string }): Promise<void> {
    const session = await deps.store.findSessionByName(input.name);
    if (!session) throw new UserError(`session 不存在：${input.name}`);
    if (session.status === "busy") {
      throw new UserError("session 正在运行，请等待完成或先 /cancel");
    }
    const binding = await deps.store.findBySession(session.id);
    if (binding) {
      try {
        await deps.lark.dissolveGroup(binding.groupId);
      } catch {
        // log-only in real impl; continue forward-biased
      }
    }
    await deps.store.deleteSessionAndBinding(session.id);
    if (deps.requestSchedulerCleanup) {
      const requestedAt = deps.clock.now();
      const date = new Date(Number(requestedAt)).toISOString().slice(0, 10);
      const clientRequestId = `${date}:session-delete-cleanup:${session.id}`;
      try {
        await deps.requestSchedulerCleanup({
          sessionId: session.id,
          sessionName: session.name,
          clientRequestId,
          requestedAt,
        });
      } catch (err) {
        // The session deletion is already committed. The requester owns the
        // durable pending/failed cleanup record; never resurrect or roll back
        // the deleted session because scheduler delivery is temporarily down.
        console.error("scheduler session cleanup delegation failed", {
          sessionId: session.id,
          sessionName: session.name,
          clientRequestId,
          error: errorMessage(err),
        });
      }
    }
    await emit({ kind: "session_deleted", sessionId: session.id });
    await catalog.regenerateCatalog(`session removed: ${input.name}`);

    await requestSync("scoped-push-current", [input.name]);
  }

  async function clearSessionContext(sessionName: string): Promise<void> {
    const session = await deps.store.findSessionByName(sessionName);
    if (!session) throw new UserError(`session 不存在：${sessionName}`);
    const prev = session.status;
    const now = deps.clock.now();
    const activeBranch = await deps.store.getActiveBranch(session.id);
    await deps.store.clearSessionBranchBackendSessionId(session.id, activeBranch.name, now);
    await deps.store.updateSessionStatus(session.id, "idle", now);
    if (prev !== "idle") {
      await emit({
        kind: "session_status_changed",
        sessionId: session.id,
        from: prev,
        to: "idle",
      });
    }
  }

  async function reset(input: { name: string }): Promise<void> {
    const session = await deps.store.findSessionByName(input.name);
    if (!session) throw new UserError(`session 不存在：${input.name}`);
    if (session.status === "busy") {
      throw new UserError("session 正在运行，请等待完成或先 /cancel");
    }
    await clearSessionContext(input.name);
  }

  async function restart(input: { name: string }): Promise<void> {
    const session = await deps.store.findSessionByName(input.name);
    if (!session) throw new UserError(`session 不存在：${input.name}`);
    if (session.status === "busy" && deps.cancelBackend) {
      await deps.cancelBackend(session.id);
    }
    await clearSessionContext(input.name);
  }

  return {
    create,
    delete: remove,
    reset,
    restart,
    regenerateCatalog: async (reason: string): Promise<void> => {
      await catalog.regenerateCatalog(reason);
    },
    // Runtime commands mirror local-authoritative current fields. A backend
    // switch also routes incompatible default migration through FP's verified
    // Feishu CAS writer, then refreshes the local cache from Feishu authority.
    syncSessionTable: async (
      scope: "current" | "backend-switch" = "current",
    ): Promise<SessionRuntimeSettingsSyncResult> => {
      const modes: SessionTableSyncMode[] = ["runtime-settings-push-current"];
      if (scope === "backend-switch") {
        modes.push("runtime-settings-normalize-main-defaults", "runtime-settings-pull");
      }
      let failed = false;
      let firstError: unknown;
      for (const mode of modes) {
        const result = await requestSync(mode);
        if (result && !result.ok) {
          if (!failed) firstError = result.error;
          failed = true;
        }
      }
      return failed ? { ok: false, error: firstError } : { ok: true };
    },
  };
}
