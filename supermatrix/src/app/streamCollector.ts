...
import { isTerminalErrorMessage, MISSING_TERMINAL_EVIDENCE_ERROR } from "./runStatus.ts";
import {
  accumulateUsage,
  ...
  try {
    for await (const event of stream) {
      ...
      case "assistant_message":
        ...
        if (event.final) {
          finalMessage = assistantTexts.join("\n\n");
          hasTerminalEvidence = finalMessage.trim().length > 0;
        }
        break;
      case "completed":
    ...
    // Keep usage already observed before a backend iterator/transport error.
    // Terminal callers can then persist that partial usage before finishing
    // the failed or timed-out message_run.
    const message = errorMessage(err);
    if (!error || isTerminalErrorMessage(message)) {
      error = message;
    }
    streamLog.push({ ts: Date.now(), kind: "error", text: message });
  }

  if (!error && !hasTerminalEvidence) {
    error = MISSING_TERMINAL_EVIDENCE_ERROR;
    streamLog.push({ ts: Date.now(), kind: "error", text: error });
  }
