...
export function normalizeCumulativeUsageEvent(
  ...
  // A new Codex thread/context sequence can restart all primary counters.
  // Treat that snapshot as the first usage of the new sequence; otherwise a
  // full regression is clamped to five zero fields and real usage is lost.
  // A single-field regression still uses per-field clamping below so cache
  // turnover cannot reintroduce cumulative-context inflation.
  if (isCumulativeSequenceReset(current, previous)) {
    return { event, nextWatermark: current };
  }
  return {
    event: {
...
function isCumulativeSequenceReset(
  current: UsageWatermark,
  previous: UsageWatermark,
): boolean {
  return (
