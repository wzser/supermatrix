...
import { asAbsolutePath, asMessageRunId, asSessionId, asTimestamp, type AbsolutePath, type LarkGroupId, type MessageRunId, type SessionId, type Timestamp } from "../domain/ids.ts";
...
import { RECEIVER_TERMINAL_COMM_KIND } from "../app/spawnClosure/receiverTerminalFence.ts";
...
import { parseOpaqueHandoffPrompt, type OpaqueHandoff } from "../app/opaqueHandoff.ts";
import {
  createImmutableIngressBridge,
  hasImmutableIngressCarrierMarker,
  IMMUTABLE_INGRESS_RECEIVER_KILL_GRACE_MS,
  IMMUTABLE_INGRESS_RESPONSE_OVERHEAD_MS,
  type ImmutableIngressBridge,
  type ImmutableIngressDeclaration,
} from "../app/immutableIngress.ts";
...
import type {
  ...
  WatcherStateRecord,
  NewCrossSessionComm,
  FinishReceiverTerminalCommInput,
...
export type ApiDeps = {
  store: {
    ...
    logCrossSessionComm?: (input: NewCrossSessionComm) => Promise<void>;
    ...
    finishReceiverTerminalComm?: (
      input: FinishReceiverTerminalCommInput,
    ) => Promise<{
      outcome: "committed" | "already_completed" | "not_committed";
      finalMessage?: string;
      reason?: string;
    }>;
  ...
  /** Admit an explicit child cwd only when it is the target's own linked Git worktree. */
  resolveSpawnWorkdir?: (input: {
    canonicalWorkdir: AbsolutePath;
  ...
  /** Framework-owned target pre-admission bridge for declared immutable ingress. */
  immutableIngressBridge?: ImmutableIngressBridge;
  /**
...
async function linkedGitIdentity(workdir: string): Promise<{
  root: string;
  commonDir: string;
}> {
  const actual = await realpath(workdir);
  const top = (await execFileAsync(
    "git", ["-C", actual, "rev-parse", "--show-toplevel"],
  )).stdout.trim();
  const root = await realpath(top);
  if (root !== actual) {
    throw new Error(`workdir must be a Git worktree root: ${actual}`);
  }
  const rawCommon = (await execFileAsync(
    "git", ["-C", actual, "rev-parse", "--git-common-dir"],
  )).stdout.trim();
  const commonDir = await realpath(
    isAbsolute(rawCommon) ? rawCommon : resolve(actual, rawCommon),
  );
  return { root, commonDir };
}

export async function resolveLinkedSpawnWorkdir(input: {
  canonicalWorkdir: AbsolutePath;
  requestedWorkdir: string;
}): Promise<AbsolutePath> {
  if (!isAbsolute(input.requestedWorkdir)) {
    throw new Error("requested workdir must be absolute");
  }
  const [canonical, requested] = await Promise.all([
    linkedGitIdentity(input.canonicalWorkdir),
    linkedGitIdentity(input.requestedWorkdir),
  ]);
  if (canonical.commonDir !== requested.commonDir) {
    throw new Error(
      `requested workdir is not linked to target repository: ${requested.root}`,
    );
  }
  return asAbsolutePath(requested.root);
}
...
export async function startApiServer(deps: ApiDeps, port: number): Promise<Server> {
  ...
  const defaultImmutableIngressBridge = createImmutableIngressBridge();
    ...
    try {
      ...
      if (method === "POST" && url.pathname === "/api/caller-identity") {
        ...
        json(res, 200, {
          ...
          messageRunId: attestation.messageRunId,
      ...
      if (method === "POST" && url.pathname === "/api/spawn2.0") {
        // receiver_terminal is synchronous-only. Its deadline starts at the
        // HTTP route entry, so body parsing and all admission preflight share
        // the same budget as receiver execution and response publication.
        const receiverTerminalRequestDeadlineAt = Date.now() + resolveSyncSpawnResponseTimeoutMs(deps);
        ...
        if (priorComm) {
          const receiverTerminalReplay = receiverTerminalSuccessReplay(priorComm, dedupKey);
          if (receiverTerminalReplay) {
            await writeJson(res, 200, {
              ok: true,
              mode: "sync_inline",
              closure: "verified",
              finalMessage: receiverTerminalReplay,
              spawnCommId: priorComm.id,
            }, { "X-SM-Spawn-Comm-Id": priorComm.id });
            return;
          }
          if (priorComm.id === receiverTerminalCommIdFor(dedupKey)) {
            const receiverTerminalFailure = receiverTerminalFailureReplay(priorComm, dedupKey);
            if (receiverTerminalFailure) {
              json(res, 400, receiverTerminalFailureResponse(receiverTerminalFailure));
              return;
            }
            const recovered = await recoverReceiverTerminalCompletion(deps, priorComm.id, dedupKey);
            json(res, recovered.durable ? 400 : 503, receiverTerminalFailureResponse(
              recovered.finalMessage,
              recovered.durable ? undefined : recovered.error,
            ));
            return;
          }
          ...
          json(res, 409, {
            ...
            existing: {
              ...
              ...(priorItem
                ...
                : {}),
              ...(priorComm.finalMessage ? { finalMessage: priorComm.finalMessage } : {}),
            },
          });
          return;
        }
        ...
        let childWorkdir = session.workdir;
        if (parsed.execution?.workdir !== undefined) {
          try {
            childWorkdir = await (deps.resolveSpawnWorkdir ?? resolveLinkedSpawnWorkdir)({
              canonicalWorkdir: session.workdir,
              requestedWorkdir: parsed.execution.workdir,
            });
          } catch (err) {
            json(res, 400, {
              ok: false,
              error: `invalid execution.workdir: ${err instanceof Error ? err.message : String(err)}`,
            });
            return;
          }
        }
        ...
        const immutableIngressDeclaration = session.capabilityPayload?.immutableIngress as ImmutableIngressDeclaration | undefined;
        const immutableIngressRequest = immutableIngressDeclaration &&
          hasImmutableIngressCarrierMarker(parsed.prompt, immutableIngressDeclaration)
          ? immutableIngressDeclaration
          : undefined;
        const receiverTerminalCompletion = immutableIngressRequest?.completionMode === "receiver_terminal";
        let verificationPredicate: NormalizedSpawnPredicate | undefined;
        if (parsed.verification_predicate !== undefined && !receiverTerminalCompletion) {
        ...
        } else if (parsed.verification_predicate !== undefined && receiverTerminalCompletion) {
          log.info("api spawn2 immutable ingress verification predicate bypassed", {
            target: parsed.target,
            clientRequestId: parsed.client_request_id,
            reason: "receiver_terminal relies on the verified terminal receipt identity",
          });
        }
        ...
        let opaqueHandoff: OpaqueHandoff | null = null;
        try {
          opaqueHandoff = parseOpaqueHandoffPrompt(parsed.prompt);
        } catch (err) {
          json(res, 400, {
            ok: false,
            error: `invalid opaque handoff: ${err instanceof Error ? err.message : String(err)}`,
          });
          return;
        }
        ...
        let promptForChild = parsed.prompt;
        if (immutableIngressRequest) {
          if (
            receiverTerminalCompletion &&
            parsed.closure.target.type !== "inline"
          ) {
            json(res, 400, {
              ok: false,
          ...
          const receiverTerminalCommId = receiverTerminalCompletion
            ? receiverTerminalCommIdFor(parsed.client_request_id)
            : undefined;
          if (receiverTerminalCompletion && (!deps.store.logCrossSessionComm || !deps.store.finishReceiverTerminalComm)) {
            json(res, 500, {
              ok: false,
              target_accepted: false,
              error: "immutable ingress receiver_terminal requires durable completion comm storage",
            });
            return;
          }
          if (receiverTerminalCommId && deps.store.logCrossSessionComm) {
            try {
              await deps.store.logCrossSessionComm({
                id: receiverTerminalCommId,
                fromSessionId: fromSession.id,
                toSessionId: session.id,
                kind: RECEIVER_TERMINAL_COMM_KIND,
                prompt: receiverTerminalCommPrompt(parsed.client_request_id, parsed.prompt, immutableIngressRequest),
                childModel: null,
                clientRequestId: parsed.client_request_id,
                originRunId,
                createdAt: Date.now() as Timestamp,
              ...
              const existing = await deps.store.findCrossSessionCommForDedup(parsed.client_request_id);
              if (existing?.id === receiverTerminalCommId) {
                const priorItem = await deps.store.getSpawnAsyncItemByComm(existing.id);
                json(res, 409, {
                  ok: false,
                  duplicate: true,
                  spawnCommId: existing.id,
                  error: `duplicate client_request_id: ${parsed.client_request_id} was admitted concurrently`,
                  existing: {
                    commId: existing.id,
                    ...
                    ...(priorItem
                      ? { ref: priorItem.ref, resultUrl: `/api/spawn_async_items/${priorItem.ref}/take` }
                      : {}),
                    ...(existing.finalMessage ? { finalMessage: existing.finalMessage } : {}),
                  },
                });
                return;
              }
              json(res, 500, {
                ok: false,
                target_accepted: false,
                error: `immutable ingress completion admission failed: ${error instanceof Error ? error.message : String(error)}`,
              });
              return;
            }
          }
          const receiverRemainingMs = receiverTerminalCompletion
            ? receiverTerminalRequestDeadlineAt - Date.now()
            : undefined;
          const receiverTimeoutMs = receiverTerminalCompletion && receiverRemainingMs !== undefined
            ? receiverTimeoutBeforeSyncDeadline(receiverRemainingMs)
            : undefined;
          if (receiverTerminalCompletion && receiverTimeoutMs === undefined) {
            const reason = receiverTerminalSyncWindowTooShortReason(receiverRemainingMs ?? 0);
            const completion = await finishReceiverTerminalCompletion(deps, {
              commId: receiverTerminalCommId!,
              clientRequestId: parsed.client_request_id,
              accepted: false,
              deadlineAt: receiverTerminalRequestDeadlineAt,
              reason,
            });
            json(res, completion.durable ? 400 : 503, receiverTerminalFailureResponse(
              completion.finalMessage,
              completion.durable ? `immutable ingress rejected: ${reason}` : completion.error,
            ));
            return;
          }
          const admissionPromise = (deps.immutableIngressBridge ?? defaultImmutableIngressBridge).admit({
            targetSessionName: parsed.target,
            targetWorkdir: childWorkdir,
            clientRequestId: parsed.client_request_id,
            prompt: parsed.prompt,
            declaration: immutableIngressRequest,
            ...(receiverTerminalCompletion
              ? { receiverDeadlineAt: receiverTerminalRequestDeadlineAt }
              : {}),
          });
          const ingress = await admissionPromise;
          if (!ingress.accepted) {
            if (receiverTerminalCommId) {
              const completion = await finishReceiverTerminalCompletion(deps, {
                commId: receiverTerminalCommId,
                clientRequestId: parsed.client_request_id,
                accepted: false,
              ...
              json(res, completion.durable ? 400 : 503, receiverTerminalFailureResponse(
                completion.finalMessage,
                completion.durable ? `immutable ingress rejected: ${ingress.reason}` : completion.error,
              ));
              return;
            }
            json(res, 400, {
              ok: false,
              target_accepted: false,
              finalMessage: immutableIngressFailureReceipt(ingress.reason),
              error: `immutable ingress rejected: ${ingress.reason}`,
            });
            return;
          }
          if (receiverTerminalCommId) {
            const receiverTerminalDeadlineExpired = Date.now() >= receiverTerminalRequestDeadlineAt;
            const completion = await finishReceiverTerminalCompletion(deps, {
              commId: receiverTerminalCommId,
              clientRequestId: parsed.client_request_id,
              accepted: !receiverTerminalDeadlineExpired,
              deadlineAt: receiverTerminalRequestDeadlineAt,
              ...(receiverTerminalDeadlineExpired
                ? { reason: "immutable ingress receiver-terminal deadline expired before completion persistence" }
                : { success: ingress }),
            });
            if (!completion.durable) {
              json(res, 503, receiverTerminalFailureResponse(completion.finalMessage, completion.error));
              return;
            }
            if (!completion.accepted) {
              json(res, 400, receiverTerminalFailureResponse(completion.finalMessage));
              return;
            }
          }
          if (receiverTerminalCompletion) {
            await writeJson(res, 200, {
              ok: true,
              mode: "sync_inline",
              closure: "verified",
              finalMessage: ingress.terminalReceiptJson,
              spawnCommId: receiverTerminalCommId,
            }, { "X-SM-Spawn-Comm-Id": receiverTerminalCommId! });
            return;
          }
          promptForChild = ingress.admittedPrompt;
          log.info("api spawn2 immutable ingress admitted", {
            target: parsed.target,
            from: parsed.from,
            artifactId: ingress.artifactId,
            promptSha256: ingress.promptSha256,
            receiptSha256: ingress.receiptSha256,
          });
        }
        log.info("api spawn2", {
          target: parsed.target,
        ...
        const spawnInput: SpawnChildInput = {
          ...
          prompt: withSpawn2DeliveryInstruction(promptForChild, parsed.closure.target),
...
function receiverTerminalCommIdFor(clientRequestId: string): string {
  return `comm_receiver_${hashOpaqueSecret(clientRequestId)}`;
}

function receiverTerminalSuccessReplay(
  comm: {
    id: string;
    status: "pending" | "completed" | "failed";
    finalMessage?: string | null;
  ...
  const finalMessage = comm.finalMessage;
  if (
    comm.status !== "completed" ||
    comm.id !== receiverTerminalCommIdFor(clientRequestId) ||
  ...
  let receipt: unknown;
  try {
    receipt = JSON.parse(finalMessage);
  } catch {
    return undefined;
  }
  if (!receipt || typeof receipt !== "object" || Array.isArray(receipt)) return undefined;
  const transport = (receipt as Record<string, unknown>).transport;
  if (
    (receipt as Record<string, unknown>).terminal !== true ||
    !transport ||
    typeof transport !== "object" ||
    Array.isArray(transport) ||
    (transport as Record<string, unknown>).target_accepted !== true
  ) {
    return undefined;
  }
  return finalMessage;
...
function receiverTerminalFailureReplay(
  comm: {
    id: string;
    status: "pending" | "completed" | "failed";
    finalMessage?: string | null;
  ...
  return (
    ...
    typeof transport === "object" &&
    !Array.isArray(transport) &&
    (transport as Record<string, unknown>).target_accepted === false
  )
    ? finalMessage
    : undefined;
}

function receiverTerminalFailureResponse(finalMessage: string, error?: string): Record<string, unknown> {
  return {
    ok: false,
    target_accepted: false,
    finalMessage,
...
type ReceiverTerminalCompletion = {
  durable: boolean;
  accepted: boolean;
  finalMessage: string;
  error?: string;
...
async function readReceiverTerminalCompletion(
  deps: ApiDeps,
  input: {
    commId: string;
    clientRequestId: string;
    accepted: boolean;
    requestedFinalMessage: string;
  },
): Promise<string | undefined> {
  const persisted = await deps.store.findCrossSessionCommForDedup(input.clientRequestId);
  if (
    persisted?.id !== input.commId ||
    persisted.status !== "completed" ||
    persisted.finalMessage !== input.requestedFinalMessage
  ) {
    return undefined;
  }
  const replay = input.accepted
    ? receiverTerminalSuccessReplay(persisted, input.clientRequestId)
    : receiverTerminalFailureReplay(persisted, input.clientRequestId);
  return replay === input.requestedFinalMessage ? replay : undefined;
}

async function finishReceiverTerminalCompletion(
  deps: ApiDeps,
  input: {
    commId: string;
    clientRequestId: string;
...
): Promise<ReceiverTerminalCompletion> {
  if (!deps.store.finishReceiverTerminalComm) {
    return {
      durable: false,
      accepted: false,
      finalMessage: immutableIngressFailureReceipt("durable completion storage is unavailable"),
      error: "immutable ingress receiver-terminal completion storage is unavailable",
    };
  }
  const finish = deps.store.finishReceiverTerminalComm.bind(deps.store);
  const requestedFinalMessage = input.accepted
    ? input.success!.terminalReceiptJson
    : immutableIngressFailureReceipt(input.reason ?? "immutable ingress receiver-terminal rejected");
  const requestedResultPreview = input.accepted
    ? receiverTerminalSuccessMetadata(input.success!)
    : receiverTerminalFailureMetadata(input.reason ?? "immutable ingress receiver-terminal rejected");
  const requestedError = input.accepted
    ? undefined
    : receiverTerminalFailureError(input.reason ?? "immutable ingress receiver-terminal rejected");
  try {
    const result = await finish({
      ...
      resultPreview: requestedResultPreview,
      ...(requestedError ? { error: requestedError } : {}),
      finalMessage: requestedFinalMessage,
    });
    if (result.outcome === "not_committed" || typeof result.finalMessage !== "string") {
      throw new Error(result.reason ?? "receiver-terminal completion was not committed");
    }
    const persisted = await readReceiverTerminalCompletion(deps, {
      commId: input.commId,
      clientRequestId: input.clientRequestId,
      accepted: input.accepted,
      requestedFinalMessage,
    });
    if (!persisted) throw new Error("durable receiver-terminal completion readback mismatch");
    return { durable: true, accepted: input.accepted, finalMessage: persisted };
  } catch (error) {
    const completionError = error instanceof Error ? error.message : String(error);
    const failClosedReason = `durable receiver-terminal completion failed: ${completionError}`;
    const failClosedMessage = immutableIngressFailureReceipt(failClosedReason);
    try {
      const result = await finish({
        id: input.commId,
        clientRequestId: input.clientRequestId,
        accepted: false,
        deadlineAt: input.deadlineAt,
        resultPreview: receiverTerminalFailureMetadata(failClosedReason),
        error: receiverTerminalFailureError(failClosedReason),
        finalMessage: failClosedMessage,
      });
      if (result.outcome === "not_committed" || typeof result.finalMessage !== "string") {
        throw new Error(result.reason ?? "receiver-terminal fail-closed completion was not committed");
      }
      const failure = await readReceiverTerminalCompletion(deps, {
        commId: input.commId,
        clientRequestId: input.clientRequestId,
        accepted: false,
        requestedFinalMessage: failClosedMessage,
      });
      if (failure) return { durable: true, accepted: false, finalMessage: failure };
    } catch {
      // Keep the response terminal and without an async URL; a later retry can
      // attempt the same deterministic fail-closed completion again.
    }
    return {
      durable: false,
      accepted: false,
      finalMessage: failClosedMessage,
      error: `immutable ingress receiver-terminal completion could not be durably recorded: ${completionError}`,
    };
  }
}

async function recoverReceiverTerminalCompletion(
  deps: ApiDeps,
  commId: string,
  clientRequestId: string,
): Promise<ReceiverTerminalCompletion> {
  return finishReceiverTerminalCompletion(deps, {
    commId,
    clientRequestId,
    accepted: false,
    deadlineAt: Date.now(),
    reason: "previous receiver-terminal completion was not durably confirmed; retry is fail-closed",
  });
}

function receiverTerminalCommPrompt(
  clientRequestId: string,
  prompt: string,
  declaration: ImmutableIngressDeclaration,
): string {
  const promptSha256 = hashOpaqueSecret(prompt);
  return JSON.stringify({
    protocol: declaration.protocol,
    completion_mode: declaration.completionMode ?? null,
    client_request_id_sha256: hashOpaqueSecret(clientRequestId),
    prompt_sha256: promptSha256,
    artifact_id: `sm-immutable-ingress-v1:${promptSha256}`,
    declaration_sha256: hashOpaqueSecret(JSON.stringify(declaration)),
  });
}

function receiverTerminalFailureMetadata(reason: string): string {
  return JSON.stringify({
    protocol: "supermatrix-immutable-ingress/v1",
    completion_mode: "receiver_terminal",
    target_accepted: false,
    reason_sha256: hashOpaqueSecret(reason),
  });
}

function receiverTerminalFailureError(reason: string): string {
  return `immutable ingress receiver-terminal failure; reason_sha256=${hashOpaqueSecret(reason)}`;
}

function receiverTerminalSuccessMetadata(input: {
  artifactId: string;
  promptSha256: string;
  receiptSha256: string;
  eventIdentity: Record<string, unknown>;
...
function immutableIngressFailureReceipt(reason: string): string {
  return JSON.stringify({
    terminal: true,
    transport: { target_accepted: false },
    error: {
      code: "receiver_terminal_rejected",
      reason_sha256: hashOpaqueSecret(reason),
    },
  });
}

function receiverTimeoutBeforeSyncDeadline(syncTimeoutMs: number): number | undefined {
...
function receiverTerminalSyncWindowTooShortReason(syncTimeoutMs: number): string {
  const requiredMarginMs =
    IMMUTABLE_INGRESS_RECEIVER_KILL_GRACE_MS + IMMUTABLE_INGRESS_RESPONSE_OVERHEAD_MS;
  return `sync response window ${syncTimeoutMs}ms cannot safely accommodate receiver termination margin ${requiredMarginMs}ms`;
}
