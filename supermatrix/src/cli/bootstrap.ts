...
import { readFileSync, writeFileSync, createWriteStream, constants as fsConstants, type WriteStream } from "node:fs";
import { readFile, access as fsAccess, stat as fsStat } from "node:fs/promises";
...
import { consumePlatformReloadPermit } from "./reloadPermit.ts";
...
} from "../app/driveCommentMentionRegistry.ts";
...
export type AppConfig = {
  ...
  driveCommentSubscriptionEnabled: boolean;
  gitActorSessionName: string;
  cardAskGatePath: string;
  mentionRegistryPath: string;
  // Physical root for brand-new workspaces (the removable sm-c volume). Only
  // produced when SM_NEW_WORKSPACE_ROOT names a root distinct from
  // workspaceRoot; absent means today's single-root behaviour. The name is
  // load-bearing: session lifecycle deps pick it up structurally.
...
const envSchema = z.object({
  ...
  LARK_CLI_PROFILE: z.string().optional(),
  ...
  SM_DRIVE_COMMENT_SUBSCRIPTION_ENABLED: z.enum(["0", "1"]).default("0"),
  SM_SESSION_NAME: z.string().optional(),
  SM_MENTION_REGISTRY_PATH: z.string().optional(),
  SM_NEW_WORKSPACE_ROOT: z.string().optional(),
});
...
export function validateEnv(env: Record<string, string | undefined>): AppConfig {
  const parsed = envSchema.parse({
    ...
    LARK_CLI_PROFILE: env["LARK_CLI_PROFILE"],
    ...
    SM_DRIVE_COMMENT_SUBSCRIPTION_ENABLED: env["SM_DRIVE_COMMENT_SUBSCRIPTION_ENABLED"],
    SM_SESSION_NAME: env["SM_SESSION_NAME"],
    SM_MENTION_REGISTRY_PATH: env["SM_MENTION_REGISTRY_PATH"],
    SM_NEW_WORKSPACE_ROOT: env["SM_NEW_WORKSPACE_ROOT"],
  });

  // Unset, empty, or identical to the legacy root => no provisioning root at
  // all, so neither the readiness guard nor the symlink layout kicks in.
  const provisioningRoot = parsed.SM_NEW_WORKSPACE_ROOT?.trim();
  const usesProvisioningRoot = provisioningRoot !== undefined
    && provisioningRoot.length > 0
    && path.resolve(provisioningRoot) !== path.resolve(parsed.SM_WORKSPACE_ROOT.trim());
  ...
  return {
    ...
    ...(parsed.LARK_CLI_PROFILE?.trim() ? { larkCliProfile: parsed.LARK_CLI_PROFILE.trim() } : {}),
    ...
    driveCommentSubscriptionEnabled: parsed.SM_DRIVE_COMMENT_SUBSCRIPTION_ENABLED === "1",
    ...
    ...(usesProvisioningRoot && provisioningRoot ? { provisioningRoot } : {}),
  };
}

type ProvisioningRootFs = {
  stat(target: string): Promise<{ dev: number }>;
  access(target: string, mode: number): Promise<void>;
};

/**
 * Fail-closed readiness guard for the removable provisioning volume.
 *
 * The only reliable signal that `sm-c` is actually mounted is the *device
 * number*: when the volume is unmounted macOS can leave a writable
 * `/Volumes/EXAMPLE_VOLUME` stub behind on the boot disk, so an existence or a
 * `/Volumes` prefix check would happily provision onto local disk. Comparing
 * `stat().dev` against the legacy workspace root's catches exactly that.
 *
 * Throws `UserError` (never `SystemError`) so commandRouter echoes the message
 * verbatim to Feishu instead of swallowing it as an internal error.
 */
export function createProvisioningRootGuard(
  opts: { provisioningRoot: string; workspaceRoot: string },
  fs: ProvisioningRootFs = { stat: fsStat, access: fsAccess },
): () => Promise<void> {
  const { provisioningRoot, workspaceRoot } = opts;
  return async () => {
    try {
      await fs.access(provisioningRoot, fsConstants.W_OK);
    } catch (err) {
      throw new UserError(
        `移动硬盘未挂载或不可写（${provisioningRoot}：${errorMessage(err)}），新建工作区已中止`,
      );
    }

    let provisioningDev: number;
    let workspaceDev: number;
    try {
      provisioningDev = (await fs.stat(provisioningRoot)).dev;
      workspaceDev = (await fs.stat(workspaceRoot)).dev;
    } catch (err) {
      throw new UserError(
        `移动硬盘未挂载，无法确认设备号（${provisioningRoot} / ${workspaceRoot}：${errorMessage(err)}），新建工作区已中止`,
      );
    }

    if (provisioningDev === workspaceDev) {
      throw new UserError(
        `移动硬盘未挂载（${provisioningRoot} 与 ${workspaceRoot} 同一设备 dev=${provisioningDev}），新建工作区已中止`,
      );
    }
  };
}
...
export async function bootstrap(env: Record<string, string | undefined>): Promise<App> {
  ...
  const mentionRegistry = createFileDriveCommentMentionRegistryLoader({
    registryPath: cfg.mentionRegistryPath,
  });
  const larkClient = createRealLarkClient({
    larkCliPath: cfg.larkCliPath,
    ...(cfg.larkCliProfile ? { profile: cfg.larkCliProfile } : {}),
    ...
    mentionRegistryPollLoader: async () =>
      deriveDriveCommentMentionPollWatches(await mentionRegistry.load()).map((watch) => ({
        ...watch,
        persistSince: (since: number) =>
          persistDriveCommentMentionPollSince(cfg.mentionRegistryPath, watch.routeId, since),
      })),
  });
  ...
  const lifecycle = createSessionLifecycle({
    ...
    // Provisioning root + its readiness guard are wired only when configured;
    // without them createSession keeps the historical single-root behaviour.
    // Note every shared-asset path below stays derived from workspaceRoot.
    ...(cfg.provisioningRoot
      ? {
          provisioningRoot: asAbsolutePath(cfg.provisioningRoot),
          assertProvisioningRootReady: createProvisioningRootGuard({
            provisioningRoot: cfg.provisioningRoot,
    ...
    eventBus,
    requestSchedulerCleanup: createScheduledTaskCleanupRequester({
  ...
  const childSessionDeps: Parameters<typeof createChildSessionService>[0] = {
    ...
    deliverSinks: async (session, finalMessage, executionEvidence) => {
  ...
  const runNotifyCli = async (args: string[]): Promise<string> => {
    ...
    let stderr = "";
    try {
      const r = await execFileP(cfg.larkCliPath, cfg.larkCliProfile ? ["--profile", cfg.larkCliProfile, ...args] : args, {
  ...
  const autoFileDelivery = createAutoFileDelivery({
    sendFile: createLarkCliAutoFileSender({
      larkCliPath: cfg.larkCliPath,
      ...(cfg.larkCliProfile ? { profile: cfg.larkCliProfile } : {}),
    }),
  ...
  registry["reload"].handler = createReloadHandler({
    lifecycle: processLifecycle,
    store,
    consumePermit: (permit) => consumePlatformReloadPermit({
      dbPath: cfg.dbPath,
      nonce: permit.nonce,
  ...
  return {
    ...
    async start() {
      ...
      if (cfg.driveCommentSubscriptionEnabled) {
        await reconcileDriveCommentSubscriptionAtStartup({
          reconcile: () => reconcileDriveCommentSubscription({
            larkCliPath: cfg.larkCliPath,
            ...(cfg.larkCliProfile ? { profile: cfg.larkCliProfile } : {}),
          }),
        ...
        logger.info("drive comment subscription reconcile disabled by configuration");
      }
...
export function createScheduledTaskReviewRequester(
  deps: {
    apiBase: string;
    store: {
      findCrossSessionCommForDedup(clientRequestId: string): Promise<unknown>;
    };
  },
  ...
  return async (request) => {
    ...
    try {
      ...
      // The request can be durably admitted before the HTTP transport times
      // out. The stable key makes that an accepted dispatch, not a failure.
      if (await deps.store.findCrossSessionCommForDedup(clientRequestId)) return;
      throw error;
    }
  };
}

type SchedulerCleanupRequesterStore = {
  beginSchedulerCleanupRequest(input: {
    clientRequestId: string;
    sessionId: SessionId;
    sessionName: string;
    createdAt: Timestamp;
  }): Promise<{ shouldDispatch: boolean }>;
  markSchedulerCleanupSubmitted(clientRequestId: string, now: Timestamp): Promise<void>;
  markSchedulerCleanupFailed(clientRequestId: string, error: string, now: Timestamp): Promise<void>;
  findCrossSessionCommForDedup(clientRequestId: string): Promise<unknown>;
};

export function createScheduledTaskCleanupRequester(deps: {
  apiBase: string;
  store: SchedulerCleanupRequesterStore;
}): (input: {
  sessionId: SessionId;
  sessionName: string;
  clientRequestId: string;
  ...
  return async (input) => {
    const started = await deps.store.beginSchedulerCleanupRequest({
      clientRequestId: input.clientRequestId,
      sessionId: input.sessionId,
      sessionName: input.sessionName,
      createdAt: input.requestedAt,
    });
    if (!started.shouldDispatch) return;

    const prompt = [
      "SESSION_SCHEDULER_CLEANUP_V1",
      `sessionName: ${input.sessionName}`,
      `sessionId: ${input.sessionId}`,
      "Delete only scheduler v2 tasks matching at least one exact condition:",
      `- task.owner === ${input.sessionName}`,
      `- task.config.body.from === ${input.sessionName}`,
      `- task.config.body.target === ${input.sessionName}`,
      "Do not match on createdBy alone. Do not use task name, description, prompt, target, or any other field as an ownership condition.",
      "If there are no matching tasks, perform a successful no-op and report matched=0.",
      "Delete no task outside this exact scope and do not recreate or modify unrelated tasks.",
    ].join("\n");
    ...
    try {
      ...
      let responseBody: { duplicate?: unknown } = {};
      if (responseText) {
        try {
          responseBody = JSON.parse(responseText) as typeof responseBody;
        } catch {
          // The status/error below retains the bounded raw response for diagnosis.
        }
      }
      if (!response.ok && response.status === 409 && responseBody.duplicate === true) {
        await deps.store.markSchedulerCleanupSubmitted(input.clientRequestId, input.requestedAt);
        return;
      }
      if (!response.ok) {
        throw new Error(`scheduler cleanup spawn failed: HTTP ${response.status}${responseText ? ` ${responseText.slice(0, 500)}` : ""}`);
      }
      await deps.store.markSchedulerCleanupSubmitted(input.clientRequestId, input.requestedAt);
    } catch (error) {
      // A transport error can happen after spawn2.0 has durably admitted the
      // comm. In that case the stable key makes the retry a duplicate, not a
      // second cleanup; preserve submitted state rather than false-failing it.
      if (await deps.store.findCrossSessionCommForDedup(input.clientRequestId)) {
        await deps.store.markSchedulerCleanupSubmitted(input.clientRequestId, input.requestedAt);
        return;
      }
      await deps.store.markSchedulerCleanupFailed(
