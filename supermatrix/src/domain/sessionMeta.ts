...
const CATEGORY_VALUES = new Set(["", "业务", "平台", "工具", "知识", "外部", "员工"]);
...
export function validateSessionCategory(value: string): void {
  if (!CATEGORY_VALUES.has(value)) {
    throw new UserError(
      `category 必须 ∈ {'', '业务', '平台', '工具', '知识', '外部', '员工'}; 收到 ${JSON.stringify(value)}`,
    );
  }
}

// 外部 sessions carry a strict non-owner trust boundary that each backend must
// enforce at execution time (claude: default permission mode; codex: read-only
// sandbox). Kimi Code ACP has no equivalent read-only execution guarantee, so
// the combination is rejected outright instead of running unprotected.
export function assertBackendAllowedForCategory(backend: string, category: string): void {
  if (category === "外部" && backend === "kimi") {
    throw new UserError(
      "外部 session 不支持 kimi backend：kimi 无 answer-only 只读安全模式，请使用 claude 或 codex",
    );
  }
}
