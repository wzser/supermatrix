...
export type NewMessageRunInput = {
  ...
  origin?: MessageOrigin; // durable ingress provenance; NULL for non-chat/historical rows
  branchName?: string;
};
...
// "receiver_terminal" = immutable-ingress request-side: a target-declared
// receiver returned the terminal result directly; no child/model/async path is
// allowed. Legacy rows may still have kind="spawn" plus the fixed sanitized
// receiver-terminal marker and are fenced by the spawn-closure lifecycle.
...
export type CrossSessionCommKind = "spawn" | "receiver_terminal" | "continuation" | "resume_main";
...
export type SchedulerCleanupRequestStatus = "pending" | "submitted" | "failed";

export type SchedulerCleanupRequest = {
  clientRequestId: string;
  sessionId: SessionId;
  sessionName: string;
  status: SchedulerCleanupRequestStatus;
  attemptCount: number;
...
export type BeginSchedulerCleanupRequestInput = Pick<
  SchedulerCleanupRequest,
  "clientRequestId" | "sessionId" | "sessionName" | "createdAt"
>;
...
export type FinishReceiverTerminalCommInput = {
  id: string;
  clientRequestId: string;
  accepted: boolean;
  deadlineAt: number;
...
export type FinishReceiverTerminalCommResult =
  | { outcome: "committed" | "already_completed"; finalMessage: string }
  | { outcome: "not_committed"; reason: string };
...
/** Terminal async receipt states. Parked results remain readable by take but
 * are no longer eligible for courier delivery or retry. */
export type SpawnAsyncItemTerminalStatus = Extract<SpawnAsyncItemStatus, "closed" | "parked">;
...
export type BindingStore = {
  ...
  listSchedulerCleanupRequests(limit?: number): Promise<SchedulerCleanupRequest[]>;
  ...
  finishMessageRun(
    ...
    canonicalRef?: { backendSessionId: string; canonicalObjectId: string },
  ): Promise<void>;
   ...
   * itself over an HTTP channel. CAS pending/waiting_child/delivering/parked
   ...
   * before finalize). Parked is terminal for courier retry, but remains
   * readable by take.
   */
  closeSpawnAsyncItemConsumed(ref: string, reason: string, now: Timestamp): Promise<boolean>;
  /**
   * Sync-inline HTTP delivery ledger: closes all courier-active items for
   * the comm (pending/waiting_child/delivering → closed, verdict='delivered');
   * closed and parked terminal receipts are left unchanged.
  ...
  /**
   * Receiver-terminal-only atomic completion. An accepted completion is
   * committed only while its deadline lease is valid; the update and
   * readback happen in one store transaction. This is optional at the port
   * boundary so older test/durable-store implementations fail closed rather
   * than silently using the unconditional generic finisher.
   */
  finishReceiverTerminalComm?(input: FinishReceiverTerminalCommInput): Promise<FinishReceiverTerminalCommResult>;
