...
export type InboundMessageOrigin = MessageOrigin;
...
// A normalized, owner-supplied polling configuration. The Lark adapter only
// schedules and emits from this shape; it never parses or writes a mention
// registry directly. `persistSince` is deliberately supplied by the config
// owner so cursor state remains alongside the registry that defines the watch.
export type DriveCommentPollWatch = {
  fileToken: string;
  fileType: DriveCommentFileType;
  tableId?: string;
  recordId?: string;
  ...
  intervalSec?: number;
  url?: string;
  persistSince?: (since: number) => Promise<void>;
};
