...
describe("resolveCodexDefaultModel", () => {
  ...
  test("preserves supported reasoning efforts from the bundled catalog", async () => {
    const stdout = JSON.stringify({
      models: [
        {
          slug: "gpt-5.6-sol",
  ...
  test("filters models rejected for the current ChatGPT login", async () => {
    const stdout = JSON.stringify({
      models: [
        { slug: "gpt-5.6-sol", priority: 1, visibility: "list", supported_in_api: true },
        { slug: "gpt-5.2", priority: 29, visibility: "list", supported_in_api: true },
      ],
    });
