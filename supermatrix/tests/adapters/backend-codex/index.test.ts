...
describe("CodexBackend (app-server transport)", () => {
  ...
  test("happy run yields started + final + completed via AsyncIterable", async () => {
    ...
    expect(events[1]).toEqual({
      kind: "canonical_ref",
      backendSessionId: "fake-thread-1",
      canonicalObjectId: "fake-turn-1",
    });
  ...
  test("rich turn maps commentary, tool call/result, usage, and final message", async () => {
    ...
    expect(events[2]).toEqual({ kind: "thinking", text: "planning the work" });
    expect(events[3]).toMatchObject({ kind: "tool_call", name: "ls /tmp" });
    ...
    expect(events[6]).toEqual({ kind: "assistant_message", text: "final answer", final: true });
    expect(events[7]).toEqual({ kind: "completed", finalMessage: "final answer" });
  });
