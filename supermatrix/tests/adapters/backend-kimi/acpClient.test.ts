...
import {
  AcpClient,
  KNOWN_IGNORABLE_SESSION_UPDATES,
...
describe("AcpClient", () => {
  ...
  test("bounds a stalled session/new RPC but keeps a live shared child", async () => {
    // 2026-09-10 mr_e263beae: a starved-but-alive shared child must not be
    // recycled — disposing it fails every other session's in-flight run.
    ...
    expect(mutable.state).toBe("ready");
    expect(mutable.conn).not.toBeNull();
  });

  test("bounds a stalled session/load RPC but keeps a live shared child", async () => {
    const client = new AcpClient({ sessionRpcTimeoutMs: 10 } as any);
    const mutable = client as unknown as {
  ...
  test("stalled session RPC still invalidates when the managed child turns dead", async () => {
    const { child } = fakeManagedChild();
    const client = new AcpClient({
      sessionRpcTimeoutMs: 10,
      childLivenessProbe: vi.fn()
        .mockResolvedValueOnce("alive") // assertManagedChildRunnable before the RPC
        .mockResolvedValue("dead"), // re-check after the timeout
      disposeTermTimeoutMs: 5,
      disposeKillTimeoutMs: 5,
    } as any);
    const mutable = client as unknown as {
  ...
  test("stalled session RPC does not recycle on an unknown liveness verdict", async () => {
    const { child } = fakeManagedChild();
    const client = new AcpClient({
      sessionRpcTimeoutMs: 10,
      childLivenessProbe: vi.fn()
        .mockResolvedValueOnce("alive")
        .mockResolvedValue("unknown"), // ps hiccup — not evidence of wedging
      disposeTermTimeoutMs: 5,
      disposeKillTimeoutMs: 5,
    } as any);
    const mutable = client as unknown as {
  ...
  test("health probe roundtrip timeout keeps a live shared child", async () => {
    const clientToServer = new PassThrough();
    const serverToClient = new PassThrough();
    let buffered = "";
    clientToServer.on("data", (chunk: Buffer) => {
      ...
      while ((newline = buffered.indexOf("\n")) >= 0) {
        ...
        // session/list is deliberately never answered: the child is alive
        // but starved, and the probe must fail without recycling it.
      }
    });
    ...
    const mutable = client as unknown as { state: "init" | "ready" | "dead" };

    await expect(client.probeHealth(20)).rejects.toThrow("raw request timeout");
    expect(mutable.state).toBe("ready");
    await client.dispose();
  });
});
...
describe("AcpClient.setSessionModel (raw side channel)", () => {
  ...
  test("dispose rejects in-flight raw requests", async () => {
    ...
    await client.dispose();
    await expect(p).rejects.toThrow("AcpClient has been disposed: raw request pending during dispose");
  });
});
...
describe("AcpClient known-ignorable session updates", () => {
  async function runTurn(scenario: "known-ignorable-updates" | "unknown-session-update") {
    const clientToServer = new PassThrough();
    const serverToClient = new PassThrough();
    let wire = "";
    ...
    runFakeAcpServer({ scenario, clientToServer, serverToClient });
    const client = new AcpClient({ streams: { stdin: clientToServer, stdout: serverToClient } });
    ...
    try {
      ...
      // The lib dispatches notifications without awaiting them; let any
      // pending decode failure reach console.error before asserting.
      await new Promise((r) => setTimeout(r, 50));
      const notificationErrors = errorSpy.mock.calls.filter(
        (args) => args[0] === "Error handling notification",
      );
      return { result, updates, wire, notificationErrors };
    } finally {
  ...
  it.each(KNOWN_IGNORABLE_SESSION_UPDATES)("drops %s before SDK decode", async (kind) => {
    const { result, updates, wire, notificationErrors } = await runTurn("known-ignorable-updates");
    expect(result.stopReason).toBe("end_turn");
    // The kind really was on the wire, so an empty error list is not vacuous.
    expect(wire).toContain(`"sessionUpdate":"${kind}"`);
    expect(notificationErrors).toEqual([]);
    expect(updates.map((u) => u.sessionUpdate)).not.toContain(kind);
    expect(updates).toContainEqual(expect.objectContaining({
      sessionUpdate: "tool_call",
      toolCallId: "tc_ignorable",
    }));
    expect(updates).toContainEqual(expect.objectContaining({ sessionUpdate: "agent_message_chunk" }));
  });

  it("sm_unknown_probe still reaches the SDK and fails its decode with -32602", async () => {
    const { result, updates, wire, notificationErrors } = await runTurn("unknown-session-update");
    expect(result.stopReason).toBe("end_turn");
    expect(wire).toContain('"sessionUpdate":"sm_unknown_probe"');
    expect(notificationErrors).toHaveLength(1);
    const [, message, error] = notificationErrors[0]!;
    expect(message).toMatchObject({
      method: "session/update",
      params: { update: { sessionUpdate: "sm_unknown_probe" } },
    });
    expect(error).toMatchObject({ code: -32602 });
    expect(updates.map((u) => u.sessionUpdate)).not.toContain("sm_unknown_probe");
    expect(updates).toContainEqual(expect.objectContaining({ sessionUpdate: "agent_message_chunk" }));
  });
});
