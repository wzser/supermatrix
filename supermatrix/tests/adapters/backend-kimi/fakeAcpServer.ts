  ...
  const conn = new AgentSideConnection(
    (clientConn) => ({
      ...
      async prompt(params) {
        ...
        if (opts.scenario === "known-ignorable-updates") {
          // kimi-code 0.42.0 emits these session/update kinds, which the ACP
          // SDK 0.4.5 union cannot decode. Raw frames (as in tool-schema-drift)
          // keep the producer shape; only the discriminant matters to the
          // client filter. The same turn carries a tool_call and a message
          // chunk so tests can prove routing is undisturbed.
          const ignorableUpdates = [
            {
            ...
            { sessionUpdate: "session_info_update", title: "probe", updatedAt: "2026-09-10T00:00:00Z" },
            { sessionUpdate: "usage_update", used: 704284, size: 1048576 },
            {
              sessionUpdate: "plan_update",
              entries: [{ content: "probe", priority: "medium", status: "pending" }],
            },
            { sessionUpdate: "plan_removed" },
          ];
          for (const update of ignorableUpdates) {
            opts.serverToClient.write(JSON.stringify({
              jsonrpc: "2.0",
              method: "session/update",
        ...
        if (opts.scenario === "unknown-session-update") {
          // Reverse control: an invented discriminant outside the ignorable
          // list must still reach the SDK and fail its decode.
          opts.serverToClient.write(JSON.stringify({
            jsonrpc: "2.0",
            method: "session/update",
            params: {
              sessionId: params.sessionId,
              update: { sessionUpdate: "sm_unknown_probe", detail: "probe" },
            },
          }) + "\n");
          await clientConn.sessionUpdate({
            sessionId: params.sessionId,
            update: {
              sessionUpdate: "agent_message_chunk",
              content: { type: "text", text: "after probe" },
            } as any,
          });
          return { stopReason: "end_turn" };
        }
