...
describe("KimiBackend", () => {
  ...
  test("cancel forwards to AcpClient.cancel with the session's backendSessionId", async () => {
    ...
    let cancelMid = false;
    acp.prompt = vi.fn(async ({ onUpdate: _ }: { onUpdate: (u: unknown) => void }) => {
...
describe("KimiBackend per-run model selection", () => {
  ...
  test("setSessionModel failure → error event, prompt never sent", async () => {
    ...
    expect(acp.setSessionModel).toHaveBeenCalledTimes(1);
  });

  test("concurrent shared-client recycle during setup is retried transparently", async () => {
    // 2026-09-10 mr_e263beae: another session's invalidation disposed the
    // shared client between ensureReady and set_model; the run must re-attach
    // and proceed instead of failing with "AcpClient not ready".
    const acp = mockAcpClient();
    acp.setSessionModel = vi.fn()
      .mockRejectedValueOnce(new AcpNotReadyError())
    ...
    expect(final).toBe("hi");
    expect(acp.setSessionModel).toHaveBeenCalledTimes(2);
    expect(acp.prompt).toHaveBeenCalled();
  });

  test("retried set_model re-loads the session first (resume run)", async () => {
    // Rereview P1-2: after a yank the retry must re-attach the ACP session —
    // a recreated shared process has never loadSession'd it.
    const acp = mockAcpClient();
    acp.setSessionModel = vi.fn()
      .mockRejectedValueOnce(new AcpNotReadyError())
      .mockResolvedValue(undefined);
    ...
    for await (const e of backend.run({
      messageRunId: TEST_MESSAGE_RUN_ID,
      session: mkSession({ backendSessionId: "acp-sid-001", model: "kimi-code/k3" }),
      prompt: "again",
    })) {
      events.push(e.kind);
      if (e.kind === "completed") final = e.finalMessage;
    ...
    expect(events).not.toContain("error");
    expect(final).toBe("hi");
    // Once during setup (empty fingerprint map), once as the retry reattach.
    expect(acp.loadSession).toHaveBeenCalledTimes(2);
    expect(acp.setSessionModel).toHaveBeenCalledTimes(2);
    expect(acp.loadSession.mock.invocationCallOrder[1]).toBeLessThan(
      acp.setSessionModel.mock.invocationCallOrder[1],
    );
  });

  test("retried set_model on a factory-recreated client loads the session onto it", async () => {
    const staleAcp = mockAcpClient();
    staleAcp.ensureReady
      .mockResolvedValueOnce(undefined) // run-start ensureAcpReady succeeds
      .mockRejectedValue(new AcpDisposedError()); // re-ensure after the yank
    staleAcp.setSessionModel = vi.fn().mockRejectedValue(new AcpDisposedError());
    const freshAcp = mockAcpClient();
    const backend = new KimiBackend({
      acpClient: staleAcp as any,
      acpClientFactory: () => freshAcp as any,
    });
    let final: string | null = null;
    const events: string[] = [];
    ...
    expect(events).not.toContain("error");
    expect(final).toBe("hi");
    expect(staleAcp.setSessionModel).toHaveBeenCalledTimes(1);
    expect(freshAcp.loadSession).toHaveBeenCalledWith({
      sessionId: "acp-sid-001",
      cwd: "/tmp",
      mcpServers: [],
    });
    expect(freshAcp.loadSession.mock.invocationCallOrder[0]).toBeLessThan(
      freshAcp.setSessionModel.mock.invocationCallOrder[0],
    );
    expect(freshAcp.prompt).toHaveBeenCalled();
  });

  test("newSession not-ready (pre-I/O guard) is retried on the recovered client", async () => {
    // AcpNotReadyError fires before any wire write, so the retry provably
    // cannot create an orphan session.
    const staleAcp = mockAcpClient();
    staleAcp.ensureReady
      .mockResolvedValueOnce(undefined) // run-start ensureAcpReady succeeds
      .mockRejectedValue(new AcpDisposedError()); // re-ensure after the yank
    staleAcp.newSession = vi.fn()
      .mockRejectedValueOnce(new AcpNotReadyError())
      .mockResolvedValue("acp-sid-stale-retry");
    const freshAcp = mockAcpClient();
    freshAcp.newSession.mockResolvedValue("acp-sid-fresh");
    const backend = new KimiBackend({
      acpClient: staleAcp as any,
      acpClientFactory: () => freshAcp as any,
    });
    ...
    expect(backendSid).toBe("acp-sid-fresh");
    expect(final).toBe("hi");
    expect(staleAcp.newSession).toHaveBeenCalledTimes(1);
    expect(freshAcp.newSession).toHaveBeenCalledTimes(1);
  });

  test("newSession disposed after possible server commit is NOT retried (no orphan)", async () => {
    // Reviewer-required ambiguous-commit contract: a disposed/newSession
    // failure after the request may have reached the server must fail the
    // run — newSession is called exactly once and no prompt is sent.
    const acp = mockAcpClient();
    acp.newSession = vi.fn().mockRejectedValue(new AcpDisposedError());
    const backend = new KimiBackend({ acpClient: acp as any });
    const events: Array<{ kind: string; message?: string }> = [];
    await expect(async () => {
      for await (const e of backend.run({
