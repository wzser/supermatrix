...
import { classifyInboundOrigin, LarkCliGateway } from "../../../src/adapters/lark-cli/index.ts";
...
describe("LarkCliGateway", () => {
  test("classifies Δ-prefixed user messages as synthetic at the ingress boundary", () => {
    expect(classifyInboundOrigin("Δ重跑命令")).toBe("lark_user_synthetic");
    expect(classifyInboundOrigin("重跑命令")).toBe("lark_user");
    expect(classifyInboundOrigin(" Δ重跑命令")).toBe("lark_user");
  });
  ...
  test("start() preserves synthetic ingress provenance", async () => {
    const fake = makeFake();
    const gw = new LarkCliGateway({
      client: fake,
      attachmentDir: () => asAbsolutePath(dir),
