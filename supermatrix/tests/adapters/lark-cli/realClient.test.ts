...
import { persistDriveCommentMentionPollSince } from "../../../src/app/driveCommentMentionRegistry.ts";
...
describe("profile-scoped lark-cli calls", () => {
  test("uses the caller-provided child environment without inheriting host credentials", async () => {
    const dir = await mkdtemp(join(tmpdir(), "supermatrix-child-env-"));
    const previousSecret = process.env.SM_TEST_HOST_SECRET;
    try {
      const envPath = join(dir, "env.json");
      const cli = join(dir, "lark-cli");
      process.env.SM_TEST_HOST_SECRET = "must-not-leak";
      await writeFile(cli, `#!/bin/sh
set -eu
printf '{"home":"%s","xdg":"%s","codex":"%s","path":"%s","secret":"%s"}\n' "$HOME" "$XDG_CONFIG_HOME" "$CODEX_HOME" "$PATH" "\${SM_TEST_HOST_SECRET-}" > ${JSON.stringify(envPath)}
printf '%s\n' '{"ok":true,"data":{"chat_id":"oc_1"}}'
`);
      ...
      const client = createRealLarkClient({
        ...
        env: {
          ...
          PATH: "/pinned/node:/pinned/codex:/pinned/python",
        },
        botAppId: "cli_app",
        ownerUserId: "ou_owner",
        noProxy: true,
      });
      await client.createGroup("onboard", "ou_owner");
      const observed = JSON.parse(await readFile(envPath, "utf8")) as Record<string, string>;
      ...
      if (previousSecret === undefined) delete process.env.SM_TEST_HOST_SECRET;
      else process.env.SM_TEST_HOST_SECRET = previousSecret;
      await rm(dir, { recursive: true, force: true });
    }
  });

  test("pins group create/read and rollback-capable calls to the configured profile", async () => {
    const dir = await mkdtemp(join(tmpdir(), "supermatrix-profile-") );
    try {
      const callsPath = join(dir, "calls.jsonl");
      const cli = join(dir, "lark-cli");
...
const args = process.argv.slice(2);
fs.appendFileSync(${JSON.stringify(callsPath)}, JSON.stringify(args) + "\\n");
const data = args.includes("+chat-create") ? { chat_id: "oc_1" } : { name: "onboard" };
process.stdout.write(JSON.stringify({ ok: true, data }));
`);
      await chmod(cli, 0o755);
      const client = createRealLarkClient({ larkCliPath: cli, profile: "isolated", botAppId: "cli_app", ownerUserId: "ou_owner" });
      await client.createGroup("onboard", "ou_owner");
      await client.getGroupName(asLarkGroupId("oc_1"));
      const calls = (await readFile(callsPath, "utf8")).trim().split("\n").map((line) => JSON.parse(line) as string[]);
      expect(calls.every((args) => args.slice(0, 2).join(" ") === "--profile isolated")).toBe(true);
    } finally {
      await rm(dir, { recursive: true, force: true });
    }
  });
...
describe("parseDriveCommentEvent", () => {
  ...
  test("preserves the target record URL when Drive comment notices include one", () => {
    const raw = parseDriveCommentEvent({
      ...
      record_url: "https://example.feishu.cn/record/SMh9rOMSuewguhcVbtrcSjFknlf",
      is_mentioned: true,
      notice_meta: {
        file_token: "base_token",
        file_type: "bitable",
    ...
    expect(raw).toEqual({
      ...
      source: {
        ...
        fromUserId: "ou_user",
        url: "https://example.feishu.cn/record/SMh9rOMSuewguhcVbtrcSjFknlf",
      },
    });
  });
    ...
    const raw = parseDriveCommentEvent({
      ...
      event: {
        ...
        target: {
          ...
          url: "https://example.feishu.cn/record/recvrSCvk4eUxc",
        },
      },
    }, { botAppId: "cli_app", botOpenId: "ou_bot" });
  ...
  test("recovers a Bitable anchor from reply notify_extra when the event omits it", async () => {
    ...
    try {
      ...
      await writeFile(fakeLarkCli, [
        ...
        `fs.appendFileSync(${JSON.stringify(callsPath)}, JSON.stringify(process.argv.slice(2)) + '\\n');`,
        "if (process.argv[2] === 'drive' && process.argv[3] === 'file.comments') {",
        "  process.stdout.write(JSON.stringify({ ok: true, data: { items: [{ comment_id: '7673460608817253339', reply_list: { replies: [{ reply_id: '7673460608838208480', content: { elements: [{ type: 'text_run', text_run: { text: '调研优惠券使用情况' } }] }, extra: { notify_extra: { record: 'recvrSqY2qbks1', table: 'tblREDACTEDTABLEID', view: 'vewREDACTED89a051f33f6f25d8' } } }] } }] } }));",
        "} else if (process.argv[2] === 'base' && process.argv[3] === '+record-get') {",
...
describe("subscribeInbound attachments", () => {
  ...
  test("derives an unmentioned poll watch from the mention registry and emits its real anchor", async () => {
    const dir = await mkdtemp(join(tmpdir(), "supermatrix-lark-cli-drive-comment-poll-unmentioned-"));
    try {
      ...
      const commentsPayload = JSON.stringify({
        ...
        data: {
          items: [{
            comment_id: "comment_poll",
            update_time: 2_000,
            extra: { notify_extra: { table: "tbl_target", record: "rec_target" } },
            reply_list: { replies: [{
              reply_id: "reply_poll",
              create_time: 2_000,
              user_id: "ou_user",
              content: { elements: [{ type: "text_run", text_run: { text: "优先级怎么判断" } }] },
            }] },
          }],
        },
      });
      ...
      const client = createRealLarkClient({
        ...
        botOpenId: "ou_bot",
        ownerUserId: "",
        mentionRegistryPollLoader: async () => [{
          fileToken: "base_token",
          fileType: "bitable",
          tableId: "tbl_target",
          recordId: "rec_target",
          ...
          intervalSec: 60,
          persistSince: (since: number) =>
            persistDriveCommentMentionPollSince(registryPath, "consult", since),
        }],
        driveCommentPollIntervalMs: 20,
      });
      const callback = vi.fn();
      ...
      expect(callback).toHaveBeenCalledWith({
        ...
        source: {
          kind: "drive_comment",
          eventId: "poll:bitable:base_token:comment_poll:reply_poll:2000",
          fileToken: "base_token",
          fileType: "bitable",
          tableId: "tbl_target",
          recordId: "rec_target",
  ...
  test("cold_start persists across restart without duplicate emit while mention watch retains history", async () => {
    const dir = await mkdtemp(join(tmpdir(), "supermatrix-lark-cli-drive-comment-cold-start-restart-"));
    try {
      const registryPath = join(dir, "mention-routes.json");
      ...
      const commentsPayload = (createTime: number, mentionsBot = false) => JSON.stringify({
        code: 0,
        data: {
          items: [{
            comment_id: "comment_poll",
            ...
            reply_list: { replies: [{
              ...
              user_id: "ou_user",
              content: { elements: [
                ...(mentionsBot ? [{ type: "person", person: { user_id: "ou_bot" } }] : []),
                { type: "text_run", text_run: { text: "优先级怎么判断" } },
              ] },
            }] },
          }],
        },
      });
      await writeFile(commentsPath, commentsPayload(1_000));
      const script = [
        "#!/usr/bin/env node",
        "const fs = require('node:fs');",
        `fs.appendFileSync(${JSON.stringify(callsPath)}, JSON.stringify(process.argv.slice(2)) + "\\n");`,
        ...
        "  setInterval(() => {}, 1000);",
        "} else if (process.argv[2] === 'drive' && process.argv[3] === 'file.comments' && process.argv[4] === 'list') {",
        `  process.stdout.write(fs.readFileSync(${JSON.stringify(commentsPath)}, 'utf8'));`,
        "}",
        "",
      ].join("\n");
      await writeFile(fakeLarkCli, script);
      ...
      const loadRegistryWatch = async () => {
        const registry = JSON.parse(await readFile(registryPath, "utf8"));
        const since = registry.routes[0].ingest.poll.since as number | undefined;
        return [{
          fileToken: "base_token",
          ...
          requireMention,
          intervalSec: 60,
          persistSince: (nextSince: number) =>
            persistDriveCommentMentionPollSince(registryPath, "consult", nextSince),
        }];
      };
      const startClient = () => {
        const callback = vi.fn();
        const client = createRealLarkClient({
          ...
          botOpenId: "ou_bot",
          ownerUserId: "",
          mentionRegistryPollLoader: loadRegistryWatch,
          driveCommentPollIntervalMs: 20,
        });
        return { callback, unsubscribe: client.subscribeInbound(callback) };
      };
      ...
      const coldStartSince = JSON.parse(await readFile(registryPath, "utf8"))
        .routes[0].ingest.poll.since as number;
      expect(coldStartSince).toBeGreaterThan(1_000);

      await writeFile(commentsPath, commentsPayload(coldStartSince + 1));
      const second = startClient();
      await vi.waitFor(() => expect(second.callback).toHaveBeenCalledTimes(1), { timeout: 5_000 });
      await second.unsubscribe();

      const third = startClient();
      await new Promise((resolve) => setTimeout(resolve, 80));
      await third.unsubscribe();
      expect(third.callback).not.toHaveBeenCalled();
      expect(JSON.parse(await readFile(registryPath, "utf8")).routes[0].ingest.poll.since)
        .toBe(coldStartSince + 1);

      const zeroCursorRegistry = JSON.parse(await readFile(registryPath, "utf8"));
      zeroCursorRegistry.routes[0].ingest.poll.since = 0;
      await writeFile(registryPath, JSON.stringify(zeroCursorRegistry));
      await writeFile(commentsPath, commentsPayload(1_000));
      const pollCallCount = async () => (await readFile(callsPath, "utf8"))
        .split("\n")
        .filter((line) => line.includes('"drive","file.comments","list"'))
        .length;
      const callsBeforeZeroCursor = await pollCallCount();
      const zeroCursor = startClient();
      await waitForCondition(async () => (await pollCallCount()) > callsBeforeZeroCursor);
      await zeroCursor.unsubscribe();
      expect(zeroCursor.callback).not.toHaveBeenCalled();
      expect(JSON.parse(await readFile(registryPath, "utf8")).routes[0].ingest.poll.since)
        .toBeGreaterThan(1_000);

      const mentionRegistry = JSON.parse(await readFile(registryPath, "utf8"));
      delete mentionRegistry.routes[0].ingest.poll.since;
      await writeFile(registryPath, JSON.stringify(mentionRegistry));
      await writeFile(commentsPath, commentsPayload(1_000, true));
      requireMention = true;
      const mentionWatch = startClient();
      await vi.waitFor(() => expect(mentionWatch.callback).toHaveBeenCalledTimes(1), { timeout: 5_000 });
      await mentionWatch.unsubscribe();
      expect(JSON.parse(await readFile(registryPath, "utf8")).routes[0].ingest.poll.since).toBe(1_000);

      const zeroCursorMentionRegistry = JSON.parse(await readFile(registryPath, "utf8"));
      zeroCursorMentionRegistry.routes[0].ingest.poll.since = 0;
      await writeFile(registryPath, JSON.stringify(zeroCursorMentionRegistry));
      const zeroCursorMentionWatch = startClient();
      await vi.waitFor(() => expect(zeroCursorMentionWatch.callback).toHaveBeenCalledTimes(1), { timeout: 5_000 });
      await zeroCursorMentionWatch.unsubscribe();
      expect(JSON.parse(await readFile(registryPath, "utf8")).routes[0].ingest.poll.since).toBe(1_000);
    } finally {
      await rm(dir, { recursive: true, force: true });
    }
  ...
  test("fails closed when the bot open ID cannot be resolved", async () => {
    const dir = await mkdtemp(join(tmpdir(), "supermatrix-lark-cli-drive-comment-poll-bot-loop-"));
    try {
      const callsPath = join(dir, "calls.jsonl");
      const fakeLarkCli = join(dir, "lark-cli");
      const commentsPayload = JSON.stringify({
        ...
        data: { items: [{
          ...
          reply_list: { replies: [{
            ...
            create_time: 2_000,
            user_id: "ou_user",
            content: { elements: [{ type: "text_run", text_run: { text: "优先级结论" } }] },
          }] },
        }] },
      });
      const script = [
        ...
        "  setInterval(() => {}, 1000);",
        "} else if (process.argv[2] === 'api' && process.argv[3] === 'GET' && process.argv[4] === '/open-apis/bot/v3/info') {",
        "  process.stdout.write(JSON.stringify({ code: 0, msg: 'ok', bot: {} }));",
        "} else if (process.argv[2] === 'drive' && process.argv[3] === 'file.comments' && process.argv[4] === 'list') {",
        `  process.stdout.write(${JSON.stringify(commentsPayload)});`,
        "}",
        "",
      ...
      const unsubscribe = client.subscribeInbound(callback);
      await waitForCondition(async () => (await readFile(callsPath, "utf8").catch(() => ""))
        .includes('"api","GET","/open-apis/bot/v3/info"'));
      await new Promise((resolve) => setTimeout(resolve, 50));
      await unsubscribe();

      expect(callback).not.toHaveBeenCalled();
      expect(await readFile(callsPath, "utf8")).not.toContain('"drive","file.comments","list"');
    } finally {
      await rm(dir, { recursive: true, force: true });
    }
  });

  test("never emits the bot's own mentioned reply when its ID is known", async () => {
    const dir = await mkdtemp(join(tmpdir(), "supermatrix-lark-cli-drive-comment-poll-self-reply-"));
    try {
      const callsPath = join(dir, "calls.jsonl");
      const watchPath = join(dir, "watch.json");
      const fakeLarkCli = join(dir, "lark-cli");
      ...
      expect(callback).not.toHaveBeenCalled();
      expect(await readFile(callsPath, "utf8")).toContain('"drive","file.comments","list"');
    } finally {
      await rm(dir, { recursive: true, force: true });
    }
  });

  test("stops an in-flight poll watch that is removed before its response arrives", async () => {
    const dir = await mkdtemp(join(tmpdir(), "supermatrix-lark-cli-drive-comment-poll-remove-"));
    try {
      const callsPath = join(dir, "calls.jsonl");
      const fakeLarkCli = join(dir, "lark-cli");
      const commentsPayload = JSON.stringify({
        ...
        data: { items: [{
          ...
          reply_list: { replies: [{
            ...
            create_time: 2_000,
            user_id: "ou_user",
            content: { elements: [{ type: "text_run", text_run: { text: "business question" } }] },
          }] },
        }] },
      });
      const script = [
      ...
      let loadCount = 0;
      const persistSince = vi.fn(async () => {});
      const client = createRealLarkClient({
        larkCliPath: fakeLarkCli,
        botAppId: "cli_app",
        botOpenId: "ou_bot",
      ...
      const pollCalls = calls.split("\n").filter(
        (line) => line.includes('"drive","file.comments","list"'),
      );
      expect(pollCalls).toHaveLength(1);
      expect(callback).not.toHaveBeenCalled();
      expect(persistSince).not.toHaveBeenCalled();
    } finally {
      await rm(dir, { recursive: true, force: true });
    }
  });

  test("filters by the reply anchor and falls back to the root reply notify_extra", async () => {
    const dir = await mkdtemp(join(tmpdir(), "supermatrix-lark-cli-drive-comment-poll-anchor-"));
    try {
      const registryPath = join(dir, "mention-routes.json");
      const fakeLarkCli = join(dir, "lark-cli");
      await writeFile(registryPath, JSON.stringify({
        ...
        routes: [{
          ...
          ingest: { poll: { interval_sec: 60, since: 1_800 } },
          triggers: [{
            id: "consult",
            ingest_modes: ["poll_unmentioned"],
            match: { any: ["优先级"] },
        ...
        reply_list: { replies: [
          {
            ...
            create_time: 1_500,
            user_id: "ou_root",
            extra: { notify_extra: { table, record } },
            content: { elements: [{ type: "text_run", text_run: { text: "root" } }] },
          },
          {
            reply_id: childReplyId,
      ...
      const commentsPayload = JSON.stringify({
        code: 0,
        data: { items: [
          thread("comment_target", "tbl_target", "rec_target", "reply_target", 2_000),
          thread("comment_other", "tbl_other", "rec_other", "reply_other", 2_001),
        ] },
      });
      const script = [
        "#!/usr/bin/env node",
...
describe("Drive comment unmentioned poll ingress (todo 451)", () => {
  const makeFakeLarkCli = (callsPath: string, commentsPayload: string): string => [
    "#!/usr/bin/env node",
    "const fs = require('node:fs');",
    `fs.appendFileSync(${JSON.stringify(callsPath)}, JSON.stringify(process.argv.slice(2)) + "\\n");`,
    "if (process.argv[2] === 'event' && process.argv[3] === '+subscribe') {",
  ...
  const textReply = (
    ...
    userId: string,
    text: string,
    notifyExtra?: { table: string; record: string },
  ) => ({
    reply_id: replyId,
    create_time: createTime,
    user_id: userId,
    ...(notifyExtra ? { extra: { notify_extra: notifyExtra } } : {}),
    content: { elements: [{ type: "text_run", text_run: { text } }] },
  });

  const pollListed = async (callsPath: string): Promise<boolean> =>
    (await readFile(callsPath, "utf8").catch(() => "")).includes('"drive","file.comments","list"');

  test("[451] emits an unmentioned poll comment with mentioned:false and the root-reply anchor", async () => {
    const dir = await mkdtemp(join(tmpdir(), "supermatrix-451-unmentioned-emit-"));
    try {
      const callsPath = join(dir, "calls.jsonl");
      const fakeLarkCli = join(dir, "lark-cli");
      const commentsPayload = JSON.stringify({
        ...
        data: {
          items: [{
            comment_id: "comment_451",
            update_time: 2_000,
            reply_list: {
              replies: [
                // Root reply carries notify_extra (Feishu only stamps it there);
                // the follow-up reply must inherit that anchor.
                textReply("comment_451", 1_000, "ou_user", "线程根", {
                  table: "tbl_451",
                  record: "rec_451",
                }),
                textReply("reply_451", 2_000, "ou_user", "经营看板的汇总口径怎么算"),
              ],
            },
          }],
        },
      });
      await writeFile(fakeLarkCli, makeFakeLarkCli(callsPath, commentsPayload));
      await chmod(fakeLarkCli, 0o755);
      ...
      expect(callback).toHaveBeenCalledWith({
        ...
        source: {
          kind: "drive_comment",
          eventId: "poll:bitable:base_451:comment_451:reply_451:2000",
          fileToken: "base_451",
          fileType: "bitable",
          tableId: "tbl_451",
          recordId: "rec_451",
          commentId: "comment_451",
          replyId: "reply_451",
          fromUserId: "ou_user",
          mentioned: false,
        },
  ...
  test("[451] never emits the bot's own reply while still emitting the human reply beside it", async () => {
    const dir = await mkdtemp(join(tmpdir(), "supermatrix-451-bot-self-reply-"));
    try {
      const callsPath = join(dir, "calls.jsonl");
      const fakeLarkCli = join(dir, "lark-cli");
      const commentsPayload = JSON.stringify({
          ...
          items: [{
            ...
            reply_list: {
              replies: [
                ...
                textReply("reply_bot", 2_000, "ou_bot", "经营快照结论：优先级闸门已触发"),
                textReply("reply_human", 3_000, "ou_user", "经营看板的汇总口径怎么算"),
              ],
            },
          }],
        },
      ...
      const callback = vi.fn();
      const unsubscribe = client.subscribeInbound(callback);
      await waitForCondition(() => pollListed(callsPath));
      await vi.waitFor(() => expect(callback).toHaveBeenCalledTimes(1), { timeout: 5_000 });
      await new Promise((resolve) => setTimeout(resolve, 60));
      await unsubscribe();

      expect(callback).toHaveBeenCalledTimes(1);
      const emitted = callback.mock.calls.map(([raw]) => raw.source.replyId);
      expect(emitted).toEqual(["reply_human"]);
      expect(emitted).not.toContain("reply_bot");
    } finally {
      await rm(dir, { recursive: true, force: true });
    }
  });

  test("[451] drops comments anchored to another table or record instead of stamping the watch IDs", async () => {
    const dir = await mkdtemp(join(tmpdir(), "supermatrix-451-anchor-filter-"));
    try {
      const callsPath = join(dir, "calls.jsonl");
      const fakeLarkCli = join(dir, "lark-cli");
      const commentsPayload = JSON.stringify({
        ...
        data: {
          items: [
            {
              comment_id: "comment_other_table",
              update_time: 2_000,
              reply_list: {
                replies: [
                  textReply("comment_other_table", 2_000, "ou_user", "别的表的经营看板讨论", {
                    table: "tbl_other",
                    record: "rec_other",
                  }),
                ],
            ...
            {
              comment_id: "comment_other_record",
              update_time: 2_000,
              reply_list: {
                replies: [
                  textReply("comment_other_record", 2_000, "ou_user", "同表别的记录", {
                    table: "tbl_451",
                    record: "rec_elsewhere",
                  }),
                ],
              },
            },
            {
              ...
              reply_list: {
                replies: [
                  textReply("comment_451", 2_000, "ou_user", "经营看板的汇总口径怎么算", {
                    table: "tbl_451",
                    record: "rec_451",
                  }),
                ],
      ...
      expect(callback).toHaveBeenCalledTimes(1);
      const sources = callback.mock.calls.map(([raw]) => raw.source);
      expect(sources.map((source) => source.commentId)).toEqual(["comment_451"]);
      // The foreign-anchor comments must be dropped, not relabelled with the
      // watch's own table/record.
      for (const source of sources) {
        expect(source.tableId).toBe("tbl_451");
        expect(source.recordId).toBe("rec_451");
      }
    } finally {
      await rm(dir, { recursive: true, force: true });
    }
  ...
  test("[451] stamps the source with the comment's own notify_extra, not the watch's IDs", async () => {
    const dir = await mkdtemp(join(tmpdir(), "supermatrix-451-anchor-stamp-"));
    try {
      const callsPath = join(dir, "calls.jsonl");
      const fakeLarkCli = join(dir, "lark-cli");
      const commentsPayload = JSON.stringify({
        ...
        data: {
          items: [{
            ...
            reply_list: {
              replies: [
                textReply("comment_451", 2_000, "ou_user", "经营看板的汇总口径怎么算", {
                  table: "tbl_from_notify_extra",
                  record: "rec_from_notify_extra",
                }),
              ],
            },
          }],
      ...
      // The watch deliberately declares no table/record, so the emitted anchor
      // can only have come from the comment payload. Under the old
      // watch-overwrite behaviour these fields would simply be absent.
      const client = createRealLarkClient({
        larkCliPath: fakeLarkCli,
        botAppId: "cli_app",
        botOpenId: "ou_bot",
      ...
      const source = callback.mock.calls[0]?.[0].source;
      expect(source.tableId).toBe("tbl_from_notify_extra");
      expect(source.recordId).toBe("rec_from_notify_extra");
      expect(source.mentioned).toBe(false);
    } finally {
      await rm(dir, { recursive: true, force: true });
    }
  });

  test("[451] keeps the registry gate when a watches-file entry shares the same watch key", async () => {
    const dir = await mkdtemp(join(tmpdir(), "supermatrix-451-require-mention-conflict-"));
    const warn = vi.spyOn(console, "warn").mockImplementation(() => {});
    try {
      const callsPath = join(dir, "calls.jsonl");
      const watchPath = join(dir, "watch.json");
      const fakeLarkCli = join(dir, "lark-cli");
      await writeFile(watchPath, JSON.stringify({
        watches: [{ fileToken: "base_451", fileType: "bitable", since: 1_000 }],
      }));
      const commentsPayload = JSON.stringify({
        code: 0,
        data: {
      ...
      // Registry-derived requireMention:false wins over the mention-only
      // watches-file entry; an OR/AND blend would have silently changed the gate.
      expect(callback.mock.calls[0][0].source.mentioned).toBe(false);
      expect(warn).toHaveBeenCalledWith(
        "[lark-cli] drive comment watch requireMention conflict resolved by registry",
        { watchKey: "bitable:base_451::", requireMention: false },
      );
    } finally {
      warn.mockRestore();
      await rm(dir, { recursive: true, force: true });
