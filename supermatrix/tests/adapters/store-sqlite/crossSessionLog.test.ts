...
describe("SqliteBindingStore cross_session_log", () => {
  ...
  test("rejects completion when the cross-session comm row is missing", async () => {
    const { store, cleanup } = await createTempStore();
    try {
      await expect(store.finishCrossSessionComm(
        "comm_missing",
        "completed",
        undefined,
        "done",
        undefined,
        "done",
      )).rejects.toThrow("cross-session comm not found: comm_missing");
    } finally {
      await cleanup();
    }
  });

  test("atomically rejects a late accepted write and lets a racing fail-closed write win", async () => {
    const { store, cleanup } = await createTempStore();
    try {
      await store.createSession({ id: asSessionId("sess_caller"), name: "caller", ...BASE });
      await store.createSession({ id: asSessionId("sess_target"), name: "target", ...BASE });
      await store.logCrossSessionComm({
        id: "comm_receiver_terminal_cas",
        fromSessionId: asSessionId("sess_caller"),
        toSessionId: asSessionId("sess_target"),
        kind: "spawn",
        prompt: "receiver-terminal metadata",
        clientRequestId: "receiver-terminal-cas-request",
        createdAt: asTimestamp(1_700_000_100_000),
      });

      const acceptedMessage = JSON.stringify({ terminal: true, transport: { target_accepted: true } });
      const failureMessage = JSON.stringify({ terminal: true, transport: { target_accepted: false } });
      const lateAccepted = await store.finishReceiverTerminalComm({
        id: "comm_receiver_terminal_cas",
        clientRequestId: "receiver-terminal-cas-request",
        accepted: true,
        deadlineAt: Date.now() - 1,
        resultPreview: "accepted",
        finalMessage: acceptedMessage,
      });
      expect(lateAccepted).toMatchObject({ outcome: "not_committed" });

      const failClosed = await store.finishReceiverTerminalComm({
        id: "comm_receiver_terminal_cas",
        clientRequestId: "receiver-terminal-cas-request",
        accepted: false,
        deadlineAt: Date.now() - 1,
        resultPreview: "fail-closed",
        error: "deadline",
        finalMessage: failureMessage,
      });
      expect(failClosed).toEqual({ outcome: "committed", finalMessage: failureMessage });

      const lateAcceptedAfterFailure = await store.finishReceiverTerminalComm({
        id: "comm_receiver_terminal_cas",
        clientRequestId: "receiver-terminal-cas-request",
        accepted: true,
      ...
      expect(lateAcceptedAfterFailure).toEqual({ outcome: "already_completed", finalMessage: failureMessage });
      const row = (await store.listAllCrossSessionComms()).find((item) => item.id === "comm_receiver_terminal_cas");
      expect(row).toMatchObject({ status: "completed", finalMessage: failureMessage });
    } finally {
      await cleanup();
    }
  });
