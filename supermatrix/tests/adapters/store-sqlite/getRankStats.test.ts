...
describe("getRankStats — global scope", () => {
  ...
  test("excludes framework-emitted as-user messages (Δ-prefixed prompts)", async () => {
    const store = await makeSeededStore();
    const db = (store as any).db;
    // Two extra rows attributed to a real user open_id but Δ-prefixed: these are
    // framework "as user" impersonations that lark-cli marked; rank must skip them.
    db.prepare(
      `INSERT INTO message_runs (id, session_id, group_id, prompt, started_at, status, sender_id)
       VALUES ('r_delta1', 'sess_1', 'oc_c', 'Δframework spoke as the user', ?, 'done', 'ou_user_a'),
              ('r_delta2', 'sess_2', 'oc_z', 'Δanother impersonated line',   ?, 'done', 'ou_user_b')`,
    ).run(Date.now(), Date.now());

    const stats = await store.getRankStats({ scope: "global" });

    // Counts are unchanged from the un-Δ'd baseline (A=6, B=4): the Δ rows were skipped.
    expect(stats.rows.find((r) => r.senderId === "ou_user_a")!.total).toBe(6);
    expect(stats.rows.find((r) => r.senderId === "ou_user_b")!.total).toBe(4);
    expect(stats.rows[0].inputChars).toBe(21);
  });
  ...
  test("allSessions includes every session while top3Sessions remains limited", async () => {
    const store = await makeSeededStore();
    const db = (store as any).db;
    db.prepare(
      `INSERT INTO sessions (id, name, alias, avatar, category, scope, backend, workdir, status, created_at, updated_at)
       VALUES ('sess_4', 'writer', '', '', '', 'user', 'claude', '/tmp', 'idle', 0, 0)`,
    ).run();
    db.prepare(
      `INSERT INTO message_runs (id, session_id, group_id, prompt, started_at, status, sender_id)
       VALUES ('r14', 'sess_4', 'oc_w', 'x', ?, 'done', 'ou_user_a')`,
    ).run(Date.now());

    const stats = await store.getRankStats({ scope: "global" });
    const userA = stats.rows.find((row) => row.senderId === "ou_user_a")!;

    expect(userA.top3Sessions).toHaveLength(3);
    expect(userA.allSessions).toEqual([
      { sessionName: "console", count: 3 },
      { sessionName: "zedong", count: 2 },
      { sessionName: "monet", count: 1 },
      { sessionName: "writer", count: 1 },
    ]);
  });
    ...
    (store as any).db
      .prepare(
        `INSERT INTO user_display_names (sender_id, display_name, fetched_at)
         VALUES ('ou_abc', 'PERSON_REDACTED', 1000000)`
      )
      .run();

    const result = await store.getDisplayNames(["ou_abc", "ou_missing"]);
    expect(result.get("ou_abc")).toEqual({ displayName: "PERSON_REDACTED", fetchedAt: 1000000 });
