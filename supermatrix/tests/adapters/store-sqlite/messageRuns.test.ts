...
describe("SqliteBindingStore message_runs", () => {
  ...
  test("persists inbound synthetic provenance separately from human user origin", async () => {
    const { store, cleanup } = await createTempStore();
    try {
      await store.createSessionWithBinding(
        { id: asSessionId("s1"), ...BASE },
      ...
      await store.startMessageRun({
        id: asMessageRunId("mr_synthetic"),
        sessionId: asSessionId("s1"),
        groupId: asLarkGroupId("oc_1"),
        prompt: "Δ重跑命令",
        startedAt: asTimestamp(1_700_000_100_000),
      ...
      const run = await store.findRunningMessageRunBySession(asSessionId("s1"));
      expect(run?.origin).toBe("lark_user_synthetic");
    } finally {
      await cleanup();
    }
  });
