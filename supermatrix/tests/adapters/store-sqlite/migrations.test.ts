import { readdir } from "node:fs/promises";
import { describe, expect, test } from "vitest";
import { asAbsolutePath, asSessionId, asTimestamp } from "../../../src/domain/ids.ts";
import { LEGACY_MAIN_SESSION_DEFAULTS } from "../../../src/ports/SessionRuntimeSettings.ts";
import { createTempStore } from "./helpers.ts";
import { SqliteBindingStore } from "../../../src/adapters/store-sqlite/index.ts";

async function migrationVersions(): Promise<number[]> {
  const files = await readdir(new URL("../../../src/adapters/store-sqlite/migrations/", import.meta.url));
  return files
    .filter((name) => name.endsWith(".sql"))
    .map((name) => Number.parseInt(name.split("_", 1)[0]!, 10))
    .sort((a, b) => a - b);
}

describe("sqlite migrations", () => {
  test("init applies all migrations and records latest schema_version", async () => {
    const { store, cleanup } = await createTempStore();
    try {
      const raw = (store as unknown as { db: { prepare: (sql: string) => { get: () => { version: number } } } }).db;
      const row = raw.prepare("SELECT version FROM schema_version ORDER BY version DESC LIMIT 1").get();
      expect(row.version).toBe((await migrationVersions()).at(-1));
    } finally {
      await cleanup();
    }
  });

  test("052 adds message_runs origin column", async () => {
    const { store, cleanup } = await createTempStore();
    try {
      const columns = store.db
        .prepare("PRAGMA table_info(message_runs)")
        .all() as Array<{ name: string }>;
      expect(columns.some((column) => column.name === "origin")).toBe(true);
    } finally {
      await cleanup();
    }
  });

  test("054 creates the scheduler cleanup request ledger", async () => {
    const store = new SqliteBindingStore(":memory:");
    await store.init();
    const columns = store.db.pragma("table_info(scheduler_cleanup_requests)") as Array<{ name: string }>;
    expect(columns.map((column) => column.name)).toEqual([
      "client_request_id",
      "session_id",
      "session_name",
      "status",
      "attempt_count",
      "last_error",
      "created_at",
      "updated_at",
      "submitted_at",
    ]);
    await store.close();
  });

  test("init is idempotent", async () => {
    const { store, cleanup } = await createTempStore();
    try {
      await store.init();
      await store.init();
      const raw = (store as unknown as { db: { prepare: (sql: string) => { all: () => { version: number }[] } } }).db;
      const rows = raw.prepare("SELECT version FROM schema_version").all();
      expect(rows.map((row) => row.version).sort((a, b) => a - b)).toEqual(await migrationVersions());
    } finally {
      await cleanup();
    }
  });

  test("duplicate column is auto-marked as applied", async () => {
    const store = new SqliteBindingStore(":memory:");
    // First init — applies all migrations normally
    await store.init();
    // Manually remove version 8 from schema_version (simulates drift)
    store.db.prepare("DELETE FROM schema_version WHERE version = 8").run();
    // Second init — migration 008 tries ALTER TABLE but column exists
    // With idempotency, this should not throw
    const result = await store.init();
    expect(result.degraded).toHaveLength(0);
    // Version 8 should be re-recorded
    const row = store.db.prepare("SELECT version FROM schema_version WHERE version = 8").get() as { version: number } | undefined;
    expect(row?.version).toBe(8);
    await store.close();
  });

  test("optional migration failure degrades instead of crashing", async () => {
    const store = new SqliteBindingStore(":memory:");
    const result = await store.init();
    expect(result).toHaveProperty("degraded");
    expect(Array.isArray(result.degraded)).toBe(true);
    await store.close();
  });

  test("critical migration failure still throws", async () => {
    const store = new SqliteBindingStore(":memory:");
    const result = await store.init();
    expect(result.degraded).toHaveLength(0);
    await store.close();
  });

  test("023-026 create spawn closure support tables", async () => {
    const store = new SqliteBindingStore(":memory:");
    await store.init();
    const rows = store.db
      .prepare("SELECT name FROM sqlite_master WHERE type = 'table' AND name IN (?, ?, ?, ?, ?, ?) ORDER BY name")
      .all(
        "result_sink_attempts",
        "spawn_predicate_patches",
        "spawn_predicates",
        "watcher_exceptions",
        "watcher_state",
        "watcher_ticks"
      ) as Array<{ name: string }>;
    expect(rows.map((r) => r.name)).toEqual([
      "result_sink_attempts",
      "spawn_predicate_patches",
      "spawn_predicates",
      "watcher_exceptions",
      "watcher_state",
      "watcher_ticks",
    ]);
    await store.close();
  });

  test("028 creates spawn async items contract table", async () => {
    const store = new SqliteBindingStore(":memory:");
    await store.init();

    const columns = store.db.pragma("table_info(spawn_async_items)") as Array<{
      name: string;
      type: string;
      notnull: number;
      dflt_value: string | null;
      pk: number;
    }>;

    expect(columns.map((c) => c.name)).toEqual([
      "ref",
      "comm_id",
      "caller_session",
      "target_session",
      "failed_phase",
      "failure_kind",
      "attempt_count",
      "status",
      "verdict",
      "verdict_reason",
      "created_at",
      "updated_at",
      "last_attempt_at",
    ]);

    expect(Object.fromEntries(columns.map((c) => [c.name, c.dflt_value]))).toMatchObject({
      attempt_count: "0",
      status: "'pending'",
    });

    const required = columns.filter((c) => c.notnull === 1).map((c) => c.name);
    expect(required).toEqual([
      "comm_id",
      "failed_phase",
      "failure_kind",
      "attempt_count",
      "status",
      "created_at",
      "updated_at",
    ]);

    const indexes = store.db
      .prepare("SELECT name FROM sqlite_master WHERE type = 'index' AND tbl_name = 'spawn_async_items' ORDER BY name")
      .all() as Array<{ name: string }>;
    expect(indexes.map((i) => i.name)).toEqual([
      "idx_spawn_async_items_comm",
      "idx_spawn_async_items_courier_status",
      "idx_spawn_async_items_status",
      "sqlite_autoindex_spawn_async_items_1",
    ]);

    await store.close();
  });

  test("030 adds indexes for boot reconciliation and cross-session sync scans", async () => {
    const store = new SqliteBindingStore(":memory:");
    await store.init();

    const rows = store.db
      .prepare(
        `SELECT name FROM sqlite_master
         WHERE type = 'index'
           AND name IN (?, ?, ?, ?)
         ORDER BY name`,
      )
      .all(
        "idx_cross_session_log_bitable_record",
        "idx_cross_session_log_child",
        "idx_cross_session_log_stale_sync",
        "idx_sessions_backend_session_id",
      ) as Array<{ name: string }>;

    expect(rows.map((r) => r.name)).toEqual([
      "idx_cross_session_log_bitable_record",
      "idx_cross_session_log_child",
      "idx_cross_session_log_stale_sync",
      "idx_sessions_backend_session_id",
    ]);

    await store.close();
  });

  test("031 creates persistent spawn throttle queue table", async () => {
    const store = new SqliteBindingStore(":memory:");
    await store.init();

    const columns = store.db.pragma("table_info(spawn_queue)") as Array<{
      name: string;
      type: string;
      notnull: number;
      dflt_value: string | null;
      pk: number;
    }>;

    expect(columns.map((c) => c.name)).toEqual([
      "id",
      "parent_id",
      "spawn_input_json",
      "caller_session",
      "comm_id",
      "status",
      "created_at",
      "dispatched_at",
      "ttl_sec",
      "updated_at",
    ]);

    expect(Object.fromEntries(columns.map((c) => [c.name, c.dflt_value]))).toMatchObject({
      status: "'pending'",
    });

    const required = columns.filter((c) => c.notnull === 1).map((c) => c.name);
    expect(required).toEqual([
      "parent_id",
      "spawn_input_json",
      "comm_id",
      "status",
      "created_at",
      "ttl_sec",
      "updated_at",
    ]);

    const indexes = store.db
      .prepare("SELECT name FROM sqlite_master WHERE type = 'index' AND tbl_name = 'spawn_queue' ORDER BY name")
      .all() as Array<{ name: string }>;
    expect(indexes.map((i) => i.name)).toEqual([
      "idx_spawn_queue_comm",
      "idx_spawn_queue_parent_status_created",
      "sqlite_autoindex_spawn_queue_1",
    ]);

    await store.close();
  });

  test("002 adds parent_id and depth columns to sessions", async () => {
    const store = new SqliteBindingStore(":memory:");
    await store.init();
    const info = store.db.pragma("table_info(sessions)") as Array<{ name: string }>;
    const columns = info.map((c) => c.name);
    expect(columns).toContain("parent_id");
    expect(columns).toContain("depth");
    await store.close();
  });

  test("038-039 add effort lock and backend runtime defaults", async () => {
    const store = new SqliteBindingStore(":memory:");
    await store.init();
    const sessionColumns = (store.db.pragma("table_info(sessions)") as Array<{ name: string }>).map((c) => c.name);
    expect(sessionColumns).toContain("effort_locked");
    const defaultsColumns = (store.db.pragma("table_info(backend_runtime_defaults)") as Array<{ name: string }>).map((c) => c.name);
    expect(defaultsColumns).toEqual(["backend", "model", "effort", "updated_at"]);
    await store.close();
  });

  test("041 creates child session defaults singleton with configured flags", async () => {

    const store = new SqliteBindingStore(":memory:");
    await store.init();

    const columns = store.db.pragma("table_info(child_session_defaults)") as Array<{
      name: string;
      notnull: number;
      pk: number;
    }>;
    expect(columns.map((column) => column.name)).toEqual([
      "singleton",
      "backend",
      "backend_configured",
      "model",
      "model_configured",
      "effort",
      "effort_configured",
      "updated_at",
    ]);
    expect(columns.find((column) => column.name === "singleton")).toMatchObject({ pk: 1 });
    expect(columns.filter((column) => column.notnull === 1).map((column) => column.name)).toEqual([
      "backend_configured",
      "model_configured",
      "effort_configured",
      "updated_at",
    ]);

    await store.close();
  });

  test("042 creates per-session runtime settings and backfills non-child sessions", async () => {

    const store = new SqliteBindingStore(":memory:");
    await store.init();
    const columns = store.db.pragma("table_info(session_runtime_settings)") as Array<{
      name: string;
      notnull: number;
      pk: number;
    }>;
    expect(columns.map((column) => column.name)).toEqual([
      "session_id",
      "main_model_default",
      "main_effort_default",
      "child_backend",
      "child_backend_configured",
      "child_model",
      "child_model_configured",
      "child_effort",
      "child_effort_configured",
      "updated_at",
    ]);
    expect(columns.find((column) => column.name === "session_id")).toMatchObject({ pk: 1 });

    store.db.exec(`
      INSERT INTO sessions (id, name, scope, backend, workdir, purpose, status, depth, created_at, updated_at)
      VALUES
        ('main-1', 'main-1', 'user', 'codex', '/tmp', '', 'idle', 0, 1, 2),
        ('child-1', 'child-1', 'child', 'codex', '/tmp', '', 'idle', 1, 1, 2);
      INSERT OR IGNORE INTO session_runtime_settings (
        session_id, child_backend_configured, child_model_configured,
        child_effort_configured, updated_at
      )
      SELECT id, 0, 0, 0, updated_at FROM sessions WHERE scope != 'child';
    `);
    const rows = store.db
      .prepare("SELECT session_id FROM session_runtime_settings ORDER BY session_id")
      .all() as Array<{ session_id: string }>;
    expect(rows.map((row) => row.session_id)).toEqual(["main-1"]);
    await store.close();
  });

  test("042 backfill reads backend runtime defaults and preserves NULL dimensions", async () => {
    const store = new SqliteBindingStore(":memory:");
    await store.init();
    await store.createSession({
      id: asSessionId("migration-defaults"),
      name: "migration-defaults",
      scope: "user",
      backend: "codex",
      workdir: asAbsolutePath("/tmp/migration-defaults"),
      purpose: "test",
      createdAt: asTimestamp(1_700_000_000_000),
    });
    await store.updateBackendRuntimeDefaults("codex", {
      model: "gpt-live-default",
      effort: null,
    });

    store.db.exec("DROP TABLE session_runtime_settings");
    store.db.prepare("DELETE FROM schema_version WHERE version = 42").run();
    await store.init();

    expect(await store.getSessionRuntimeSettings(asSessionId("migration-defaults"))).toMatchObject({
      mainModelDefault: "gpt-live-default",
      mainEffortDefault: null,
    });
    await store.close();
  });

  test("042 rerun preserves existing session runtime settings", async () => {
    const store = new SqliteBindingStore(":memory:");
    await store.init();
    const sessionId = asSessionId("migration-existing-settings");
    await store.createSession({
      id: sessionId,
      name: "migration-existing-settings",
      scope: "user",
      backend: "codex",
      workdir: asAbsolutePath("/tmp/migration-existing-settings"),
      purpose: "test",
      createdAt: asTimestamp(1_700_000_000_000),
    });
    await store.updateSessionRuntimeSettings(sessionId, {
      mainModelDefault: "keep-existing-model",
      mainEffortDefault: "low",
    });
    await store.updateBackendRuntimeDefaults("codex", {
      model: "replacement-default",
      effort: "ultra",
    });

    store.db.prepare("DELETE FROM schema_version WHERE version = 42").run();
    await store.init();

    expect(await store.getSessionRuntimeSettings(sessionId)).toMatchObject({
      mainModelDefault: "keep-existing-model",
      mainEffortDefault: "low",
    });
    await store.close();
  });

  test("042 backfill falls back when the backend runtime defaults row is missing", async () => {
    const store = new SqliteBindingStore(":memory:");
    await store.init();
    const sessionId = asSessionId("migration-missing-backend-defaults");
    await store.createSession({
      id: sessionId,
      name: "migration-missing-backend-defaults",
      scope: "user",
      backend: "claude",
      workdir: asAbsolutePath("/tmp/migration-missing-backend-defaults"),
      purpose: "test",
      createdAt: asTimestamp(1_700_000_000_000),
    });

    store.db.prepare("DELETE FROM backend_runtime_defaults WHERE backend = 'claude'").run();
    store.db.exec("DROP TABLE session_runtime_settings");
    store.db.prepare("DELETE FROM schema_version WHERE version = 42").run();
    await store.init();

    expect(await store.getSessionRuntimeSettings(sessionId)).toMatchObject(
      LEGACY_MAIN_SESSION_DEFAULTS.claude,
    );
    await store.close();
  });

  test("043 adds the user-session governance affiliation column", async () => {
    expect(await migrationVersions()).toContain(43);

    const store = new SqliteBindingStore(":memory:");
    await store.init();
    const columns = (store.db.pragma("table_info(sessions)") as Array<{ name: string }>).map(
      (column) => column.name,
    );
    expect(columns).toContain("affiliated_to");
    await store.close();
  });

  test("044 adds a default-off workspace lock to sessions", async () => {
    const store = new SqliteBindingStore(":memory:");
    await store.init();
    const columns = store.db.pragma("table_info(sessions)") as Array<{
      name: string;
      notnull: number;
      dflt_value: string | null;
    }>;
    expect(columns.find((column) => column.name === "workspace_locked")).toMatchObject({
      notnull: 1,
      dflt_value: "0",
    });
    await store.close();
  });

  test("045 creates the backend_account_switches idempotency ledger", async () => {
    // This test owns migration 045's schema, not the repository's newest
    // migration number. Newer migrations must not make this historical
    // ledger assertion fail.
    expect(await migrationVersions()).toContain(45);

    const store = new SqliteBindingStore(":memory:");
    await store.init();
    const columns = (store.db.pragma("table_info(backend_account_switches)") as Array<{
      name: string;
      pk: number;
    }>).map((c) => c.name);
    expect(columns).toEqual([
      "client_request_id",
      "backend",
      "caller",
      "from_profile",
      "to_profile",
      "switched_at",
      "cleared_sessions",
      "cleared_branches",
      "created_at",
    ]);
    await store.close();
  });

  test("049 creates durable backend maintenance lease and audit tables", async () => {
    expect(await migrationVersions()).toContain(49);

    const store = new SqliteBindingStore(":memory:");
    await store.init();
    const leaseColumns = (store.db.pragma("table_info(backend_maintenance_leases)") as Array<{
      name: string;
    }>).map((column) => column.name);
    expect(leaseColumns).toEqual([
      "backend",
      "owner",
      "token_hash",
      "request_id",
      "acquired_at",
    ]);
    const eventColumns = (store.db.pragma("table_info(backend_maintenance_lease_events)") as Array<{
      name: string;
    }>).map((column) => column.name);
    expect(eventColumns).toEqual([
      "id",
      "backend",
      "action",
      "outcome",
      "owner",
      "request_id",
      "lease_owner",
      "running_message_run_count",
      "created_at",
    ]);
    await store.close();
  });

  test("046 creates the Codex state snapshot audit ledger", async () => {
    const store = new SqliteBindingStore(":memory:");
    await store.init();
    const columns = (store.db.pragma("table_info(codex_state_snapshot_audit)") as Array<{
      name: string;
      notnull: number;
    }>).map((column) => column.name);
    expect(columns).toEqual([
      "id",
      "snapshot_id",
      "event",
      "source_path",
      "owner",
      "reason",
      "snapshot_path",
      "snapshot_sha256",
      "snapshot_bytes",
      "receipt_path",
      "expires_at",
      "release_reason",
      "created_at",
    ]);
    const indexes = store.db.prepare(
      "SELECT name FROM sqlite_master WHERE type = 'index' AND name = ?",
    ).all("idx_codex_state_snapshot_audit_snapshot_created") as Array<{ name: string }>;
    expect(indexes).toEqual([{ name: "idx_codex_state_snapshot_audit_snapshot_created" }]);
    await store.close();
  });

  test("047 creates the generic managed SQLite snapshot audit ledger", async () => {
    const store = new SqliteBindingStore(":memory:");
    await store.init();
    const columns = (store.db.pragma("table_info(managed_sqlite_snapshot_audit)") as Array<{
      name: string;
    }>).map((column) => column.name);
    expect(columns).toEqual([
      "id",
      "snapshot_id",
      "operation_id",
      "event",
      "source_path",
      "owner",
      "reason",
      "snapshot_path",
      "snapshot_sha256",
      "snapshot_bytes",
      "receipt_path",
      "expires_at",
      "release_reason",
      "created_at",
    ]);
    const copied = store.db.prepare(
      "SELECT name FROM sqlite_master WHERE type = 'index' AND name = ?",
    ).all("idx_managed_sqlite_snapshot_audit_snapshot_created") as Array<{ name: string }>;
    expect(copied).toEqual([{ name: "idx_managed_sqlite_snapshot_audit_snapshot_created" }]);
    await store.close();
  });

  test("013 adds child capability columns to sessions", async () => {
    const store = new SqliteBindingStore(":memory:");
    await store.init();
    const info = store.db.pragma("table_info(sessions)") as Array<{ name: string }>;
    const columns = info.map((c) => c.name);
    expect(columns).toContain("child_type");
    expect(columns).toContain("trigger_kind");
    expect(columns).toContain("post_identity");
    expect(columns).toContain("caller_invocation");
    expect(columns).toContain("continuation_hook");
    expect(columns).toContain("capability_payload");
    await store.close();
  });

  test("013 backfills pre-existing child rows by status heuristic", async () => {
    // Verify the backfill UPDATE logic directly against the applied schema.
    // Insert rows with child_type intentionally NULL to simulate pre-013 state,
    // then execute the same UPDATE statements the migration runs.
    const store = new SqliteBindingStore(":memory:");
    await store.init();

    store.db.exec(`
      INSERT INTO sessions (id, name, scope, backend, workdir, purpose, status, parent_id, depth, child_type, created_at, updated_at)
      VALUES
        ('p1', 'parent', 'user', 'claude', '/tmp', '', 'idle', NULL, 0, NULL, 0, 0),
        ('s-idle-child', 'idle_child', 'child', 'claude', '/tmp', '', 'idle', 'p1', 1, NULL, 0, 0),
        ('s-busy-child', 'busy_child', 'child', 'claude', '/tmp', '', 'busy', 'p1', 1, NULL, 0, 0),
        ('s-deleted-child', 'deleted_child', 'child', 'claude', '/tmp', '', 'deleted', 'p1', 1, NULL, 0, 0),
        ('s-user-session', 'user_session', 'user', 'claude', '/tmp', '', 'idle', NULL, 0, NULL, 0, 0);
    `);

    store.db.exec(`
      UPDATE sessions
      SET child_type = 'ephemeral_conversation'
      WHERE scope = 'child' AND status = 'idle' AND child_type IS NULL;

      UPDATE sessions
      SET child_type = 'one_shot_delegation'
      WHERE scope = 'child' AND child_type IS NULL;
    `);

    const rows = store.db
      .prepare("SELECT id, child_type FROM sessions")
      .all() as Array<{ id: string; child_type: string | null }>;
    const byId = Object.fromEntries(rows.map((r) => [r.id, r.child_type]));

    expect(byId["s-idle-child"]).toBe("ephemeral_conversation");
    expect(byId["s-busy-child"]).toBe("one_shot_delegation");
    expect(byId["s-deleted-child"]).toBe("one_shot_delegation");
    expect(byId["s-user-session"]).toBeNull();
    expect(byId["p1"]).toBeNull();

    await store.close();
  });

  test("020 backfills heartbeat for existing non-child active sessions", async () => {
    const store = new SqliteBindingStore(":memory:");
    await store.init();

    store.db.exec(`
      INSERT INTO sessions (id, name, scope, backend, workdir, purpose, status, parent_id, depth, heartbeat_enabled, created_at, updated_at)
      VALUES
        ('s-user', 'user_session', 'user', 'claude', '/tmp', '', 'idle', NULL, 0, 0, 0, 0),
        ('s-root', 'root_session', 'root', 'claude', '/tmp', '', 'idle', NULL, 0, 0, 0, 0),
        ('s-child', 'child_session', 'child', 'claude', '/tmp', '', 'idle', 's-user', 1, 0, 0, 0),
        ('s-deleted', 'deleted_session', 'user', 'claude', '/tmp', '', 'deleted', NULL, 0, 0, 0, 0),
        ('s-heartbeat', 'heartbeat', 'user', 'claude', '/tmp', '', 'idle', NULL, 0, 0, 0, 0);
    `);

    store.db.exec(`
      UPDATE sessions
      SET heartbeat_enabled = 1
      WHERE status != 'deleted'
        AND scope != 'child'
        AND name != 'heartbeat';
    `);

    const rows = store.db
      .prepare("SELECT id, heartbeat_enabled FROM sessions WHERE id LIKE 's-%'")
      .all() as Array<{ id: string; heartbeat_enabled: number }>;
    const byId = Object.fromEntries(rows.map((r) => [r.id, r.heartbeat_enabled]));

    expect(byId["s-user"]).toBe(1);
    expect(byId["s-root"]).toBe(1);
    expect(byId["s-child"]).toBe(0);
    expect(byId["s-deleted"]).toBe(0);
    expect(byId["s-heartbeat"]).toBe(0);
    await store.close();
  });

  test("032 adds client_request_id to cross_session_log with lookup index", async () => {
    const store = new SqliteBindingStore(":memory:");
    await store.init();

    const columns = store.db.pragma("table_info(cross_session_log)") as Array<{ name: string; type: string }>;
    expect(columns).toContainEqual(expect.objectContaining({
      name: "client_request_id",
      type: "TEXT",
    }));

    const row = store.db
      .prepare("SELECT name FROM sqlite_master WHERE type = 'index' AND name = ?")
      .get("idx_cross_session_log_client_request_id") as { name: string } | undefined;
    expect(row?.name).toBe("idx_cross_session_log_client_request_id");

    await store.close();
  });

  test("035 adds origin_run_id to cross_session_log with lookup index", async () => {
    const store = new SqliteBindingStore(":memory:");
    await store.init();

    const columns = store.db.pragma("table_info(cross_session_log)") as Array<{ name: string; type: string }>;
    expect(columns).toContainEqual(expect.objectContaining({
      name: "origin_run_id",
      type: "TEXT",
    }));

    const row = store.db
      .prepare("SELECT name FROM sqlite_master WHERE type = 'index' AND name = ?")
      .get("idx_cross_session_log_origin_run_id") as { name: string } | undefined;
    expect(row?.name).toBe("idx_cross_session_log_origin_run_id");

    await store.close();
  });

  test("036 creates conversation branch tables and message run branch index", async () => {
    const store = new SqliteBindingStore(":memory:");
    await store.init();

    const branchTables = store.db
      .prepare("SELECT name FROM sqlite_master WHERE type = 'table' AND name IN (?, ?) ORDER BY name")
      .all("session_branch_state", "session_branches") as Array<{ name: string }>;
    expect(branchTables.map((r) => r.name)).toEqual([
      "session_branch_state",
      "session_branches",
    ]);

    const messageRunColumns = store.db.pragma("table_info(message_runs)") as Array<{
      name: string;
      type: string;
      notnull: number;
      dflt_value: string | null;
    }>;
    expect(messageRunColumns).toContainEqual(expect.objectContaining({
      name: "branch_name",
      type: "TEXT",
      notnull: 1,
      dflt_value: "'main'",
    }));

    const indexes = store.db
      .prepare("SELECT name FROM sqlite_master WHERE type = 'index' AND name IN (?, ?) ORDER BY name")
      .all("idx_message_runs_session_branch", "idx_session_branches_session") as Array<{ name: string }>;
    expect(indexes.map((i) => i.name)).toEqual([
      "idx_message_runs_session_branch",
      "idx_session_branches_session",
    ]);

    await store.close();
  });
});
