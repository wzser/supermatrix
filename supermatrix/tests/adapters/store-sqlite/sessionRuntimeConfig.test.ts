...
import * as sqliteDbModule from "../../../src/adapters/store-sqlite/db.ts";
...
describe("SqliteBindingStore session runtime config mutations", () => {
  ...
  test("rejects direct backend writes and permits only the atomic runtime-config path", async () => {
    const { store, cleanup } = await createTempStore();
    try {
      await seedSession(store, "s1");

      expect(() => store.db.prepare("UPDATE sessions SET backend = ? WHERE id = ?").run("codex", "s1"))
        .toThrow("sessions.backend changes require applySessionRuntimeConfigMutations");
      expect((await store.findSessionById(asSessionId("s1")))?.backend).toBe("claude");

      await expect(store.applySessionRuntimeConfigMutations([mutation("s1")])).resolves.toEqual({ updated: 1 });
      expect((await store.findSessionById(asSessionId("s1")))?.backend).toBe("codex");
    } finally {
      await cleanup();
    }
  });

  test("does not export a low-level backend write authorization capability", () => {
    expect(sqliteDbModule).not.toHaveProperty("withRuntimeConfigBackendWrite");
  });

  test("prevents the public database handle from replacing the backend write guard", async () => {
    const { store, cleanup } = await createTempStore();
    try {
      await seedSession(store, "s1");

      expect(() => store.db.function("sm_runtime_config_backend_write_allowed", () => 1))
        .toThrow("runtime config backend write guard is reserved");
      expect(() => Reflect.apply(Database.prototype.function, store.db, [
        "sm_runtime_config_backend_write_allowed",
        () => 1,
      ])).toThrow();
      expect(() => store.db.prepare("UPDATE sessions SET backend = ? WHERE id = ?").run("codex", "s1"))
        .toThrow("sessions.backend changes require applySessionRuntimeConfigMutations");
      expect((await store.findSessionById(asSessionId("s1")))?.backend).toBe("claude");
      expect(await store.listSessionRuntimeConfigAudit(asSessionId("s1"))).toHaveLength(0);
    } finally {
      await cleanup();
    }
  });

  test("rejects backend writes from an external SQLite connection", async () => {
    const { store, dir, cleanup } = await createTempStore();
    const external = new Database(join(dir, "console.db"));
    try {
      await seedSession(store, "s1");

      expect(() => external.prepare("UPDATE sessions SET backend = ? WHERE id = ?").run("codex", "s1"))
        .toThrow(/sm_runtime_config_backend_write_allowed|applySessionRuntimeConfigMutations/u);
      expect((await store.findSessionById(asSessionId("s1")))?.backend).toBe("claude");
    } finally {
      external.close();
      await cleanup();
...
describe("fake binding store session runtime config contract", () => {
  ...
  test("new main sessions read the current backend runtime defaults", async () => {
    const store = createFakeBindingStore();
    await store.updateBackendRuntimeDefaults("claude", {
      model: "claude-live-default",
      effort: "high",
    });
    await store.createSession({
      id: asSessionId("runtime-defaults"),
      name: "runtime-defaults",
      scope: "user",
      backend: "claude",
      workdir: asAbsolutePath("/tmp/runtime-defaults"),
      purpose: "test",
      createdAt: CREATED_AT,
    });

    expect(await store.getSessionRuntimeSettings(asSessionId("runtime-defaults"))).toMatchObject({
      mainModelDefault: "claude-live-default",
