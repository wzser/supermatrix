...
describe("SqliteBindingStore sessions", () => {
  ...
  test("updateSessionBackendSessionId persists value", async () => {
    ...
    try {
      ...
      const afterThreadStart = await store.findSessionById(asSessionId("s1"));
      expect(afterThreadStart?.backendSessionId).toBe("bks-1");
      expect(afterThreadStart?.backendSessionUpdatedAt).toEqual(expect.any(Number));

      await store.updateSessionModel(asSessionId("s1"), "claude-opus-5");
      expect((await store.findSessionById(asSessionId("s1")))?.backendSessionUpdatedAt).toBe(
        afterThreadStart?.backendSessionUpdatedAt,
      );
      ...
      expect((await store.findSessionById(asSessionId("s1")))?.backendSessionUpdatedAt).toBeNull();
    } finally {
      await cleanup();
    }
  });
  ...
  test("updateSessionEffortLocked round-trips independently", async () => {
    const { store, cleanup } = await createTempStore();
    try {
      const id = asSessionId("s_effort_lock");
      await store.createSession({ id, ...BASE, name: "effort-lock-test" });
      expect((await store.findSessionById(id))?.effortLocked).toBe(false);
      await store.updateSessionEffortLocked(id, true);
      expect(await store.findSessionById(id)).toMatchObject({ modelLocked: false, effortLocked: true });
      await store.updateSessionEffortLocked(id, false);
      expect((await store.findSessionById(id))?.effortLocked).toBe(false);
    } finally {
      await cleanup();
    }
  ...
  test("backend runtime defaults preserve the untouched field", async () => {
    const { store, cleanup } = await createTempStore();
    try {
      await store.updateBackendRuntimeDefaults("codex", { model: "gpt-5.4" });
      await store.updateBackendRuntimeDefaults("codex", { effort: "high" });
      expect(await store.getBackendRuntimeDefaults("codex")).toMatchObject({
        backend: "codex", model: "gpt-5.4", effort: "high",
      });
      expect(await store.listBackendRuntimeDefaults()).toHaveLength(1);
    } finally {
      await cleanup();
    }
  });

  test("concurrent model and effort default patches do not erase each other", async () => {
    const { store, cleanup } = await createTempStore();
    try {
      await Promise.all([
        store.updateBackendRuntimeDefaults("codex", { model: "gpt-5.4" }),
        store.updateBackendRuntimeDefaults("codex", { effort: "high" }),
      ]);
      expect(await store.getBackendRuntimeDefaults("codex")).toMatchObject({
        model: "gpt-5.4",
        effort: "high",
  ...
  test("child session defaults distinguish unconfigured values from configured null", async () => {
    const { store, cleanup } = await createTempStore();
    try {
      ...
      await store.updateChildSessionDefaults({ model: { configured: true, value: null } });
  ...
  test("concurrent child session default patches do not erase each other", async () => {
    const { store, cleanup } = await createTempStore();
    try {
      await Promise.all([
        store.updateChildSessionDefaults({ backend: { configured: true, value: "codex" } }),
        store.updateChildSessionDefaults({ model: { configured: true, value: "gpt-5.4" } }),
      ]);

      expect(await store.getChildSessionDefaults()).toMatchObject({
        backend: { configured: true, value: "codex" },
        model: { configured: true, value: "gpt-5.4" },
        effort: { configured: false, value: null },
      });
    } finally {
      await cleanup();
  ...
  test("child session defaults CAS only applies a patch to the exact read tuple", async () => {
    const { store, cleanup } = await createTempStore();
    try {
      const initial = await store.getChildSessionDefaults();
      expect(await store.compareAndSetChildSessionDefaults(initial, {
        backend: { configured: true, value: "codex" },
      })).toBe(true);

      const persisted = await store.getChildSessionDefaults();
      expect(await store.compareAndSetChildSessionDefaults(persisted, {
  ...
  test("empty child session defaults patch does not create or update defaults", async () => {
    const { store, cleanup } = await createTempStore();
    const now = vi.spyOn(Date, "now").mockReturnValueOnce(1_700_000_000_000);
    try {
      await store.updateChildSessionDefaults({});
      const emptyRows = store.db
        .prepare("SELECT COUNT(*) AS count FROM child_session_defaults")
        .get() as { count: number };
      expect(emptyRows.count).toBe(0);
      expect((await store.getChildSessionDefaults()).updatedAt).toBeNull();
      expect(now).not.toHaveBeenCalled();

      await store.updateChildSessionDefaults({ effort: { configured: true, value: "high" } });
      const beforeNoop = await store.getChildSessionDefaults();
      await store.updateChildSessionDefaults({});

      expect(await store.getChildSessionDefaults()).toEqual(beforeNoop);
  ...
  test.each([
    ...
    ["kimi", "kimi-code/k3", null],
  ] as const)(
    "creates legacy-fallback runtime settings for a %s main session",
    async (backend, mainModelDefault, mainEffortDefault) => {
      const { store, cleanup } = await createTempStore();
      try {
        const id = asSessionId(`settings-${backend}`);
        await store.createSession({
          id,
          ...BASE,
        ...
        expect(await store.getSessionRuntimeSettings(id)).toMatchObject({
          sessionId: id,
          mainModelDefault,
          mainEffortDefault,
  ...
  test("new main session settings read live backend defaults and preserve local NULL dimensions", async () => {
    const { store, cleanup } = await createTempStore();
    try {
      ...
      await store.createSession({
        id: asSessionId("settings-live-first"),
        ...BASE,
        name: "settings-live-first",
      });
      ...
      await store.createSession({
        id: asSessionId("settings-live-second"),
        ...BASE,
        name: "settings-live-second",
      });

      expect(await store.getSessionRuntimeSettings(asSessionId("settings-live-first"))).toMatchObject({
        mainModelDefault: "claude-opus-5",
        mainEffortDefault: "high",
      });
      expect(await store.getSessionRuntimeSettings(asSessionId("settings-live-second"))).toMatchObject({
        mainModelDefault: "claude-opus-5-1",
  ...
  test("new main session settings fall back to the legacy map when defaults table is missing", async () => {
    const { store, cleanup } = await createTempStore();
    try {
      store.db.exec("DROP TABLE backend_runtime_defaults");
      await store.createSession({
        id: asSessionId("settings-no-defaults-table"),
        ...BASE,
        name: "settings-no-defaults-table",
      });

      expect(await store.getSessionRuntimeSettings(asSessionId("settings-no-defaults-table"))).toMatchObject({
  ...
  test("does not create runtime settings for a child session", async () => {
    const { store, cleanup } = await createTempStore();
    try {
      const mainId = asSessionId("settings-main");
      await store.createSession({ id: mainId, ...BASE, name: "settings-main" });
      await store.createSession({
        id: asSessionId("settings-child"),
        ...BASE,
        name: "settings-child",
        scope: "child",
        parentId: mainId,
      ...
      expect(await store.getSessionRuntimeSettings(asSessionId("settings-child"))).toBeNull();
    } finally {
      await cleanup();
    }
  });

  test("session runtime settings patches preserve untouched fields", async () => {
    const { store, cleanup } = await createTempStore();
    try {
      const id = asSessionId("settings-patch");
      await store.createSession({ id, ...BASE, name: "settings-patch" });
      await store.updateSessionRuntimeSettings(id, {
        mainModelDefault: "claude-sonnet-4-6",
  ...
  test("session runtime settings reject an inconsistent follow/locked pair", async () => {
    const { store, cleanup } = await createTempStore();
    try {
      const id = asSessionId("settings-invalid-lock");
      await store.createSession({ id, ...BASE, name: "settings-invalid-lock" });

      expect(() => store.db.prepare(
        "UPDATE session_runtime_settings SET child_model_configured = 1, child_model = NULL WHERE session_id = ?",
      ).run(id)).toThrow(/CHECK constraint failed/u);
      expect(() => store.db.prepare(
        "UPDATE session_runtime_settings SET child_model_configured = 0, child_model = 'gpt-5.5' WHERE session_id = ?",
      ).run(id)).toThrow(/CHECK constraint failed/u);
    } finally {
      await cleanup();
    }
  ...
  test("cleanupStaleChildSessions marks idle children as deleted", async () => {
    ...
    try {
      await store.createSession({
        id: asSessionId("caller_stale"),
        name: "caller-stale",
        scope: "user",
        backend: "claude",
        workdir: asAbsolutePath("/ws/caller-stale"),
        purpose: "",
        createdAt: asTimestamp(1000),
      });
      await store.createSession({
      ...
      await store.logCrossSessionComm({
        id: "comm_stale_orphan",
        fromSessionId: asSessionId("caller_stale"),
        toSessionId: asSessionId("p2"),
        kind: "spawn",
      ...
      expect(cleaned.failedComms).toEqual([
        {
          commId: "comm_stale_orphan",
          callerSessionId: asSessionId("caller_stale"),
          callerSessionName: "caller-stale",
          targetSessionName: "parent2",
          childSessionId: asSessionId("stale1"),
          childSessionName: "child_p2_stale",
        },
      ]);
      ...
      const comm = store.db
        .prepare("SELECT status, error_message, finished_at FROM cross_session_log WHERE id = ?")
        .get("comm_stale_orphan") as { status: string; error_message: string | null; finished_at: number | null };
      expect(comm).toMatchObject({
        status: "failed",
        error_message: "orphaned by console restart",
        finished_at: 5_000_000,
      });
  ...
  test("cleanupErroredChildSessions marks only stale error children as deleted", async () => {
    ...
    try {
      await store.createSession({
        id: asSessionId("caller_err"),
        name: "caller-err",
        scope: "user",
        backend: "claude",
        workdir: asAbsolutePath("/ws/caller-err"),
        purpose: "",
        createdAt: asTimestamp(1000),
      });
      await store.createSession({
      ...
      await store.logCrossSessionComm({
        id: "comm_error_orphan",
        fromSessionId: asSessionId("caller_err"),
        toSessionId: asSessionId("p_err"),
        kind: "spawn",
        prompt: "please do work",
        createdAt: asTimestamp(40_000),
      });
      store.db
        .prepare("UPDATE cross_session_log SET child_session_id = ? WHERE id = ?")
        .run("old_error", "comm_error_orphan");
      ...
      expect(cleaned.failedComms).toEqual([
        {
          commId: "comm_error_orphan",
          callerSessionId: asSessionId("caller_err"),
          callerSessionName: "caller-err",
          targetSessionName: "parent_err",
          childSessionId: asSessionId("old_error"),
          childSessionName: "child_p_err_old_error",
        },
      ]);
      ...
      const comm = store.db
        .prepare("SELECT status, error_message, finished_at FROM cross_session_log WHERE id = ?")
        .get("comm_error_orphan") as { status: string; error_message: string | null; finished_at: number | null };
      expect(comm).toMatchObject({
        status: "failed",
        error_message: "orphaned by console restart",
        finished_at: 5_000_000,
...
// Source: SuperMatrixRuntime/workspaces/first-principle/rules/session-meta-fields.md
describe("SqliteBindingStore session-meta v1.0 validation", () => {
  ...
  test("createSessionWithBinding accepts each category enum value (and empty)", async () => {
    const { store, cleanup } = await createTempStore();
    try {
      const values = ["", "业务", "平台", "工具", "知识", "外部", "员工"] as const;
