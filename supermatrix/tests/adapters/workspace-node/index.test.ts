...
describe("NodeWorkspaceFs", () => {
  ...
  test("entryExists sees a dangling symlink without changing exists semantics", async () => {
    const link = asAbsolutePath(join(dir, "dangling"));
    await symlink(join(dir, "missing-target"), link);

    expect(await fs.exists(link)).toBe(false);
    expect(await fs.entryExists(link)).toBe(true);
    expect(await fs.entryExists(asAbsolutePath(join(dir, "missing")))).toBe(false);
  });
