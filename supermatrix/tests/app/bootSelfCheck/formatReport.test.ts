...
describe("formatReport", () => {
  ...
  it("renders runtime config summary counts and omits zero problem groups", () => {
    const s = renderLarkSelfCheckMessage([{
      name: "codex-runtime-config",
      status: "warn",
      message: "unchanged=3 clamped=1 fallbackModel=0 conflict=0 failed=1; clamped: alpha; failed: beta",
    }]);
    expect(s).toContain("unchanged=3 clamped=1 fallbackModel=0 conflict=0 failed=1");
    expect(s).toContain("clamped: alpha");
    expect(s).toContain("failed: beta");
    expect(s).not.toContain("fallbackModel:");
    expect(s).not.toContain("conflict:");
  });
});
