...
describe("bootstrap Drive comment subscription reconciliation", () => {
  ...
  it("starts the primary Lark ingress before invoking the real reconciler", () => {
    ...
    expect(startSource).toContain("if (cfg.driveCommentSubscriptionEnabled)");
    expect(startSource).toContain("drive comment subscription reconcile disabled by configuration");
...
describe("validateEnv", () => {
  it("returns parsed config on valid env", () => {
    ...
    expect(cfg.driveCommentSubscriptionEnabled).toBe(false);
  ...
  it("enables drive comment reconciliation only with the explicit switch", () => {
    const cfg = validateEnv({
      SM_ROOT_GROUP_ID: "g_root",
      SM_ROOT_USER_ID: "u_owner",
      SM_WORKSPACE_ROOT: "/tmp/sm-work",
      ...
      LARK_APP_ID: "cli_test",
      LARK_APP_SECRET: "secret",
      SM_DRIVE_COMMENT_SUBSCRIPTION_ENABLED: "1",
    });
    expect(cfg.driveCommentSubscriptionEnabled).toBe(true);
  });
  ...
  it("resolves SM_NEW_WORKSPACE_ROOT into provisioningRoot", () => {
    const base = {
      SM_ROOT_GROUP_ID: "g",
      SM_ROOT_USER_ID: "u",
      SM_WORKSPACE_ROOT: "/w",
    ...
    expect(validateEnv({ ...base, SM_NEW_WORKSPACE_ROOT: "/Volumes/EXAMPLE_VOLUME/workspaces" }).provisioningRoot)
      .toBe("/Volumes/EXAMPLE_VOLUME/workspaces");
    // Unset / empty / same-as-legacy-root all keep today's single-root behaviour.
    expect(validateEnv(base).provisioningRoot).toBeUndefined();
    expect(validateEnv({ ...base, SM_NEW_WORKSPACE_ROOT: "" }).provisioningRoot).toBeUndefined();
    expect(validateEnv({ ...base, SM_NEW_WORKSPACE_ROOT: "  " }).provisioningRoot).toBeUndefined();
    expect(validateEnv({ ...base, SM_NEW_WORKSPACE_ROOT: " /w " }).provisioningRoot).toBeUndefined();
  });

  it.each([
    ["new root trailing slash", "/w", "/w/"],
    ["workspace root trailing slash", "/w/", "/w"],
    ["new root dot segment", "/w", "/w/./"],
    ["workspace root dot segment", "/w/./", "/w"],
  ])("does not enable provisioning for the same lexical root: %s", (_case, workspaceRoot, newRoot) => {
    const dbPath = "/runtime/data/./supermatrix.db";
    const cfg = validateEnv({
      SM_ROOT_GROUP_ID: "g",
      SM_ROOT_USER_ID: "u",
      SM_WORKSPACE_ROOT: workspaceRoot,
      SM_NEW_WORKSPACE_ROOT: newRoot,
      SM_DB_PATH: dbPath,
      SM_BACKEND: "claude",
      SM_LOG_LEVEL: "info",
      LARK_APP_ID: "cli_test",
    ...
    expect(cfg.provisioningRoot).toBeUndefined();
    expect(cfg.workspaceRoot).toBe(workspaceRoot);
    expect(cfg.dbPath).toBe(dbPath);
  });

  it("provisioning root guard rejects a root that shares a device with the workspace root", async () => {
    const workspaceRoot = "/Users/LOCAL_USER/SuperMatrixRuntime/workspaces";
    const guard = createProvisioningRootGuard(
      { provisioningRoot: "/Volumes/EXAMPLE_VOLUME/workspaces", workspaceRoot },
      {
        // Unmounted volume leaving a writable stub behind: access() succeeds,
        // but the stub lives on the boot disk — same device number.
        access: async () => {},
        stat: async () => ({ dev: 16777229 }),
      },
    );

    await expect(guard()).rejects.toBeInstanceOf(UserError);
    await expect(guard()).rejects.toThrow(/未挂载/);
    await expect(guard()).rejects.toThrow(/\/Volumes\/EXAMPLE_VOLUME\/workspaces/);
    await expect(guard()).rejects.toThrow(new RegExp(workspaceRoot.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")));
  });

  it("provisioning root guard rejects a root that is missing or not writable", async () => {
    const guard = createProvisioningRootGuard(
      { provisioningRoot: "/Volumes/EXAMPLE_VOLUME/workspaces", workspaceRoot: "/w" },
      {
        access: async () => {
          throw new Error("ENOENT: no such file or directory");
        },
        stat: async () => {
          throw new Error("stat must not run once access failed");
        },
      },
    );
  ...
  it("provisioning root guard passes when the root is on a separate device", async () => {
    const statCalls: string[] = [];
    const accessCalls: string[] = [];
    const guard = createProvisioningRootGuard(
      { provisioningRoot: "/Volumes/EXAMPLE_VOLUME/workspaces", workspaceRoot: "/Users/LOCAL_USER/SuperMatrixRuntime/workspaces" },
      {
        access: async (target) => {
        ...
        stat: async (target) => {
          ...
          return { dev: target.startsWith("/Volumes/") ? 16777244 : 16777229 };
        },
      },
    );

    await expect(guard()).resolves.toBeUndefined();
    expect(accessCalls).toEqual(["/Volumes/EXAMPLE_VOLUME/workspaces"]);
    expect(statCalls).toEqual([
      "/Volumes/EXAMPLE_VOLUME/workspaces",
