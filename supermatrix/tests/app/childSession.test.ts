...
import type { OpaqueHandoffExecutionInput, OpaqueHandoffExecutionResult } from "../../src/app/opaqueHandoff.ts";
...
function mkService(opts: {
  ...
  deliverSinks?: NonNullable<Parameters<typeof createChildSessionService>[0]["deliverSinks"]>;
  ...
  opaqueHandoffExecutor?: (input: OpaqueHandoffExecutionInput) => Promise<OpaqueHandoffExecutionResult>;
} = {}) {
  ...
  const service = createChildSessionService({
    ...
    ...(opts.opaqueHandoffExecutor ? { opaqueHandoffExecutor: opts.opaqueHandoffExecutor } : {}),
  });
...
describe("childSession", () => {
  describe("spawnChild", () => {
    ...
    test("passes framework-observed tool results to declared result sinks", async () => {
      const deliverSinks = vi.fn(async () => ({ delivered: [] }));
      const { store, service } = mkService({
        script: [
          { kind: "started", backendSessionId: "bks-tool-evidence" },
          {
            kind: "tool_result",
            name: "python3 scripts/todo_append.py",
            command: "python3 scripts/todo_append.py --payload-file payload.json",
            result: { output: '{"ok":true}', exitCode: 0 },
          },
          { kind: "assistant_message", text: "child prose", final: true },
          { kind: "completed", finalMessage: "child prose" },
        ],
        deliverSinks,
      });
      const parent = seedParent(store);
      const requester = seedRequester(store, "sess_requester_tool_evidence");
      ...
      await service.spawnChild({
        ...
        requestedBy: requester.id,
        type: "one_shot_delegation",
        resultSinks: [{ kind: "chat_post", chatRef: { kind: "explicit", chatId: "oc_target" }, identity: "bot" }],
      });

      expect(deliverSinks).toHaveBeenCalledWith(
  ...
  describe("opaque handoff", () => {
    test("bypasses the agent and gives the fixed consumer exact bytes plus digest", async () => {
      const bytes = Buffer.from('{"handoff":{"delivery_mode":"spawn2_todo_pool"}}', "utf8");
      const encodedBytes = bytes.toString("base64");
      const calls: OpaqueHandoffExecutionInput[] = [];
      const setup = mkService({
        backend: {
          kind: "claude",
          async *run() {
            throw new Error("agent must not receive an opaque handoff");
          },
          async cancel() {},
        },
        opaqueHandoffExecutor: async (input) => {
          calls.push(input);
      ...
      const result = await setup.service.spawnChild({
        parentId: parent.id,
        requestedBy: parent.id,
        backend: "claude",
        callerInvocation: "async_kickoff",
        workdir: asAbsolutePath("/ws/parent"),
        prompt: "opaque handoff",
        type: "one_shot_delegation",
        resultSinks: [{ kind: "pollable_endpoint" }],
        opaqueHandoff: {
          encodedBytes,
          bytesSha256: createHash("sha256").update(bytes).digest("hex"),
          executable: "bin/fixed-consumer",
          args: ["--input", "-", "--handoff-comm-id", "'<framework comm_id>'"],
        },
      });

      expect(result.finalMessage).toBe('{"ok":true}');
      expect(calls).toHaveLength(1);
      expect(calls[0]).toMatchObject({
        cwd: asAbsolutePath("/ws/parent"),
        commId: expect.stringMatching(/^comm_/u),
        handoff: {
          encodedBytes,
          bytesSha256: createHash("sha256").update(bytes).digest("hex"),
          executable: "bin/fixed-consumer",
