...
describe("commandRegistry", () => {
  ...
  test("reload and cancel help forbid copying backend SIGKILL into platform lifecycle operations", () => {
    const r = buildCommandRegistry();
    expect(r.reload.command.notes).toContain("禁止直接 kill -9");
    expect(r.reload.command.notes).toContain("spawn2.0 target=codexroot");
    expect(r.cancel.command.notes).toContain("仅限当前 session 已绑定的 backend 进程组");
    expect(r.cancel.command.notes).toContain("不得对 SuperMatrix、localwatch 或其他 session 使用 kill -9");
  });
  ...
  test("/help model and /help effort expose GPT-6-Astra and its bundled efforts", () => {
    setCodexModelCatalogEntries(
      [{
        slug: "gpt-6-astra",
    ...
    try {
      ...
      expect(registry.model.command.notes).toContain("gpt-6-astra");
      expect(registry.effort.command.notes).toContain(
  ...
  test("command notes advertise the accepted input aliases", () => {
    ...
    expect(r.model.command.notes).toContain("astra / gpt6-astra / 6-astra");
    expect(r.model.command.notes).toContain("裸 gpt6 / 6 不映射");
