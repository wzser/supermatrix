...
describe("commandRouter", () => {
  ...
  test("rejects removed shipment writeback command as unknown", async () => {
    const reg = bindAll(async () => ({ replyText: "" }));
    const router = createCommandRouter(reg);
    const result = await router.route({
      scope: "user",
  ...
  test("runtime config contention becomes an actionable retry reply", async () => {
    const reg = bindAll(async () => {
      throw new RuntimeConfigConflictError(asSessionId("sess_busy"));
    });
    const router = createCommandRouter(reg);
    const result = await router.route({
      scope: "root",
    ...
    if (!("replyText" in result)) throw new Error("expected replyText");
    expect(result.replyText).toMatch(/正忙|并发修改/u);
    expect(result.replyText).toContain("稍后重试");
    expect(result.replyText).not.toContain("未知错误");
  });
});
