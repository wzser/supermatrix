...
describe("/btw handler", () => {
  test("first call spawns child with keepAlive=true and replies final message", async () => {
    const store = setup();
    const spawnChild = vi.fn(async (_input: SpawnChildInput) => childResult("sess_child_1", "hello from btw"));
  ...
  test("first claude btw child leaves execution selection to the system-wide resolver", async () => {
    const store = setup();
    store.seedSession({ ...PARENT_SESSION, model: "claude-opus-5" });
    const spawnChild = vi.fn(async (_input: SpawnChildInput) => childResult("sess_child_1", "hello from btw"));
    ...
    expect(spawnChild.mock.calls[0]?.[0]).not.toHaveProperty("model");
    expect(spawnChild.mock.calls[0]?.[0]).not.toHaveProperty("executionOverride");
    btw.shutdown();
  });

  test("first codex btw child leaves execution selection to the system-wide resolver", async () => {
    const store = setup();
    store.seedSession({ ...PARENT_SESSION, backend: "codex", model: "gpt-5.6-luna" });
    const spawnChild = vi.fn(async (_input: SpawnChildInput) => childResult("sess_child_1", "hello from btw"));
    ...
    expect(spawnChild).toHaveBeenCalledWith(expect.objectContaining({ backend: "codex" }));
    expect(spawnChild.mock.calls[0]?.[0]).not.toHaveProperty("model");
    expect(spawnChild.mock.calls[0]?.[0]).not.toHaveProperty("executionOverride");
    btw.shutdown();
  });

  test("codex btw does not impose a command-local model policy", async () => {
    const store = setup();
    store.seedSession({ ...PARENT_SESSION, backend: "codex", model: "gpt-5.4" });
    const spawnChild = vi.fn(async (_input: SpawnChildInput) => childResult("sess_child_1", "resolved centrally"));
    const lark = makeLark();
    ...
    expect(lark.sendMessage).toHaveBeenCalledOnce();
    expect(spawnChild).toHaveBeenCalledOnce();
    expect(spawnChild.mock.calls[0]?.[0]).not.toHaveProperty("model");
    expect(result).toEqual({ replyText: "resolved centrally" });
    btw.shutdown();
  });
