...
describe("/heartbeat", () => {
  ...
  test("metadata keeps canonical state signature and declares all state aliases", () => {
    const registry = buildCommandRegistry();
    const stateParam = registry["heartbeat"].command.params.find((param) => param.name === "state");

    expect(stateParam).toMatchObject({
      kind: "positional",
      type: "enum",
  ...
  test("root scope resolves state aliases via the parser", async () => {
    const store = createFakeBindingStore();
    seed(store, "s1", "scheduler");
    const router = bindHeartbeatRouter(store);

    await expect(
      replyText(await router.route({ scope: "root", msg: makeMsg("oc_root", "/heartbeat scheduler 开启") })),
    ).resolves.toBe("✓ heartbeat「scheduler」已开启");
    await expect(store.getSessionHeartbeatEnabled(asSessionId("s1"))).resolves.toBe(true);

    await expect(
      replyText(await router.route({ scope: "root", msg: makeMsg("oc_root", "/heartbeat scheduler 状态") })),
    ).resolves.toBe("heartbeat「scheduler」当前：on");

    await expect(
      replyText(await router.route({ scope: "root", msg: makeMsg("oc_root", "/heartbeat scheduler disable") })),
    ).resolves.toBe("✓ heartbeat「scheduler」已关闭");
    await expect(store.getSessionHeartbeatEnabled(asSessionId("s1"))).resolves.toBe(false);
  });

  test("unknown heartbeat state fails closed before writes", async () => {
    const store = createFakeBindingStore();
    seed(store, "s1", "scheduler");
    const router = bindHeartbeatRouter(store);

    await expect(
      replyText(await router.route({ scope: "root", msg: makeMsg("oc_root", "/heartbeat scheduler unknown") })),
    ).resolves.toContain("必须是以下之一：on|off|status|stop|resume");
    await expect(store.getSessionHeartbeatEnabled(asSessionId("s1"))).resolves.toBe(false);
  });
