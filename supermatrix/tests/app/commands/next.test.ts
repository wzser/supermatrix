...
describe("/next command", () => {
  ...
  test("acknowledges idle /next as immediate delivery", async () => {
    const store = createFakeBindingStore();
    ...
    const result = await handler({
      args: { text: "run immediately" },
      scope: "user",
      msg: msg("/next run immediately"),
    });

    expect(result).toEqual({ replyText: "✓ 当前 session 空闲，已直接投递" });
    expect(queued.map((entry) => entry.text)).toEqual(["run immediately"]);
  });

  test("does not enqueue when /next does not come from trusted Lark user inbound", async () => {
    const store = createFakeBindingStore();
    store.seedSession(makeSession("busy"));
    const queued: Array<{ text: string; groupId: typeof USER_GROUP; userId: string }> = [];
    const handler = createNextHandler({
    ...
    const trustedMsg = msg("/next missing");
    const { origin: _origin, ...messageWithoutOrigin } = trustedMsg;
    const missingOrigin = await handler({
      args: { text: "missing origin prompt" },
      scope: "user",
      msg: messageWithoutOrigin,
    });
    const syntheticOrigin = await handler({
      args: { text: "synthetic prompt" },
      scope: "user",
      msg: { ...msg("/next synthetic"), origin: "framework_synthetic" as const },
    });

    expect(missingOrigin).toEqual({ handled: true });
    expect(syntheticOrigin).toEqual({ handled: true });
    expect(queued).toEqual([]);
  });

  test("accepts lark user synthetic /next after the dispatcher narrow gate", async () => {
    const store = createFakeBindingStore();
    store.seedSession(makeSession("busy"));
    const queued: Array<{ text: string; groupId: typeof USER_GROUP; userId: string }> = [];
    const handler = createNextHandler({
    ...
    const result = await handler({
      args: { text: "synthetic prompt" },
      scope: "user",
      msg: { ...msg("/next synthetic"), origin: "lark_user_synthetic" as const },
    });

    expect(result).toEqual({ replyText: "✓ 已排队，将在当前任务完成后执行" });
    expect(queued.map((entry) => entry.text)).toEqual(["synthetic prompt"]);
  });
});
