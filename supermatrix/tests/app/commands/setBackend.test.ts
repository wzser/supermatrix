...
import type { MessageOrigin } from "../../../src/domain/messageOrigin.ts";
...
function msg(groupId: string, text: string, origin: MessageOrigin = "lark_user") {
  return {
...
describe("createSetBackendHandler", () => {
  ...
  test.each(["framework_synthetic", "lark_user_synthetic"] as const)(
    "rejects %s backend mutations before any runtime write",
    async (origin) => {
      const { store } = fakeStore({ name: "foo", row: baseRow });
      ...
      await expect(handler({
        ...
        scope: "root",
        msg: msg("oc_root", "/backend foo codex", origin),
      })).rejects.toThrow("只能由用户显式 /backend 命令触发");
      expect(applySessionRuntimeConfigMutations).not.toHaveBeenCalled();
    },
  );

  test("rejects backend mutations when ingress provenance is missing", async () => {
    const { store } = fakeStore({ name: "foo", row: baseRow });
    const applySessionRuntimeConfigMutations = vi.fn(store.applySessionRuntimeConfigMutations);
    const handler = createSetBackendHandler({
      store: { ...store, applySessionRuntimeConfigMutations },
    });
    const { origin: _origin, ...withoutOrigin } = msg("oc_root", "/backend foo codex");
  ...
  test("successful backend switch requests a session-table sync after the confirmation", async () => {
    const { store } = fakeStore({ name: "foo", row: baseRow });
    ...
    if (!("replyText" in result) || typeof result.afterReply !== "function") {
      throw new Error("expected backend confirmation with sync follow-up");
    }
  ...
  test("returns the backend confirmation before session-table sync completes", async () => {
    const { store, sessions } = fakeStore({ name: "foo", row: baseRow });
    let releaseSync!: (result: { ok: true }) => void;
    const syncSessionTable = vi.fn(() => new Promise<{ ok: true }>((resolve) => {
      releaseSync = resolve;
    }));
    const handler = createSetBackendHandler({ store, syncSessionTable });
    ...
    const earlyResult = await Promise.race([
      pendingResult,
      new Promise<"sync-blocked">((resolve) => setTimeout(() => resolve("sync-blocked"), 20)),
    ]);

    expect(sessions.get("foo")?.backend).toBe("codex");
    if (earlyResult === "sync-blocked") {
      releaseSync({ ok: true });
      await pendingResult;
      expect.fail("backend confirmation waited for session-table sync");
    }
    if (!("replyText" in earlyResult)) throw new Error("expected immediate backend confirmation");
    expect(earlyResult.replyText).toContain("已从 claude 切换为 codex");
    expect("afterReply" in earlyResult).toBe(true);

    if (!("afterReply" in earlyResult) || typeof earlyResult.afterReply !== "function") {
      throw new Error("expected session-table sync follow-up");
    }
    const followUp = earlyResult.afterReply();
    releaseSync({ ok: true });
    await expect(followUp).resolves.toBeNull();
  });

  test("reports a failed session-table sync as a follow-up after the backend confirmation", async () => {
    const { store, sessions } = fakeStore({ name: "foo", row: baseRow });
    const syncFailure = new Error("session table push down");
    const syncSessionTable = vi.fn(async () => ({ ok: false as const, error: syncFailure }));
    const handler = createSetBackendHandler({ store, syncSessionTable });
    ...
    if (!("replyText" in result)) throw new Error("expected replyText");
    expect(result.replyText).toContain("已从 claude 切换为 codex");
    expect(result.replyText).not.toContain("同步失败");
    if (typeof result.afterReply !== "function") throw new Error("expected sync follow-up");
    await expect(result.afterReply()).resolves.toBe(
      "⚠ session「foo」已切换为 codex，但飞书每日默认值同步失败：session table push down",
    );
    expect(sessions.get("foo")?.backend).toBe("codex");
  });
  ...
  test("post-commit binding lookup failure returns success with a warning and does not claim rollback", async () => {
    ...
    expect(result.replyText).toContain("后续同步或核查未完成");
