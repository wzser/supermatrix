...
describe("/effort — kimi model-aware thinking levels (kimi-code 0.30.0)", () => {
  ...
  test("session persisted with the now-verified k3-256k model accepts K3 effort mapping", async () => {
    const store = createFakeBindingStore();
    seedKimi(store, "k1", "kimi-k3xl", { model: "kimi-code/k3-256k" });
    const handler = createSetEffortHandler({ store });

    const result = await handler({ scope: "root", args: { name: "kimi-k3xl", level: "max" }, msg: makeMsg("oc_root", "") });
    expect("replyText" in result).toBe(true);
    expect((await store.findSessionByName("kimi-k3xl"))?.effort).toBe("max");
  });
