...
import {
  ...
  getCodexModelAliasCatalogDrift,
...
describe("resolveModelAlias — Codex short aliases", () => {
  ...
  test("Astra aliases resolve and persist through the bundled catalog", async () => {
    setCodexModelCatalogEntries([
      {
        slug: "gpt-6-astra",
    ...
    try {
      ...
      for (const [index, alias] of (["astra", "gpt6-astra", "6-astra"] as const).entries()) {
        const name = `astra-${index}`;
        seed(store, `s${index}`, name, "codex");
        expect(resolveAndValidateModel(alias, "codex")).toBe("gpt-6-astra");
        const result = await handler({
          args: { name, model: alias },
          scope: "root",
          msg: makeMsg("oc_root", `/model ${name} ${alias}`),
        });

        if (!("replyText" in result)) throw new Error("expected replyText");
        expect((await store.findSessionByName(name))?.model).toBe("gpt-6-astra");
        expect(result.replyText).toContain("gpt-6-astra");
      }
    } finally {
      resetCodexModelCatalogForTests();
    }
  ...
  test("bare gpt6 and 6 stay unresolved when Astra is the only GPT-6 catalog entry", () => {
    setCodexModelCatalogEntries([{ slug: "gpt-6-astra" }], "bundled");
    try {
      expect(() => resolveAndValidateModel("gpt6", "codex")).toThrow(UserError);
      expect(() => resolveAndValidateModel("6", "codex")).toThrow(UserError);
    } finally {
      resetCodexModelCatalogForTests();
    }
  });
...
describe("Codex static aliases and the dynamic bundled catalog", () => {
  test("all alias targets, including Astra, are present in the bundled catalog", () => {
    setCodexModelCatalogEntries(
      [...getCodexBundledModels(), "gpt-6-astra"].map((slug) => ({ slug })),
      "bundled",
    );
    try {
      expect(getCodexModelAliasCatalogDrift()).toEqual([]);
      expect(() => assertCodexModelAliasesInCatalog()).not.toThrow();
    } finally {
      resetCodexModelCatalogForTests();
    }
  });
...
describe("resolveModelAlias — cross-backend rejection", () => {
  ...
  for (const token of ["astra", "gpt6-astra", "6-astra", "gpt-6-astra"]) {
    test(`Astra token ${token} is rejected on Claude and Kimi sessions`, () => {
