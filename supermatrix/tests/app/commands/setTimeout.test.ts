...
describe("/timeout command handler", () => {
  ...
  test("默认 resets inactivity timeout", async () => {
    const store = createFakeBindingStore();
    seedFoo(store);
    const handler = createSetTimeoutHandler({
      store,
    ...
    const r = await handler({
      args: { timeout: "默认" },
      scope: "user",
      msg: makeMsg("oc_foo", "/timeout 默认"),
    });
    if (!("replyText" in r)) throw new Error("expected replyText");
    expect(r.replyText).toContain("默认 (900s)");
    expect((await store.findSessionById(asSessionId("s1")))?.inactivityTimeoutS).toBeNull();
  });
  ...
  test("关闭 clears max runtime via --maxrun", async () => {
    const store = createFakeBindingStore();
    seedFoo(store);
    const handler = createSetTimeoutHandler({
      store,
    ...
    const r = await handler({
      args: { maxrun: "关闭" },
      scope: "user",
      msg: makeMsg("oc_foo", "/timeout --maxrun 关闭"),
    });
    if (!("replyText" in r)) throw new Error("expected replyText");
    expect(r.replyText).toContain("无限制");
    expect((await store.findSessionById(asSessionId("s1")))?.maxRuntimeS).toBeNull();
  });
