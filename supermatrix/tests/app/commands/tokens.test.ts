...
describe("tokens handler", () => {
  ...
  test("normalizes provider token bases across backends", async () => {
    // codex reports input_tokens INCLUDING cached_input_tokens and the stream
    // parser splits reasoning out of output_tokens; claude reports
    // input_tokens EXCLUDING both cache halves and folds thinking into
    // output_tokens. Rendering the raw columns double-counted codex's cached
    // input in the `all` row and understated its `out` row by the reasoning
    // share, which made per-session rows and the 合计 row incomparable.
    const fake = createFakeBindingStore();
    ...
    const store = withUsage(fake, {
      s1: {
        today: {
          ...empty(),
          inputTokens: 10_000_000, // already includes the 8M cached below
      ...
      s2: {
        today: {
          ...empty(),
          inputTokens: 1_000_000, // excludes both cache halves
          cacheReadTokens: 8_000_000,
    ...
    // codex: in = 10M total (NOT 10M+8M), out = 1M+2M reasoning, all = 13M
    expect(body).toMatch(/codexroot[\s\S]*?\n\s+in\s+10\.00\s/);
    expect(body).toMatch(/codexroot[\s\S]*?\n\s+out\s+3\.00\s/);
    expect(body).toMatch(/codexroot[\s\S]*?\n\s+all\s+13\.00\s/);
    // claude: in = 1M+8M+1M = 10M total, out = 0.5M, all = 10.5M
    expect(body).toMatch(/watchdog[\s\S]*?\n\s+in\s+10\.00\s/);
    expect(body).toMatch(/watchdog[\s\S]*?\n\s+out\s+0\.50\s/);
    expect(body).toMatch(/watchdog[\s\S]*?\n\s+all\s+10\.50\s/);
    // 合计 sums the normalized values, not the raw columns
    expect(body).toMatch(/合计[\s\S]*?\n\s+in\s+20\.00\s/);
    expect(body).toMatch(/合计[\s\S]*?\n\s+out\s+3\.50\s/);
    expect(body).toMatch(/合计[\s\S]*?\n\s+all\s+23\.50\s/);
    // miss is unchanged by normalization: both reduce to totalInput-cacheRead
    expect(body).toMatch(/codexroot[\s\S]*?\n\s+miss\s+2\.00\s/);
    expect(body).toMatch(/watchdog[\s\S]*?\n\s+miss\s+2\.00\s/);
  });
