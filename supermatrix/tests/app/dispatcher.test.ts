...
describe("dispatcher", () => {
  it("sends a command confirmation before awaiting its follow-up warning", async () => {
    ...
    const afterReply = vi.fn(async () => {
      await followUpGate;
      return "⚠ post-reply sync failed";
    });
    const dispatcher = createDispatcher({
      ...
      router: {
        async route() {
          return { replyText: "✓ backend switched", afterReply };
        },
      },
    ...
    const handling = dispatcher.handleInbound(makeMsg(ROOT_GROUP, "/backend foo kimi"));
    await vi.waitFor(() => {
      expect(lark.sent.map((message) => message.text)).toEqual(["✓ backend switched"]);
    ...
    expect(lark.sent.map((message) => message.text)).toEqual([
      "✓ backend switched",
      "⚠ post-reply sync failed",
    ]);
  });

  it("does not send a second command message when the follow-up succeeds", async () => {
    const lark = createFakeLarkGateway();
    const dispatcher = createDispatcher({
      ...
      router: {
        async route() {
          return {
            replyText: "✓ backend switched",
            afterReply: async () => null,
          };
        },
      },
      backend: makeFakeBackendRegistry(),
    ...
    await dispatcher.handleInbound(makeMsg(ROOT_GROUP, "/backend foo kimi"));
  ...
  it("attributes lifecycle in-flight accounting to the exact message run", async () => {
    const store = createFakeBindingStore();
    const sessionId = asSessionId("sess_lifecycle_identity");
    store.seedSession(makeSession("sess_lifecycle_identity", "idle"));
    store.seedBinding({ groupId: USER_GROUP, sessionId, createdAt: asTimestamp(100) });
    ...
    const dispatcher = createDispatcher({
      ...
      idFactory: makeIdFactory(),
      lifecycle: {
        runStarted: (id) => { started.push(id); },
        runFinished: (id) => { finished.push(id); },
      },
    });

    await dispatcher.handleInbound(makeMsg(USER_GROUP, "a prompt"));

    expect(started).toEqual(["id_1"]);
    expect(finished).toEqual(["id_1"]);
  });
  ...
  it("executes lark user synthetic prompts but keeps them out of human card-ask routing", async () => {
    const store = createFakeBindingStore();
    const lark = createFakeLarkGateway();
    const replier = createFakeReplier();

    const sessionId = asSessionId("sess_lark_synthetic");
    store.seedSession(makeSession("sess_lark_synthetic", "idle"));
    store.seedBinding({ groupId: USER_GROUP, sessionId, createdAt: asTimestamp(100) });

    const { registry, captured } = makeCapturingBackendRegistry([
      { kind: "started", backendSessionId: "back_lark_synthetic" },
      { kind: "completed", finalMessage: "done" },
    ]);
    const gate = vi.fn(async () => ({ enabled: true }));
    ...
    await dispatcher.handleInbound(makeMsg(USER_GROUP, "Δ失败原因\n请按命令重跑", {
    ...
    expect(gate).not.toHaveBeenCalled();
    expect(captured).toHaveLength(1);
    expect(captured[0].prompt).toContain("Δ失败原因");
    expect(captured[0].cardAskEnabled).toBeUndefined();
  });

  it("keeps legacy omitted-origin prompts out of human card-ask routing", async () => {
    const store = createFakeBindingStore();
    const sessionId = asSessionId("sess_legacy_no_card_ask");
    store.seedSession(makeSession("sess_legacy_no_card_ask", "idle"));
    store.seedBinding({ groupId: USER_GROUP, sessionId, createdAt: asTimestamp(100) });
    const { registry, captured } = makeCapturingBackendRegistry([
      { kind: "completed", finalMessage: "done" },
    ]);
    ...
    await dispatcher.handleInbound(makeMsg(USER_GROUP, "legacy prompt"));
  ...
  it.each(["/help", "/now inspect", "/next inspect"])(
    "treats lark user synthetic %s as a prompt, not a command",
    async (text) => {
      const store = createFakeBindingStore();
      const sessionId = asSessionId(`sess_synthetic_command_${text.slice(1, 5)}`);
      store.seedSession(makeSession(sessionId, "idle"));
      store.seedBinding({ groupId: USER_GROUP, sessionId, createdAt: asTimestamp(100) });
      const { registry, captured } = makeCapturingBackendRegistry([
        { kind: "completed", finalMessage: "done" },
      ]);
      const route = vi.fn(async () => ({ replyText: "should not route" }));
      const dispatcher = createDispatcher({
        store,
        lark: createFakeLarkGateway(),
        router: { route },
      ...
      await dispatcher.handleInbound(makeMsg(USER_GROUP, text, {
        origin: "lark_user_synthetic",
      }));

      expect(route).not.toHaveBeenCalled();
      expect(captured.map((input) => input.prompt)).toEqual([text]);
    },
  );
  ...
  it("does not apply workspace lock semantics to legacy omitted-origin prompts", async () => {
    const store = createFakeBindingStore();
    const sessionId = asSessionId("sess_workspace_legacy_origin");
    store.seedSession(makeSession("sess_workspace_legacy_origin"));
    store.seedBinding({ groupId: USER_GROUP, sessionId, createdAt: asTimestamp(100) });
    await store.updateSessionWorkspaceLocked(sessionId, true);
    const { registry, captured } = makeCapturingBackendRegistry([
      { kind: "completed", finalMessage: "done" },
    ...
    await dispatcher.handleInbound(makeMsg(USER_GROUP, "legacy non-owner prompt", {
      userId: "ou_employee",
    }));

    expect(captured.map((input) => input.prompt)).toEqual(["legacy non-owner prompt"]);
  });
