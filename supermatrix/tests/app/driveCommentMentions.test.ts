...
describe("resolveDriveCommentMentionRoute registry predicates", () => {
  ...
  test("does not match a legacy mention-only trigger for an unmentioned poll", async () => {
    const request: DriveCommentMentionRequest = {
      source: {
        ...req("ignored").source,
        fileToken: "base_token",
  ...
  test("narrows poll_unmentioned trigger text without changing mention matching", async () => {
    const registry = {
      async load() {
        return {
          version: 1 as const,
          routes: [{
            ...
            triggers: [{
              id: "proposal-comment",
              priority: 100,
              ingestModes: ["mention", "poll_unmentioned"] as Array<"mention" | "poll_unmentioned">,
              match: { any: ["proposal"] },
              sopRef: "sop/mention/proposal.md",
            }],
          }],
        };
      },
      async loadSop() {
        return { name: "proposal", targetSession: "tobedone", body: "处理 proposal 评论" };
      },
    };
    const request: DriveCommentMentionRequest = {
      source: {
    ...
    await expect(resolveDriveCommentMentionRoute({
      ...request,
      source: { ...request.source, mentioned: true },
    }, registry)).resolves.toMatchObject({
      kind: "registered",
      route: { matchedRule: "registry:poll-comments:proposal-comment" },
    });
  });
...
describe("buildDriveCommentTargetPrompt", () => {
  test("[451] describes poll_unmentioned intake without claiming an explicit mention", () => {
    const request: DriveCommentMentionRequest = {
      ...req("经营快照的汇总口径应该怎么定？"),
      source: {
        ...req("ignored").source,
    ...
    const prompt = buildDriveCommentTargetPrompt(request, {
      ...
      reason: "matched registered poll_unmentioned trigger",
      matchedRule: "registry:todolist-comment-kanban-consult:kanban-consult",
    });

    expect(prompt).toContain("polled without an explicit SuperMatrix mention");
    expect(prompt).not.toContain("that mentioned SuperMatrix");
  });
  ...
  test("includes the target record URL and prior comment thread for child sessions", () => {
    ...
    expect(prompt).toContain(
      "target_url: https://example.feishu.cn/record/SMh9rOMSuewguhcVbtrcSjFknlf",
    );
...
describe("createDriveCommentMentionProcessor", () => {
  test("drops an unmentioned registry miss before claim, reply, or response logging", async () => {
    ...
    const processor = createDriveCommentMentionProcessor({
      ...
      mentionRegistry: {
        ...
        async loadSop() {
          throw new Error("should not load SOP");
        },
      },
      store: {
        ...sweepStoreStubs,
        async findSessionByName() { throw new Error("should not resolve a session"); },
        async claimDriveCommentMention(input: unknown) { claims.push(input); return true; },
        async finishDriveCommentMention() { throw new Error("should not finish"); },
        async recordResponseLog(input: unknown) { responseLogs.push(input); },
        async finishResponseLog() { throw new Error("should not finish a response log"); },
      },
      lark: {
        async getDriveCommentContext() { return { text: "查一下昨天销量", threadReplies: [] }; },
        async replyToDriveComment(input: unknown) { replies.push(input); },
        async createDriveComment() { throw new Error("should not fallback"); },
      },
      childSession: {
        async spawnChild() { throw new Error("should not spawn"); },
      },
    });

    await processor.handle({
      ...req("ignored").source,
      eventId: "poll:bitable:base_token:comment_1:reply_1:2000",
      fileToken: "base_token",
      fileType: "bitable",
      tableId: "tbl_todo",
      recordId: "rec_1",
  ...
  test("uses a registered source trigger and SOP before the standard flow", async () => {
    ...
    const source = {
      ...
      tableId: "tbl_growth",
      recordId: "rec_1",
      url: "https://example.feishu.cn/record/rec_1",
    };
    ...
    expect(prompt).toContain("target_url: https://example.feishu.cn/record/rec_1");
...
describe("[451] unmentioned poll intake admission", () => {
  const pilotRegistry = {
    async load() {
      return {
        version: 1 as const,
            ...
            triggers: [
              {
                id: "kanban-consult",
                priority: 50,
                ingestModes: ["poll_unmentioned", "mention"] as Array<"mention" | "poll_unmentioned">,
                match: { any: ["经营快照"] },
                sopRef: "sop/mention/kanban-comment-consult.md",
              },
            ],
          },
        ],
    ...
    async loadSop(sopRef: string) {
      if (sopRef === "sop/mention/todo.md") {
        return { name: "todo", targetSession: "tobedone", body: "处理 todo 评论" };
      }
      if (sopRef !== "sop/mention/kanban-comment-consult.md") {
        throw new Error(`unexpected sop_ref: ${sopRef}`);
      }
      return {
        name: "kanban-comment-consult",
        targetSession: "product-tracker",
        body: "把评论、目标记录链接和历史评论投递给看板娘，让它给一段可直接贴回评论区的结论。",
      };
    },
  };

  test("[451] drops an unmentioned registry miss before claim, reply, and response log", async () => {
    const claims: unknown[] = [];
    const replies: unknown[] = [];
    const responseLogs: unknown[] = [];
    const processor = createDriveCommentMentionProcessor({
      callerSessionName: "pinglunmaster",
      clock: { now: () => asTimestamp(10_000) },
      mentionRegistry: pilotRegistry,
      store: {
        ...sweepStoreStubs,
        async findSessionByName() { throw new Error("should not resolve a session"); },
        async claimDriveCommentMention(input: unknown) { claims.push(input); return true; },
        async finishDriveCommentMention() { throw new Error("should not finish a mention"); },
        async recordResponseLog(input: unknown) { responseLogs.push(input); },
        async finishResponseLog() { throw new Error("should not finish a response log"); },
      },
      lark: {
        async getDriveCommentContext() {
          return {
            text: "这个查一下昨天销量",
            threadReplies: [],
            bitableRecord: { recordId: "recOtherRecord", fields: { "Todo owner alias": "tobedone" } },
          };
        },
        async replyToDriveComment(input: unknown) { replies.push(input); },
        async createDriveComment() { throw new Error("should not fallback"); },
  ...
  test("[451] a legacy trigger without ingest_modes never matches an unmentioned poll", async () => {
    const request: DriveCommentMentionRequest = {
      source: {
        ...req("ignored").source,
        fileToken: "YOUR_RESOURCE_ID",
    ...
    await expect(resolveDriveCommentMentionRoute(request, pilotRegistry)).resolves.toMatchObject({
      kind: "standard",
    });
    ...
    await expect(resolveDriveCommentMentionRoute({
      ...request,
      source: { ...request.source, mentioned: true },
    }, pilotRegistry)).resolves.toMatchObject({
      kind: "registered",
      route: { matchedRule: "registry:todolist-agent-todo-comments:agent-todo-record-comment" },
    });
  });

  test("[451] a poll_unmentioned trigger matches and delivers to the SOP target session", async () => {
    const productTracker = session("sess_product_tracker", "product-tracker");
    const caller = session("sess_pinglun", "pinglunmaster");
    const claims: unknown[] = [];
    const spawns: unknown[] = [];
    const replies: unknown[] = [];
    ...
    const processor = createDriveCommentMentionProcessor({
      ...
      store: {
        ...sweepStoreStubs,
        async findSessionByName(name: string) {
          return name === "product-tracker" ? productTracker : name === "pinglunmaster" ? caller : null;
        },
        async claimDriveCommentMention(input: unknown) { claims.push(input); return true; },
        async finishDriveCommentMention(input: unknown) { finishes.push(input); },
        async recordResponseLog() {},
      ...
      lark: {
        async getDriveCommentContext() {
          return {
            text: "经营快照的汇总口径应该怎么定？",
            threadReplies: [],
            bitableRecord: { recordId: "recvsPavOYlcOA", fields: { "Todo owner alias": "tobedone" } },
          };
        },
        async replyToDriveComment(input: unknown) { replies.push(input); },
        async createDriveComment() { throw new Error("should not fallback"); },
      ...
      childSession: {
        async spawnChild(input: unknown) {
          spawns.push(input);
          return {
            session: productTracker,
            finalMessage: "看板娘结论：按父 ASIN 日粒度汇总。",
            backendSessionId: null,
            messageRunId: asMessageRunId("mr_451"),
          };
        },
      },
    });
    ...
    expect(spawns).toHaveLength(1);
    expect(spawns[0]).toMatchObject({
      parentId: productTracker.id,
      clientRequestId: buildDriveCommentMentionDedupeKey(source),
    });
    expect(replies).toEqual([{ source, text: "看板娘结论：按父 ASIN 日粒度汇总。" }]);
    expect(finishes[0]).toMatchObject({ status: "completed" });
  });
});
