...
describe("processLifecycle", () => {
  ...
  it("ignores the exact authorized maintenance run when evaluating restart drain", async () => {
    const onExit = vi.fn().mockResolvedValue(undefined);
    const lc = createProcessLifecycle({ onExit, logger: noopLogger });
    lc.runStarted("run_codexroot_maintenance");
    lc.requestRestart("scheduled maintenance", {
      source: "scheduled-daily",
      ignoreInFlightRunId: "run_codexroot_maintenance",
    });

    await new Promise((resolve) => setImmediate(resolve));

    expect(onExit).toHaveBeenCalledTimes(1);
    expect(lc.isForce()).toBe(false);
  });

  it("continues draining while any run other than the authorized maintenance child is active", async () => {
    const onExit = vi.fn().mockResolvedValue(undefined);
    const lc = createProcessLifecycle({ onExit, logger: noopLogger });
    lc.runStarted("run_codexroot_maintenance");
    lc.runStarted("run_same_session_other");
    lc.requestRestart("scheduled maintenance", {
      source: "scheduled-daily",
      ignoreInFlightRunId: "run_codexroot_maintenance",
    });

    lc.runFinished("run_codexroot_maintenance");
    await new Promise((resolve) => setImmediate(resolve));
    expect(onExit).not.toHaveBeenCalled();

    lc.runFinished("run_same_session_other");
    await new Promise((resolve) => setImmediate(resolve));
    expect(onExit).toHaveBeenCalledTimes(1);
  });
