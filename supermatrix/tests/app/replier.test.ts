...
describe("replier", () => {
  ...
  test("synthetic-origin runs get a Δ title prefix and violet completed header", async () => {
    // Continuation receipts, /next drains, and spawn result reinjections all
    // start runs with origin=framework_synthetic while the user said nothing.
    // Their cards used to be indistinguishable from user-triggered runs, so a
    // session "moving on its own" (e.g. auto-executing dispatch.py after a
    // child completed) looked like an anomaly. The Δ prefix matches the Δ the
    // framework already prepends to injected prompts; violet matches the kimi
    // autonomous-turn convention for non-user-triggered completions.
    const lark = createFakeLarkGateway();
    const now = { value: 1_000 };
    const replier = createReplier({
      lark,
    ...
    await replier.consume({
      groupId: asLarkGroupId("oc_1"),
      sessionId: asSessionId("s1"),
      runId: asMessageRunId("mr_cont"),
      sessionName: "tobedone",
      sessionModel: "gpt-5.6-sol",
      sessionEffort: "xhigh",
      sessionBackend: "codex",
      execution: { backend: "codex", model: "gpt-5.6-sol", effort: "xhigh" },
      syntheticOrigin: true,
      stream: (async function* () {
        yield { kind: "started", backendSessionId: "bks-cont" } satisfies AgentEvent;
        yield { kind: "assistant_message", text: "done", final: true } satisfies AgentEvent;
        yield { kind: "completed", finalMessage: "done" } satisfies AgentEvent;
      })(),
    } as Parameters<typeof replier.consume>[0]);
    ...
    const titles = lark.titleHistory.map((t) => t.title);
    expect(titles[0]).toBe("Δ tobedone | GPT-5.6 Sol · running | XHIGH | mr_cont");
    const [final] = lark.finalized;
    expect(final.title).toBe("Δ tobedone | GPT-5.6 Sol · done | XHIGH | mr_cont");
    expect(final.completedTemplate).toBe("violet");
  });

  test("an explicit completedTemplate wins over the synthetic-origin violet default", async () => {
    const lark = createFakeLarkGateway();
    const now = { value: 1_000 };
    const replier = createReplier({
      lark,
    ...
    await replier.consume({
      groupId: asLarkGroupId("oc_1"),
      sessionId: asSessionId("s1"),
      runId: asMessageRunId("mr_kimi_auto"),
      sessionName: "wechat-help",
      sessionModel: "k3",
      sessionBackend: "kimi",
      syntheticOrigin: true,
      completedTemplate: "grey",
      stream: (async function* () {
        yield { kind: "started", backendSessionId: "bks-k" } satisfies AgentEvent;
        yield { kind: "completed", finalMessage: "ok" } satisfies AgentEvent;
      })(),
    } as Parameters<typeof replier.consume>[0]);

    expect(lark.finalized[0]?.completedTemplate).toBe("grey");
  });
  ...
  for (const config of [
    ...
    test(`${config.backend} final process log keeps narrative and hides tool details`, async () => {
      ...
      // Tool NAMES are safe to surface (payloads above are not): without them
      // a finalized card cannot distinguish "model did nothing useful" from
      // "framework swallowed the answer". Real case mr_b258cad6: 3 Bash calls
      // that only rewrote USER_PROFILE.md, an 11-char final message, and a
      // collapsed line that named nothing — the user reasonably read it as a
      // framework bug.
      expect(finalCall.processLog).toContain("Bash×1");
  ...
  test("formats the GPT-6-Astra display name", () => {
    expect(formatModel("gpt-6-astra", "codex")).toBe("GPT-6-Astra");
  });
