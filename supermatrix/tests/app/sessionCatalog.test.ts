...
describe("session catalog service", () => {
  ...
  test("regenerateCatalog includes fpManaged=false active sessions", async () => {
    const { store, fs, service } = mkService();
    store.seedSession(mkSession("kept", { fpManaged: null }));
    store.seedSession(mkSession("attached", { fpManaged: false }));

    await service.regenerateCatalog("scope");

    const parsed = JSON.parse(fs.files.get("/ws/session-catalog.json")!) as {
      sessions: Array<{ name: string; fp_managed: boolean | null }>;
    };
    expect(parsed.sessions.map((s) => s.name)).toEqual(["attached", "kept"]);
    expect(parsed.sessions.find((s) => s.name === "attached")!.fp_managed).toBe(false);
  });
});
