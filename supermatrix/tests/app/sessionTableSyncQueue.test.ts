...
describe("session table sync request queue", () => {
  ...
  test("serializes adjacent full requests and coalesces the queued duplicate", async () => {
    ...
    const firstOutcome = request("full");
    request("full");
    request("full");
    ...
    await expect(firstOutcome).resolves.toEqual({ ok: true });
  ...
  test("returns a failed outcome to the requester without rejecting", async () => {
    const failure = new Error("sync failed");
    const request = createSessionTableSyncRequester(async () => {
      throw failure;
    }, () => {});

    await expect(request("runtime-settings-push-current")).resolves.toEqual({
      ok: false,
      error: failure,
    });
  });
