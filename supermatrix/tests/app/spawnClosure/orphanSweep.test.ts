...
describe("recoverSpawnCommOrphans", () => {
  ...
  test.each([
    ...
    "excludes $label from both startup and watcher orphan recovery",
    async ({ kind, prompt }) => {
      const store = new SqliteBindingStore(":memory:");
      await store.init();
      try {
        await seedSessions(store);
        await store.logCrossSessionComm({
          id: `comm_receiver_terminal_${kind}`,
          fromSessionId: asSessionId("sess_caller"),
          toSessionId: asSessionId("sess_target"),
          kind,
          prompt,
        ...
        for (const source of ["startup", "watcher_tick"] as const) {
          expect(recoverSpawnCommOrphans({
            db: store.db,
            now: asTimestamp(101_000),
            thresholdSec: 60,
            source,
  ...
  test.each([
    ...
    { suffix: "trailing_metadata", prompt: `${LEGACY_RECEIVER_TERMINAL_PROMPT},"extra":true` },
    {
      suffix: "short_digest",
  ...
  ])("recovers an ordinary spawn with $suffix near-marker metadata", async ({ suffix, prompt }) => {
    const store = new SqliteBindingStore(":memory:");
    await store.init();
    try {
      await seedSessions(store);
      const commId = `comm_near_marker_${suffix}`;
      await store.logCrossSessionComm({
        id: commId,
        fromSessionId: asSessionId("sess_caller"),
        toSessionId: asSessionId("sess_target"),
      ...
      expect(recoverSpawnCommOrphans({
        ...
        expect.objectContaining({
          commId,
          ref: `async_orphan_${commId}`,
        }),
      ]);
      expect(store.db
        .prepare("SELECT status FROM spawn_async_items WHERE comm_id = ?")
        .get(commId)).toEqual({ status: "pending" });
    } finally {
      await store.close();
    }
  });
