...
describe("inbox-message predicate evaluator", () => {
  ...
  test("matches pointerized final_message through the injected canonical resolver", async () => {
    const dir = await mkdtemp(join(tmpdir(), "sm-predicate-inbox-"));
    const dbPath = join(dir, "supermatrix.db");
    seedDb(dbPath);
    const db = new Database(dbPath);
    db.exec(`
      ...
      UPDATE message_runs SET final_message = NULL WHERE id = 'mr_1';
      INSERT INTO message_run_canonical_pointers VALUES ('mr_1', 'final_message');
    `);
    db.close();
    const predicate = validateSpawnPredicate({
      type: "inbox-message",
      ...
      field: "final_message",
      since: { kind: "timestamp_ms", value: 1_699_999_999_999 },
      contains_all: ["comm_msg_pointer"],
    }).predicate;
    if (predicate.type !== "inbox-message") throw new Error("expected inbox-message predicate");
    try {
      await expect(evaluateInboxMessagePredicate(predicate, {
        dbRegistry: registryFor(dbPath),
        resolveMessageRunContent: (_path, messageRunId, contentKind) => {
          expect(messageRunId).toBe("mr_1");
          expect(contentKind).toBe("final_message");
          return "done for comm_msg_pointer";
        },
      })).resolves.toMatchObject({ matched: true, observed_count: 1 });
    } finally {
      await rm(dir, { force: true, recursive: true });
    }
  });
