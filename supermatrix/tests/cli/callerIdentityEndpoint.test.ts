...
describe("POST /api/caller-identity", () => {
  ...
  test("an unknown or revoked attestation is unattested, never a fallback identity", async () => {
    const token = registry.mint({ sessionId: "s1", messageRunId: "run-1", sessionName: "a", backend: "codex", now: 1 });
  ...
  test("a guessed attestation does not resolve", async () => {
    registry.mint({ sessionId: "s-fp", messageRunId: "run-fp-1", sessionName: "first-principle", backend: "codex", now: 1 });
