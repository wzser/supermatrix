...
`,
    ...
    try {
      ...
      expect(
        ...
        detail: {
          ...
          aliases: [
            { alias: "astra", target: "gpt-6-astra" },
            { alias: "gpt6-astra", target: "gpt-6-astra" },
            { alias: "6-astra", target: "gpt-6-astra" },
    ...
    // Must exceed the 30_000 spawnSync budget above, otherwise vitest's 5s default kills the test
    // before the child's own bound can apply and a loaded machine reports a bogus timeout.
  }, 40_000);
  ...
  test("cli-upgrade profile maps a failed check to ok=false and exit 1", () => {
    ...
    // Same reason as above: exceed this test's own 15_000 spawnSync budget.
  }, 25_000);
});
