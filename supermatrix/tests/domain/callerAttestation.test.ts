...
describe("callerAttestation registry", () => {
  ...
  test("an unknown, empty, or whitespace token resolves to null (never a default identity)", () => {
    const registry = mkRegistry();
    registry.mint({ sessionId: "s1", messageRunId: "run-1", sessionName: "a", backend: "codex", now: 1 });
  ...
  test("re-minting for a session rotates: the previous token stops resolving", () => {
    const registry = mkRegistry();
    const first = registry.mint({ sessionId: "s1", messageRunId: "run-1", sessionName: "a", backend: "codex", now: 1 });
    const second = registry.mint({ sessionId: "s1", messageRunId: "run-2", sessionName: "a", backend: "codex", now: 2 });
  ...
  test("revokeSession invalidates the live token for that session only", () => {
    const registry = mkRegistry();
    const a = registry.mint({ sessionId: "s1", messageRunId: "run-1", sessionName: "a", backend: "codex", now: 1 });
    const b = registry.mint({ sessionId: "s2", messageRunId: "run-2", sessionName: "b", backend: "claude", now: 1 });
