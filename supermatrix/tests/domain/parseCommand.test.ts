...
const REGISTRY = { new: NEW_CMD, reset: RESET_CMD, help: HELP_CMD, rank: RANK_CMD };
...
describe("parseCommand", () => {
  ...
  test("/rank accepts name in root scope and rejects it in user scope", () => {
    expect(parseCommand("/rank PERSON_REDACTED", REGISTRY, "root")).toEqual({
      kind: "ok",
      name: "rank",
      args: { name: "PERSON_REDACTED" },
    });
    expect(parseCommand("/rank PERSON_REDACTED", REGISTRY, "user")).toEqual({
      kind: "error",
      message: "多余的参数：PERSON_REDACTED",
    });
  });
  ...
  test("preserves a synthetic Δ/next free body with truncated JSON", () => {
    const body = [
      "完整内容已写入本机文件，以下仅预览：",
      '{"todo":"保留 Markdown、单双引号与截断 JSON","preview":"',
    ].join("\n");

    const result = parseCommand(`Δ/next ${body}`.slice(1), {
      next: {
        name: "next",
        description: "排队消息",
        scope: ["user"],
    ...
    expect(result).toEqual({ kind: "ok", name: "next", args: { text: body } });
    expect(result.kind === "error" ? result.message : "").not.toContain("引号未闭合");
  });
});

// --- selector aliasing (D1-B) ---
const NEW_ALIAS_CMD: Command = {
  name: "new",
  description: "新建 session",
  scope: ["root"],
  params: [
    {
      ...
      enumAliases: { "claude-code": "claude", "codex-cli": "codex", k2: "kimi" },
    },
    { name: "name", type: "string", required: true, kind: "positional" },
    { name: "purpose", type: "string", required: false, kind: "rest" },
  ],
};
const HEARTBEAT_ALIAS_CMD: Command = {
  name: "heartbeat",
  description: "巡检",
  scope: ["root", "user"],
  params: [
...
const TIMEOUT_ALIAS_CMD: Command = {
  name: "timeout",
  description: "超时",
  scope: ["root", "user"],
  params: [
...
const CANCEL_ALIAS_CMD: Command = {
  name: "cancel",
  description: "取消",
  scope: ["root", "user"],
  params: [
    { name: "next", type: "enum", required: false, kind: "positional", scope: ["user"], enum: ["next"] },
    { name: "target", type: "string", required: false, kind: "rest", scope: ["root"] },
  ],
};
const ALIAS_REGISTRY = {
  new: NEW_ALIAS_CMD,
  heartbeat: HEARTBEAT_ALIAS_CMD,
  timeout: TIMEOUT_ALIAS_CMD,
  cancel: CANCEL_ALIAS_CMD,
};

describe("parseCommand selector aliasing (D1-B)", () => {
  test("enum member matches case- and NFKC-insensitively", () => {
    expect(parseCommand("/new CLAUDE foo", ALIAS_REGISTRY, "root")).toEqual({
      kind: "ok",
      name: "new",
      args: { backend: "claude", name: "foo" },
    });
    expect(parseCommand("/new ＣＯＤＥＸ foo", ALIAS_REGISTRY, "root")).toEqual({
      kind: "ok",
      name: "new",
  ...
  test("enumAliases map to the canonical member", () => {
    expect(parseCommand("/new claude-code foo", ALIAS_REGISTRY, "root")).toEqual({
      kind: "ok",
      name: "new",
      args: { backend: "claude", name: "foo" },
    });
    expect(parseCommand("/new k2 foo", ALIAS_REGISTRY, "root")).toEqual({
      kind: "ok",
      name: "new",
  ...
  test("heartbeat state aliases resolve to canonical members", () => {
    expect(parseCommand("/heartbeat s 开启", ALIAS_REGISTRY, "root")).toEqual({
      kind: "ok",
      name: "heartbeat",
      args: { name: "s", state: "on" },
    });
    expect(parseCommand("/heartbeat s STATUS", ALIAS_REGISTRY, "root")).toEqual({
      kind: "ok",
      name: "heartbeat",
      args: { name: "s", state: "status" },
    });
  });

  test("named-flag key folds case; its value stays untouched", () => {
    expect(parseCommand("/timeout s 10 --MAXRUN 60", ALIAS_REGISTRY, "root")).toEqual({
      kind: "ok",
      name: "timeout",
      args: { name: "s", timeout: "10", maxrun: "60" },
    });
  });

  test("user-scope /cancel NEXT folds the enum member to next", () => {
    // user scope: `next` is an enum param → parser canonicalizes NEXT → next.
    expect(parseCommand("/cancel NEXT", ALIAS_REGISTRY, "user")).toEqual({
      kind: "ok",
      name: "cancel",
      args: { next: "next" },
    });
  ...
  test("unknown enum value still errors with canonical members", () => {
    const r = parseCommand("/new gpt4 foo", ALIAS_REGISTRY, "root");
    expect(r.kind).toBe("error");
  });

  test("D1-B: free-text positional keeps its case (no lowercasing of names)", () => {
    expect(parseCommand("/new claude MySession", ALIAS_REGISTRY, "root")).toEqual({
      kind: "ok",
      name: "new",
      args: { backend: "claude", name: "MySession" },
    });
  });
});
