...
describe("buildSessionCatalog", () => {
  ...
  test("keeps fpManaged=false active sessions and projects the governance flag", () => {
    const out = buildSessionCatalog(
      [
        mkSession("nullmgd", { fpManaged: null }),
        mkSession("truemgd", { fpManaged: true }),
        mkSession("falsemgd", { fpManaged: false }),
      ],
      asTimestamp(1),
    );
    expect(out.sessions.map((s) => s.name)).toEqual(["falsemgd", "nullmgd", "truemgd"]);
    expect(out.sessions.find((s) => s.name === "falsemgd")!.fp_managed).toBe(false);
