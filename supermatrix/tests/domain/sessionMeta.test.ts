...
describe("assertBackendAllowedForCategory", () => {
  test("rejects kimi backend on 外部 category", () => {
    expect(() => assertBackendAllowedForCategory("kimi", "外部")).toThrow(UserError);
    expect(() => assertBackendAllowedForCategory("kimi", "外部")).toThrow(/外部 session 不支持 kimi/);
  });

  test("allows claude/codex on 外部 and kimi on non-外部 categories", () => {
    expect(() => assertBackendAllowedForCategory("claude", "外部")).not.toThrow();
    expect(() => assertBackendAllowedForCategory("codex", "外部")).not.toThrow();
    expect(() => assertBackendAllowedForCategory("kimi", "")).not.toThrow();
    expect(() => assertBackendAllowedForCategory("kimi", "平台")).not.toThrow();
  });
});
