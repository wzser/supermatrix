...
import { createCloneHandler, createNewHandler } from "../../src/app/commands/newSession.ts";
...
export type Harness = {
  ...
  setSteer(steer: (input: SteerInput) => Promise<SteerResult>): void;
  readonly backendGets: BackendKind[];
  readonly runInputs: RunInput[];
  readonly steerCalls: SteerInput[];
  readonly cancelCalls: SessionId[];
  pendingNextCount(sessionId: SessionId): number;
...
export async function createHarness(opts: {
  ...
  const backend = makeFakeBackend(opts.script ?? (() => []));
  const backendGets: BackendKind[] = [];
  const backendRegistry: BackendRegistry = {
    get: (kind) => {
      backendGets.push(kind);
      return backend;
    },
  ...
  registry.clone.handler = createCloneHandler({ lifecycle, store, resolveUserGroupSession });
  ...
  return {
    ...
    setSteer: (next) => backend.setSteer(next),
    backendGets,
    runInputs: backend.runInputs,
    steerCalls: backend.steerCalls,
    cancelCalls: backend.cancelCalls,
    pendingNextCount: (sessionId) => pendingNextMap.get(sessionId)?.length ?? 0,
    rootGroupId,
    async cleanup() {
