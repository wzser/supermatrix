...
describe("e2e spawn closure async watcher path", () => {
  ...
  test("empty inline spawn2 switches async, watcher persists caller failure notice before parking, and trace stays keyed by comm_id", async () => {
    ...
    const res = await fetch(`${baseUrl}/api/spawn2.0`, {
      ...
      body: JSON.stringify({
        ...
        client_request_id: "2026-05-31:e2e:spawn-closure-empty",
    ...
    expect(body.spawnCommId).toMatch(/^comm_e2e_1_/u);
    ...
    const heartbeatCalls = await readFile(heartbeat.callsPath, "utf8");
    expect(heartbeatCalls).toContain("--session caller");
    expect(heartbeatCalls).toContain(`--key ${body.spawnCommId}:failure-notice`);
    expect(heartbeatCalls).toContain(`--source-ref ${body.spawnCommId}`);
    expect(heartbeatCalls).toContain("--todo-type status_reconcile");
    expect(heartbeatCalls).toContain("--batch-mode single");
    expect(heartbeatCalls).toContain(`async_ref:`);
    expect(heartbeatCalls).toContain("执行状态未知，先核业务副作用，再由 caller 显式决定是否用原 client_request_id 重试");
    expect(watcher.stdout).toContain(`"comm_id":"${body.spawnCommId}"`);
    expect(watcher.stdout).toContain('"route":"failure_notice"');
    expect(watcher.stdout).toContain('"action":"failure_notice"');
    expect(attempts).toBe(1);

    const updated = store.db
      .prepare("SELECT status, attempt_count, verdict FROM spawn_async_items WHERE comm_id = ?")
      .get(body.spawnCommId) as { status: string; attempt_count: number; verdict: string };
    ...
    expect(closureEvents).toEqual(expect.arrayContaining(["phase_check", "async_switch"]));
  });
});
