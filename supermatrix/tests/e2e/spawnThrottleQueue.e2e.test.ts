...
describe("e2e spawn throttle queue", () => {
  ...
  test("queues throttled API spawns and exposes their completed results through take", async () => {
    ...
    const childSession = createChildSessionService({
      ...
      availability: { probe: async () => ({ kind: "available", checkedAt: Date.now() }) },
      idFactory: (() => {
        let i = 0;
    ...
    const spawn2Body = (prompt: string) => ({
      from: "caller",
      target: "target",
      prompt,
      client_request_id: `2026-05-31:e2e:spawn-throttle:${prompt}`,
    ...
    const first = fetch(`${baseUrl}/api/spawn2.0`, {
      ...
      body: JSON.stringify(spawn2Body("first")),
    });
    await backend.waitForRunCount(1);
    expect(backend.runInputs[0]!.session).toMatchObject({
      backend: "codex",
      model: "gpt-5.5",
      effort: "high",
    });
    ...
    const queuedResponses = await Promise.all([
      fetch(`${baseUrl}/api/spawn2.0`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(spawn2Body("second")),
      }),
      fetch(`${baseUrl}/api/spawn2.0`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(spawn2Body("third")),
      }),
    ]);
    const queuedBodies = await Promise.all(
      queuedResponses.map((res) => res.json() as Promise<{
        ok: boolean;
        status: string;
        ref: string;
        comm_id: string;
    ...
    const pendingSnapshots = await Promise.all(queuedBodies.map(async (body) => {
      const pending = await fetch(`${baseUrl}/api/spawn_async_items/${body.ref}`);
      return { status: pending.status, body: await pending.json() };
    }));
    await store.updateChildSessionDefaults({
    ...
    await eventually(() => {
      expect(backend.prompts.map((prompt) => prompt.split("\n\n").at(-1))).toEqual(["first", "second", "third"]);
      ...
      expect(backend.runInputs.map((input) => ({
    ...
    for (const [index, body] of queuedBodies.entries()) {
      expect(body.resultUrl).toBe(`/api/spawn_async_items/${body.ref}/take`);
      expect(pendingSnapshots[index]).toMatchObject({
        status: 200,
        body: {
          ok: true,
          ref: body.ref,
    ...
    const takenResponses = await Promise.all(
      queuedBodies.map((body) => fetch(`${baseUrl}${body.resultUrl}`, { method: "POST" })),
    );
    expect(takenResponses.map((res) => res.status)).toEqual([200, 200]);
    const takenBodies = await Promise.all(takenResponses.map((res) => res.json()));
    expect(takenBodies).toEqual([
      expect.objectContaining({ ok: true, commStatus: "completed", finalMessage: "second done" }),
      expect.objectContaining({ ok: true, commStatus: "completed", finalMessage: "third done" }),
    ]);
  });
});
