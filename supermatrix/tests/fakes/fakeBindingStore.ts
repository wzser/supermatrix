...
function defaultSessionRuntimeSettings(
  ...
  updatedAt: Timestamp,
  backend?: BackendKind,
  runtimeDefaults?: { model: string | null; effort: EffortLevel | null },
): SessionRuntimeSettings {
  const defaults = backend
    ? runtimeDefaults
      ? {
          mainModelDefault: runtimeDefaults.model,
          mainEffortDefault: runtimeDefaults.effort,
        }
      : LEGACY_MAIN_SESSION_DEFAULTS[backend]
...
export function createFakeBindingStore(): BindingStore & {
  ...
  _listSchedulerCleanupRequests(): SchedulerCleanupRequest[];
} {
  ...
  const schedulerCleanupRequests = new Map<string, SchedulerCleanupRequest>();
  ...
  return {
      ...
      if (input.scope !== "child") {
        sessionRuntimeSettings.set(
          ...
          defaultSessionRuntimeSettings(
            ...
            session.updatedAt,
            session.backend,
            backendRuntimeDefaults.get(session.backend),
          ),
        );
      }
    ...
    async updateSessionRuntimeSettings(
      ...
      const current = sessionRuntimeSettings.get(sessionId)
        ?? defaultSessionRuntimeSettings(
          ...
          session?.backend,
          session ? backendRuntimeDefaults.get(session.backend) : undefined,
        );
    ...
    async beginSchedulerCleanupRequest(input) {
      const existing = schedulerCleanupRequests.get(input.clientRequestId);
      if (existing) {
        if (existing.status === "failed") {
          const retried: SchedulerCleanupRequest = {
            ...existing,
            status: "pending",
            attemptCount: existing.attemptCount + 1,
            lastError: null,
            updatedAt: input.createdAt as Timestamp,
            submittedAt: null,
          };
          schedulerCleanupRequests.set(input.clientRequestId, retried);
          return { request: { ...retried }, shouldDispatch: true };
        }
        return { request: { ...existing }, shouldDispatch: false };
      }
      const created: SchedulerCleanupRequest = {
        ...input,
        status: "pending",
        attemptCount: 1,
        lastError: null,
      ...
      schedulerCleanupRequests.set(input.clientRequestId, created);
      return { request: { ...created }, shouldDispatch: true };
    },

    async markSchedulerCleanupSubmitted(clientRequestId: string, now: Timestamp): Promise<void> {
      const existing = schedulerCleanupRequests.get(clientRequestId);
      if (!existing || existing.status !== "pending") return;
      schedulerCleanupRequests.set(clientRequestId, {
        ...existing,
        status: "submitted",
        lastError: null,
        updatedAt: now,
    ...
    async listSchedulerCleanupRequests(limit = 100): Promise<SchedulerCleanupRequest[]> {
      return [...schedulerCleanupRequests.values()]
    ...
    _listSchedulerCleanupRequests() {
      return [...schedulerCleanupRequests.values()].map((request) => ({ ...request }));
    },
  };
}
