...
export function createFakeLarkGateway(opts: FakeLarkOptions = {}) {
  ...
  const gateway: LarkGateway & {
    ...
    async finalizeCard(cardId, text, title, processLog, runStatus, completedTemplate) {
      ...
      if (completedTemplate !== undefined) entry.completedTemplate = completedTemplate;
