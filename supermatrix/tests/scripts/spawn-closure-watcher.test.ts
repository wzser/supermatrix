...
describe("scripts/spawn-closure-watcher.sh", () => {
  ...
  test("delivery-ready scan and delivery reconstruct a pointerized final message", async () => {
    const seeded = await seedWatcherDb();
    const heartbeat = await makeHeartbeatStub(seeded.dir);
    const api = await startMockApi();
    try {
      const finalMessage = "pointer-only watcher result";
      const commId = insertAsyncItem(seeded.dbPath, {
        status: "waiting_child",
        failureKind: "late_result",
        childSessionId: "sess_pointer_child",
        finalMessage,
      });
      const turnId = "turn_pointer_watcher";
      const rolloutPath = join(seeded.dir, "pointer-rollout.jsonl");
      const record = `${JSON.stringify({
        type: "event_msg",
        payload: { type: "task_complete", turn_id: turnId, last_agent_message: finalMessage },
      })}\n`;
      await writeFile(rolloutPath, record, "utf8");
      const db = new Database(seeded.dbPath);
      try {
        db.prepare(
          `INSERT INTO message_run_canonical_pointers
           ...
           VALUES (?, 'final_message', 'thread_pointer', ?, ?, ?, 0, ?, 1, ?)`,
        ).run(
          `mr_${commId}`,
          turnId,
          createHash("sha256").update(finalMessage, "utf8").digest("hex"),
          rolloutPath,
          Buffer.byteLength(record),
          Date.now(),
        );
        db.prepare("UPDATE message_runs SET final_message = NULL WHERE id = ?").run(`mr_${commId}`);
        db.prepare("UPDATE cross_session_log SET final_message = NULL WHERE id = ?").run(commId);
      } finally {
        db.close();
      }
      ...
      const calls = await readTextIfExists(heartbeat.callsPath);
      expect(calls).toContain(finalMessage);
      expect(result.stdout).toContain('"route":"deliver"');
      expect(readAsyncItem(seeded.dbPath, commId)).toMatchObject({ status: "closed", attempt_count: 1 });
    } finally {
      await api.close();
    }
  });
