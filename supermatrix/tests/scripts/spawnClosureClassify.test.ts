...
import { RECEIVER_TERMINAL_COMM_KIND, RECEIVER_TERMINAL_LEGACY_PROMPT_PREFIX } from "../../src/app/spawnClosure/receiverTerminalFence.ts";

const LEGACY_RECEIVER_TERMINAL_PROMPT =
  `${RECEIVER_TERMINAL_LEGACY_PROMPT_PREFIX}${"a".repeat(64)}","prompt_sha256":"${"b".repeat(64)}","artifact_id":"sm-immutable-ingress-v1:${"b".repeat(64)}","declaration_sha256":"${"c".repeat(64)}"}`;
...
`,
  ...
  test.each([
    { label: "new durable kind", kind: RECEIVER_TERMINAL_COMM_KIND, prompt: "receiver metadata" },
    {
      label: "legacy sanitized marker",
      kind: "spawn",
      prompt: LEGACY_RECEIVER_TERMINAL_PROMPT,
    },
  ])("suppresses async recovery and redrive for $label receiver-terminal comms", async ({ kind, prompt }) => {
    process.env.SPAWN_CLOSURE_ENABLE_AUTO_REDRIVE = "1";
    const { db } = await makeDb();
    const item = seedAsyncItem(db, { failureKind: "spawn_not_started" });
    db.prepare("UPDATE cross_session_log SET kind = ?, prompt = ?, status = 'pending' WHERE id = ?")
      .run(kind, prompt, item.comm_id);
    const fetchSpy = vi.fn();
    vi.stubGlobal("fetch", fetchSpy);

    expect(classifyAsyncItem(item, db)).toEqual({
      route: "noop",
      reason: "receiver-terminal comm is terminal-only; async recovery and redrive suppressed",
    });
    await expect(classifyAndRoute({ item, db, apiBase: "http://sm.test" })).resolves.toEqual({
      decision: {
        route: "noop",
        reason: "receiver-terminal comm is terminal-only; async recovery and redrive suppressed",
      },
    ...
    expect(db.prepare("SELECT COUNT(*) AS count FROM spawn_async_items").get()).toEqual({ count: 1 });
    expect(db.prepare("SELECT status FROM spawn_async_items WHERE ref = ?").get(item.ref)).toEqual({ status: "pending" });
  });

  test.each([
    {
      label: "LIKE-wildcard key substitutions",
      prompt: '{"protocol":"supermatrix-immutable-ingress/v1","completionXmode":"receiverXterminal","clientXrequestXidXsha256":"not-a-legacy-row',
    },
    { label: "trailing metadata", prompt: `${LEGACY_RECEIVER_TERMINAL_PROMPT},"extra":true` },
    {
      label: "short digest",
      prompt: `${RECEIVER_TERMINAL_LEGACY_PROMPT_PREFIX}${"a".repeat(63)}","prompt_sha256":"${"b".repeat(64)}","artifact_id":"sm-immutable-ingress-v1:${"b".repeat(64)}","declaration_sha256":"${"c".repeat(64)}"}`,
    },
    {
      label: "artifact digest mismatch",
      prompt: `${RECEIVER_TERMINAL_LEGACY_PROMPT_PREFIX}${"a".repeat(64)}","prompt_sha256":"${"b".repeat(64)}","artifact_id":"sm-immutable-ingress-v1:${"d".repeat(64)}","declaration_sha256":"${"c".repeat(64)}"}`,
    },
  ])("keeps ordinary spawn redrive enabled for $label near-markers", async ({ prompt }) => {
    process.env.SPAWN_CLOSURE_ENABLE_AUTO_REDRIVE = "1";
    const { db } = await makeDb();
    const item = seedAsyncItem(db, { failureKind: "spawn_not_started" });
    db.prepare("UPDATE cross_session_log SET prompt = ?, status = 'pending' WHERE id = ?")
      .run(prompt, item.comm_id);
  ...
  test.each([
    { label: "new durable kind", kind: RECEIVER_TERMINAL_COMM_KIND, prompt: "receiver metadata" },
    { label: "legacy exact marker", kind: "spawn", prompt: LEGACY_RECEIVER_TERMINAL_PROMPT },
  ])("fails closed when the $label identity lookup fails", async ({ kind, prompt }) => {
    process.env.SPAWN_CLOSURE_ENABLE_AUTO_REDRIVE = "1";
    const { db } = await makeDb();
    const item = seedAsyncItem(db, { failureKind: "spawn_not_started" });
    db.prepare("UPDATE cross_session_log SET kind = ?, prompt = ?, status = 'pending' WHERE id = ?")
    ...
    const faultDb = new Proxy(db, {
      get(target, property) {
        if (property === "prepare") {
          return (sql: string) => {
            if (sql.includes("SELECT 1 AS matched") && sql.includes("cross_session_log c")) {
              throw new Error("injected receiver-terminal fence lookup failure");
            }
    ...
    expect(classifyAsyncItem(item, faultDb)).toEqual({
      route: "noop",
      reason: "receiver-terminal identity lookup failed; generic classification suppressed",
    });
    await expect(classifyAndRoute({ item, db: faultDb, apiBase: "http://sm.test" })).resolves.toEqual({
      decision: {
        route: "noop",
        reason: "receiver-terminal identity lookup failed; generic classification suppressed",
      },
