...
describe("scripts/watcher-tick.sh trigger routing", () => {
  ...
  test.each(signalCases)("$signal routes to sk-watcher and writes watcher_state dedup", async (scenario) => {
    ...
    try {
      ...
      expect(api.calls.filter((call) => call.path === "/api/spawn2.0")).toHaveLength(1);
      expect(api.calls.filter((call) => call.path === "/api/notify")).toHaveLength(0);

      const spawnCall = api.calls.find((call) => call.path === "/api/spawn2.0")!;
      expect(spawnCall.body).toMatchObject({
        target: "sk-watcher",
        from: "sk-watcher",
        closure: { kind: "message", target: { type: "todo_pool" } },
      });
      expect(spawnCall.body.client_request_id).toMatch(/^20\d\d-\d\d-\d\d:watcher-tick:/u);
      expect(spawnCall.body.supermatrix_internal).toBeUndefined();
